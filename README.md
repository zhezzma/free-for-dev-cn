...
本特别列表的范围仅限于基础设施开发人员（系统管理员、DevOps 从业者等）可能觉得有用的东西。我们喜欢那里所有的免费服务，但保持主题相关会很好。有时这是一条灰色地带，所以这是有意见的；如果我不接受你的贡献，请不要感到冒犯。

此列表是通过拉取请求、评论、想法和 1600 多人的工作得出的。你也可以通过发送[拉取请求](https://github.com/ripienaar/free-for-dev)来添加更多服务或删除那些提供的内容已经更改或已退役的服务来提供帮助。

[![跟踪优秀列表](https://www.trackawesomelist.com/badge.svg)](https://www.trackawesomelist.com/ripienaar/free-for-dev)

**注意**：此列表仅适用于即服务产品，不适用于自托管软件。要符合条件，服务必须提供免费层级，而不仅仅是免费试用。如果是时间桶化的免费层级，必须至少为一年。我们还从安全角度考虑免费层级，因此 SSO 可以接受，但我不会接受将 TLS 限制为仅付费层级的服务。
...
# 目录
  ...
  * [访问者会话记录（VisitorSessionRecording）](#访问者会话记录（VisitorSessionRecording）)
  * [虚拟主机（WebHosting）](#虚拟主机（WebHosting）)
  * [评论平台](#评论平台)
  * [基于浏览器的硬件仿真（在JavaScript中编写）](#基于浏览器的硬件仿真（在JavaScript中编写）)
  * [远程桌面工具](#远程桌面工具)
  * [游戏开发](#游戏开发)
  * [其他免费资源](#其他免费资源)
...
## 主要云服务提供商的免费使用限制
  ...
  * [Google Cloud Platform](https://cloud.google.com)
    ...
    * Compute Engine - 1 个非抢占性 e2-micro，30GB HDD，5GB 快照存储（受某些地区限制），每月从北美到所有地区目的地（不包括中国和澳大利亚）的 1GB 网络出站流量
  ...
  * [Microsoft Azure](https://azure.microsoft.com)
    ...
    * [Azure Pipelines](https://azure.microsoft.com/services/devops/pipelines/)——Linux、macOS 和 Windows 的开源版本每月有 10 个免费并行作业，不限时间
    ...
    * [Cosmos DB](https://azure.microsoft.com/services/cosmos-db/) - 25GB 存储和 1000 RU 提供的吞吐量
    ...
    * [Cognitive Services](https://azure.microsoft.com/services/cognitive-services/) - AI/ML API（计算机视觉、翻译、面部检测、机器人等），免费层包括有限的事务
  ...
  * [Oracle Cloud](https://www.oracle.com/cloud/)
    * 计算
       ...
       - 实例在被视为空闲时将被收回[（https://docs.oracle.com/en-us/iaas/Content/FreeTier/freetier_topic-Always_Free_Resources.htm#compute__idleinstances）
    * 块卷 - 2 个卷，总计 200GB（用于计算）
    * 对象存储 - 10GB
    * 负载均衡器 - 1 个实例，速度为 10Mbps
    * 数据库 - 2 个数据库，每个数据库 20GB
    * 监控 - 5 亿个摄入数据点，10 亿个检索数据点
    * 带宽 - 每月 10TB 出站流量，x64 基于 VM 的速度限制为 50Mbps，基于 ARM 的 VM 速度限制为 500Mbps * 核心数
...
## 云管理解决方案
  ...
  * [Cloud 66](https://www.cloud66.com/) - 个人项目免费（包括一个部署服务器，一个静态站点），Cloud 66 为您提供在任何云平台上构建、部署和扩展应用程序所需的一切，而无需担心“服务器问题”。
  ...
  * [scalr.com](https://scalr.com/) - Scalr 是一个 Terraform 自动化和协作（TACO）产品，用于更好地协作和自动化由 Terraform 管理的基础架构和配置。完全支持 Terraform CLI，OPA 集成和分层配置模型。无单点登录税。所有功能都包含在内。免费使用高达 50 次运行/月。
...
## 源代码存储库
  ...
  * [Bitbucket](https://bitbucket.org/)——无限数量的公共和私人 Git 存储库，最多可容纳 5 名用户，并提供持续集成/持续部署（CI/CD）管道
...
## API、数据和机器学习
  ...
  * [JSONGrid](https://jsongrid.com) - 免费工具，用于可视化、编辑、过滤复杂的 JSON 数据到漂亮的表格网格中。通过链接共享 JSON 数据。
  ...
  * [Abstract API](https://www.abstractapi.com) - 用于各种用例的 API 套件，包括 IP 地理位置定位、性别检测或电子邮件验证。
  ...
  * [APIVerve](https://apiverve.com) - 立即免费访问 120 多个 API，注重质量、一致性和可靠性。免费计划每月覆盖最多 50 个 API 令牌。
  ...
  * [Country-State-City Microservice API](https://country-state-city.rebuscando.info/) - 提供广泛信息的 API 和微服务，包括国家、地区、省份、城市、邮政编码等。免费层每天包括最多 100 次请求。
  * [Coupler](https://www.coupler.io/) - 在应用程序之间进行数据集成的工具。它可以创建实时仪表板和报告，转换和操作值，并收集和备份见解。免费计划具有无限用户、每月 100 次运行、1000 行每月和无限集成。
  ...
  * [Dataimporter.io](https://www.dataimporter.io) - 将数据连接、清理和导入 Salesforce 的工具。免费计划每月最多可容纳 20,000 条记录。
  ...
  * [DB-IP](https://db-ip.com/api/free) - 免费的 IP 地理位置定位 API，每个 IP 每天限制 1k 请求。基于 CC-BY 4.0 许可证的 lite 数据库也是免费的。
  * [DB Designer](https://www.dbdesigner.net/) - 基于云的数据库模式设计和建模工具，免费入门计划为每个模型提供 2 个数据库模型和 10 个表。
  * [DeepAR](https://developer.deepar.ai) - 用于任何平台的增强现实面部过滤器，一个 SDK 即可。免费计划提供最多 10 个每月活跃用户 (MAU)，并跟踪最多 4 个面部
  * [Deepnote](https://deepnote.com) - 一种新的数据科学笔记本。兼容 Jupyter，具有实时协作和在云端运行的功能。免费层包括无限个人项目、高达 750 小时的标准硬件以及最多 3 个编辑器的团队。
  ...
  * [dreamfactory.com](https://dreamfactory.com/)——用于移动、网络和物联网应用程序的开源 REST API 后端。连接任何 SQL/NoSQL 数据库、文件存储系统或外部服务，它立即创建一个具有实时文档和用户管理的全面 REST API 平台。
  ...
  * [file.coffee](https://file.coffee/)——一个平台，你可以在其中存储最多 15MB/文件（使用帐户可存储 30MB/文件）。
  * [FraudLabs Pro](https://www.fraudlabspro.com)——对信用卡支付欺诈进行订单交易筛查。此 REST API 将根据订单的输入参数检测所有可能的欺诈特征。免费的微计划每月有 500 笔交易。
  * [Geekflare API](https://geekflare.com/api)——Geekflare API 让你可以截图、审核网站、TLS 扫描、DNS 查找、测试 TTFB 等。免费计划提供 3000 次 API 请求。
  ...
  * [Glitterly](https://glitterly.app/)——从基础模板动态生成图像的程序。Restful API 和无代码集成。免费层每月有 50 张图像和五个模板。
  ...
  * [Hook0](https://www.hook0.com/)——Hook0 是一个开源的 Webhooks-as-a-service (WaaS)，使在线产品提供 Webhooks 变得容易。免费提供 7 天历史记录，每月最多可调度 3000 次事件。
  ...
  * [Invantive Cloud](https://cloud.invantive.com/)——访问超过 70 个（云）平台，如 Exact Online、Twinfield、ActiveCampaign 或 Visma 使用 Invantive SQL 或 OData4（通常是 Power BI 或 Power Query）。包括数据复制和交换。开发人员和实施顾问的免费计划。对于特定平台，免费计划具有数据量限制。
  * [ipaddress.sh](https://ipaddress.sh)——一个简单的服务，用于以不同的[格式](https://about.ipaddress.sh/)获取公共 IP 地址。
  * [ipbase.com](https://ipbase.com)——IP 地理定位 API——永久免费计划，每月 150 次请求。
  * [IP Geolocation](https://ipgeolocation.io/)——IP 地理定位 API——开发人员的永久免费计划，每月 30k 请求（每天 1k）限制。
  ...
  * [IP2Location.io](https://www.ip2location.io/)——免费增值、快速可靠的 IP 地理定位 API，用于确定城市、坐标、ISP 等地理定位数据。免费计划每月提供 30k 信用额度。订阅付费计划以获得更多高级功能或联系我们获取个性化计划。
  ...
  * [JSON Serve](https://jsonserve.com/)——一个免费的服务，帮助开发人员存储 JSON 对象，并在其应用程序中使用该 JSON 作为 REST API。
  ...
  * [Market Data API](https://www.marketdata.app) - 提供股票、期权、共同基金等实时和历史金融数据。免费永久 API 层允许每天无限制地进行 100 次 API 请求。
  * [Meteosource Weather API](https://www.meteosource.com/)——全球天气 API，提供当前和预测天气数据。预测基于机器学习对多个天气模型的组合，以提高准确性。免费计划每天提供 400 次调用。
  * [microlink.io](https://microlink.io/)——将任何网站转换为数据，例如元标记规范化、美化链接预览、抓取功能或屏幕截图。每天免费提供 100 个请求。
  ...
  * [Mockfly](https://www.mockfly.dev/)——Mockfly 是一款值得信赖的 API 模拟和功能标志管理开发工具。通过直观的界面快速生成和控制模拟 API。免费层每天提供 500 个请求。
  ...
  * [microenv.com](https://microenv.com)——为开发人员创建虚假的 REST API，具有在 Docker 容器中生成代码和应用程序的可能性。
  ...
  * [GoCardless](https://gocardless.com/)——免费开放银行数据 API。PSD2。在欧盟+英国连接 2300 多家银行。
  ...
  * [Duply.co](https://duply.co)——从 API 和 URL 创建动态图像，设计模板一次并重复使用。免费层每月提供 70 个图像的创建（来自 API 和 URL），最多 100 个通过表单。
  ...
  * [Postman](https://postman.com)——用于简化工作流程并使用 Postman 创建更好的 API 的协作平台，Postman 是 API 开发的协作平台。免费使用 Postman App 永久。Postman 云功能也永久免费，但有一定限制。
  * [Insomnia](https://insomnia.rest)——用于设计和测试 API 的开源 API 客户端，它支持 REST 和 GraphQL
  * [PrefectCloud](https://www.prefect.io/cloud/)——用于数据流程自动化的完整平台。所有计划每月包括 20,000 次免费运行。这足以为大多数小企业提供 ETL 动力。
  * [Preset Cloud](https://preset.io/)——托管的 Apache Superset 服务。最多 5 个用户的团队永久免费，提供无限的仪表板和图表、无代码图表构建器和协作式 SQL 编辑器。
  * [PromptLeo](https://promptleo.com/)——创作者和开发人员的提示工程平台。它提供提示工程库、表单和 API。免费计划提供一个提示形成、一个提示 API 端点和每月 30 次生成。
  * [PromptLoop](https://www.promptloop.com/)——使用 AI 和大型语言模型（如 GPT-3），通过简单的电子表格公式在 Google Sheets 中转换、理解和分析文本。每月免费提供前 2000 个信用额度。
  * [Crawlbase](https://crawlbase.com/)——无需代理、基础设施或浏览器即可抓取和抓取网站。我们为您解决验证码问题并防止您被阻止。前 1000 个调用免费。
  ...
  * [QuickMocker](https://quickmocker.com/)——管理自己子域下的在线虚假 API 终结点，将请求转发到本地主机 URL 进行 Webhooks 开发和测试，使用 RegExp 和多个 HTTP 方法进行 URL 路径，对端点进行优先级排序，每个端点超过 100 个短代码（动态或虚假响应值）用于响应模板化，从 OpenAPI（Swagger）规范以 JSON 格式导入，代理请求，根据 IP 地址和授权标头限制端点。免费帐户提供一个随机子域、十个端点、五个 RegExp URL 路径、每个端点 50 个短代码、每天 100 个请求和请求日志中的 50 个历史记录。
  ...
  * [Select Star](https://www.selectstar.com/)——是一个智能数据发现平台，可自动分析和记录您的数据。免费的轻量级套餐，包括 1 个数据源、最多 100 个表和 10 个用户。
  ...
  * [Sofodata](https://www.sofodata.com/)——从 CSV 文件创建安全的 RESTful API。上传 CSV 文件并通过其 API 立即访问数据，从而加速应用程序开发。免费计划包括 2 个 API 和每月 2,500 个 API 调用。您无需信用卡。
  ...
  * [TinyMCE](https://www.tiny.cloud)——富文本编辑 API。核心功能免费供无限使用。
  * [Treblle](https://www.treblle.com)——Treblle 帮助团队构建、交付和管理 API。具有高级 API 日志聚合、可观察性、文档和调试功能。所有功能都是免费的，但免费层每月的请求限制高达 250k。
  ...
  * [Zenscrape](https://zenscrape.com/web-scraping-api)——具有无头浏览器、住宅 IP 和直接定价的网络抓取 API。每月一千次免费 API 调用，学生和非营利组织有额外积分。
  * [ip-api](https://ip-api.com)——IP 地理位置 API，免费供非商业使用，无需 API 密钥，免费计划每个 IP 地址每分钟限制 45 次请求。
  ...
  * [Volca](https://volca.io#api)——免费提供编程语言和数据库系统等技术列表的 API。无限制的免费请求。
...
## 团队和协作工具
  ...
  * [Duckly](https://duckly.com/) - 与团队实时交谈和协作。支持配对编程、IDE、终端共享、语音、视频和屏幕共享。免费供小团队使用。
  ...
  * [gitter.im](https://gitter.im/) - 用于 GitHub 的聊天。团队最多可容纳 25 人，免费使用公共和私人房间。
  ...
  * [/meet for Slack](https://meetslack.com) - 使用 /meet 在任何频道、组或 DM 中从 Slack 直接启动 Google 会议。免费无任何限制。
  ...
  * [Notion](https://www.notion.so/) - 具有 markdown 支持的笔记记录和协作应用程序，它集成了任务、维基和数据库。该公司将该应用程序描述为笔记记录、项目管理和任务管理的一体化工作空间。除了跨平台应用程序，还可以通过大多数网络浏览器访问它。
  ...
  * [PageShare.dev](https://www.pageshare.dev) - 在 GitHub Pull Requests 中添加视觉审查功能，无需部署网站。免费供每月最多 10 页和总计 100MB 的存储。
  * [Pendulums](https://pendulums.io/) - Pendulums 是一个免费的时间跟踪工具，它具有易于使用的界面和有价值的统计信息，可帮助您更好地管理时间。
  * [Pumble](https://pumble.com) - 免费团队聊天应用程序。无限制的用户和消息历史记录，永远免费。
  * [Raindrop.io](https://raindrop.io) - macOS、Windows、Android、iOS 和 Web 上的私有和安全书签应用程序。免费无限书签和协作。
  ...
  * [Slab](https://slab.com/)——一个现代的团队知识管理服务。免费提供给最多 10 个用户。
  * [slack.com](https://slack.com/)——免费，无限用户，但有一些功能限制
  * [Spectrum](https://spectrum.chat/)——免费创建公共或私人社区。
  * [StatusPile](https://www.statuspile.com/)——状态页面的状态页面。您能跟踪您的上游提供商的状态页面吗？
  * [Stickies](https://stickies.app/)——用于头脑风暴、内容策划和笔记的可视化协作应用程序。免费提供给最多 3 个墙、无限用户和 1GB 存储空间。
  * [talky.io](https://talky.io/)——免费的小组视频聊天。匿名。点对点。无需插件、注册或支付。
  * [Teamhood](https://teamhood.com/)——免费的项目、任务和问题跟踪软件。支持带有泳道和完整 Scrum 实现的看板。具有集成的时间跟踪功能。免费提供给五个用户和三个项目组合。
  * [Teamplify](https://teamplify.com)——通过团队分析和智能日常站立改进团队开发流程。包括适用于远程优先团队的全功能休假管理。免费提供给最多 5 个用户的小团体。
  ...
  * [BookmarkOS.com](https://bookmarkos.com/)——免费的一体化书签管理器、选项卡管理器和任务管理器，具有可定制的在线桌面，支持文件夹协作。
  * [typetalk.com](https://www.typetalk.com/)——通过网络或移动设备上的即时消息与团队分享和讨论想法。
  * [Tugboat](https://tugboat.qa)——自动和按需预览每个拉取请求。免费提供给所有人，非盈利组织提供免费的 Nano 层。
  * [whereby.com](https://whereby.com/)——一键式视频对话，免费（以前称为 appear.in）
  * [windmill.dev](https://windmill.dev/)——Windmill 是一个开源开发者平台，可从最少的 Python 和 Typescript 脚本快速构建生产级多步骤自动化和内部应用程序。作为免费用户，您最多可以创建和成为三个非高级工作空间的成员。
  * [vadoo.tv](https://vadoo.tv/)——简单的视频托管和营销。只需单击一下即可上传视频。记录、管理、共享等。免费层提供最多 10 个视频、1GB 存储空间和 10GB 带宽/每月
  * [userforge.com](https://userforge.com/)——互联在线个人资料、用户故事和上下文映射。有助于保持设计和开发的同步，最多免费提供给 3 个个人资料和两个协作者。
  ...
  * [Wormhole](https://wormhole.app/)——使用端到端加密共享最多 5GB 的文件，长达 24 小时。对于大于 5GB 的文件，它使用点对点传输将您的文件直接发送。
  * [zoom.us](https://zoom.us/)——提供安全的视频和网络会议附加组件。免费计划限制为 40 分钟。
  * [Zulip](https://zulip.com/)——具有独特电子邮件式线程模型的实时聊天。免费计划包括 10,000 条消息搜索历史和最多 5GB 的文件存储。还提供自托管的开源版本。
  ...
  * [Chanty.com](https://chanty.com/)——Chanty 是 Slack 的另一种替代品。它为小型团队（最多 10 个）提供免费的永久计划，包括无限的公共和私人对话、可搜索的历史记录、无限的 1:1 音频通话、无限的语音消息、10 个集成和每个团队 20GB 的存储空间。
  * [ruttl.com](https://ruttl.com/)——收集数字反馈和审查网站、PDF 和图像的最佳一体化反馈工具。
  * [Mattermost](https://mattermost.com/)——适用于技术团队的安全协作。免费计划包括无限频道、剧本、看板、用户、10GB 存储等。
  * [Webvizio](https://webvizio.com)——用于直接在实时网站和网络应用程序、图像、PDF 和设计文件上的任务上简化网站开发协作的网站反馈工具、网站审查软件和错误报告工具。
  * [Pullflow](https://pullflow.com)——在 GitHub、Slack 和 VS Code 上提供 AI 增强的代码审查协作平台。
  * [Webex](https://www.webex.com/)——提供免费计划，其中视频会议每个会议可提供 40 分钟，最多可容纳 100 名参会者。
  * [RingCentral](https://www.ringcentral.com/)——提供免费计划，其中视频会议每个会议可提供 50 分钟，最多可容纳 100 名参与者。
  * [GitBook](https://www.gitbook.com/)——用于捕获和记录技术知识的平台——从产品文档到内部知识库和 API。个人开发者的免费计划。
  * [transfernow](https://www.transfernow.net/)——最简单、最快和最安全的界面，用于传输和共享文件。无需订阅即可发送照片、视频和其他大文件。
  * [paste.sh](https://paste.sh/)——这是一个基于 JavaScript 和 Crypto 的简单粘贴网站。
  * [Revolt.chat](https://revolt.chat/)——[Discord](https://discord.com/)的开源替代品，尊重您的隐私。它还免费提供了 Discord 的大多数专有功能。Revolt 是一个安全快速的全功能应用程序，同时完全免费。所有功能都是免费的。与大多数主流聊天应用程序不同，它还支持（官方和非官方）插件支持。
  * [Tencent RTC](https://trtc.io/)——腾讯实时通信 (TRTC) 提供组音频/视频通话解决方案。第一年每月提供 10,000 分钟的免费分钟数。
  * [Pastefy](https://pastefy.app/)——具有可选客户端加密、多标签粘贴、API、突出显示的编辑器等的美观简洁的 Pastebin。
...
## 内容管理系统（CMS）
  ...
  * [DatoCMS](https://www.datocms.com/) - 为小项目提供免费层级。DatoCMS 是一个基于 GraphQL 的 CMS。在较低层级，您有 100k/月的调用。
  ...
  * [kontent.ai](https://www.kontent.ai) - 一个内容即服务平台，为您提供无头 CMS 的所有好处，同时赋予营销人员权力。开发者计划为每个用户提供两个用户、无限项目、两个环境、500 个内容项目、两种语言的交付和管理 API 以及自定义元素支持。您可以使用更详细的计划来满足您的需求。
  ...
  * [sensenet](https://sensenet.com) - 提供企业级解决方案的 API 优先无头 CMS，适用于各种规模的企业。开发者计划为三个用户、500 个内容项目、三个内置角色、25+5 种内容类型、完全可访问的 REST API、文档预览生成和 Office Online 编辑提供服务。
  * [TinaCMS](https://tina.io/) - 取代 Forestry.io。开源 Git 支持的无头 CMS，支持 Markdown、MDX 和 JSON。基本套餐免费，可提供两个用户。
  * [GatsbyjsCMS](https://www.gatsbyjs.com/) - Gatsby 是快速灵活的框架，使使用任何 CMS、API 或数据库构建网站再次变得有趣。构建和部署无头网站，以增加流量、更好地转化和获得更多收入！
  * [Hygraph](https://hygraph.com/) - 为小项目提供免费层级。GraphQL 优先 API。摆脱遗留解决方案，采用 GraphQL 原生无头 CMS - 并首先提供多渠道内容 API。
  * [Squidex](https://squidex.io/) - 为小项目提供免费层级。API/GraphQL 优先。开源且基于事件源（自动跟踪每个更改）。
  * [InstaWP](https://instawp.com/) - 在几秒钟内启动 WordPress 网站。免费层级包括 5 个活动站点、500MB 空间、48 小时站点过期。
  * [Storyblok](https://www.storyblok.com) - 为开发人员和营销人员提供的无头 CMS，适用于所有现代框架。社区（免费）层提供管理 API、可视化编辑器、十个源、自定义字段类型、国际化（无限语言/地区）、资产管理器（最多 2500 个资产）、图像优化服务、搜索查询、Webhook + 250GB 流量/月包含在内。
  * [WPJack](https://wpjack.com) - 在不到 5 分钟的时间内在任何云平台上设置 WordPress！免费层级包括 1 个服务器、2 个站点、免费 SSL 证书和无限 cron 作业。没有时间限制或过期-您的网站，您的方式。
...
## 代码生成
  ...
  * [Cody AI](https://sourcegraph.com/cody)——Cody 是一款编码 AI 助手，它使用 AI 和对代码库的深入理解来帮助你更快地编写和理解代码。Cody 为开发人员提供了对各种 LLM（包括本地推理）的选择，支持各种 IDE，支持所有流行的编程语言，并提供免费计划。免费计划每月为开发人员提供 20 条聊天消息（使用 Claude 3 Sonnet 作为 LLM）和 500 次自动完成（使用 Starcoder 16b）。
  * [DhiWise](https://www.dhiwise.com/)——借助 DhiWise 的创新代码生成技术，将 Figma 设计无缝转换为动态的 Flutter 和 React 应用程序，优化工作流程，帮助你以前所未有的速度打造出色的移动和 Web 体验。
  ...
  * [Metalama](https://www.postsharp.net/metalama)——仅适用于 C#。Metalama 在编译时动态生成代码的样板，使你的源代码保持整洁。它对开源项目免费，其商业友好的免费层包括三个方面。
  ...
  * [v0.dev](https://v0.dev/)——v0 使用 AI 模型根据简单的文本提示生成代码。它根据 shadcn/ui 和 Tailwind CSS 生成可复制粘贴的友好 React 代码，人们可以在他们的项目中使用。每次生成至少需要 30 个学分。你开始时有 1200 个学分，并且每月获得 200 个免费学分。
...
## 代码质量
  ...
  * [Codeac.io](https://www.codeac.io/infrastructure-as-code.html?ref=free-for-dev)——用于 DevOps 的自动化基础架构即代码审查工具，与 GitHub、Bitbucket 和 GitLab 集成（甚至可以自托管）。除了标准语言外，它还分析 Ansible、Terraform、CloudFormation、Kubernetes 等。（开源免费）
  ...
  * [CodSpeed](https://codspeed.io)——在您的 CI 管道中自动跟踪性能。由于精确和一致的指标，在部署前捕获性能回归。对于开源项目免费永久。
  ...
  * [DeepSource](https://deepsource.io/)——DeepSource 持续分析源代码更改，查找和修复归类为安全、性能、反模式、错误风险、文档和样式的问题。与 GitHub、GitLab 和 Bitbucket 原生集成。
  * [DiffText](https://difftext.com)——立即找到两段代码之间的差异。完全免费使用。
  * [eversql.com](https://www.eversql.com/)——EverSQL - 数据库优化的#1 平台。自动获取有关您的数据库和 SQL 查询的关键见解。
  * [gerrithub.io](https://review.gerrithub.io/)——适用于 GitHub 存储库的 Gerrit 代码审查免费
  ...
  * [reviewable.io](https://reviewable.io/)——适用于 GitHub 存储库的代码审查，对于公共或个人存储库免费。
...
## 代码搜索和浏览
  ...
  * [tickgit.com](https://www.tickgit.com/)——将`TODO`注释（和其他标记）表面化，以确定值得返回进行改进的代码区域。
...
## 持续集成和持续部署（CI和CD）
  ...
  * [AccessLint](https://github.com/marketplace/accesslint) - AccessLint 将自动化的网络可访问性测试引入您的开发工作流程。对于开源和教育目的是免费的。
  * [appcircle.io](https://appcircle.io) - 企业级移动 DevOps 平台，可自动构建、测试和发布移动应用程序，以实现更快、更高效的发布周期。免费使用，最长 30 分钟的构建时间，20 个每月构建和 1 个并发构建。
  * [appveyor.com](https://www.appveyor.com/) - Windows 的持续集成服务，开源免费
  * [Argonaut](https://argonaut.dev/) - 在您的云平台上几分钟内部署应用程序和基础架构。支持在 Kubernetes 和 Lambda 环境中自定义和第三方应用程序的部署。免费层允许在 5 个域和 2 个用户的情况下无限制的应用程序和部署。
  * [bitrise.io](https://www.bitrise.io/) - 移动应用程序的持续集成/持续部署，适用于原生或混合应用程序。每月有 200 次免费构建、10 分钟构建时间和两个团队成员。OSS 项目获得 45 分钟构建时间、+1 并发性和无限团队规模。
  * [buddy.works](https://buddy.works/) - 具有五个免费项目和一个并发运行（每月 120 次执行）的持续集成/持续部署
  * [Buildkite](https://buildkite.com) 持续集成管道免费使用 3 个用户和 5k 作业分钟/月。测试分析免费开发人员层包括每月 100 万次测试执行，对于开源项目还有更多免费功能。
  * [bytebase.com](https://www.bytebase.com/) - 数据库持续集成/持续交付和 DevOps。免费使用人数在 20 人以下，数据库实例在 10 个以下
  * [CircleCI](https://circleci.com/) - 全面的免费计划，包含在托管的持续集成/持续部署服务中，适用于 GitHub、GitLab 和 BitBucket 存储库的所有功能。多个资源类、Docker、Windows、Mac OS、ARM 执行器、本地运行器、测试拆分、Docker 层缓存和其他高级持续集成/持续部署功能。免费使用时间每月高达 6000 分钟，无限制的协作者，私有项目中的 30 个并行作业，以及开源项目的 80,000 个免费构建分钟。
  ...
  * [drone](https://cloud.drone.io/) - Drone Cloud 使开发人员能够在多个架构（包括 x86 和 Arm（32 位和 64 位））上运行持续交付管道，所有这些都在一个地方
  * [LayerCI](https://layerci.com) - 全栈项目的持续集成
  * [semaphoreci.com](https://semaphoreci.com/) - 开源免费，每月 100 个私有构建
  * [Squash Labs](https://www.squash.io/) - 为每个分支创建一个 VM，并使您的应用程序可从唯一的 URL 访问，无限的公共和私有存储库，最大 2GB 的 VM 大小。
  * [styleci.io](https://styleci.io/) - 仅限公共 GitHub 存储库
  * [Mergify](https://mergify.io) - GitHub 的工作流程自动化和合并队列 - 免费用于公共 GitHub 存储库
  * [Make](https://www.make.com/en) - 工作流程自动化工具，允许您通过 UI 连接应用程序并自动化工作流程。它支持许多应用程序和最流行的 API。免费用于公共 GitHub 存储库，以及免费层，包括 100Mb、1000 次操作和 15 分钟的最小间隔。
  * [Spacelift](https://spacelift.io/) - 基础设施即代码的管理平台。免费计划功能：IaC 协作、Terraform 模块注册表、ChatOps 集成、与 Open Policy Agent 的持续资源合规性、与 SAML 2.0 的单点登录以及对公共工作者池的访问：每月最多 200 分钟
  ...
  * [Terramate](https://terramate.io/) - Terramate 是用于基础设施即代码 (IaC) 工具（如 Terraform、OpenTofu 和 Terragrunt）的编排和管理平台。免费使用，最多 2 个用户，包括所有功能。

**[⬆️ 返回顶部](#目录)**

## 测试

  * [Applitools.com](https://applitools.com/) - 用于 Web、原生移动和桌面应用的智能视觉验证。与几乎所有自动化解决方案（如 Selenium 和 Karma）和远程运行器（Sauce Labs、Browser Stack）集成。开源免费。为每周检查点有限的单个用户提供免费层。
  * [Appetize](https://appetize.io) - 在这个基于云的 Android 手机/平板电脑模拟器和 iPhone/iPad 模拟器上直接在您的浏览器中测试您的 Android 和 iOS 应用。免费层包括两个并发会话，每月使用 30 分钟。无应用大小限制。
  * [Apptim](https://apptim.com) - 一款移动测试工具，使没有性能工程技能的人员能够评估应用的性能和用户体验（UX）。使用您自己的设备的桌面版本完全免费，iOS 和 Android 上的测试次数不限。
  * [Argos](https://argos-ci.com) - 面向开发人员的开源视觉测试。无限制的项目，每月 5,000 张屏幕截图。开源项目免费。
  ...
  * [cypress.io](https://www.cypress.io/) - 在浏览器中运行的任何内容的快速、简单和可靠的测试。Cypress Test Runner 始终免费且开源，没有任何限制和限制。Cypress Dashboard 对开源项目免费，最多可用于 5 个用户。
  * [Cypress Recorder by Preflight](https://cypress.preflight.com/) - 在您的浏览器上创建基于人工智能的 Cypress 测试/POM 模型。它是开源的，人工智能部分除外。它在免费的每月测试创建中提供五个，具有自我修复脚本、电子邮件和视觉测试。
  * [everystep-automation.com](https://www.everystep-automation.com/) - 记录并重放在网络浏览器中执行的所有步骤，并创建脚本，免费但选项较少
  * [Gremlin](https://www.gremlin.com/gremlin-free-software) - Gremlin 的混沌工程工具允许您安全地将故障注入到您的系统中，以在它们导致面向客户的问题之前找到弱点。Gremlin Free 提供对多达 5 个主机或容器的关闭和 CPU 攻击的访问权限。
  * [gridlastic.com](https://www.gridlastic.com/) - 具有免费计划的 Selenium Grid 测试，最多可同时使用 4 个 Selenium 节点/10 个网格启动/4,000 个测试分钟/月
  ...
  * [knapsackpro.com](https://knapsackpro.com) - 通过在任何 CI 提供程序上进行最佳测试套件并行化来加快您的测试。在并行 CI 节点上拆分 Ruby、JavaScript 测试以节省时间。免费计划最多可用于 10 分钟的测试文件，开源项目无限制免费计划。
  ...
  * [lost-pixel.com](https://lost-pixel.com) - 用于您的 Storybook、Ladle、Histoire 故事和 Web Apps 的整体视觉回归测试。无限团队成员，完全免费开源，每月 7,000 个快照。
  * [octomind.dev](https://www.octomind.dev/) - 使用人工智能辅助的测试用例生成自动生成、运行和维护的 Playwright UI 测试
  * [percy.io](https://percy.io) - 将视觉测试添加到任何网络应用程序、静态站点、样式指南或组件库中。无限团队成员、演示应用程序和无限项目，每月 5,000 个快照。
  ...
  * [qase.io](https://qase.io) - 用于 Dev 和 QA 团队的测试管理系统。管理测试用例，组合测试运行，执行测试，跟踪缺陷，并衡量影响。免费层包括所有核心功能，可用于附件的 500MB，最多 3 个用户。
  * [Repeato](https://repeato.app/) 基于计算机视觉和人工智能的无代码移动应用程序测试自动化工具。适用于原生应用程序、Flutter、React Native、Web、Ionic 等众多应用程序框架。免费计划限制为 iOS 上的 10 个测试和 Android 上的 10 个测试，但包括付费计划的大部分功能，包括无限测试运行。
  * [Requestly](https://requestly.com/) 开源 Chrome 扩展，用于拦截、重定向和模拟 HTTP 请求。具有[调试器](https://requestly.com/products/web-debugger/)、[模拟服务器](https://requestly.com/products/mock-server/)、[API 客户端](https://requestly.com/products/api-client/)和[会话记录](https://requestly.com/products/session-book/)。重定向 URL、修改 HTTP 标头、模拟 API、注入自定义 JS、修改 GraphQL 请求、生成模拟 API 终结点、记录带有网络和控制台日志的会话。免费层中可以创建最多 10 个规则。开源免费。
  * [seotest.me](https://seotest.me/) - 免费的页面 SEO 网站测试器。每天 10 次免费网站抓取。有关如何提高任何网站页面 SEO 结果的有用 SEO 学习资源和建议，无论技术如何。
  * [snippets.uilicious.com](https://snippets.uilicious.com) - 它就像 CodePen 但用于跨浏览器测试。UI-licious 允许您像用户故事一样编写测试，并提供一个免费平台 - UI-licious Snippets - 允许您在 Chrome 上无限次免费测试，无需注册，每次测试运行最多 3 分钟。发现错误？您可以复制测试的唯一 URL 并将其提供给开发人员，以确切地向他们展示如何重现该错误。
  ...
  * [Vaadin](https://vaadin.com) - 使用 Java 或 TypeScript 构建可扩展的用户界面，并使用集成的工具、组件和设计系统来更快地迭代、更好地设计，并简化开发过程。无限项目，五年免费维护。
  ...
  * [webhook.site](https://webhook.site) - 使用自定义 URL 验证 Webhook、出站 HTTP 请求或电子邮件。临时 URL 和电子邮件地址始终是免费的。
...
## 安全性和公钥基础设施（PKI）
  ...
  * [aikido.dev](https://www.aikido.dev)——一体化的应用安全平台，涵盖 SCA、SAST、CSPM、DAST、Secrets、IaC、Malware、Container scanning、EOL 等。免费计划包括两个用户，对 10 个存储库、1 个云、2 个容器和 1 个域的扫描。
  ...
  * [Public Cloud Threat Intelligence](https://cloudintel.himanshuanand.com/)——针对公共云基础设施的高可信度指标(IOC)，部分内容可在 github（https://github.com/unknownhad/AWSAttacks）上获得。完整列表可通过 API 获取
  ...
  * [CyberChef](https://gchq.github.io/CyberChef/)——一个简单、直观的网络应用程序，用于分析和解码/编码数据，无需处理复杂的工具或编程语言。就像一个加密和加密的瑞士军刀。所有功能均可免费使用，无限制。如果您希望自行托管，则为开源。
  * [DAS](https://signup.styra.com/)——Styra DAS 免费，完整的生命周期策略管理，用于创建、部署和管理 Open Policy Agent(OPA)授权
  * [Datree](https://www.datree.io/)——开源 CLI 工具，用于防止 Kubernetes 配置错误，确保清单和 Helm 图表遵循最佳实践以及您组织的策略
  ...
  * [Dotenv](https://dotenv.org/)——快速、安全地同步您的.env 文件。停止通过不安全的渠道（如 Slack 和电子邮件）共享您的.env 文件，再也不会丢失重要的.env 文件。最多供 3 个队友使用。
  ...
  * [hostedscan.com](https://hostedscan.com)——在线漏洞扫描程序，适用于 Web 应用程序、服务器和网络。每月免费 10 次扫描。
  * [Infisical](https://infisical.com/)——开源平台，可让您在团队和基础架构中管理开发人员的秘密：从本地开发到登台/生产第三方服务。最多 5 名开发人员免费。
  ...
  * [meterian.io](https://www.meterian.io/)——监控 Java、Javascript、.NET、Scala、Ruby 和 NodeJS 项目中的安全漏洞。免费用于一个私有项目，开源项目不限。
  ...
  * [pixee.ai](https://pixee.ai)——自动产品安全工程师，作为免费的 GitHub 机器人，会自动提交修复漏洞的 PR。其他语言即将推出！
  * [pyup.io](https://pyup.io)——监控 Python 依赖项的安全漏洞并自动更新它们。免费用于一个私有项目，开源项目不限。
  ...
  * [seclookup.com](https://www.seclookup.com/)——Seclookup API 可以丰富 SIEM 中的域名威胁指标，提供域名的全面信息，并提高威胁检测和响应能力。在此处（https://account.seclookup.com/）免费获得 50K 次查找。
  ...
  * [StackHawk](https://www.stackhawk.com/)在整个管道中自动扫描应用程序，以在生产中发现和修复安全漏洞。单个应用程序的无限扫描和环境。
  ...
  * [Ubiq Security](https://ubiqsecurity.com/)——使用三行代码加密和解密数据，并自动进行密钥管理。免费用于一个应用程序，每月最多 1,000,000 次加密。
  * [Virgil Security](https://virgilsecurity.com/)——在您的数字解决方案中实现端到端加密、数据库保护、物联网安全等的工具和服务。免费用于最多 250 个用户的应用程序。
  * [Vulert](https://vulert.com)——Vulert 持续监控您的开源依赖项以发现新的漏洞，并提供修复建议，无需安装或访问您的代码库。适用于开源项目。

  * [HasMySecretLeaked](https://gitguardian.com/hasmysecretleaked) - 在公共 GitHub 存储库、代码片段、问题和注释中搜索 2000 万个公开泄露的秘密，免费使用

**[⬆️ 返回顶部](#目录)**

## 身份验证、授权和用户管理

  * [Aserto](https://www.aserto.com) - 细粒度授权即服务，适用于应用程序和 API。免费使用额度高达 1000 MAUs 和 100 个授权器实例。
  * [asgardeo.io](https://wso2.com/asgardeo) - 无缝集成单点登录、多因素身份验证、无密码身份验证等。包括适用于前端和后端应用程序的 SDK。免费使用额度高达 1000 MAUs 和 5 个身份提供商。
  * [Auth0](https://auth0.com/) - 托管式单点登录。免费计划包括 25,000 MAUs、无限社交连接、自定义域名等。
  * [Authgear](https://www.authgear.com) - 在数分钟内为您的应用带来无密码、一次性密码、双因素身份验证、单点登录等功能。所有前端均包含在内。免费使用额度高达 5000 MAUs。
  * [Authress](https://authress.io/) - 身份验证登录和访问控制，适用于任何项目的无限身份提供商。Facebook、Google、Twitter 等。前 1000 次 API 调用免费。
  * [Authy](https://authy.com) - 多设备上的双因素身份验证（2FA），并提供备份。可替代 Google Authenticator。免费使用额度高达 100 次成功验证。
  * [Cerbos Hub](https://www.cerbos.dev/product-cerbos-hub) - 用于编写、测试和部署访问策略的完整授权管理系统。细粒度授权和访问控制，免费使用额度高达 100 个每月活跃主体。
  * [Clerk](https://clerk.com) - 用户管理、身份验证、2FA/MFA、用于登录、注册、用户个人资料等的预构建 UI 组件。免费使用额度高达 10,000 个每月活跃用户。
  * [Cloud-IAM](https://www.cloud-iam.com/) - Keycloak 身份和访问管理即服务。免费使用额度高达 100 用户和一个领域。
  * [Corbado](https://www.corbado.com/) - 将无密码优先身份验证添加到新的或现有的应用程序中。免费使用额度不限 MAUs。
  * [Descope](https://www.descope.com/) - 高度可定制的身份验证流程，同时提供无代码和 API/SDK 方法，免费使用额度为 7,500 个活跃用户/月，50 个租户（最多 5 个 SAML/SSO 租户）。
  * [duo.com](https://duo.com/) - 网站或应用程序的双因素身份验证（2FA）。免费使用额度为 10 个用户，所有身份验证方法、无限集成、硬件令牌。
  * [Kinde](https://kinde.com/) - 简单、强大的身份验证，您可以在几分钟内集成到您的产品中。开始使用 7,500 个免费 MAU 所需的一切。
  * [logintc.com](https://www.logintc.com/) - 通过推送通知进行双因素身份验证（2FA），免费使用额度为 10 个用户，VPN、网站和 SSH
  * [MojoAuth](https://mojoauth.com/) - MojoAuth 使您能够在几分钟内轻松实现 Web、移动或任何应用程序的无密码身份验证。
  * [Okta](https://developer.okta.com/signup/) - 用户管理、身份验证和授权。免费使用额度高达 100 个每月活跃用户。
  * [onelogin.com](https://www.onelogin.com/) - 身份即服务（IDaaS）、单点登录身份提供商、云 SSO IdP、三个公司应用程序和五个个人应用程序，无限制用户
  * [Ory](https://ory.sh/) - 身份验证/授权/OAuth2.0/零信任管理的安全平台。具有所有安全功能的永久免费开发者帐户，无限团队成员，200 个日常活跃用户和 25k/月的权限检查。
  * [Phase Two](https://phasetwo.io) - Keycloak 开源身份和访问管理。免费领域最多 1000 用户，最多 10 个 SSO 连接，利用 Phase Two 的增强型 Keycloak 容器，其中包括 [Organization](https://phasetwo.io/product/organizations/) 扩展。
  ...
  * [Stack Auth](https://stack-auth.com) - 开源身份验证，不会令人讨厌。最适合开发者的解决方案，只需五分钟即可入门。可免费自托管，或提供带有 10k 免费每月活跃用户的托管 SaaS 版本。
  * [SuperTokens](https://supertokens.com/) - 开源用户身份验证，可本机集成到您的应用程序中-使您能够快速启动，同时控制用户和开发人员体验。免费使用额度高达 5000 MAUs。
  * [Warrant](https://warrant.dev/) - 为您的应用提供托管企业级授权和访问控制服务。免费层包括 100 万个每月 API 请求和 1,000 个授权规则。
  * [ZITADEL Cloud](https://zitadel.com) - 一个适用于您的一键式用户和访问管理，支持多租户（B2B）用例。免费使用额度高达 25,000 个经过身份验证的请求，具有所有安全功能（OTP、无密码、策略等无付费墙）。
  * [PropelAuth](https://propelauth.com) - 只需几行代码即可立即向任何规模的公司销售，免费使用额度高达 200 用户和 10k 交易电子邮件（带有水印品牌：“由 PropelAuth 提供支持”）。
  * [Logto](https://logto.io/) - 开发、保护和管理您产品的用户身份（用于身份验证和授权）。免费使用额度高达 5,000 MAUs，提供开源自托管选项。
  * [WorkOS](https://workos.com/) - 高达 100 万 MAUs 的免费用户管理和身份验证。支持电子邮件+密码、社交身份验证、Magic Auth、MFA 等。
...
## 移动应用程序分发和反馈
  ...
  * [Appho.st](https://appho.st) - 移动应用程序托管平台。免费计划包括五个应用程序、每月 50 次下载和最大 100MB 的文件大小。
...
## 管理系统
  ...
  * [Miradore](https://miradore.com)——设备管理服务。保持设备清单的最新状态，并免费安全保护无限数量的设备。免费计划提供基本功能。
...
## 消息传递和流
  ...
  * [cloudamqp.com](https://www.cloudamqp.com/) —— RabbitMQ 即服务。小狐猴计划：每月最多 100 万条消息、最多 20 个并发连接、最多 100 个队列、最多 10,000 条排队消息，不同 AZ 中有多个节点
  ...
  * [knock.app](https://knock.app) – 开发人员的通知基础设施。使用单个 API 调用将消息发送到多个通道，如应用内、电子邮件、SMS、Slack 和推送。免费计划包括每月 10,000 条消息。
  * [NotificationAPI.com](https://www.notificationapi.com/) —— 在 5 分钟内将用户通知添加到任何软件中。免费计划包括每月 10,000 个通知+100 条 SMS 和自动呼叫。
  * [Novu.co](https://novu.co) —— 开发人员的开源通知基础架构。用于在一个地方管理所有通信渠道（电子邮件、SMS、直接、应用内和推送）的简单组件和 API。免费计划包括每月 30,000 个通知，保留期为 90 天。
  ...
  * [synadia.com](https://synadia.com/ngs) —— [NATS.io](https://nats.io) 即服务。全球、AWS、GCP 和 Azure。免费永久使用，4k 消息大小，50 个活动连接，每月 5GB 数据。
  ...
  * [eyeson API](https://developers.eyeson.team/) - 基于 WebRTC（SFU、MCU）的视频通信 API 服务，用于构建视频平台。允许实时数据注入、视频布局、录制、功能齐全的托管网络 UI（快速入门）或自定义 UI 的软件包。为开发人员提供[免费层](https://apiservice.eyeson.com/api-pricing)，每月 1000 分钟会议。
  * [webpushr](https://www.webpushr.com/) - Web 推送通知 - 免费适用于最多 10k 订户，无限推送通知，浏览器内消息传递
  ...
  * [Pocket Alert](https://pocketalert.app) - 将推送通知发送到您的 iOS 和 Android 设备。通过 API 或 Webhooks 轻松集成，并完全控制您的警报。免费计划：每天向 1 个设备和 1 个应用程序发送 50 条消息。
...
## 日志管理
  ...
  * [loggly.com](https://www.loggly.com/) - 免费（单用户），每天最多可记录 200MB 数据，保留时间为 7 天
  ...
  * [logzab.com](https://logzab.com/) - 审计跟踪管理系统。每月最多可免费记录 1000 个用户活动日志，保留时间为 1 个月，最多可用于 5 个项目
  * [openobserve.ai](https://openobserve.ai/) - 每月免费摄入 200GB，保留时间为 15 天
...
## 翻译管理
  ...
  * [gitlocalize.com](https://gitlocalize.com)——免费且无限制，适用于私有和公共存储库
  ...
  * [localazy.com](https://localazy.com/)——免费 1000 个源语言字符串，无限制语言，无限制贡献者，创业和开源交易
  * [Localeum](https://localeum.com)——免费最多 1000 个字符串，一个用户，无限制语言，无限制项目
  * [localizely.com](https://localizely.com/)——开源免费
  * [Loco](https://localise.biz/)——免费最多 2000 个翻译，无限译者，每个项目 10 种语言，每个项目 1000 个可翻译资产
  * [oneskyapp.com](https://www.oneskyapp.com/)——最多 5 个用户的有限免费版，开源免费
  * [POEditor](https://poeditor.com/)——免费最多 1000 个字符串
  * [SimpleLocalize](https://simplelocalize.io/)——免费最多 100 个翻译键，无限制字符串，无限制语言，创业交易
  ...
  * [weblate.org](https://weblate.org/)——对于免费层，对于自由项目是免费的，最多 10,000 个字符串源，对于免费套餐是无限的自托管本地部署。
...
## 监控
  ...
  * [checklyhq.com](https://checklyhq.com/) - 开发者的开源端到端/合成监控和深度 API 监控。免费计划有 5 个用户和 50k+检查运行。
  ...
  * [cronitor.io](https://cronitor.io/) - 对 cron 作业、网站、API 等的性能洞察和监控。免费层有 5 个监控器。
  ...
  * [economize.cloud](https://economize.cloud) - 通过组织云资源进行优化并报告相同的内容，帮助用户了解云基础设施成本。免费适用于每月在 Google Cloud Platform 上花费最多 5,000 美元的情况。
  * [elastic.co](https://www.elastic.co/solutions/apm) - 为 JavaScript 开发人员提供即时性能洞察。免费，保留 24 小时数据。
  * [Grafana Cloud](https://grafana.com/products/cloud/) - Grafana Cloud 是一个可组合的观测平台，它将指标和日志与 Grafana 集成在一起。免费：3 个用户，10 个仪表板，100 个警报，Prometheus 和 Graphite 中的指标存储（10,000 个系列，14 天保留），Loki 中的日志存储（50GB 日志，14 天保留）
  ...
  * [linkok.com](https://linkok.com/) - 在线断链检查程序，免费适用于最多 100 页的小型网站，对开源项目完全免费。
  ...
  * [Middleware.io](https://middleware.io/) - 中间件观测平台提供对应用程序和堆栈的完整可见性，以便您可以大规模监控和诊断问题。他们有一个免费的永久计划供 Dev 社区使用，允许日志监控最多 1M 个日志事件，基础设施监控和 APM 最多 2 个主机。
  * [nixstats.com](https://nixstats.com/) - 免费 1 台服务器。电子邮件通知，公共状态页面，60 秒间隔等。
  * [OnlineOrNot.com](https://onlineornot.com/) - 提供网站和 API 的监控，以及 cron 作业和计划任务的监控。还提供状态页面。免费层的前五个检查间隔为 3 分钟。免费层通过 Slack、Discord 和电子邮件发送警报。
  ...
  * [phare.io](https://phare.io/) - 免费为最多 100,000 个事件提供监控，无限制的项目和状态页面。
  * [pingbreak.com](https://pingbreak.com/) - 现代监控服务。检查无限数量的 URL，并通过 Discord、Slack 或电子邮件获取停机通知。
  ...
  * [sematext.com](https://sematext.com/) - 免费 24 小时指标，无限制的服务器，10 个自定义指标，500,000 个自定义指标数据点，无限制的仪表板，用户等。
  ...
  * [statusgator.com](https://statusgator.com/)——状态页面监控，免费提供 3 个监视器
  * [SweetUptime](https://dicloud.net/sweetuptime-server-uptime-monitoring/)——服务器监控、运行状况监控、DNS 和域名监控。免费监控 10 台服务器、10 个运行状况和 10 个域名。
  * [thousandeyes.com](https://www.thousandeyes.com/)——网络和用户体验监控。免费提供 3 个位置和 20 个主要网络服务的数据Feed
  * [uptimerobot.com](https://uptimerobot.com/)——网站监控，免费提供 50 个监视器
  * [uptimetoolbox.com](https://uptimetoolbox.com/)——免费监控五个网站，间隔 60 秒，公共状态页面。
  * [zenduty.com](https://www.zenduty.com/)——端到端事件管理、警报、值班管理和响应编排平台，适用于网络运营、站点可靠性工程和 DevOps 团队。免费供最多 5 个用户使用。
  * [instatus.com](https://instatus.com)——在 10 秒内获得漂亮的状态页面。免费无限订阅和无限团队。
  * [Squadcast.com](https://squadcast.com)——Squadcast 是一款端到端事件管理软件，旨在帮助您推广 SRE 最佳实践。提供最多 10 个用户的免费永久计划。
  * [RoboMiri.com](https://robomiri.com/)——RoboMiri 是一个稳定的运行状况监视器，提供广泛的监视器：cronjob、关键字、网站、端口、ping。免费提供 25 次运行状况检查，间隔 3 分钟检查一次。通过电话呼叫、短信、电子邮件和 Webhooks 发出警报。
  * [Better Stack](https://betterstack.com/better-uptime)——在单个产品中进行运行状况监控、事件管理、值班调度/警报和状态页面。免费计划包括 10 个监视器，检查频率为 3 分钟，状态页面。
  * [Pulsetic](https://pulsetic.com)——10 个监视器，6 个月的历史运行状况/日志，无限的状态页面和自定义域名！免费无限时间和无限电子邮件警报。您不需要信用卡。
  * [Wachete](https://www.wachete.com)——监控五个页面，每 24 小时检查一次。
  * [Xitoring.com](https://xitoring.com/)——运行状况监控：免费 20 个，Linux 和 Windows Server 监控：免费 5 个，状态页面：免费 1 个——移动应用程序，多个通知渠道等！
...
## Crash and Exception Handling
  ...
  * [exceptionless](https://exceptionless.com) - 实时错误、功能、日志报告等。每月免费 3000 个事件/1 个用户。开源且易于自行托管，无限使用。
  ...
  * [memfault.com](https://memfault.com) - 云设备可观测性和调试平台。[Nordic](https://app.memfault.com/register-nordic)、[NXP](https://app.memfault.com/register-nxp)和[Laird](https://app.memfault.com/register-laird)设备免费 100 个。
  ...
  * [Axiom](https://axiom.co/) - 存储长达 30 天的日志，最高可达 0.5TB。包括与 Vercel 等平台的集成以及通过电子邮件/Discord 通知进行高级数据查询。
  * [Semaphr](https://semaphr.com) - 移动应用的免费一体化 Kill Switch。
  * [Jam](https://jam.dev) - 一键点击生成开发者友好的错误报告。免费计划，无限 Jams。
  * [Whitespace](https://whitespace.dev) - 在浏览器中一键生成错误报告。免费计划，个人使用无限录制。

**[⬆️ 返回顶部](#目录)**

## 搜索

  * [algolia.com](https://www.algolia.com/) - 具有容错性、相关性和用户界面库的托管搜索解决方案，可轻松创建搜索体验。免费的“Build”计划包括 100 万份文件和 10 万次搜索/月。还提供免费的[开发者文档搜索](https://docsearch.algolia.com/)。
  ...
  * [Orama Cloud](https://orama.com/pricing) - 免费 3 个索引，10 万份文档/索引，无限制全文/向量/混合搜索，60 天分析
...
## 教育和职业发展
  ...
  * [免费专业简历模板和编辑器](https://www.overleaf.com/latex/templates/tagged/cv) - 免费平台，有很多经验丰富的专业人士的简历模板，可完全克隆、编辑并下载，ATS 优化。
  ...
  * [Roadmap.sh](https://roadmap.sh) - 涵盖从区块链到用户体验设计等各个方面的免费学习路线图。
  * [Cisco Networking Academy, Skills for All](https://skillsforall.com/) - 提供免费的与认证相关的课程，主题包括网络安全、网络和 Python。
  * [MIT 开放式课程](https://ocw.mit.edu/) - MIT 开放式课程是来自 2500 多门 MIT 课程的材料的在线出版物，与世界各地的学习者和教育工作者免费分享知识。Youtube 频道可在 [@mitocw](https://www.youtube.com/@mitocw/featured) 找到

**[⬆️ 返回顶部](#目录)**

## 电子邮件

  * [10minutemail](https://10minutemail.com) - 免费的临时测试邮件。
  * [AhaSend](https://ahasend.com) - 交易邮件服务，每月免费 1000 封邮件，免费计划中无限制的域名、团队成员、Webhooks 和消息路由。
  * [AnonAddy](https://anonaddy.com) - 开源匿名邮件转发，可免费创建无限的邮件别名。
  * [Antideo](https://www.antideo.com) - 免费版每小时提供 10 次 API 请求，用于邮件验证、IP 和电话号码验证。无需信用卡。
  ...
  * [Buttondown](https://buttondown.email/) - 新闻通讯服务。免费提供最多 100 名订阅者。
  ...
  * [DNSExit](https://dnsexit.com/) - 免费提供您域名下最多 2 个电子邮件地址，存储空间 100MB。支持 IMAP、POP3、SMTP、SPF/DKIM。
  * [EmailLabs.io](https://emaillabs.io/en) - 每月免费发送高达 9000 封电子邮件，每天最多 300 封。
  * [EmailOctopus](https://emailoctopus.com) - 免费提供最多 2500 名订阅者和 10,000 封邮件。
  * [EmailJS](https://www.emailjs.com/) - 这不是完整的电子邮件服务器；这只是一个电子邮件客户端，您可以使用它从客户端直接发送电子邮件，而无需暴露您的凭据，免费版每月有 200 次请求，2 个电子邮件模板，请求大小高达 50KB，联系人历史记录有限。
  * [EtherealMail](https://ethereal.email) - Ethereal 是一个虚假的 SMTP 服务，主要针对 Nodemailer 和 EmailEngine 用户（但不限于）。这是一个完全免费的反事务性邮件服务，消息永远不会送达。
  * [Temp-Mail.org](https://temp-mail.org/en/) - 利用各种域名的临时/一次性邮件生成实用程序。每次重新加载页面时，电子邮件地址都会刷新。它完全免费，不包括任何服务费用。
  * [TempMailDetector.com](https://tempmaildetector.com/) - 免费每月验证最多 200 封电子邮件，并查看邮件是否临时。
  ...
  * [forwardemail.net](https://forwardemail.net) - 免费自定义域名的邮件转发。使用您的域名创建和转发无限数量的电子邮件地址（**注意**：如果您使用.casa、.cf、.click、.email、.fit、.ga、.gdn、.gq、.lat、.loan、.london、.men、.ml、.pl、.rest、.ru、.tk、.top、.work TLD，则需要付费）。
  ...
  * [EForw](https://www.eforw.com) - 免费为一个域转发邮件。从您的域名接收和发送邮件。
  * [Inboxes App](https://inboxesapp.com) - 每天最多创建 3 个临时电子邮件，完成后可以在方便的 Chrome 扩展中删除它们。非常适合测试注册流程。
  * [inboxkitten.com](https://inboxkitten.com/) - 免费的临时/一次性电子邮件收件箱，最多可自动删除 3 天的邮件。开源且可自托管。
  * [mail-tester.com](https://www.mail-tester.com) - 测试邮件的 DNS/SPF/DKIM/DMARC 设置是否正确，每月免费 20 次。
  * [dkimvalidator.com](https://dkimvalidator.com/) - 测试邮件的 DNS/SPF/DKIM/DMARC 设置是否正确，由 roundsphere.com 提供的免费服务。
  * [mailcatcher.me](https://mailcatcher.me/) - 捕获邮件并通过 Web 界面提供服务。
  * [mailchannels.com](https://www.mailchannels.com) - 带有 REST API 和 SMTP 集成的电子邮件 API，免费提供给每月最多 3000 封邮件的用户。
  ...
  * [Mailjet](https://www.mailjet.com/) - 每月 6000 封免费邮件（每天 200 封邮件发送限制）。
  ...
  * [Mail7.io](https://www.mail7.io/) - 免费的临时电子邮件地址。使用 Web 界面或 API 立即创建电子邮件地址。
  ...
  * [Parsio.io](https://parsio.io/) - 免费的电子邮件解析器（转发邮件，提取数据，发送到您的服务器）。
  * [pepipost.com](https://pepipost.com) - 第一个月免费 30k 封邮件，然后每天免费 100 封邮件。
  * [Plunk](https://useplunk.com) - 每月免费 3000 封邮件。
  * [Postmark](https://postmarkapp.com/) - 每月免费 100 封邮件，无限制的 DMARC 每周摘要。
  * [Proton Mail](https://proton.me/mail) - 提供内置端到端加密的免费安全电子邮件帐户服务提供商。免费 1GB 存储空间。
  * [Queuemail.dev](https://queuemail.dev) - 可靠的电子邮件交付 API。免费版（每月 10,000 封邮件）。异步发送。使用多个 SMTP 服务器。阻止列表、日志记录、跟踪、Webhooks 等。
  * [QuickEmailVerification](https://quickemailverification.com) - 在免费层免费每天验证 100 封电子邮件，以及其他免费 API，如 DEA 检测器、DNS 查询、SPF 检测器等。
  * [Resend](https://resend.com) - 用于开发人员的事务性电子邮件 API。每月 3000 封邮件，每天 100 封邮件免费，一个自定义域名。
  ...
  * [Sweego](https://www.sweego.io/) - 适用于开发人员的欧洲交易性电子邮件 API。每天免费 500 封邮件。
  ...
  * [Zoho](https://www.zoho.com)——最初是作为电子邮件提供商，但现在提供了一系列服务，其中一些提供免费计划。提供免费计划的服务列表：
     ...
     - [Connect](https://zoho.com/connect)——团队协作免费供 25 个用户使用，包括三个组、三个自定义应用程序、三个看板、三个手册和 10 个集成以及频道、事件和论坛。
     - [Meeting](https://zoho.com/meeting)——最多 3 个会议参与者和 10 个网络研讨会参与者的会议。
     ...
     - [Subscriptions](https://zoho.com/subscriptions)——无限制客户/订阅和 1 个用户的经常性计费管理，所有支付由 Zoho 托管。存储最后 40 个订阅指标
     - [Checkout](https://zoho.com/checkout)——具有 3 页和最多 50 笔支付的产品计费管理。
     - [Desk](https://zoho.com/desk)——具有三个代理、私有知识库和电子邮件票证的客户支持管理。与[Assist](https://zoho.com/assist)集成，用于一名远程技术人员和 5 台无人参与的计算机。
     - [Cliq](https://zoho.com/cliq)——具有 100GB 存储、无限用户、每个频道 100 个用户和 SSO 的团队聊天软件。
...
## 功能切换管理平台
  ...
  * [Toggled.dev](https://www.toggled.dev) - 企业级、可扩展的多区域功能切换管理平台。免费计划最多 10 个标志、两个环境、无限制的请求。SDK、分析仪表板、发布日历、Slack 通知等所有其他功能都包含在无尽的免费计划中。
...
## 字体
  ...
  * [Fontshare](https://www.fontshare.com/) - 是一个免费字体服务。它是一个不断增长的专业级字体集合，个人和商业用途完全免费。
  ...
  * [Fontsensei](https://fontsensei.com/) 由用户标记的开源 Google 字体。带有 CJK（中文、日语、韩语）字体标签。

**[⬆️ 返回顶部](#目录)**

## 表单

  * [Feathery](https://feathery.io) - 功能强大、开发者友好的表单构建器。可构建注册/登录、用户引导、支付流程、复杂金融应用等。免费计划允许每月最多提交 250 次，并且有五个活动表单。
  ...
  * [Form.taxi](https://form.taxi/)——HTML 表单提交的端点。具有通知、垃圾邮件阻止程序和符合 GDPR 的数据处理。免费计划用于基本使用。
  ...
  * [Formsubmit.co](https://formsubmit.co/)——适用于 HTML 表单的简单表单端点。免费永久。无需注册。
  ...
  * [HeroTofu.com](https://herotofu.com/) - 具有机器人检测和加密存档的表单后端。通过 UI 将提交内容转发到电子邮件、Slack 或 Zapier。使用您自己的前端。不需要服务器代码。免费计划每月提供无限表单和 100 次提交。
  ...
  * [WaiverStevie.com](https://waiverstevie.com) - 具有 REST API 的电子签名平台。您可以使用 Webhooks 接收通知。免费计划对已签名的文件加水印，但允许无限数量的信封+签名。
  * [Web3Forms](https://web3forms.com) - 适用于静态和 JAMStack 网站的联系表单，无需编写后端代码。免费计划允许每月无限表单、无限域名和 250 次提交。
  * [WebAsk](https://webask.io) - 调查和表单构建器。免费计划每个帐户有三个调查，每月有 100 次响应，每个调查有十个元素。
  * [Wufoo](https://www.wufoo.com/)——在网站上快速使用的表单。免费计划每月每个表单的限制为 100 次提交。
  * [formpost.app](https://formpost.app) - 免费、无限的表单到电子邮件服务。免费设置自定义重定向、自动回复、Webhooks 等。
  * [Formester.com](https://formester.com) - 在您的网站上分享和嵌入独特外观的表单——不受计划限制创建的表单数量或功能的限制。每月免费获得高达 100 次提交。
  * [SimplePDF.eu](https://simplepdf.eu/embed) - 在您的网站上嵌入 PDF 编辑器，并将任何 PDF 转换为可填写的表单。免费计划允许每个 PDF 有无限个 PDF 和每次提交三个 PDF。
  * [forms.app](https://forms.app/)——使用条件逻辑、自动评分计算器和 AI 等强大功能创建在线表单。免费计划可收集最多 100 次响应，可嵌入网站或使用链接。
  * [Qualli](https://usequalli.com)——应用内调查，专为移动设备设计。使用 Qualli AI 打造完美问题。您可以在我们的免费计划中尝试，最高可达 500 MAU，创建无限表单和触发器。
...
## 生成式人工智能（GenerativeAI）
  ...
  * [Clair](https://askclair.ai/) - 临床人工智能参考。学生可以免费访问专业工具套件，其中包括 Open Search、临床摘要、Med 评论、药物相互作用、ICD-10 代码和管理。此外，还提供专业套件的免费试用。
  * [Langtrace](https://langtrace.ai) - 使开发人员能够跟踪、评估、管理提示和数据集，并调试与 LLM 应用程序性能相关的问题。它为任何 LLM 创建开放遥测标准跟踪，有助于可观测性，并与任何可观测性客户端兼容。免费计划提供每月 50K 个跟踪。
...
## 内容分发网络（CDN）和防护
  ...
  * [bootstrapcdn.com](https://www.bootstrapcdn.com/) - 引导程序、引导主题和字体真棒的 CDN
  * [cdnjs.com](https://cdnjs.com/) - 简单。快速。可靠。内容交付的最佳选择。cdnjs 是一个免费和开源的 CDN 服务，超过 11%的网站都信任它，由 Cloudflare 提供支持。
...
## 平台即服务（PaaS）
  ...
  * [anvil.works](https://anvil.works) - 使用 Python 进行 Web 应用开发。免费层提供无限数量的应用和 30 秒超时限制。
  * [appwrite](https://appwrite.io) - 无限项目，无项目暂停（支持 WebSockets）和身份验证服务。免费层每个项目包含 1 个数据库、3 个存储桶、5 个函数。
  ...
  * [deco.cx](https://www.deco.cx/en/dev)——边缘原生前端平台，具有可视化 CMS，由 TypeScript 代码自动生成。内置 A/B 测试、内容分段和实时分析。非常适合内容丰富和企业电子商务网站。免费版每月可免费访问 5k 页面或开源/个人项目。
  ...
  * [gigalixir.com](https://gigalixir.com/)——Gigalixir 提供一个永不休眠的免费实例和一个免费的 PostgreSQL 数据库，限制为 2 个连接、10,000 行，并且不提供备份，适用于 Elixir/Phoenix 应用。
  * [Glitch](https://glitch.com/)——具有代码共享和实时协作功能的免费公共托管。免费计划每月有 1000 小时的限制。
  * [Lade](https://www.lade.io/)——Lade 是为开发人员构建的云平台。免费层包括三个服务，可作为应用程序或数据库。每个服务包括 128MB RAM 和 1GB 存储。
  ...
  * [pythonanywhere.com](https://www.pythonanywhere.com/)——云 Python 应用托管。入门帐户是免费的，在您的-username.pythonanywhere.com 域中有 1 个 Python Web 应用程序，512MB 私有文件存储，1 个 MySQL 数据库
  ...
  * [Napkin](https://www.napkin.io/)——具有 500Mb 内存、默认超时时间为 15 秒和每月 5000 次免费 API 调用（限制为每秒 5 次调用）的 FaaS。
  ...
  * [WunderGraph](https://cloud.wundergraph.com)——一个开源平台，允许您快速构建、交付和管理现代 API。内置 CI/CD、GitHub 集成和自动 HTTPS。免费计划每月最多可使用 3 个项目、1GB 出站流量和 300 分钟构建时间[免费计划](https://wundergraph.com/pricing)
...
## 后端即服务（BaaS）
  ...
  * [Activepieces](https://www.activepieces.com) - 使用自动化流程将多个应用程序连接到您的应用程序后端。例如，当您的应用程序中发生事件时，发送一条 Slack 消息或向 Google Sheet 中添加一行。每月免费使用高达 5000 个任务。
  * [back4app.com](https://www.back4app.com) - Back4App 是一个易于使用、灵活且可扩展的后端，基于 Parse 平台。
  * [backendless.com](https://backendless.com/) - 移动和网络 Baas，提供 1GB 文件存储免费，每月推送通知 50,000 个，表中 1000 个数据对象。
  * [bismuth.cloud](https://www.bismuth.cloud/) - 我们的人工智能将在我们的函数运行时和托管存储上引导您的 Python API，在我们的在线编辑器中免费构建和托管，或在您喜欢的工具中本地构建和托管。
  * [BMC Developer Program](https://developers.bmc.com/site/global/bmc_helix_platform/program/overview/index.gsp) - BMC Developer Program 提供文档和资源，用于为您的企业构建和部署数字创新。访问全面的个人沙盒，其中包括平台、SDK 和可用于构建和定制应用程序的组件库。
  * [connectycube.com](https://connectycube.com) - 无限聊天消息、p2p 语音和视频通话、文件附件和推送通知。对于最多 1000 个用户的应用程序免费。
  * [convex.dev](https://convex.dev/) - 作为服务的反应式后端，托管您的数据（具有关系和可序列化 ACID 事务的文档）、无服务器函数和 WebSockets，以将更新流式传输到各个客户端。小型项目免费 - 最多 1M 记录，每月 5M 函数调用。
  ...
  * [hasura.io](https://hasura.io/) - Hasura 扩展了您所在位置的现有数据库，并提供了一个即时 GraphQL API，可安全访问网络、移动和数据集成工作负载。免费 1GB/月的数据通过量。
  ...
  * [onesignal.com](https://onesignal.com/) - 无限免费推送通知。每月 10,000 封电子邮件发送，无限制的联系人并访问自动预热。
  ...
  * [pushcrew.com](https://pushcrew.com/) - 推送通知服务。对于最多 2000 个订阅者，无限制通知
  * [pusher.com](https://pusher.com/beams) - 免费、无限制的推送通知，适用于每月 2000 名活跃用户。适用于 iOS 和 Android 设备的单个 API。
  ...
  * [Salesforce Developer Program](https://developer.salesforce.com/signup) - 使用拖放工具快速构建、测试和部署应用程序。免费开发者计划可访问完整的 Lightning 平台。
  * [ServiceNow Developer Program](https://developer.servicenow.com/) - 快速构建、测试和部署可改善您组织工作的应用程序。免费实例和早期预览访问权限。
  * [simperium.com](https://simperium.com/) - 立即在各个位置自动移动数据，多平台，无限制发送和存储结构化数据，每月最多 2500 个用户。
  ...
  * [zapier.com](https://zapier.com/) - 将您使用的应用程序连接起来以自动化任务。每 15 分钟五个 zaps，每月 100 个任务
  * [IFTTT](https://ifttt.com) - 自动化您喜欢的应用程序和设备。免费 2 个小程序
...
## 低代码平台
  ...
  * [UI Bakery](https://uibakery.io)——一个低代码平台，能够更快地构建自定义 Web 应用程序。支持使用拖放构建 UI，并通过 JavaScript、Python 和 SQL 进行高度定制。提供云和自托管解决方案。免费供最多 5 个用户使用。
  * [Mendix](https://www.mendix.com/)——企业快速应用开发，无限制的可访问沙盒环境支持总用户，每个应用程序 0.5GB 存储空间和 1GB RAM。免费层还允许使用 Studio 和 Studio Pro IDE。
...
## 虚拟主机（WebHosting）
  ...
  * [DigitalOcean](https://www.digitalocean.com/pricing)——在应用平台入门层上免费构建和部署三个静态网站。
  ...
  * [tilda.cc](https://tilda.cc/)——一个站点，50 页，50MB 存储，仅提供 170 多个可用的主要预定义块，无字体，无 favicon，无自定义域名。
...
## 域名系统（DNS）
  ...
  * [1.1.1.1](https://developers.cloudflare.com/1.1.1.1/) - 由 Cloudflare 提供的免费公共 DNS 解析器，速度快且安全（加密 DNS 查询）。可用于绕过互联网提供商的 DNS 阻塞、防止 DNS 查询监视以及[阻止成人和恶意软件内容](https://developers.cloudflare.com/1.1.1.1/1.1.1.1-for-families)。它也可以通过[API 使用](https://developers.cloudflare.com/1.1.1.1/encrypted-dns/dns-over-https/make-api-requests)。注意：只是 DNS 解析器，不是 DNS 主机。
  * [1984.is](https://www.1984.is/product/freedns/)— 带有 API 和许多其他免费 DNS 功能的免费 DNS 服务。
  * [cloudns.net](https://www.cloudns.net/)—免费 DNS 托管，最多可托管 1 个域名，记录数为 50 条。
  ...
  * [noip.at](https://noip.at/)—免费 DDNS 服务，无需注册、跟踪、记录或广告。域名数量不限。
  ...
  * [Hetzner](https://www.hetzner.com/dns-console)—具有 API 支持的免费 DNS 托管，来自 Hetzner。
...
## 托管数据服务
  ...
  * [SeaTable](https://seatable.io/) — 由 Seafile 团队构建的灵活、类似电子表格的数据库。无限表格，2,000 行，1 个月版本控制，最多 25 个团队成员。
  ...
  * [Turso by ChiselStrike](https://chiselstrike.com/) - Turso 是边缘数据库中的 SQLite 开发人员体验。Turso 提供免费永久入门计划，总存储量为 9GB，最多可容纳 500 个数据库，最多可容纳 3 个位置，每月读取 10 亿行数据，并提供本地开发支持和 SQLite。
  * [InfluxDB](https://www.influxdata.com/) — 时间序列数据库，免费使用高达 3MB/5 分钟写入，30MB/5 分钟读取和 10,000 个基数系列
  ...
  * [Neo4j Aura](https://neo4j.com/cloud/aura/) — 管理本机图形数据库管理系统/分析平台，具有 Cypher 查询语言和 REST API。图形大小限制（50k 个节点，175k 个关系）。
  * [Neon](https://neon.tech/) — 托管 PostgreSQL，总存储量为 0.5GB，1 个项目，10 个分支，无限数据库，始终可用的主分支（非主分支计算后 5 分钟自动暂停），每月活动时间 20 小时
  * [Dgraph Cloud](https://cloud.dgraph.io/pricing?type=free) — 管理本机图形数据库管理系统，具有 GraphQL API。每天限制 1MB 数据传输。
  * [Tinybird](https://tinybird.co) - 无服务器管理的 ClickHouse，通过 HTTP 进行无连接数据摄取，并允许您将 SQL 查询发布为托管 HTTP API。免费层没有时间限制，10GB 存储+每天 1000 个 API 请求。
  ...
  * [TerminusCMS](https://terminusdb.com/pricing) — TerminusDB 的托管免费服务，一种用 Prolog 和 Rust 编写的文档和图形数据库。免费用于开发，付费用于企业部署和支持。
  ...
  * [8base.com](https://www.8base.com/) - 8base 是一个为 JavaScript 开发人员构建的全栈低代码开发平台，构建在 MySQL 和 GraphQL 之上，以及无服务器后端即服务之上。它允许您使用 UI 应用程序构建器快速构建 Web 应用程序并快速扩展，免费层包括行：2,500，存储：500，无服务器计算：1Gb/h，客户端应用程序用户：5。
...
## 隧道、WebRTC、WebSocket服务器和其他路由器
  ...
  * [Hamachi](https://www.vpn.net/) - LogMeIn Hamachi 是一项托管 VPN 服务，允许你使用免费计划（最多允许 5 人使用，无限制网络）安全地将类似 LAN 的网络扩展到分布式团队。
  * [Mirna Sockets](https://mirna.cloud/) - 免费的套接字即服务平台，当你部署 Web Socket 服务器代码时，它会为你提供一个 wss://URL，还允许你监控其性能。
  ...
  * [Radmin VPN](https://www.radmin-vpn.com/) - 通过 VPN 启用的类似 LAN 的网络将多台计算机连接在一起。无限对等点。（Hamachi 替代方案）
  * [segment.com](https://segment.com/) - 用于将事件翻译成并路由到其他第三方服务的中心。免费计划每月 100,000 个事件和 100,000 次尝试，保留三天。
  * Google STUN - [stun:stun.l.google.com:19302](stun:stun.l.google.com:19302)
  * Twilio STUN - [stun:global.stun.twilio.com:3478?transport=udp](stun:global.stun.twilio.com:3478?transport=udp)
  * [Tailscale](https://tailscale.com/) - 使用开放源代码的 WireGuard 协议的零配置 VPN。安装在 MacOS、iOS、Windows、Linux 和 Android 设备上。个人免费计划可使用 100 个设备和 3 个用户。
  * [webhookrelay.com](https://webhookrelay.com) - 管理、调试、分流和代理你所有的 Webhook 到公共或内部（即 localhost）目的地。此外，通过获取公共 HTTP 终结点（`https://yoursubdomain.webrelay.io <----> http://localhost:8080`），可以将在私有网络中运行的服务器暴露出来。
  * [Hookdeck](https://hookdeck.com/pricing) - 在任何地方开发、测试和监控你的 Webhook。每月 100K 请求和 100K 尝试，保留三天。
  * [Xirsys](https://www.xirsys.com/pricing/) - 无限 STUN 使用+500MB 每月 TURN 带宽，限制带宽，单个地理区域。
  * [ZeroTier](https://www.zerotier.com) - 免费的托管虚拟以太网即服务。免费计划可在一个网络中拥有 25 个客户端的无限端到端加密网络。客户端适用于桌面/移动/NA；Web 界面用于配置自定义路由规则和批准私有网络上的新客户端节点。
...
## 问题跟踪和项目管理
  ...
  * [asana.com](https://asana.com/)——私人项目免费，最多可与合作者一起使用
  ...
  * [Cloudcraft](https://cloudcraft.co/)——使用 Cloudcraft 可视化设计器在几分钟内设计专业的架构图，该设计器针对 AWS 进行了优化，具有智能组件，可显示实时数据。免费计划为单个用户提供无限私有图表。
  * [Codegiant](https://codegiant.io)——项目管理与存储库托管和 CI/CD。免费计划提供无限存储库、项目和文档，最多 5 个团队成员。每月 500 分钟的 CI/CD 分钟数。每月 30000 个无服务器代码运行分钟数 1GB 存储库存储。
  ...
  * [Crosswork](https://crosswork.app/)——通用项目管理平台。免费最多 3 个项目，无限用户，1GB 存储空间。
  ...
  * [easyretro.io](https://www.easyretro.io/)——简单直观的冲刺回顾工具。免费计划每月有三个公共板和每个板上的一个调查。
  ...
  * [Instabug](https://instabug.com)——适用于移动应用的全面错误报告和应用内反馈 SDK。免费计划最多适用于 1 个应用和 1 个成员。
  * [WishKit](https://wishkit.io)——为您的 iOS/macOS 应用收集应用内用户反馈，并根据用户投票确定功能的优先级。免费计划最多适用于 1 个应用。
  ...
  * [Kitemaker.co](https://kitemaker.co)——通过 Slack、Discord、Figma 和 Github 协作完成产品开发过程的各个阶段，并跟踪工作。无限用户，无限空间。免费计划最多可处理 250 个工作项。
  * [Kiter.app](https://www.kiter.app/)——让任何人组织他们的求职并跟踪面试、机会和联系。功能强大的网络应用程序和 Chrome 扩展程序。完全免费。
  * [Kumu.io](https://kumu.io/)——具有动画、装饰、过滤器、聚类、电子表格导入等功能的关系图。免费层允许无限公开项目。图形大小不受限制。免费的学生私人项目。如果您不想将文件公开在线（上传、编辑、下载、丢弃），可以使用沙盒模式。
  * [Linear](https://linear.app/)——界面简洁的问题跟踪器。免费供无限成员使用，文件上传大小最大为 10MB，问题数量（不包括存档）最多为 250 个。
  ...
  * [nTask](https://www.ntaskmanager.com/)——项目管理软件，使您的团队能够协作、计划、分析和管理日常任务。Essential 计划永久免费，存储空间为 100MB，用户/团队数量不限。无限工作空间、会议、任务分配、时间表和问题跟踪。
  * [Ora](https://ora.pm/)——敏捷任务管理和团队协作。免费最多供 3 个用户使用，文件大小限制为 10MB。
  * [pivotaltracker.com](https://www.pivotaltracker.com/)——免费供无限公共项目和两个私人项目使用，每个项目最多有三个活跃用户（读写）和无限个被动用户（只读）。
  * [plan.io](https://plan.io/)——项目管理，带有存储库托管和更多选项。免费供两个用户使用，最多有十个客户和 500MB 存储空间。
  * [Plane](https://plane.so/)——Plane 是一个简单、可扩展、开源的项目和产品管理工具。免费供无限成员使用，文件上传大小最大为 5MB，问题数量最多为 1000 个。
  ...
  * [ScrumFast](https://www.scrumfast.com)——具有非常直观界面的 Scrum 看板，免费最多供 5 个用户使用。

  * [快捷键](https://shortcut.com/) - 项目管理平台。免费使用，最多可容纳 10 名用户，永久免费。
  ...
  * [taskade.com](https://www.taskade.com/)——实时协作任务列表和团队大纲。免费计划有一个工作区，无限任务和项目；1GB 文件存储；项目历史记录为 1 周；以及每次视频会议最多 5 名与会者。
  * [taskulu.com](https://taskulu.com/)——基于角色的项目管理。免费最多 5 个用户。与 GitHub/Trello/Dropbox/Google Drive 集成
  * [Teaminal](https://www.teaminal.com) - 适用于远程团队的站立会议、回顾和冲刺规划工具。免费供最多 15 名用户使用。
  ...
  * [testlio.com](https://testlio.com/)——问题跟踪、测试管理和测试版测试平台。私人使用免费
  * [terrastruct.com](https://terrastruct.com/)——专门用于软件架构的在线图表制作工具。免费层图数量最多为 4 层。
  * [todoist.com](https://todoist.com/)——协作和个人任务管理。免费计划包括：5 个活动项目、项目中的 5 个用户、上传文件最大 5MB、3 个过滤器和一周的活动历史记录。
  ...
  * [Wikifactory](https://wikifactory.com/)——具有项目、VCS 和问题的产品设计服务。免费计划提供无限项目和协作者以及 3GB 存储。
  * [Yodiz](https://www.yodiz.com/)——敏捷开发和问题跟踪。免费最多 3 个用户，无限项目。
  * [YouTrack](https://www.jetbrains.com/youtrack/buy/#edition=incloud)——免费托管的 YouTrack（InCloud）适用于 FOSS 项目和私人项目（免费供 3 个用户）。包括时间跟踪和敏捷看板
  * [zenhub.com](https://www.zenhub.com)——GitHub 内部唯一的项目管理解决方案。免费适用于公共存储库、OSS 和非营利组织
  * [zenkit.com](https://zenkit.com)——项目管理和协作工具。免费供最多 5 名成员使用，5GB 附件。
  * [Zube](https://zube.io)——具有免费计划的项目管理，适用于 4 个项目和 4 个用户。提供 GitHub 集成。
  * [Toggl](https://toggl.com/)——提供两个免费的生产力工具。[Toggl Track](https://toggl.com/track/)用于时间管理和跟踪应用程序，具有免费计划，提供无缝时间跟踪和报告，专为自由职业者设计。它具有无限的跟踪记录、项目、客户端、标签、报告等。和[Toggl Plan](https://toggl.com/plan/)用于任务规划，具有免费计划，适用于独奏开发人员，无限任务、里程碑和时间线。
  * [Sflow](https://sflow.io)——sflow.io 是为敏捷软件开发、营销、销售和客户支持构建的项目管理工具，特别是对于外包和跨组织协作项目。免费计划最多可容纳 3 个项目和 5 个成员。
...
## 存储和媒体处理
  ...
  * [AndroidFileHost](https://androidfilehost.com/) - 一个免费的文件共享平台，速度、带宽、文件数量、下载次数等不受限制。它主要面向与 Android 开发相关的文件，如 APK 构建、自定义 ROM 和修改等，但似乎也接受其他任何文件。
  * [borgbase.com](https://www.borgbase.com/) — 用于 Borg Backup 的简单安全的离线备份托管。免费 10GB 备份空间和两个存储库。
  * [icedrive.net](https://www.icedrive.net/) - 简单的云存储服务。免费 10GB 存储空间
  * [sync.com](https://www.sync.com/) - 端到端云存储服务。免费 5GB 存储空间
  * [pcloud.com](https://www.pcloud.com/) - 云存储服务。最多可免费获得 10GB 存储空间
  * [sirv.com](https://sirv.com/) — 带有即时图像优化和调整大小功能的智能图像 CDN。免费层包括 500MB 存储空间和 2GB 带宽。
  * [cloudimage.io](https://www.cloudimage.io/en/home) — 全球拥有 1500 多个节点的全面图像优化和 CDN 服务。具有多种图像缩放、压缩和水印功能。适用于响应式图像、360 度图像制作和图像编辑的开源插件。免费计划每月提供 25GB 的 CDN 流量、25GB 的缓存存储和无限转换。
  * [cloudinary.com](https://cloudinary.com/) — 适用于网站和应用程序的图像上传、强大的操作、存储和交付，具有 Ruby、Python、Java、PHP、Objective-C 等更多库。免费层包括 25 个每月积分。一个信用等于 1000 次图像转换、1GB 存储空间或 1GB CDN 使用量。
  * [embed.ly](https://embed.ly/) — 为网页中的媒体嵌入、响应式图像缩放和从网页中提取元素提供 API。免费使用，每月最多可处理 5000 个 URL，每秒 15 个请求
  * [filestack.com](https://www.filestack.com/) — 文件选择、转换和交付，免费 250 个文件、500 次转换和 3GB 带宽
  * [file.io](https://www.file.io) - 2GB 文件存储。文件下载一次后自动删除。提供与存储交互的 REST API。请求限制为每分钟一个请求。
  * [freetools.site](https://freetools.site/) — 免费在线工具。转换或编辑文档、图像、音频、视频等。
  * [GoFile.io](https://gofile.io/) - 免费文件共享和存储平台，可通过基于网络的 UI 和 API 使用。无文件大小、带宽、下载次数等限制。但文件在不活动（10 天以上没有下载）时会被删除。
  ...
  * [Imgbot](https://github.com/marketplace/imgbot) — Imgbot 是一个友好的机器人，可优化您的图像并为您节省时间。优化后的图像意味着更小的文件大小，而不会牺牲质量。对开源免费。
  ...
  * [nitropack.io](https://nitropack.io/) - 通过完整的前端优化（缓存、图像和代码优化、CDN）自动加速您网站的速度。免费计划适用于每月最多 5000 次页面浏览量
  ...
  * [piio.co](https://piio.co/) — 适用于每个网站的响应式图像优化和交付。开发人员和个人网站的免费计划。免费 CDN、WebP 和懒加载开箱即用。
  ...
  * [redbooth.com](https://redbooth.com) — P2P 文件同步，最多可供 2 个用户使用
  ...
  * [Shotstack](https://shotstack.io) - 用于按比例生成和编辑视频的 API。每月免费最多 20 分钟的渲染视频
  * [tinypng.com](https://tinypng.com/) — 用于压缩和调整 PNG 和 JPEG 图像大小的 API，每月免费提供 500 次压缩
  * [transloadit.com](https://transloadit.com/) — 处理文件上传以及视频、音频、图像、文档的编码。免费适用于开源、慈善机构和学生，通过 GitHub 学生开发者包。商业应用程序在测试驱动时可获得 2GB 免费使用
  * [twicpics.com](https://www.twicpics.com) - 响应式图像即服务。它提供图像 CDN、媒体处理 API 和前端库，可自动优化图像。服务免费，每月最多可使用 3GB 流量。
  * [uploadcare.com](https://uploadcare.com/hub/developers/) — Uploadcare 为开发人员提供了基于最先进算法的媒体管道的终极工具包。所有功能对开发人员完全免费：文件上传 API 和 UI、图像 CDN 和源服务、自适应交付和智能压缩。免费层包括 3000 次上传、3GB 流量和 3GB 存储。
  * [imagekit.io](https://imagekit.io) - 具有自动优化、实时转换和存储的图像 CDN，您可以在几分钟内集成到现有设置中。免费计划每月包括高达 20GB 的带宽。
  * [internxt.com](https://internxt.com) - Internxt Drive 是基于绝对隐私和毫不妥协的安全性的零知识文件存储服务。注册并免费获得 10GB，永远！
  * [degoo.com](https://degoo.com/) - 基于人工智能的云存储，免费提供高达 20GB，三个设备，5GB 推荐奖金（90 天账户不活动）。

  * [ImageEngine](https://imageengine.io/)——ImageEngine 是一个易于使用的全球图像 CDN。设置时间在 60 秒以内。支持 AVIF 和 JPEGXL，有 WordPress、Magento、React、Vue 等插件等。在此[申请](https://imageengine.io/developer-program/)免费开发者账号。
...
## 设计和用户界面（UI）
  ...
  * [Float UI](https://floatui.com/) - 一个免费的 Web 开发工具，用于快速创建具有时尚设计的现代响应式网站，即使对于非设计师也是如此。
  * [Ant Design Landing Page](https://landing.ant.design/) - Ant Design Landing Page 提供了一个由 Ant Motion 的运动组件构建的模板。它有一个丰富的主页模板，下载模板代码包，可以快速使用。您还可以使用编辑器快速构建自己的专用页面。
  * [Backlight](https://backlight.dev/)——以开发人员和设计师的合作为核心，Backlight 是一个完整的编码平台，团队可以在其中构建、记录、发布、扩展和维护设计系统。免费计划允许最多 3 个编辑器在一个设计系统上工作，没有限制的查看者。
  ...
  * [css-gradient.com](https://www.css-gradient.com/) - 免费工具，可快速生成自定义跨浏览器 CSS 梯度。以 RGB 和 HEX 格式。
  * [easyvectors.com](https://easyvectors.com/)——免费的 SVG 向量艺术库存。下载最佳的向量图形绝对免费。
  * [figma.com](https://www.figma.com)——在线团队协作设计工具；免费层包括无限文件和查看器，最大有 2 个编辑器和三个项目。
  * [Flyon UI](https://flyonui.com/) - 用于 Tailwind CSS 的最简单组件库。
  * [framer.com](https://www.framer.com/) - Framer 帮助您为您的下一个应用程序、网站或产品迭代和动画界面想法——从强大的布局开始。对于将 Framer 验证为专业原型制作工具的任何人：无限的查看者，最多 2 个编辑器和最多 3 个项目。
  ...
  * [landen.co](https://www.landen.co)——生成、编辑和发布您的初创公司的漂亮网站和登录页面。无需代码。免费层允许您拥有一个网站，完全可定制并发布在网络上。
  ...
  * [LottieFiles](https://lottiefiles.com/) - 世界上最大的设计师、开发人员等在线平台，用于世界上最小的动画格式。访问 Android、iOS 和 Web 的 Lottie 动画工具和插件。
  * [MagicPattern](https://www.magicpattern.design/tools)——一个 CSS 和 SVG 背景生成器和工具的集合，用于梯度、图案和斑点。
  * [marvelapp.com](https://marvelapp.com/)——设计、原型制作和协作，免费计划仅限于一个用户和一个项目。
  * [Mindmup.com](https://www.mindmup.com/)——无限的思维导图免费，并存储在云中。您的思维导图随时随地，从任何设备都可以立即使用。
  ...
  * [Proto.io](https://www.proto.io) - 创建无需编码的全交互式 UI 原型。免费层在免费试用结束后可用。免费层包括一个用户、一个项目、五个原型、100MB 的在线存储和 proto.io 应用程序的预览。
  ...
  * [tabler-icons.io](https://tabler-icons.io/) - 1500 多个免费可复制和粘贴的 SVG 可编辑图标。
  * [UI Avatars](https://ui-avatars.com/) - 根据名称的首字母生成头像。URL 可以直接在您的网络/应用程序中链接。支持通过 URL 配置参数。
  ...
  * [xLayers](https://xlayers.dev)——预览和将 Sketch 设计文件转换为 Angular、React、Vue、LitElement、Stencil、Xamarin 等（免费和开源，网址为 [https://github.com/xlayers/xlayers](https://github.com/xlayers/xlayers)）
  ...
  * [Unicorn Platform](https://unicornplatform.com/)——轻松的 landing 页构建器，提供托管。免费一个网站。
  * [react-favicon.com](https://react-favicon.com/)——使用 React 和 JSX 使用任何字体和图标库为您的网站生成 favicons。
  * [SVGmix.com](https://www.svgmix.com/)——拥有 30 多万个免费 SVG 图标、集合和品牌徽标的大型存储库。它在浏览器中提供了一个简单的向量编辑程序，用于快速文件编辑。
  ...
  * [Superdesigner](https://superdesigner.co)——一系列免费的设计工具，可通过点击几下创建独特的背景、图案、形状、图像等。
  * [TeleportHQ](https://teleporthq.io/)——低代码前端设计和开发平台。TeleportHQ 是一个协作前端平台，可立即创建和发布无后端静态网站。三个免费项目，无限协作者，免费代码导出。
  * [vector.express](https://vector.express)——快速、轻松地转换您的 AI、CDR、DWG、DXF、EPS、HPGL、PDF、PLT、PS 和 SVG 向量。
  ...
  * [Logo.dev](https://www.logo.dev)——具有 4400 万多个品牌的公司徽标 API，就像调用 URL 一样简单。前 10,000 次 API 调用免费。
  * [MDBootstrap](https://mdbootstrap.com/)——免费用于个人和商业使用的 Bootstrap、Angular、React 和 Vue UI 工具包，带有 700 多个组件、精美的模板、1 分钟安装、广泛的教程和庞大的社区。
  ...
  * [iconify.design](https://icon-sets.iconify.design/)——超过 100 个图标的集合，具有统一的界面。允许您在包中搜索图标并将单个图标导出为 SVG 或用于流行的网络框架。
  ...
  * [Webstudio](https://webstudio.is/)——Webflow 的开源替代方案。免费计划在其域名上提供无限数量的网站。五个带自定义域名的网站。每月十万次页面浏览量。2GB 资产存储。
...
## 设计灵感
  ...
  * [Landings](https://landings.dev/) - [网页截图] 根据您的偏好，找到您设计灵感的最佳着陆页。
  * [Lapa Ninja](https://www.lapa.ninja/) - [着陆页/UI 套件/网页截图] Lapa Ninja 是一个画廊，展示了最好的 6025 个着陆页示例，为设计师提供免费书籍和免费的 UI 套件。
  * [LovelyLanding.net](https://www.lovelylanding.net/) - [着陆页设计] 定期更新的着陆页屏幕截图。包括桌面、平板电脑和手机屏幕截图。
  * [Mobbin](https://mobbin.design/) - [手机屏幕截图] 通过我们的 50,000+个可完全搜索的手机应用程序屏幕截图库，节省数小时的 UI&UX 研究时间。
  * [Uiland Design](https://uiland.design/) - [手机屏幕截图] 探索来自非洲和世界领先公司的移动和网络 UI 设计。
  * [Mobile Patterns](https://www.mobile-patterns.com/) - [手机屏幕截图] 一个设计灵感库，其中包含设计师、开发人员和产品制造商可参考的 iOS 和 Android 上最精美的 UI UX 模式。
  * [Page Flows](https://pageflows.com/) - [手机/web 视频和屏幕截图] 许多手机和网络应用的完整流程视频。还包括屏幕截图。可高度搜索和索引。
  * [Screenlane](https://screenlane.com/) - [手机屏幕截图] 获取灵感并跟上最新的网络和移动应用程序 UI 设计趋势。可按模式和应用程序进行筛选。
  * [scrnshts](https://scrnshts.club/) - [手机屏幕截图] 精心挑选的最佳应用商店设计屏幕截图集合。
  * [UI Garage](https://uigarage.net/) - [手机/web 屏幕截图] 设计师和开发人员日常 UI 灵感和模式，以找到项目的灵感、工具和最佳资源。
...
## 地图上的数据可视化
  ...
  * [Clockwork Micro](https://clockworkmicro.com/)——像时钟一样精确的地图工具。每月免费查询 50,000 次（地图瓦片、db2vector、高程）。
  * [developers.arcgis.com](https://developers.arcgis.com)——用于地图、地理空间数据存储、分析、地理编码、路由等的 API 和 SDK。每月免费提供 200 万张基础地图瓦片、20,000 个未存储的地理编码、20,000 条简单路线、5,000 次驾车时间计算以及 5GB 免费的瓦片+数据存储。
  ...
  * [nextbillion.ai](https://nextbillion.ai/)——与地图相关的服务：地理编码、导航（方向、路由、路线优化、距离矩阵）、地图 SDK（矢量、静态、移动 SDK）。[免费，有特定配额](https://nextbillion.ai/pricing)。
  ...
  * [positionstack](https://positionstack.com/)——免费为全球地点和坐标进行地理编码。每月个人使用 25,000 次请求。
...
## 集成开发环境（IDE）和代码编辑
  ...
  * [apiary.io](https://apiary.io/) - 具有即时 API 模拟和生成文档的协作式设计 API（免费提供无限的 API 蓝图和无限数量的用户，每个管理员帐户一个，托管文档）。
  ...
  * [BlueJ](https://bluej.org) - 一款免费的 Java 开发环境，专为初学者设计，全球有数百万用户使用。由 Oracle 提供支持，具有简单的 GUI，可帮助初学者。
  ...
  * [cacher.io](https://www.cacher.io) - 具有标签支持的代码片段组织者，支持 100 多种编程语言。
  * [Code::Blocks](https://codeblocks.org) - 免费的 Fortran 和 C / C++ IDE。开源，可在 Windows、macOS 和 Linux 上运行。
  * [Cody](https://sourcegraph.com/cody) - 免费的人工智能编码助手，可编写（代码块、自动完成、单元测试）、理解（整个代码库的知识）、修复和查找代码。适用于 VS Code、JetBrains 和在线使用。
  * [codiga.io](https://codiga.io/) - 在 IDE 中直接搜索、定义和重用代码片段的编码助手。个人和小组织免费使用。
  * [codesnip.com.br](https://codesnip.com.br) - 具有类别、搜索和标签的简单代码片段管理器。免费且无限。
  * [cocalc.com](https://cocalc.com/) - （以前是 cloud.sagemath.com 上的 SageMathCloud）- 云协作计算。通过内置协作和大量免费软件（包括 Python、LaTeX、Jupyter Notebooks、SageMath、scikitlearn 等）访问完整的 Ubuntu。
  * [code.cs50.io](https://code.cs50.io/) - 适用于 CS50 的 Visual Studio Code 是 code.cs50.io 上的网络应用程序，它为学生和教师适应了 GitHub Codespaces。
  ...
  * [Eclipse Che](https://www.eclipse.org/che/) - 用于开发团队的基于 Web 和 Kubernetes 的 IDE，支持多种语言。开源且由社区驱动。红帽提供了一个在线实例，可在[workspaces.openshift.com](https://workspaces.openshift.com/)访问。
  ...
  * [ide.goorm.io](https://ide.goorm.io) goormIDE 是云全功能 IDE。多语言支持，基于 Linux 的容器通过功能齐全的基于网络的终端、端口转发、自定义 URL、实时协作和聊天、共享链接、Git / Subversion 支持。还有许多其他功能（免费层包括每个容器 1GB RAM 和 10GB 存储，5 个容器插槽）。
  * [JDoodle](https://www.jdoodle.com) - 用于 60 多种编程语言的在线编译器和编辑器，免费计划可用于 REST API 代码编译，每天最多 200 个信用额度。
  ...
  * [Lazarus](https://www.lazarus-ide.org/) - Lazarus 是一个适用于 Rapid Application Development 的与 Delphi 兼容的跨平台 IDE。
  ...
  * [mockaroo](https://mockaroo.com/) - 允许您以 CSV、JSON、SQL 和 Excel 格式生成逼真的测试数据。您还可以为后端 API 创建模拟。
  ...
  * [Prepros](https://prepros.io/) - Prepros 可以开箱即用编译 Sass、Less、Stylus、Pug / Jade、Haml、Slim、CoffeeScript 和 TypeScript，重新加载浏览器，使开发和测试变得容易，您可以专注于使它们完美。您还可以通过点击几下添加自己的工具。
...
## 分析、事件和统计
  ...
  * [Avo](https://avo.app/)——简化的分析发布工作流程。单一来源的跟踪计划、类型安全的分析跟踪库、应用内调试程序和数据观测，可在发布前捕获所有数据问题。两个工作区成员和 1 小时数据观测回溯免费。
  ...
  * [Databox](https://databox.com)——通过结合其他分析和 BI 平台提供业务洞察力和分析。免费计划提供 3 个用户、仪表板和数据源。1100 万条历史数据记录。
  * [Hitsteps.com](https://hitsteps.com/)——每月 2000 次页面浏览量，适用于一个网站
  * [amplitude.com](https://amplitude.com/)——每月 100 万次事件，最多 2 个应用程序
  * [GoatCounter](https://www.goatcounter.com/)——GoatCounter 是一个开源的网络分析平台，可作为托管服务（免费用于非商业用途）或自托管应用程序使用。它旨在提供易于使用和有意义的隐私友好型网络分析，作为 Google Analytics 或 Matomo 的替代方案。免费层适用于非商业用途，包括无限站点、6 个月的数据保留和每月 10 万次页面浏览量。
  ...
  * [heap.io](https://heap.io)——自动捕获 iOS 或网络应用中的每个用户操作。免费提供高达 10K 个每月会话。
  * [Hotjar](https://hotjar.com)——网站分析和报告。免费计划允许每天 2000 次页面浏览量。每天一百个快照（最大容量：300）。可以存储三个快照热图 365 天。无限的团队成员。还包括应用内调查和反馈小部件以及屏幕截图。免费层允许每月创建 3 个调查和 3 个反馈小部件并收集 20 个响应。
  * [Keen](https://keen.io/)——自定义分析，用于数据收集、分析和可视化。每月 1000 个事件免费
  ...
  * [Similar Web](https://similarweb.com)——适用于网络和移动应用的分析。免费计划提供每个指标的五个结果，一个月的移动应用数据和三个月的网站数据。
  ...
  * [Beampipe.io](https://beampipe.io)——Beampipe 是一种简单、注重隐私的网络分析。免费适用于最多 5 个域和 10k 每月页面浏览量。
  * [Aptabase](https://aptabase.com)——开源、隐私友好且简单的移动和桌面应用分析。适用于 Swift、Kotlin、React Native、Flutter、Electron 等的 SDK。免费适用于每月最多 20,000 个事件。
  ...
  * [TrackWith Dicloud](https://dicloud.net/trackwith-privacy-focused-analytics/)——免费的轻量级注重隐私的 Google Analytics 替代方案。无限制的页面浏览量、无限制的访问者、无限制的页面热图和目标跟踪。免费适用于最多 3 个域和每个域 600 次会话回放。
  * [AppFit](https://appfit.io)——AppFit 是一款全面的分析和产品管理工具，旨在促进跨平台管理分析和产品更新的无缝衔接。免费计划包括每月 10,000 次事件、产品日记和每周洞察。

**[⬆️ 返回顶部](# 目录 )**

## 访问者会话记录（VisitorSessionRecording）

  * [Reactflow.com](https://www.reactflow.com/)——按站点计费：每天 1000 页浏览量，三个热点图，三个小部件，免费错误跟踪
  * [OpenReplay.com](https://www.openreplay.com)——开源会话重放，具有用于错误重现的开发工具、实时会话支持和产品分析套件。每月一千个会话，可访问所有功能和 7 天保留期。
  ...
  * [hotjar.com](https://www.hotjar.com/)——按站点计费：每月 1050 页浏览量，无限制热点图，数据存储三个月
  * [inspectlet.com](https://www.inspectlet.com/)——一个网站每月 2500 次会话免费
  * [Microsoft Clarity](https://clarity.microsoft.com/)——会话记录完全免费，“无流量限制”，无项目限制，也不进行采样
  * [mouseflow.com](https://mouseflow.com/)——一个网站每月 500 次会话免费
  * [mousestats.com](https://www.mousestats.com/)——一个网站每月 100 次会话免费
  * [smartlook.com](https://www.smartlook.com/)——网络和移动应用的免费套餐（每月 1500 次会话），三个热点图，一个漏斗，1 个月数据历史
...
## 支付和计费集成
  ...
  * [currencylayer](https://currencylayer.com/)——可靠的汇率和货币转换服务，适用于您的企业，每月免费 100 次 API 请求。
  ...
  * [Nami ML](https://www.namiml.com/)——适用于 iOS 和 Android 的应用内购买和订阅的完整平台，包括无代码支付墙、CRM 和分析。免费使用所有基本功能来运行 IAP 业务。
  ...
  * [Currencyapi](https://currencyapi.com)——免费的货币转换和汇率数据 API。每月免费 300 次请求，每分钟 10 次请求供个人使用。
...
## 开发博客网站
  ...
  * [BearBlog](https://bearblog.dev/) - 一款极简的、基于 Markdown 的博客和网站生成器。
...
## 评论平台
  ...
  * [Disqus](https://disqus.com/) - Disqus 是一个网络社区平台，被数十万网站使用。
  * [Remarkbox](https://www.remarkbox.com/) - 开源托管评论平台，可按需支付“在几个域上有一位管理员，完全控制行为和外观”
...
## 屏幕截图API
  ...
  * [ApiFlash](https://apiflash.com)——基于 Aws Lambda 和 Chrome 的截图 API。处理整页截图、捕获时间和viewport 尺寸。
  * [microlink.io](https://microlink.io/)——它将任何网站转换为数据，例如元标记规范化、美化链接预览、抓取功能或截图即服务。每天免费 250 次请求。
...
## 与Flutter相关和无需Mac即可构建IOS应用程序
  ...
  * [CodeMagic](https://codemagic.io/) - CodeMagic 是一个完全托管和管理的移动应用 CI/CD。您可以使用基于 GUI 的 CI/CD 工具进行构建、测试和部署。免费层提供 500 分钟/月的免费时长和一个配备 2.3GHz 处理器和 8GB 内存的 Mac Mini 实例。
  * [FlutterFlow](https://flutterflow.io/) - FlutterFlow 是一个基于浏览器的拖放界面，用于使用 Flutter 构建移动应用。

**[⬆️ 返回顶部](#目录)**

## 基于浏览器的硬件仿真（在JavaScript中编写）
...
## 隐私管理
  ...
  * [Bearer](https://www.bearer.sh/) - 通过审计和持续工作流程帮助实现隐私设计，确保组织符合 GDPR 等法规。免费层仅限于较小的团队，SaaS 版本也是如此。
...
## 杂项
  ...
  * [Blynk](https://blynk.io) - 一个具有 API 的 SaaS，用于控制、构建和评估物联网设备。免费开发者计划包括 5 个设备、免费云存储和数据存储。还提供移动应用程序。
  ...
  * [Cronhooks](https://cronhooks.io/) - 按时或定期安排网络钩子。免费计划允许 5 个临时计划。
  ...
  * [Format Express](https://www.format-express.dev) - 在线即时格式，用于 JSON / XML / SQL。
  * [FOSSA](https://fossa.com/) - 第三方代码、许可证合规性和漏洞的可扩展端到端管理。
  * [fullcontact.com](https://www.fullcontact.com/developer/pricing/) - 通过向您的应用添加社交资料，帮助您的用户更多地了解他们的联系人。每月 500 个免费个人 API 匹配
  ...
  * [RandomKeygen](https://randomkeygen.com/) - 免费的移动友好工具，提供多种随机生成的密钥和密码，可用于保护任何应用程序、服务或设备。
  ...
  * [superfeedr.com](https://superfeedr.com/) - 实时 PubSubHubbub 兼容的提要、导出、分析。免费提供较少的自定义
  ...
  * [Versionfeeds](https://versionfeeds.com) - 您喜爱的软件版本的自定义 RSS 饲料。拥有您编程语言、库或喜爱的工具的最新版本在一个饲料中。（前 3 个饲料是免费的）
...
## 远程桌面工具
  ...
  * [RemSupp](https://remsupp.com)——按需支持和对设备的永久访问权限（每天免费 2 个会话）
...
## 游戏开发
  ...
  * [Game Icons](https://game-icons.net/) - 免费的可风格化的 SVG/PNG 图标，根据 CC-BY 许可证提供。
  ...
  * [Kenney](https://www.kenney.nl/assets/) - 免费（CC0 1.0 通用许可证）的 2D、3D、音频和 UI 游戏资产。
...
## 其他免费资源
  ...
  * [红帽开发者](https://developers.redhat.com) - 免费访问红帽产品，包括 RHEL、OpenShift、CodeReady 等，仅供开发者使用。仅限个人计划。还提供免费电子书作为参考。
  ...
  * [Web.Dev](https://web.dev/measure/) - 这是一个免费工具，允许您查看网站的性能并改善 SEO，以在搜索引擎中获得更高的排名。
  * [SmallDev.tools](https://smalldev.tools/) - 开发者的免费工具，允许您对各种格式进行编码/解码、最小化 HTML/CSS/Javascript、美化、以 JSON/CSV 及多种其他格式生成虚假/测试数据集等功能。具有令人愉快的界面。
  * [UseCSV by Layercode](https://layercode.com/usecsv) - 在几分钟内将 CSV 和 Excel 导入您的网络应用程序。为您的用户提供愉快和强大的数据导入体验。免费开始，无需任何信用卡详细信息，今天就开始集成 UseCSV。您可以创建无限的导入器，上传的文件最大为 100Mb。
  ...
  * [免费代码工具](https://freecodetools.org/) - 有效的免费代码工具。包括 Markdown 编辑器、代码最小化/美化器、QR 码生成器、Open Graph 生成器、Twitter 卡生成器等。
  * [regex101](https://regex101.com/) - 免费网站，允许您测试和调试正则表达式（regex）。它提供了一个正则表达式编辑器和测试器，以及学习 regex 的有用文档和资源。
  * [Kody 工具](https://www.kodytools.com/dev-tools) - 100 多种开发工具，包括格式化程序、最小化程序和转换器。
  ...
  * [QRCodeBest](https://qrcode.best/) - 使用 13 种模板创建自定义 QR 码，具有完全隐私和个人品牌。功能包括跟踪像素、项目分类和无限团队座位 QRCode.Best。
  ...
  * [MySQL Visual Explain](https://mysqlexplain.com) - 易于理解和免费的 MySQL EXPLAIN 输出可视化工具，用于优化慢速查询。
