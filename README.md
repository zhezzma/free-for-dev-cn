...
开发人员和开源作者现在有许多提供免费层的服务，但找到它们全部需要时间来做出明智的决策。

这是一个带有免费开发者层的软件（SaaS、PaaS、IaaS 等）和其他产品的列表。

此特定列表的范围仅限于基础架构开发人员（系统管理员、DevOps 从业者等）可能会发现有用的东西。我们喜欢那里所有的免费服务，但保持主题相关性会很好。有时这是一条灰色地带，所以这是有意见的；如果我不接受你的贡献，请不要感到冒犯。

此列表是通过拉取请求、评论、想法和 1600 多人的工作得出的。你也可以通过发送[拉取请求](https://github.com/ripienaar/free-for-dev)来添加更多服务或删除那些提供的服务已经更改或已退役的服务。

[![跟踪精彩列表](https://www.trackawesomelist.com/badge.svg)](https://www.trackawesomelist.com/ripienaar/free-for-dev)

**注意**：此列表仅适用于即服务产品，不适用于自托管软件。要符合条件，服务必须提供免费层，而不仅仅是免费试用。如果是时间桶化的免费层，必须至少为一年。我们还从安全角度考虑免费层，因此单点登录是可以的，但我不会接受将 TLS 限制为仅付费层的服务。
...
# Table of Contents
  ...
  * [隐私管理](#隐私管理)
  * [屏幕截图API](#屏幕截图API)
  * [与Flutter相关以及无需Mac即可构建IOS应用](#与Flutter相关以及无需Mac即可构建IOS应用)
  * [搜索](#搜索)
...
## 主要云服务提供商的免费服务限制
  ...
  * [Google Cloud Platform](https://cloud.google.com)
    ...
    * Compute Engine - 1 个非抢占式 e2-micro，30GB HDD，5GB 快照存储（限制在某些地区），每月从北美到所有地区目的地（不包括中国和澳大利亚）的 1GB 网络出口
    ...
    * Cloud Run - 每月 200 万次请求，360,000GB 秒内存，180,000vCPU 秒计算时间，每月从北美到所有地区目的地的 1GB 网络出口
  ...
  * [Microsoft Azure](https://azure.microsoft.com)
    ...
    * [Cosmos DB](https://azure.microsoft.com/services/cosmos-db/) - 25GB 存储和 1,000 RU 的预配吞吐量
    ...
    * [Cognitive Services](https://azure.microsoft.com/services/cognitive-services/) - AI/ML  API（计算机视觉、翻译、面部检测、机器人等），免费层包括有限的交易
  ...
  * [Oracle Cloud](https://www.oracle.com/cloud/)
    * 计算
       ...
       - 实例在被视为空闲时会被回收[https://docs.oracle.com/en-us/iaas/Content/FreeTier/freetier_topic-Always_Free_Resources.htm#compute__idleinstances]
...
## 云管理解决方案
  ...
  * [Cloud 66](https://www.cloud66.com/) - 个人项目免费（包括一个部署服务器，一个静态网站），Cloud 66 为您提供在任何云上构建、部署和扩展应用程序所需的一切，而无需担心“服务器问题”。
  ...
  * [scalr.com](https://scalr.com/) - Scalr 是一个 Terraform 自动化和协作（TACO）产品，用于更好地协作和自动化由 Terraform 管理的基础架构和配置。完全支持 Terraform CLI，集成 OPA，以及分层配置模型。无 SSO 税。所有功能都包含在内。免费使用高达 50 次/月的运行次数。
  * [deployment.io](https://deployment.io) - Deployment.io 帮助开发人员在 AWS 上自动化部署。在我们的免费层中，开发人员（单个用户）可以无限部署静态网站、Web 服务和环境。我们在免费层中提供每月 20 次免费的作业执行，其中包括预览和自动部署。

**[⬆️ 返回顶部](# 目录)**

## 源代码存储库

  * [Bitbucket](https://bitbucket.org/)——无限制的公共和私人 Git 存储库，最多可容纳 5 名用户，并提供持续集成/持续部署管道
  * [chiselapp.com](https://chiselapp.com/)——无限制的公共 Fossil 存储库
  * [codebasehq.com](https://www.codebasehq.com/)——一个免费项目，提供 100MB 空间和两个用户
  * [Codeberg](https://codeberg.org/)——无限的公共和私人 Git 存储库，适用于免费和开源项目（无限制的协作者）。由[Forgejo](https://forgejo.org/)提供支持。静态网站托管[Codeberg Pages](https://codeberg.page/)。持续集成/持续部署托管[Codeberg 的 CI](https://docs.codeberg.org/ci/)。翻译托管[Codeberg Translate](https://translate.codeberg.org/)。包括包和容器托管、项目管理和问题跟踪
  * [GitGud](https://gitgud.io)——无限的私有和公共存储库。永远免费。由 GitLab 和 Sapphire 提供支持。不提供持续集成/持续部署。
  * [GitHub](https://github.com/)——无限的公共存储库和无限的私人存储库（无限制的协作者）。包括持续集成/持续部署、开发环境、静态托管、包和容器托管、项目管理和 AI Copilot
  * [gitlab.com](https://about.gitlab.com/)——无限制的公共和私人 Git 存储库，最多可容纳 5 名协作者。包括持续集成/持续部署、静态托管、容器注册表、项目管理和问题跟踪
  ...
  * [ionicframework.com](https://ionicframework.com/appflow) - 用于使用 Ionic 开发应用程序的存储库和工具；您还可以拥有一个 ionic 存储库
  ...
  * [pijul.com](https://pijul.com/) - 无限免费和开源的分布式版本控制系统。其独特之处在于基于可靠的补丁理论，使其易于学习、使用和分发。解决了 git/hg/svn/darcs 的许多问题。
  * [plasticscm.com](https://plasticscm.com/)——免费供个人、OSS 和非营利组织使用
  * [projectlocker.com](https://projectlocker.com)——一个免费的私人项目（Git 和 Subversion），提供 50MB 空间
  * [RocketGit](https://rocketgit.com)——基于 Git 的存储库托管。无限公共和私人存储库。
...
## API、数据和ML
  ...
  * [JSONGrid](https://jsongrid.com) - 免费工具，用于可视化、编辑、过滤复杂的 JSON 数据为漂亮的表格网格。通过链接共享 JSON 数据并保存。
  * [Zerosheets](https://zerosheets.com) - 将您的 Google Sheets 电子表格转换为强大的 API，以便快速开发原型、网站、应用程序等。每月可免费使用 500 次请求。
  ...
  * [Apify](https://www.apify.com/) - 网络抓取和自动化平台，可创建适用于任何网站的 API 并提取数据。现成的抓取器、集成的代理和定制解决方案。免费计划每月包含 5 美元的平台信用额度。
  ...
  * [APIVerve](https://apiverve.com) - 立即免费访问 120 多个 API，注重质量、一致性和可靠性。免费计划每月涵盖最多 50 个 API 令牌。
  ...
  * [Browse AI](https://www.browse.ai) - 网络数据提取和监控。每月免费 50 个信用额度。
  * [BrowserCat](https://www.browsercat.com) - 无头浏览器 API，用于自动化、抓取、AI 代理网络访问、图像/pdf 生成等。免费计划每月 1k 次请求。
  * [Bruzu](https://bruzu.com/) - 自动生成图像。使用 API、集成或无代码表生成大量图像变体。API 免费，但带有水印。
  * [Calendarific](https://calendarific.com) - 企业级公共假日 API 服务，适用于 200 多个国家。免费计划包括每月 1,000 次调用。
  * [Canopy](https://www.canopyapi.co/) - 适用于亚马逊产品、搜索和类别数据的 GraphQL API。免费计划包括每月 100 次调用。
  ...
  * [Commerce Layer](https://commercelayer.io) - 可组合的商务 API，可以从任何前端构建、放置和管理订单。开发人员计划每月免费 100 个订单，最多 1,000 个 SKU。
  * [Conversion Tools](https://conversiontools.io/) - 在线文件转换器，适用于文档、图像、视频、音频和电子书。提供 REST API。适用于 Node.js、PHP、Python 的库。支持的文件最大为 50GB（付费计划）。免费层受文件大小和每天转换次数的限制。
  ...
  * [Coupler](https://www.coupler.io/) - 用于在应用程序之间同步数据的工具。它可以创建实时仪表板和报告，转换和操作值，并收集和备份见解。免费计划具有无限用户，每月 100 次运行，每次运行 1000 行，以及无限集成。
  ...
  * [Cube](https://cube.dev/) - Cube 帮助数据工程师和应用程序开发人员从现代数据存储中访问数据，将其组织为一致的定义，并将其交付到每个应用程序。使用 Cube 的最快方法是使用 Cube Cloud，它提供每月 1GB 数据通过的免费层。
  ...
  * [Datalore](https://datalore.jetbrains.com) - Jetbrains 的 Python 笔记本。每月包括 10GB 存储和 120 小时的运行时间。
  ...
  * [DB-IP](https://db-ip.com/api/free) - 免费的 IP 地理位置定位 API，每个 IP 每天 1k 次请求。基于 CC-BY 4.0 许可证的 lite 数据库也是免费的。
  ...
  * [Deepnote](https://deepnote.com) - 一个新的数据科学笔记本。兼容 Jupyter，支持实时协作和在云端运行。免费层包括无限个人项目、高达 750 小时的标准硬件以及最多 3 个编辑器的团队。
  ...
  * [dreamfactory.com](https://dreamfactory.com/)——用于移动、Web 和 IoT 应用程序的开源 REST API 后端。连接任何 SQL/NoSQL 数据库、文件存储系统或外部服务，它立即创建一个具有实时文档和用户管理的全面 REST API 平台。
  ...
  * [GoodData](https://www.gooddata.com/)——数据即服务——创建互动和有洞察力的仪表板。免费层包含五个工作区和 100MB/工作区。
  ...
  * [Hybiscus](https://hybiscus.dev/)——使用简单的声明性 API 构建 pdf 报告。免费层每月最多包含 100 份单页报告，能够自定义调色板和字体。
  * [Invantive Cloud](https://cloud.invantive.com/)——访问超过 70 个（云）平台，如 Exact Online、Twinfield、ActiveCampaign 或 Visma，使用 Invantive SQL 或 OData4（通常为 Power BI 或 Power Query）。包括数据复制和交换。开发人员和实施顾问的免费计划。对于特定平台，免费计划在数据量上有限制。
  ...
  * [IP Geolocation](https://ipgeolocation.io/)——IP 地理定位 API——开发人员的永久免费计划，每月有 30k 请求（每天 1k）限制。
  * [IP Geolocation API](https://www.abstractapi.com/ip-geolocation-api)——Abstract 的 IP 地理定位 API——提供广泛的免费计划，允许每月进行 20,000 次请求。
  * [IP2Location](https://www.ip2location.com)——免费增值 IP 地理定位服务。LITE 数据库可免费下载。将数据库导入服务器并在本地查询以确定城市、坐标和 ISP 信息。
  * [IP2Location.io](https://www.ip2location.io/)——免费增值、快速可靠的 IP 地理定位 API，用于确定城市、坐标、ISP 等地理定位数据。免费计划每月可获得 30k 信用额度。订阅付费计划以获得更多高级功能或联系我们以获得个性化计划。
  * [ipapi](https://ipapi.co/)——由 Kloudend, Inc 提供的 IP 地址位置 API——基于 AWS 构建的可靠地理定位 API，受到 Fortune 500 的信任。免费层每月提供 30k 次查找（每天 1k），无需注册。
  * [ipapi.is](https://ipapi.is/)——为开发人员提供的可靠 IP 地址 API——具有最佳的主机检测功能。免费计划提供 1000 次无签名查找。
  * [IPinfo](https://ipinfo.io/)——快速、准确且免费（每月最多 50k）的 IP 地址数据 API。提供有关地理位置、公司、运营商、IP 范围、域、滥用联系人等的 API。所有付费 API 均可免费试用。
  * [BigDataCloud](https://www.bigdatacloud.com/)——提供快速、准确且免费（无限或每月 10K-50K）的现代网络 API，如 IP 地理定位、反向地理编码、网络洞察、电子邮件和电话验证、客户端信息等。
  ...
  * [JSON Serve](https://jsonserve.com/)——一项免费服务，可帮助开发人员存储 JSON 对象，并在其应用程序中将该 JSON 用作 REST API。
  ...
  * [Lightly](https://www.lightly.ai/)——使用正确的数据改进您的机器学习模型。免费使用最多 1000 个样本的数据集。
  ...
  * [Meteosource 天气 API](https://www.meteosource.com/)——全球天气 API，提供当前和预测的天气数据。预测基于机器学习对多个天气模型的组合，以提高准确性。免费计划每天提供 400 次调用。
  * [microlink.io](https://microlink.io/)——将任何网站转换为数据，例如元标记标准化、美化链接预览、抓取功能或屏幕截图服务。每天 100 个请求免费。
  ...
  * [Mockfly](https://www.mockfly.dev/)——Mockfly 是一款值得信赖的 API 模拟和功能标志管理工具。通过直观的界面快速生成和控制模拟 API。免费层每天提供 500 次请求。
  ...
  * [Nordigen](https://nordigen.com)——免费的开放银行数据 API。PSD2。在欧盟+英国连接 2300 多家银行。
  ...
  * [Duply.co](https://duply.co)——从 API 和 URL 创建动态图像，设计模板一次并重复使用。免费层每月提供 70 张 API 和 URL 创建的图像，以及最多 100 张通过表单创建的图像。
  ...
  * [pdfEndpoint.com](https://pdfendpoint.com)——使用简单 API 将 HTML 或 URL 轻松转换为 PDF。每月免费进行 100 次转换。
  ...
  * [Postman](https://postman.com)——用于简化工作流程并更快地创建和测试更好的 API 的协作平台。免费使用 Postman App 永久。Postman 云功能也永久免费，但有一定限制。
  ...
  * [PromptLoop](https://www.promptloop.com/)——使用 AI 和大型语言模型（如 GPT-3）和简单的电子表格公式在 Google Sheets 中转换、理解和分析文本。每月免费提供前 2000 个信用额度。
  ...
  * [QuickMocker](https://quickmocker.com/)——管理自己子域下的在线虚假 API 端点，将请求转发到本地主机 URL 进行 Webhooks 开发和测试，使用 RegExp 和多个 HTTP 方法进行 URL 路径，对端点进行优先级排序，每个端点超过 100 个短代码（动态或虚假响应值）用于响应模板，从 OpenAPI（Swagger）规范以 JSON 格式导入，代理请求，根据 IP 地址和授权标头限制端点。免费帐户提供一个随机子域、十个端点、五个 RegExp URL 路径、每个端点 50 个短代码、每天 100 个请求和请求日志中的 50 条历史记录。
  * [Rapidapi](https://rapidapi.com/)——世界上最大的 API 中心数百万开发人员找到并连接到数千个 API，使用有趣的挑战（和解决方案！）和交互示例进行 API 开发。
  ...
  * [Select Star](https://www.selectstar.com/) - 是一个智能数据发现平台，它自动分析和记录您的数据。免费轻量级套餐，包括 1 个数据源、最多 100 个表和 10 个用户。
  ...
  * [Sqlable](https://sqlable.com/) - 一系列免费的在线 SQL 工具，包括 SQL 格式化程序和验证器、SQL 正则表达式测试器、虚假数据生成器和交互式数据库游乐场。
  ...
  * [TemplateTo](https://templateto.com) - 使用我们的拖放编辑器和简单 API，从可重用模板自动生成 PDF/TXT 文档。免费计划包括每月 450 份 PDF 和三个模板。
  * [TinyMCE](https://www.tiny.cloud) - 富文本编辑 API。核心功能免费，无使用限制。
  * [Treblle](https://www.treblle.com) - Treblle 帮助团队构建、交付和管理 API。具有高级 API 日志聚合、可观测性、文档和调试功能。所有功能都是免费的，但免费层每月的请求限制高达 250k。
  ...
  * [Weights & Biases](https://wandb.ai) — 开发者优先的 MLOps 平台。通过实验跟踪、数据集版本控制和模型管理更快地构建更好的模型。个人项目的免费层仅包含 100GB 的存储空间。
  * [wit.ai](https://wit.ai/) — 面向开发人员的 NLP。
  * [wolfram.com](https://wolfram.com/language/) — 基于云的内置知识算法。
  ...
  * [Zenscrape](https://zenscrape.com/web-scraping-api) — 具有无头浏览器、住宅 IP 和简单定价的 Web 抓取 API。每月一千次免费 API 调用，学生和非营利组织有额外积分。
  ...
  * [Zipcodebase](https://zipcodebase.com) - 免费邮政编码 API，可访问全球邮政编码数据。每月十万次免费请求。
  * [huggingface.co](https://huggingface.co) - 用于 Pytorch、TensorFlow 和 JAX 的 NLP 模型的构建、训练和部署。免费使用高达 30k 输入字符/月。
  ...
  * [ERD Lab](https://www.erdlab.io) — 面向开发人员的免费云基础实体关系图 (ERD) 工具。
  * [What The Diff](https://whatthediff.ai) - 由人工智能驱动的代码审查助手。免费计划每月有 25,000 个令牌限制（~10 个 PR）。
  ...
  * [Composio](https://composio.dev/) - AI 代理和大语言模型的集成平台。集成代理互联网上的 200 多个工具。

**[⬆️ 返回顶部](#table-of-contents)**

## 工件存储库

  * [Artifactory](https://jfrog.com/start-free/) - 一个支持多种软件包格式的工件存储库，如 Maven、Docker、Cargo、Helm、PyPI、CocoaPods 和 GitLFS。它包含了包扫描工具 XRay 和 CI/CD 工具 Pipelines（以前称为 Shippable），每月有 2,000 分钟的免费 CI/CD 分钟数。
  ...
  * [cloudsmith.io](https://cloudsmith.io) - 用于 Java/Maven、RedHat、Debian、Python、Ruby、Vagrant 等的简单、安全和集中的存储库服务。免费层+开源免费。
  * [jitpack.io](https://jitpack.io/) - GitHub 上的 JVM 和 Android 项目的 Maven 存储库，公共项目免费。
...
## 团队和协作工具
  ...
  * [Calendly](https://calendly.com) — Calendly 是连接和安排会议的工具。免费计划为每个用户提供 1 个日历连接，无限制的会议。还提供桌面和移动应用程序。
  * [Discord](https://discord.com/) — 与公共/私人房间聊天。支持 Markdown 文本、语音、视频和屏幕共享功能。免费供无限用户使用。
  * [Telegram](https://telegram.org/) — Telegram 适合所有想要快速、可靠消息传递和通话的人。商业用户和小团队可能喜欢大群组、用户名、桌面应用程序和强大的文件共享选项。
  * [Dubble](https://dubble.so/) — 免费的分步指南创建者。截图、记录流程并与团队协作。还支持异步屏幕录制。
  * [Duckly](https://duckly.com/) — 与团队实时交谈和协作。与 IDE 进行配对编程，终端共享，语音、视频和屏幕共享。适用于小团队。
  ...
  * [Fibery](https://fibery.io/) — 连接工作区平台。免费供单个用户使用，高达 2GB 磁盘空间。
  * [Filestash](https://www.filestash.app) — 类似于 Dropbox 的文件管理器，可连接多种协议和平台：S3、FTP、SFTP、Minio、Git、WebDAV、Backblaze、LDAP 等。
  * [flock.com](https://flock.com) — 团队更快速的沟通方式。免费无限消息、频道、用户、应用程序和集成。
  * [Gather](https://www.gather.town/) - 在线见面的更好方式。围绕完全可定制的空间，Gather 使与您的社区共度时光就像现实生活一样容易。免费供最多 10 个并发用户使用。
  * [gokanban.io](https://gokanban.io) - 基于语法的、无需注册的看板，可快速使用。免费且无限制。
  * [flat.social](https://flat.social) - 用于团队会议和欢乐时光社交活动的交互式可定制空间。无限会议，免费供最多 8 个并发用户使用。
  * [GitDailies](https://gitdailies.com) - 您团队在 GitHub 上的提交和拉取请求活动的每日报告。包括推送可视化器、同伴认可系统和自定义警报构建器。免费层有无限用户、三个存储库和 3 个警报配置。
  * [gitter.im](https://gitter.im/) — GitHub 的聊天工具。无限的公共和私人房间，团队最多可容纳 25 人免费
  * [Hackmd.io](https://hackmd.io/) - 用于 markdown 格式文档/文件的实时协作和编写工具。就像 Google Docs 但适用于 markdown 文件。免费无限数量的“笔记”，但私人笔记和模板的协作者（受邀者）数量将受到限制[将受到限制](https://hackmd.io/pricing)。
  ...
  * [MarkUp](https://www.markup.io/) — MarkUp 让您可以直接在网站、PDF 和图像上收集反馈。
  * [Proton Pass](https://proton.me/pass) — 具有内置电子邮件别名、2FA 身份验证器、共享和密码的密码管理器。可在网络、浏览器扩展和移动应用程序以及桌面设备上使用。
  ...
  * [nootiz](https://www.nootiz.com/) - 用于收集和管理任何网站上视觉反馈的首选工具
  * [Notion](https://www.notion.so/) - 具有 markdown 支持的笔记记录和协作应用程序，它将任务、维基和数据库集成在一起。该公司将该应用程序描述为笔记记录、项目管理和任务管理的一体化工作空间。除了跨平台应用程序，还可以通过大多数网络浏览器访问它。
  * [Nuclino](https://www.nuclino.com) - 适用于您团队所有知识、文档和笔记的轻量级协作维基。免费计划包含所有基本功能，最多 50 个项目和 5GB 存储空间。
  ...
  * [PageShare.dev](https://www.pageshare.dev) - 在 GitHub Pull 请求中添加视觉评论功能，无需部署网站。免费供每月最多 10 页和总计 100MB 的存储空间。
  * [Pendulums](https://pendulums.io/) - Pendulums 是一个免费的时间跟踪工具，它具有易于使用的界面和有价值的统计信息，可以帮助您更好地管理时间。
  ...
  * [Spectrum](https://spectrum.chat/) - 创建公共或私人社区，免费。
  * [StatusPile](https://www.statuspile.com/) - 状态页面的状态页面。您能否跟踪您的上游提供商的状态页面？
  * [Stickies](https://stickies.app/) - 用于头脑风暴、内容策展和笔记的可视化协作应用程序。最多可免费使用 3 个墙、无限用户和 1GB 存储空间。
  * [talky.io](https://talky.io/) - 免费的小组视频聊天。匿名。点对点。无需插件、注册或支付。
  * [Teamhood](https://teamhood.com/) - 免费的项目、任务和问题跟踪软件。支持带有泳道和完整 Scrum 实现的看板。集成了时间跟踪功能。免费供五名用户和三个项目组合使用。
  * [Teamplify](https://teamplify.com) - 通过团队分析和智能日常站立改进团队开发流程。包括适用于远程优先团队的全面的请假管理功能。免费供最多 5 名用户的小团体使用。
  * [Tefter](https://tefter.io) - 具有强大的 Slack 集成的书签应用程序。免费供开源团队使用。
  ...
  * [twist.com](https://twist.com/) - 一种异步友好的团队通信应用程序，其中对话保持组织有序且主题相关。提供免费和无限计划。有资格的团队可享受折扣。
  * [tldraw.com](https://tldraw.com/) - 免费的开源白板和绘图工具，具有智能箭头、捕捉、便签和 SVG 导出功能。支持多人协作编辑的多人模式。还有免费的官方 VS Code 扩展。
  * [BookmarkOS.com](https://bookmarkos.com) - 免费的一体化书签管理器、选项卡管理器和任务管理器，可在可自定义的在线桌面上使用，具有文件夹协作功能。
  ...
  * [windmill.dev](https://windmill.dev/) - 一个开源的开发者平台，用于从最小的 Python 和 TypeScript 脚本快速构建生产级的多步骤自动化和内部应用程序。作为免费用户，您最多可以创建和成为三个非高级工作空间的成员。
  * [vadoo.tv](https://vadoo.tv/) - 简单的视频托管和营销。只需点击一下即可上传视频。记录、管理、共享等。免费层提供多达 10 个视频、1GB 存储空间和 10GB 带宽/每月
  * [userforge.com](https://userforge.com/) - 相互关联的在线个人资料、用户故事和上下文映射。有助于保持设计和开发的同步，免费供最多 3 个个人资料和两个协作者使用。
  * [wistia.com](https://wistia.com/) - 具有观众分析、高清视频交付和营销工具的视频托管，可帮助您了解访问者，25 个视频，以及 Wistia 品牌播放器
  * [wormhol.org](https://www.wormhol.org/) - 简单的文件共享服务。与任意数量的同行共享高达 5GB 的无限文件。
  ...
  * [Zulip](https://zulip.com/) - 具有独特的电子邮件式线程模型的实时聊天。免费计划包括 10,000 条消息搜索历史记录和最多 5GB 的文件存储。还提供可自托管的开源版本。
  * [robocorp.com](https://robocorp.com) - 用于推动自动化运营的开源堆栈。免费试用云功能并实现简单自动化。机器人工作 240 分钟/月，10 个助手运行，存储 100MB。
  * [Fleep.io](https://fleep.io/) - Fleep 是 Slack 的替代品。它为小团队提供免费计划，包括完整的消息历史记录、无限的 1:1 对话、1 个组对话和 1GB 文件存储。
  * [Chanty.com](https://chanty.com/) - Chanty 是 Slack 的另一种替代品。它为小团队（最多 10 人）提供免费的永久计划，包括无限的公共和私人对话、可搜索的历史记录、无限的 1:1 音频通话、无限的语音消息、10 个集成和每个团队 20GB 的存储空间。
  ...
  * [Webvizio](https://webvizio.com) - 用于在直接在实时网站和网络应用程序、图像、PDF 和设计文件上的任务上简化网络开发协作的网站反馈工具、网站审查软件和错误报告工具。
  ...
  * [Revolt.chat](https://revolt.chat/) - [Discord](https://discord.com/)的开源替代品，尊重您的隐私。它还免费提供了 Discord 的大多数专有功能。Revolt 是一个安全、快速且 100%免费的一体化应用程序。每个功能都是免费的。它们还支持（官方和非官方）插件，这与大多数主流聊天应用程序不同。
  * [Tencent RTC](https://trtc.io/) - 腾讯实时通信 (TRTC) 提供群组音频/视频通话解决方案。第一年每月免费 10,000 分钟。
  * [Pastefy](https://pastefy.app/) - 美观简洁的 Pastebin，可选客户端加密、多标签粘贴、API、突出显示的编辑器等。
...
## 内容管理系统（CMS）
  ...
  * [Contentful](https://www.contentful.com/) - 无头 CMS。云内的内容管理和交付 API。附带一个免费的社区空间，其中包括五个用户、25K 条记录、48 种内容类型、2 个地区。
  ...
  * [DatoCMS](https://www.datocms.com/) - 为小型项目提供免费层。DatoCMS 是一个基于 GraphQL 的 CMS。在较低层，您每月有 100 万次调用。
  * [Directus](https://directus.io) - 无头 CMS。一个完全免费和开源的平台，用于管理本地或云环境中的资产和数据库内容。没有限制或付费墙。
  * [FrontAid](https://frontaid.io/) - 无头 CMS，将 JSON 内容直接存储在您的 Git 存储库中。无限制。
  * [kontent.ai](https://www.kontent.ai) - 内容即服务平台，为您提供无头 CMS 的所有好处，同时赋予营销人员权力。开发人员计划为每个用户提供两个用户、无限项目、两个环境、500 个内容项、两种语言的交付和管理 API 以及自定义元素支持。您可以使用更详细的计划来满足您的需求。
  * [Prismic](https://www.prismic.io/) - 无头 CMS。具有完全托管和可扩展 API 的内容管理界面。社区计划为一个用户提供无限的 API 调用、文档、自定义类型、资产和地区。您下一个项目所需的一切。对于开源/开源项目，有更大的免费计划。
  ...
  * [sensenet](https://sensenet.com) - 提供企业级解决方案的 API 优先无头 CMS，适用于各种规模的企业。开发者计划为三个用户、500 个内容项目、三个内置角色、25+5 种内容类型、完全可访问的 REST API、文档预览生成和 Office Online 编辑提供支持。
  * [TinaCMS](https://tina.io/) - 取代 Forestry.io。开源 Git 支持的无头 CMS，支持 Markdown、MDX 和 JSON。基本套餐免费，可提供两个用户。
  * [GatsbyjsCMS](https://www.gatsbyjs.com/) - Gatsby 是快速灵活的框架，使使用任何 CMS、API 或数据库构建网站再次变得有趣。构建和部署无服务器网站，以增加流量、更好地转化和赚取更多收入！
  * [Hygraph](https://hygraph.com/) - 为小型项目提供免费层。GraphQL 优先 API。摆脱遗留解决方案，采用 GraphQL 原生无头 CMS - 并首先提供多渠道内容 API。
  * [Squidex](https://squidex.io/) - 为小型项目提供免费层。API/GraphQL 优先。开源且基于事件源（自动响应每个更改）。
  * [InstaWP](https://instawp.com/) - 在几秒钟内启动 WordPress 网站。免费层包括 5 个活动网站、500MB 空间、48 小时网站过期。
  * [Storyblok](https://www.storyblok.com) - 适用于开发人员和营销人员的无头 CMS，可与所有现代框架配合使用。社区（免费）层提供管理 API、可视化编辑器、十个源、自定义字段类型、国际化（无限语言/地区）、资产管理器（最多 2500 个资产）、图像优化服务、搜索查询、Webhook + 250GB 流量/月包含在内。
  * [WPJack](https://wpjack.com) - 在不到 5 分钟的时间内在任何云平台上设置 WordPress！免费层包括 1 个服务器、2 个站点、免费 SSL 证书和无限 cron 作业。没有时间限制或过期 - 您的网站，您的方式。

**[⬆️ 返回顶部](#table-of-contents)**

## 代码生成

  * [Appinvento](https://appinvento.io/)——AppInvento 是一款免费的无代码应用构建器。在自动生成的后端代码中，用户可以完全访问源代码以及无限的 API 和路由，从而实现广泛的集成。免费计划包括三个项目、五个表格和一个 Google 附加组件。
  * [Cody AI](https://sourcegraph.com/cody)——Cody 是一个编码 AI 助手，它使用 AI 和对代码库的深入理解来帮助您更快地编写和理解代码。Cody 为开发人员提供了对 LLM（包括本地推理）的选择，支持各种 IDE，支持所有流行的编程语言，并提供免费计划。免费计划每月为开发人员提供 20 条聊天消息（使用 Claude 3 Sonnet 作为 LLM）和 500 次自动完成（使用 Starcoder 16b）。
  * [DhiWise](https://www.dhiwise.com/)——借助 DhiWise 的创新代码生成技术，将 Figma 设计无缝转换为动态的 Flutter 和 React 应用程序，优化您的工作流程，帮助您比以往更快地打造出色的移动和 Web 体验。
  ...
  * [Fern](https://buildwithfern.com)——编写 API 定义，并使用它们在 TypeScript、Python、Java、Go 等流行语言中生成 SDK/client 库。完全支持 OpenAPI。免费层最多为 20 个端点生成代码。
  * [Metalama](https://www.postsharp.net/metalama)——仅适用于 C#。Metalama 在编译时动态生成代码的样板，从而使您的源代码保持整洁。它对开源项目免费，其商业友好的免费层包括三个方面。
  * [Supermaven](https://www.supermaven.com/)——Supermaven 是适用于 VSCode、JetBrains 和 Neovim 的快速 AI 代码完成插件。免费层提供无限内联完成。
  * [tabnine.com](https://www.tabnine.com/)——Tabnine 通过提供从世界上所有代码中学习到的见解，帮助开发人员更快地创建更好的软件。提供插件。
  * [v0.dev](https://v0.dev/)——v0 使用 AI 模型根据简单的文本提示生成代码。它基于 shadcn/ui 和 Tailwind CSS 生成可复制粘贴的 React 代码，人们可以在他们的项目中使用。每次生成至少需要 30 个学分。您开始时拥有 1200 个学分，并且每月获得 200 个免费学分。

**[⬆️ 返回顶部](#目录)**

## 代码质量

  * [beanstalkapp.com](https://beanstalkapp.com/)——一个完整的工作流程，用于编写、审查和部署代码），一个免费帐户供一个用户使用，以及一个存储容量为 100MB 的存储库
  ...
  * [CodeBeat](https://codebeat.co)——适用于多种语言的自动化代码审查平台。免费版永久适用于公共存储库，带有 Slack 和电子邮件集成。
  * [codeclimate.com](https://codeclimate.com/)——自动化代码审查，对于开源免费，并且组织拥有的私有存储库数量不受限制（最多 4 个协作者）。对于学生和机构也免费。
  * [codecov.io](https://codecov.io/)——代码覆盖率工具（SaaS），对于开源免费，一个免费私有存储库
  ...
  * [CodSpeed](https://codspeed.io)——在 CI 管道中自动跟踪性能。由于精确和一致的指标，在部署之前捕获性能回归。对于开源项目免费永久。
  * [coveralls.io](https://coveralls.io/)——显示测试覆盖率报告，对于开源免费
  * [dareboost](https://dareboost.com)——每月免费提供 5 份针对网络性能、可访问性和安全性的分析报告
  * [deepcode.ai](https://www.deepcode.ai)——DeepCode 基于人工智能发现错误、安全漏洞、性能和 API 问题。DeepCode 的分析速度允许我们在您的 IDE 中保存按钮时实时分析您的代码并提供结果。支持的语言包括 Java、C/C++、JavaScript、Python 和 TypeScript。与 GitHub、BitBucket 和 GitLab 的集成。对于开源和私有存储库以及最多 30 个开发人员免费。
  * [deepscan.io](https://deepscan.io/)——针对 JavaScript 代码自动查找运行时错误的高级静态分析，对于开源免费
  * [DeepSource](https://deepsource.io/)——DeepSource 持续分析源代码更改，查找和修复分类为安全、性能、反模式、错误风险、文档和样式的问题。与 GitHub、GitLab 和 Bitbucket 的本机集成。
  * [DiffText](https://difftext.com)——立即找到两个代码块之间的差异。完全免费使用。
  * [eversql.com](https://www.eversql.com/)——EverSQL - 数据库优化的#1 平台。自动获取您的数据库和 SQL 查询的关键见解。
  * [gerrithub.io](https://review.gerrithub.io/)——针对 GitHub 存储库的 Gerrit 代码审查免费
  * [gocover.io](https://gocover.io/)——针对任何[Go](https://golang.org/)包的代码覆盖率
  * [goreportcard.com](https://goreportcard.com/)——Go 项目的代码质量，对于开源免费
  * [gtmetrix.com](https://gtmetrix.com/)——报告和彻底的建议来优化网站
  ...
  * [Moderne.io](https://app.moderne.io)——自动源代码重构。Moderne 提供框架迁移、带修复的代码分析以及无与伦比的大规模代码转换，因此开发人员可以将时间花在构建新事物上，而不是维护旧事物上。对于开源免费。
...
## 持续集成和持续部署（CI和CD）
  ...
  * [appcircle.io](https://appcircle.io) - 一个企业级移动 DevOps 平台，可自动化移动应用的构建、测试和发布存储，以实现更快、更高效的发布周期。免费使用，最长 30 分钟的构建时间，20 个每月构建和 1 个并发构建。
  * [appveyor.com](https://www.appveyor.com/) - Windows 的持续集成服务，开源免费
  * [Argonaut](https://argonaut.dev/) - 在您的云平台上几分钟内部署应用程序和基础架构。支持在 Kubernetes 和 Lambda 环境中自定义和第三方应用程序部署。免费层允许无限数量的应用程序和部署，以及 5 个域和 2 个用户。
  * [bitrise.io](https://www.bitrise.io/) - 适用于移动应用程序的持续集成/持续部署（CI/CD），无论是原生的还是混合的。每月有 200 次免费构建、10 分钟构建时间和两个团队成员。开源项目获得 45 分钟构建时间、+1 并发性和无限团队规模。
  ...
  * [bytebase.com](https://www.bytebase.com/) - 数据库 CI/CD 和 DevOps。免费使用，用户数量不超过 20，数据库实例数量不超过 10
  * [CircleCI](https://circleci.com/) - 全面的免费计划，包含托管 CI/CD 服务中针对 GitHub、GitLab 和 BitBucket 存储库的所有功能。多个资源类、Docker、Windows、Mac OS、ARM 执行器、本地运行器、测试拆分、Docker 层缓存和其他高级 CI/CD 功能。免费使用，每月执行时间最多 6000 分钟，无限制的协作者，私有项目中最多 30 个并行作业，以及开源项目最多 80,000 个免费构建分钟。
  * [cirrus-ci.org](https://cirrus-ci.org) - 针对公共 GitHub 存储库免费
  * [cirun.io](https://cirun.io) - 针对公共 GitHub 存储库免费
  ...
  * [LayerCI](https://layerci.com) - 全栈项目的 CI
  * [semaphoreci.com](https://semaphoreci.com/) - 开源免费，每月 100 个私有构建
  * [Squash Labs](https://www.squash.io/) - 为每个分支创建一个 VM，并使您的应用程序可从一个独特的 URL 访问，无限的公共和私有存储库，最大 2GB VM 大小。
  * [styleci.io](https://styleci.io/) - 仅限公共 GitHub 存储库
  * [Mergify](https://mergify.io) - GitHub 的工作流程自动化和合并队列 - 针对公共 GitHub 存储库免费
  * [Make](https://www.make.com/en) - 工作流程自动化工具，可让您通过 UI 连接应用程序并自动化工作流程。它支持许多应用程序和最流行的 API。针对公共 GitHub 存储库免费，并且具有免费层，包括 100Mb、1000 次操作和 15 分钟的最小间隔。
  * [Spacelift](https://spacelift.io/) - 基础设施即代码 (IaC) 的管理平台。免费计划功能：IaC 协作、Terraform 模块注册表、ChatOps 集成、使用 Open Policy Agent 进行持续资源合规性、使用 SAML 2.0 的单点登录以及访问公共工作者池：最多 200 分钟/月
  * [microtica.com](https://microtica.com/) - 具有即用型基础设施组件的启动环境，可免费在 AWS 上部署应用程序，并支持您的生产工作负载。免费层包括 1 个环境（在您的 AWS 账户上）、2 个 Kubernetes 服务、每月 100 分钟构建时间和每月 20 次部署。
  * [Nx Cloud](https://nx.dev/ci) - Nx Cloud 通过远程缓存、任务在机器上的分布甚至自动拆分端到端测试运行等功能，加快您的 CI 中的 monorepos 速度。它提供了一个免费计划，最多可容纳 30 名贡献者，并包含慷慨的 150k 信用额度。
  * [blacksmith](https://www.blacksmith.sh/) - 用于 GitHub Actions 的托管性能运行器，每月提供 3000 分钟的免费时间，无需信用卡。
  * [Terramate](https://terramate.io/) - Terramate 是一个用于基础设施即代码 (IaC) 工具（如 Terraform、OpenTofu 和 Terragrunt）的编排和管理平台。免费使用，最多 2 个用户，包括所有功能。
...
## 测试
  ...
  * [Apptim](https://apptim.com) - 一款移动测试工具，使没有性能工程技能的人员能够评估应用程序的性能和用户体验（UX）。使用您自己的设备的桌面版本完全免费，在 iOS 和 Android 上的测试次数不限。
  * [Bencher](https://bencher.dev/) - 持续基准测试工具套件，可捕获 CI 性能回归。对所有公共项目免费。
  * [BugBug](https://bugbug.io/) - 用于 Web 应用程序的轻量级测试自动化工具。易于学习，不需要编码。您可以在自己的计算机上免费运行无限数量的测试。您还可以获得云监控和 CI/CD 集成，额外收取每月费用。
  ...
  * [cypress.io](https://www.cypress.io/) - 快速、简单和可靠的浏览器测试。Cypress Test Runner 始终免费且开源，没有任何限制和限制。Cypress Dashboard 对开源项目的免费使用最多可容纳 5 个用户。
  * [Cypress Recorder by Preflight](https://cypress.preflight.com/) - 在浏览器上创建基于人工智能的 Cypress 测试/POM 模型。除了人工智能部分，它是开源的。免费使用 5 个月，可进行自我修复脚本、电子邮件和视觉测试。
  ...
  * [gridlastic.com](https://www.gridlastic.com/) - 带有免费计划的 Selenium 网格测试，最多可同时使用 4 个 Selenium 节点/10 个网格启动/4000 分钟的测试/月
  * [katalon.com](https://katalon.com) - 提供测试平台，可帮助不同测试成熟度级别的各种规模的团队，包括 Katalon Studio、TestOps（包括视觉测试免费）、TestCloud 和 Katalon Recorder。
  * [Keploy](https://keploy.io/) - Keploy 是面向开发人员的功能测试工具包。记录 API 调用为 API（KTests）和模拟或存根（KMocks）生成端到端测试。它对开源项目免费。
  ...
  * [preflight.com](https://preflight.com) - 无代码自动化网络测试。在您的浏览器上记录对 UI 更改具有弹性的测试，并在 Windows 计算机上运行它们。可以与您的 CI/CD 集成吗？免费计划包括 50 个每月测试运行，带有视频、HTML 会话等。
  * [Argos](https://argos-ci.com) - 面向开发人员的开源视觉测试。无限制项目，每月 5000 张屏幕截图。开源项目免费。
  * [percy.io](https://percy.io) - 将视觉测试添加到任何网络应用程序、静态网站、样式指南或组件库。无限团队成员、演示应用程序和无限项目，每月 5000 个快照。
  * [lost-pixel.com](https://lost-pixel.com) - 您的 Storybook、Ladle、Histoire 故事和 Web Apps 的整体视觉回归测试。无限团队成员，完全免费的开源，每月 7000 个快照。
  * [Repeato](https://repeato.app/) 基于计算机视觉和人工智能的无代码移动应用程序测试自动化工具。适用于原生应用程序、Flutter、React Native、Web、Ionic 等许多应用程序框架。免费计划限制为 iOS 上的 10 个测试和 Android 上的 10 个测试，但包括付费计划的大多数功能，包括无限的测试运行。
  ...
  * [seotest.me](https://seotest.me/) - 免费的页面内 SEO 网站测试器。每天 10 个免费网站抓取。有关如何改善任何网站的页面内 SEO 结果的有用 SEO 学习资源和建议，无论技术如何。
  * [snippets.uilicious.com](https://snippets.uilicious.com) - 它就像 CodePen 但用于跨浏览器测试。UI-licious 允许您像用户故事一样编写测试，并提供一个免费平台 - UI-licious Snippets - 允许您在 Chrome 上无限制地进行测试，无需注册，每次测试运行最多 3 分钟。发现错误了吗？您可以复制测试的唯一 URL 并将其提供给开发人员，以确切地向他们展示如何重现该错误。
  ...
  * [tesults.com](https://www.tesults.com) - 测试结果报告和测试用例管理。与流行的测试框架集成。开源软件开发者、个人、教育工作者和小型团队可以请求超出基本免费项目的折扣和免费产品。
  * [websitepulse.com](https://www.websitepulse.com/tools/) - 各种免费的网络和服务器工具。
  * [qase.io](https://qase.io) - Dev 和 QA 团队的测试管理系统。管理测试用例、组成测试运行、执行测试、跟踪缺陷并衡量影响。免费层包括所有核心功能，可用于附件的容量为 500MB，最多可容纳 3 个用户。

  * [UseWebhook.com](https://usewebhook.com) - 在浏览器中捕获和检查 Webhooks。可以转发到本地主机，或从历史记录中重放。免费使用。
...
## 安全性和公钥基础设施（PKI）
  ...
  * [aikido.dev](https://www.aikido.dev)——一体化应用安全平台，涵盖 SCA、SAST、CSPM、DAST、Secrets、IaC、Malware、Container scanning、EOL 等。免费计划包括两个用户，对 10 个存储库、1 个云、2 个容器和 1 个域进行扫描。
  ...
  * [CyberChef](https://gchq.github.io/CyberChef/)——一个简单直观的网络应用程序，用于分析和解码/编码数据，无需使用复杂的工具或编程语言。就像一把瑞士军刀的密码学和加密工具。所有功能均可免费使用，无限制。如果愿意，可以自行托管。
  ...
  * [Dependabot](https://dependabot.com/)——自动进行 Ruby、JavaScript、Python、PHP、Elixir、Rust、Java (Maven 和 Gradle)、.NET、Go、Elm、Docker、Terraform、Git Submodules 和 GitHub Actions 的依赖项更新。
  * [DJ Checkup](https://djcheckup.com)——使用此免费、自动化的检查工具扫描您的 Django 网站的安全漏洞。源自 Pony Checkup 网站。
  ...
  * [hostedscan.com](https://hostedscan.com)——在线漏洞扫描程序，用于 Web 应用程序、服务器和网络。每月可免费进行 10 次扫描。
  * [Infisical](https://infisical.com/)——开源平台，可让您在团队和基础架构中管理开发人员的秘密：从本地开发到登台/生产第三方服务的各个方面。免费供最多 5 名开发人员使用。
  ...
  * [meterian.io](https://www.meterian.io/)——监控 Java、Javascript、.NET、Scala、Ruby 和 NodeJS 项目中的安全漏洞。免费供一个私有项目使用，开源项目不限数量。
  * [Mozilla Observatory](https://observatory.mozilla.org/)——查找和修复您网站上的安全漏洞。
  ...
  * [pixee.ai](https://pixee.ai)——免费的 GitHub 机器人，可将 PR 提交到您的 Java 代码库，以自动解决漏洞。其他语言即将推出！
  * [pyup.io](https://pyup.io)——监控 Python 依赖项的安全漏洞并自动更新它们。免费供一个私有项目使用，开源项目不限数量。
  ...
  * [seclookup.com](https://www.seclookup.com/)——Seclookup API 可以丰富 SIEM 中的域名威胁指标，提供域名的全面信息，并提高威胁检测和响应能力。在这里免费获得 50K 次查找[https://account.seclookup.com/]。
  * [snyk.io](https://snyk.io)——可以找到并修复开源依赖项中的已知安全漏洞。对开源项目无限制的测试和修复。对于您的私有项目，每月限制为 200 次测试。
  ...
  * [StackHawk](https://www.stackhawk.com/)——在整个管道中自动执行应用程序扫描，以在安全漏洞进入生产环境之前找到并修复它们。单个应用程序的无限扫描和环境。
  ...
  * [Virgil Security](https://virgilsecurity.com/)——在您的数字解决方案中实现端到端加密、数据库保护、物联网安全等的工具和服务。免费供最多 250 个用户的应用程序使用。
  * [Vulert](https://vulert.com)——Vulert 持续监控您的开源依赖项以发现新漏洞，并提供修复建议，无需安装或访问您的代码库。适用于开源项目。

  * [HasMySecretLeaked](https://gitguardian.com/hasmysecretleaked) - 在公共 GitHub 存储库、gist、问题和评论中搜索超过 2000 万个公开泄露的机密，免费使用
...
## 身份验证、授权和用户管理
  ...
  * [asgardeo.io](https://wso2.com/asgardeo) - 无缝集成单点登录 (SSO)、多因素身份验证 (MFA)、无密码身份验证等。包括适用于前端和后端应用程序的 SDK。免费使用额度为 1000 MAUs 和五个身份提供商。
  * [Auth0](https://auth0.com/) - 托管式单点登录。最多支持 7000 个活跃用户和两个社交身份提供商。
  * [Authgear](https://www.authgear.com) - 在数分钟内为您的应用带来无密码、一次性密码 (OTP)、双因素身份验证 (2FA)、单点登录 (SSO) 等功能。所有前端功能均包含在内。免费使用额度为 5000 MAUs。
  ...
  * [Cerbos Hub](https://www.cerbos.dev/product-cerbos-hub) - 用于创作、测试和部署访问策略的完整授权管理系统。细粒度授权和访问控制，免费使用额度为每月 100 个活跃主体。
  ...
  * [Corbado](https://www.corbado.com/) - 为新应用或现有应用添加密码优先身份验证。无限 MAUs 免费。
  * [Descope](https://www.descope.com/) - 高度可定制的身份验证流程，同时提供无代码和 API/SDK 方法。免费使用额度为每月 7500 个活跃用户，50 个租户（最多 5 个 SAML/SSO 租户）。
  * [duo.com](https://duo.com/) - 网站或应用的双因素身份验证 (2FA)。免费使用额度为 10 个用户，所有身份验证方法、无限制、集成、硬件令牌。
  * [Kinde](https://kinde.com/) - 简单、强大的身份验证，您可以在几分钟内集成到您的产品中。免费使用额度为 7500 个 MAU。
  * [logintc.com](https://www.logintc.com/) - 通过推送通知进行双因素身份验证 (2FA)，免费使用额度为 10 个用户，VPN、网站和 SSH。
  * [MojoAuth](https://mojoauth.com/) - MojoAuth 可帮助您在几分钟内轻松实现 Web、移动或任何应用程序的无密码身份验证。
  ...
  * [Ory](https://ory.sh/) - 身份验证/授权/OAuth2.0/零信任管理安全平台。开发者账户永久免费，包含所有安全功能，无限团队成员，每天 200 个活跃用户，每月 25k 次权限检查。
  * [Phase Two](https://phasetwo.io) - Keycloak 开源身份和访问管理。免费领域最多支持 1000 个用户，最多 10 个 SSO 连接，利用 Phase Two 的增强型 Keycloak 容器，其中包含 [Organization](https://phasetwo.io/product/organizations/) 扩展。
  * [Stytch](https://www.stytch.com/) - 具有强大密码保护功能的灵活身份验证 API 和 SDK，具有无密码登录流程、MFA、SSO 等功能。内置电子邮件/短信发送和故障转移以及欺诈保护。提供 5000 个每月活跃用户免费（B2C）或 1000 个每月活跃用户和 25 个组织免费（B2B）。
  * [Stack Auth](https://stack-auth.com) - 开源身份验证，不逊于其他产品。最适合开发者的解决方案，只需五分钟即可开始使用。可免费自托管，或提供带有 10k 免费每月活跃用户的托管 SaaS 版本。
  * [SuperTokens](https://supertokens.com/) - 开源用户身份验证，可本机集成到您的应用程序中，使您能够快速启动，同时控制用户和开发者体验。免费使用额度为每月最多 5000 MAUs。
  * [Warrant](https://warrant.dev/) - 为您的应用提供托管企业级授权和访问控制服务。免费层包括每月 100 万次 API 请求和 1000 个授权规则。
  * [ZITADEL Cloud](https://zitadel.com) - 一站式用户和访问管理，适用于您和支持多租户 (B2B) 用例。免费使用额度为每月最多 25,000 次经过身份验证的请求，所有安全功能（OTP、无密码、策略等无付费墙）。
  * [PropelAuth](https://propelauth.com) - 立即向任何规模的公司销售，只需几行代码，免费使用额度为最多 200 个用户和 10k 笔事务性电子邮件（带有水印品牌：“由 PropelAuth 提供支持”）。
  * [Logto](https://logto.io/) - 开发、保护和管理您产品的用户身份，同时支持认证和授权。免费使用额度为每月最多 5000 MAUs，提供开源自托管选项。
...
## 移动应用分发和反馈
  ...
  * [Loadly](https://loadly.io) - iOS 和 Android 测试版应用程序分发服务提供完全免费的服务，包括无限制的下载、高速下载和无限制的上传。
  * [Diawi](https://www.diawi.com) - 直接将 iOS 和 Android 应用部署到设备上。免费计划：应用上传、密码保护链接、1 天有效期、10 次安装。
...
## 管理系统
  ...
  * [Esper](https://esper.io)——适用于 Android 设备的 MDM 和 MAM，带有 DevOps。一个用户许可证和 25MB 应用程序存储可免费使用 100 个设备。
  * [jamf.com](https://www.jamf.com/)——适用于 iPad、iPhone 和 Mac 的设备管理，免费提供 3 个设备
  * [Miradore](https://miradore.com)——设备管理服务。随时了解您的设备机队并免费安全保护无限数量的设备。免费计划提供基本功能。
  * [moss.sh](https://moss.sh)——帮助开发人员部署和管理他们的 Web 应用程序和服务器。每月免费高达 25 次 git 部署
  * [runcloud.io](https://runcloud.io/)——专注于 PHP 项目的服务器管理。最多免费 1 个服务器
  * [ploi.io](https://ploi.io/)——服务器管理工具，可轻松管理和部署您的服务器和网站。免费 1 个服务器
...
## 消息和流
  ...
  * [cloudamqp.com](https://www.cloudamqp.com/) - RabbitMQ 即服务。小 Lemur 计划：每月最多 100 万条消息，最多 20 个并发连接，最多 100 个队列，最多 10,000 条排队消息，不同可用区的多个节点
  * [courier.com](https://www.courier.com/) - 用于推送、应用内、电子邮件、聊天、短信和其他消息通道的单一 API，具有模板管理等功能。免费计划包括每月 10,000 条消息。
  ...
  * [synadia.com](https://synadia.com/ngs) - [NATS.io](https://nats.io) 即服务。全球、AWS、GCP 和 Azure。免费无限期使用，4k 消息大小，50 个活动连接，每月 5GB 数据。
  ...
  * [EMQX Serverless](https://www.emqx.com/en/cloud/serverless-mqtt) - 可在几秒钟内获得的可扩展且安全的无服务器 MQTT 代理。每月 1M 会话分钟免费永久（无需信用卡）。

**[⬆️ 返回顶部](#table-of-contents)**

## 日志管理

  * [bugfender.com](https://bugfender.com/) — 免费，每天最多可存储 100k 行日志，保留时间为 24 小时
  * [logentries.com](https://logentries.com/) — 免费，每月最多可存储 5GB，保留时间为七天
  * [loggly.com](https://www.loggly.com/) — 免费，单个用户，每天 200MB，保留时间为七天
  * [logz.io](https://logz.io/) — 免费，每天最多可存储 1GB，保留时间为一天
  ...
  * [papertrailapp.com](https://papertrailapp.com/) — 搜索时间为 48 小时，存档时间为七天，每月 50MB
  * [sematext.com](https://sematext.com/logsene) — 每天最多可存储 500MB，保留时间为七天
  * [sumologic.com](https://www.sumologic.com/) — 每天最多可存储 500MB，保留时间为七天
  * [logflare.app](https://logflare.app/) — 每个应用每月最多可免费存储 12960000 条条目，保留时间为三天
  * [logtail.com](https://logtail.com/) — 基于 ClickHouse 的 SQL 兼容日志管理。每月最多可免费使用 1GB，保留时间为三天
...
## 翻译管理
  ...
  * [Lecto](https://lecto.ai/)——机器翻译 API，提供免费层（30 个免费请求，每个请求 1000 个已翻译字符）。与 Loco Translate Wordpress 插件集成。
  * [lingohub.com](https://lingohub.com/)——最多 3 个用户免费，开源始终免费
  * [localazy.com](https://localazy.com/)——免费 1000 个源语言字符串，无限语言，无限贡献者，创业和开源交易
  * [Localeum](https://localeum.com)——免费最多 1000 个字符串，一个用户，无限语言，无限项目
  * [localizely.com](https://localizely.com/)——开源免费
  * [Loco](https://localise.biz/)——免费最多 2000 次翻译，无限制翻译人员，每个项目 10 种语言/项目，每个项目 1000 个可翻译资产/项目
  * [oneskyapp.com](https://www.oneskyapp.com/)——对于最多 5 个用户的有限免费版，开源免费
  * [POEditor](https://poeditor.com/)——免费最多 1000 个字符串
  * [SimpleLocalize](https://simplelocalize.io/)——免费最多 100 个翻译键，无限制字符串，无限制语言，创业优惠
  ...
  * [Translized](https://translized.com)——免费最多 1000 个字符串，一个用户，无限语言，无限项目
  * [webtranslateit.com](https://webtranslateit.com/)——免费最多 500 个字符串
  * [weblate.org](https://weblate.org/)——对于自由项目免费，免费层最多 10,000 个字符串源，无限的自托管本地版本
...
## 监控
  ...
  * [UptimeObserver.com](https://uptimeobserver.com/) - 获得 20 个具有 5 分钟间隔的监控器，并可自定义状态页面-甚至可用于商业用途。通过电子邮件和 Telegram 享受无限制的实时通知。无需信用卡即可开始使用。
  ...
  * [bleemeo.com](https://bleemeo.com/) - 免费 3 台服务器、5 个监控器、无限制用户、无限制仪表板、无限制警报规则。
  ...
  * [deadmanssnitch.com](https://deadmanssnitch.com/) - 对 cron 作业的监控。一个免费的监控器（监视器），如果你推荐其他人注册，还可以获得更多。
  * [downtimemonkey.com](https://downtimemonkey.com/) - 60 个监控器，5 分钟间隔。电子邮件、Slack 警报。
  * [economize.cloud](https://economize.cloud/) - 通过组织云资源以优化和报告相同的内容来帮助消除云基础设施成本的神秘感。免费适用于每月在 Google Cloud Platform 上花费最多 5,000 美元。
  ...
  * [healthchecks.io](https://healthchecks.io/) - 监控你的 cron 作业和后台任务。免费最多 20 次检查。
  * [Hydrozen.io](https://hydrozen.io/) - 监控和状态页面，免费计划：10 个监控器，5 个心跳监控器，1 个域名监控和 1 个状态页面免费。
  ...
  * [linkok.com](https://linkok.com/) - 在线断链检查，免费适用于小型网站（最多 100 页），完全免费适用于开源项目。
  ...
  * [newrelic.com](https://www.newrelic.com) - 为帮助工程师创建更完美的软件而构建的 New Relic 观测平台。从单体到无服务器，您可以对所有内容进行仪器化，然后分析、排除故障和优化整个软件堆栈。免费层提供 100GB/月的免费数据摄入，一个免费的全访问用户和无限的免费主要用户。
  * [Middleware.io](https://middleware.io/) - 中间件观测平台提供对应用程序和堆栈的完整可见性，以便您可以大规模监控和诊断问题。他们为开发人员社区提供免费的永久计划，允许 Log 监控最多 1M 个日志事件，基础设施监控和 APM 最多 2 个主机。
  * [nixstats.com](https://nixstats.com/) - 免费 1 台服务器。电子邮件通知、公共状态页面、60 秒间隔等。
  * [OnlineOrNot.com](https://onlineornot.com/) - 提供网站和 API 的在线监控，以及 cron 作业和计划任务的监控。还提供状态页面。前五个检查间隔为 3 分钟，免费。免费层通过 Slack、Discord 和电子邮件发送警报。
  * [OntarioNet.ca CN Test](https://cntest.ontarionet.ca) - 检查网站是否被中国的防火墙阻止。它通过比较由中国服务器和美国服务器检测到的 DNS 结果和 ASN 信息来识别 DNS 污染。
  * [opsgenie.com](https://www.opsgenie.com/) - 用于始终在线服务的强大警报和值班管理。免费最多 5 个用户。
  * [paessler.com](https://www.paessler.com/) - 功能强大的基础设施和网络监控解决方案，包括警报、强大的可视化功能和基本报告。免费最多 100 个传感器。
  ...
  * [pingbreak.com](https://pingbreak.com/) - 现代的监控服务。检查无限数量的 URL，并通过 Discord、Slack 或电子邮件获得停机通知。
  * [pingpong.one](https://pingpong.one/) - 具有监控功能的高级状态页面平台。免费层包括一个带有 SSL 子域的公共可定制状态页面。Pro 计划免费提供给开源项目和非营利组织。
  * [robusta.dev](https://home.robusta.dev/) - 基于 Prometheus 的强大 Kubernetes 监控。自带 Prometheus 或安装一体化捆绑包。免费层包括最多 20 个 Kubernetes 节点。通过 Slack、Microsoft Teams、Discord 等发送警报。与 PagerDuty、OpsGenie、VictorOps、DataDog 等许多其他工具集成。
  * [sematext.com](https://sematext.com/) - 24 小时指标免费，无限制的服务器，10 个自定义指标，500,000 个自定义指标数据点，无限制的仪表板，用户等。
  ...
  * [instatus.com](https://instatus.com) - 在 10 秒内获得漂亮的状态页面。免费无限子账户和无限团队
  * [Squadcast.com](https://squadcast.com) - Squadcast 是一款端到端事件管理软件，旨在帮助您推广 SRE 最佳实践。免费提供长达 10 个用户的永久计划
  * [RoboMiri.com](https://robomiri.com/) - RoboMiri 是一个稳定的正常运行时间监视器，提供广泛的监视器：cronjob、关键字、网站、端口、ping。免费提供 25 次正常运行时间检查，间隔 3 分钟检查。通过电话呼叫、短信、电子邮件和 Webhooks 发出警报
  * [Better Stack](https://betterstack.com/better-uptime) - 单一产品中包含正常运行时间监控、事件管理、值班安排/警报和状态页面。免费计划包括 10 个监视器，检查频率为 3 分钟，状态页面
...
## Crash and Exception Handling
  ...
  * [exceptionless](https://exceptionless.com) - 实时错误、功能、日志报告等。每月免费 3k 事件/1 用户。开源且易于自行托管，无限使用。
  ...
  * [sentry.io](https://sentry.io/) - Sentry 实时跟踪应用程序异常，有一个小的免费计划。每月免费 5k 个错误/1 用户，自托管不限使用
  * [Axiom](https://axiom.co/) - 存储长达 0.5 TB 的日志，保留 30 天。包括与 Vercel 等平台的集成，以及通过电子邮件/Discord 通知进行高级数据查询。
  * [Semaphr](https://semaphr.com) - 移动应用的免费一体化 Kill Switch。
  * [Jam](https://jam.dev) - 一键点击即可生成开发者友好的错误报告。免费计划无限 Jams。
  * [Whitespace](https://whitespace.dev) - 在浏览器中一键生成错误报告。免费计划个人使用无限录制。
...
## 搜索
  ...
  * [CommandBar](https://www.commandbar.com/) - 统一搜索栏即服务，基于网络的用户界面小部件/插件，允许用户在产品中搜索内容、导航、功能等，有助于提高可发现性。对于最多 1000 名每月活跃用户免费，无限制命令。
...
## 教育和职业发展
  ...
  * [免费专业简历模板和编辑器](https://www.overleaf.com/latex/templates/tagged/cv) - 免费平台，提供大量经验丰富的专业人士的简历模板，可完全克隆、编辑和下载，符合 ATS 优化。
  ...
  * [Cisco Networking Academy，面向所有人的技能](https://skillsforall.com/) - 提供与免费认证相关的课程，涵盖网络安全、网络和 Python 等主题。
  * [MIT 开放式课程](https://ocw.mit.edu/) - MIT 开放式课程是来自 2500 多门 MIT 课程的材料的在线出版物，与世界各地的学习者和教育工作者免费分享知识。可以在 Youtube 频道上找到[@mitocw](https://www.youtube.com/@mitocw/featured)

**[⬆️ 返回顶部](#目录)**

## 电子邮件

  * [10minutemail](https://10minutemail.com) - 免费的临时测试电子邮件。
  ...
  * [Antideo](https://www.antideo.com) - 免费版每小时 10 次 API 请求，用于电子邮件验证、IP 和电话号码验证。不需要信用卡。
  * [Brevo](https://www.brevo.com/) - 每月 9000 封电子邮件，每天 300 封电子邮件免费。
  * [Bump](https://bump.email/) - 免费 10 个 Bump 电子邮件地址，1 个自定义域名。
  ...
  * [EmailLabs.io](https://emaillabs.io/en) - 每月免费发送最多 9000 封电子邮件，每天最多 300 封。
  * [EmailOctopus](https://emailoctopus.com) - 免费提供最多 2500 个订阅者和 10,000 封电子邮件。
  * [EmailJS](https://www.emailjs.com/) - 这不是完整的电子邮件服务器；这只是一个电子邮件客户端，您可以使用它从客户端发送电子邮件，而无需暴露您的凭据，免费版每月有 200 个请求，2 个电子邮件模板，请求大小上限为 50Kb，联系人历史记录有限。
  ...
  * [Temp-Mail.org](https://temp-mail.org/en/) - 使用各种域名的临时/一次性邮件生成实用程序。每次重新加载页面时，电子邮件地址都会刷新。它完全免费，不包括任何服务费用。
  * [TempMailDetector.com](https://tempmaildetector.com/) - 每月免费验证最多 200 封电子邮件，并查看电子邮件是否为临时的。
  * [Emailvalidation.io](https://emailvalidation.io) - 每月免费进行 100 次电子邮件验证。
  * [FakeMailGenerator.com](https://www.fakemailgenerator.com/) - 德国临时/一次性邮件生成器。支持 10 个域名，同时让您自由创建无限数量的地址。（包含广告）但除此之外，该服务不包括任何费用，完全免费。
  * [forwardemail.net](https://forwardemail.net) - 免费的自定义域名电子邮件转发。创建并转发无限数量的电子邮件地址（**注意**：如果您使用.casa、.cf、.click、.email、.fit、.ga、.gdn、.gq、.lat、.loan、.london、.men、.ml、.pl、.rest、.ru、.tk、.top、.work TLD，则需要付费）。
  * [Imitate Email](https://imitate.email) - 用于测试电子邮件功能的沙盒电子邮件服务器，适用于构建/qa 和 ci/cd。免费帐户每天可获得 15 封电子邮件，永久有效。
  ...
  * [MailerSend.com](https://www.mailersend.com) - 电子邮件 API、SMTP，每月 3000 封免费的交易邮件。
  ...
  * [mailsac.com](https://mailsac.com) - 免费的临时电子邮件测试 API，免费的公共电子邮件托管，出站捕获，电子邮件到-slack/websocket/webhook（每月 1500 次 API 限制）。
  * [Mailtrap.io](https://mailtrap.io/) - 用于开发的虚假 SMTP 服务器，免费计划包括一个收件箱、100 条消息、没有团队成员、每秒两封电子邮件、没有转发规则。
  * [Mail7.io](https://www.mail7.io/) - 免费的 Temp 电子邮件地址，用于 QA 开发人员。使用 Web 界面或 API 即时创建电子邮件地址。
  ...
  * [Queuemail.dev](https://queuemail.dev) - 可靠的电子邮件传递 API。免费版（每月 10,000 封电子邮件）。异步发送。使用多个 SMTP 服务器。阻止列表、日志记录、跟踪、Webhooks 等。
  * [QuickEmailVerification](https://quickemailverification.com) - 在免费层免费每天验证 100 封电子邮件，以及其他免费 API，如 DEA 检测器、DNS 查找、SPF 检测器等。
  * [Resend](https://resend.com) - 开发人员的事务性电子邮件 API。每月 3000 封电子邮件，每天 100 封电子邮件，一个自定义域名。
  * [Sender](https://www.sender.net) - 每月最多 15000 封电子邮件，最多 2500 个订阅者。
  * [SendGrid](https://sendgrid.com/) - 每天 100 封电子邮件和 2000 个联系人免费。
  * [Sendpulse](https://sendpulse.com) - 500 个订阅者/月，15,000 封电子邮件/月免费。
  * [SimpleLogin](https://simplelogin.io/) - 开源、可自托管的电子邮件别名/转发解决方案。免费 5 个别名，无限带宽，无限回复/发送。免费适用于教育工作者（学生、研究人员等）。
  * [Substack](https://substack.com) - 无限免费新闻通讯服务。当您开始收费时。
  * [Sweego](https://www.sweego.io/) - 适用于开发人员的欧洲交易性电子邮件 API。每天免费 500 封电子邮件。
  ...
  * [Zoho](https://www.zoho.com)——最初是作为电子邮件提供商，但现在提供了一系列服务，其中一些提供免费计划。提供免费计划的服务列表：
     ...
     - [Connect](https://zoho.com/connect)——团队协作免费供 25 个用户使用，包括 3 个组、3 个自定义应用程序、3 个 Boards、3 个手册和 10 个集成以及频道、事件和论坛。
     - [Meeting](https://zoho.com/meeting)——会议最多可容纳 3 个会议参与者和 10 个网络研讨会参与者。
     - [Vault](https://zoho.com/vault)——个人可访问密码管理。
     - [Showtime](https://zoho.com/showtime)——另一个用于培训的会议软件，最多可容纳 5 个远程与会者。
     ...
     - [Subscriptions](https://zoho.com/subscriptions)——循环计费管理免费供 20 个客户/订阅者和 1 个用户使用，所有支付托管由 Zoho 完成。最后 40 个订阅指标存储
     - [Checkout](https://zoho.com/checkout)——产品计费管理，有 3 个页面和最多 50 笔付款。
     - [Desk](https://zoho.com/desk)——客户支持管理，有 3 个代理、私人知识库和电子邮件票证。与[Assist](https://zoho.com/assist)集成，用于 1 个远程技术人员和 5 个无人参与的计算机。
...
## 功能切换管理平台
  ...
  * [ConfigCat](https://configcat.com) - ConfigCat 是一款以开发人员为中心的功能标志服务，具有无限的团队规模、出色的支持和合理的价格标签。免费计划可支持高达 10 个标志、两个环境、1 个产品和每月 500 万次请求。
  ...
  * [Molasses](https://www.molasses.app) - 强大的功能标志和 A/B 测试。免费提供高达 3 个环境，每个环境最多 5 个功能标志。
  * [Toggled.dev](https://www.toggled.dev) - 企业级、可扩展的多区域功能标志管理平台。免费计划最多可支持 10 个标志、两个环境、无限制的请求。SDK、分析仪表板、发布日历、Slack 通知和所有其他功能都包含在无尽的免费计划中。
  * [Statsig](https://www.statsig.com) - 功能管理、A/B 测试、分析等的强大平台。其慷慨的免费计划提供无限席位、标志、实验和动态配置，每月支持高达 100 万次事件。
...
## 字体
  ...
  * [dafont](https://www.dafont.com/) - 该网站上提供的字体是其作者的财产，并且是免费软件、共享软件、演示版或公共领域的。
  * [Everything Fonts](https://everythingfonts.com/) - 提供多种工具；@font-face、单位换算器、字体黑客和字体提交器。
  ...
  * [Bunny](https://fonts.bunny.net) 以隐私为导向的 Google 字体
...
## 表单
  ...
  * [Feathery](https://feathery.io) - 功能强大、开发者友好的表单构建器。可构建注册/登录、用户引导、支付流程、复杂金融应用等。免费计划允许每月最多提交 250 次，并且有五个有效表单。
  * [Form-Data](https://form-data.com) - 无代码表单后端。提供垃圾邮件过滤器、电子邮件通知和自动回复、Webhooks、zapier、重定向、AJAX 或 POST 等功能。免费计划提供无限表单、每月 20 次提交，以及带有 Form-Data 徽章的额外 2000 次提交。
  ...
  * [formspark.io](https://formspark.io/) - 表单到电子邮件服务，免费计划允许每月无限表单，每次提交 250 次，有客户支持团队支持。
  ...
  * [Formlick.com](https://formlick.com) - Typeform 替代品，带有终身交易。用户可以创建一个免费表单并接收无限提交。在高级版中，用户可以创建无限表单和无限提交。
  * [getform.io](https://getform.io/) - 设计师和开发人员的表单后端平台，1 个表单，50 次提交，单文件上传，100MB 文件存储。
  * [HeroTofu.com](https://herotofu.com/) - 具有机器人检测和加密存档的表单后端。通过 UI 将提交内容转发到电子邮件、Slack 或 Zapier。使用您自己的前端。无需服务器代码。免费计划每月提供无限表单和 100 次提交。
  * [HeyForm.net](https://heyform.net/) - 拖放在线表单构建器。免费层允许创建无限表单并收集无限提交。附带预制模板、反垃圾邮件和 100MB 文件存储。
  * [Tally.so](https://tally.so/) - 99%的功能都是免费的。免费层允许您拥有：无限表单、无限提交、电子邮件通知、表单逻辑、收集付款、文件上传、自定义感谢页面等。
  * [Hyperforms.app](https://hyperforms.app/) - 在几秒钟内创建表单到电子邮件等，无需后端代码。个人帐户每月为您提供最多 50 次表单提交的免费使用。
  ...
  * [smartforms.dev](https://smartforms.dev/) - 功能强大且易于使用的网站表单后端，永久免费计划每月允许 50 次提交，250MB 文件存储，Zapier 集成，CSV/JSON 导出，自定义重定向，自定义响应页面，Telegram 和 Slack 机器人，单个电子邮件通知。
  * [Survicate](https://survicate.com/) - 从所有来源收集反馈并使用一个工具发送后续调查。自动使用 AI 分析反馈并提取见解。免费电子邮件、网站、应用内或移动调查、AI 调查创建者和每月 25 次响应。
  ...
  * [Typeform.com](https://www.typeform.com/) - 在网站上包含精美的设计表单。免费计划每个表单允许 10 个字段，每月允许 100 次响应。
  ...
  * [Wufoo](https://www.wufoo.com/) - 在网站上快速使用的表单。免费计划每月有 100 次提交限制。
  * [formpost.app](https://formpost.app) - 免费、无限的表单到电子邮件服务。免费设置自定义重定向、自动响应、Webhooks 等。
  * [Formester.com](https://formester.com) - 在您的网站上分享和嵌入独特外观的表单——不受计划限制的表单数量或功能限制。每月免费获得高达 100 次提交。
  * [SimplePDF.eu](https://simplepdf.eu/embed) - 在您的网站上嵌入 PDF 编辑器，并将任何 PDF 转换为可填写的表单。免费计划允许每个 PDF 有无限个 PDF 和三次提交。
  * [forms.app](https://forms.app/) - 使用强大功能（如条件逻辑、自动评分计算器和 AI）创建在线表单。免费计划可收集高达 100 次响应，可嵌入网站或使用链接。
...
## 生成式AI
  ...
  * [Braintrust](https://www.braintrustdata.com/) - Gen AI 的评估、提示符游乐场和数据管理。免费计划每周提供高达 1,000 条私有评估行。
  * [Findr](https://www.usefindr.com/) - 通用搜索，可让您一次搜索所有应用程序。搜索助手，可让您使用您的信息回答问题。免费计划提供无限统一搜索和 5 个每日合作飞行员查询。
  * [ReportGPT](https://report-gpt.io) - AI 写作助手。整个平台只要您提供自己的 API 密钥，就是免费的。
  * [Clair](https://askclair.ai/) - 临床 AI 参考。学生可以免费访问专业工具套件，其中包括 Open Search、临床摘要、Med 评论、药物相互作用、ICD-10 代码和管理。此外，还提供专业套件的免费试用。
  * [Langtrace](https://langtrace.ai) - 使开发人员能够跟踪、评估、管理提示和数据集，并调试与 LLM 应用程序性能相关的问题。它为任何 LLM 创建开放遥测标准跟踪，有助于可观测性，并与任何可观测性客户端配合使用。免费计划提供每月 50K 个跟踪。
...
## 内容分发网络（CDN）和保护
  ...
  * [toranproxy.com](https://toranproxy.com/) - Packagist 和 GitHub 的代理。永不失败的 CD。个人使用免费，一个开发人员，不提供支持
  * [UNPKG](https://unpkg.com/) - npm 上的所有内容的 CDN
  * [weserv](https://images.weserv.nl/) - 图像缓存和调整大小服务。使用全球缓存动态地操作图像。
  ...
  * [CacheFly](https://portal.cachefly.com/signup/free2023) - 每月高达 5TB 的免费 CDN 流量，19 个核心 PoPs，1 个域和通用 SSL。

**[⬆️ 返回顶部](#table-of-contents)**

## 平台即服务（PaaS）

  * [anvil.works](https://anvil.works) - 使用 Python 开发 Web 应用程序，无需其他任何东西。免费层提供无限数量的应用程序和 30 秒超时。
  * [appwrite](https://appwrite.io) - 无限项目，无项目暂停（支持 websockets）和身份验证服务。免费层每个项目有 1 个数据库、3 个存储桶、5 个函数。
  * [configure.it](https://www.configure.it/)——移动应用开发平台，免费提供两个项目，功能有限，但无资源限制
  ...
  * [Lade](https://www.lade.io/)——Lade 是为开发人员构建的云平台。免费层包括三个服务，可以是应用程序或数据库。每个服务包括 128MB RAM 和 1GB 存储。
  * [pipedream.com](https://pipedream.com)——为开发人员构建的集成平台。基于任何触发器开发任何工作流。工作流是您可以免费运行的代码[免费](https://docs.pipedream.com/pricing/)。无需管理服务器或云资源。
  * [pythonanywhere.com](https://www.pythonanywhere.com/)——云 Python 应用程序托管。入门帐户是免费的，在您的-your-username.pythonanywhere.com 域中有 1 个 Python Web 应用程序，512MB 私有文件存储，1 个 MySQL 数据库
  ...
  * [Koyeb](https://www.koyeb.com)——Koyeb 是一个面向开发者的无服务器平台，可在全球范围内部署应用程序。通过基于 git 的部署、本机自动缩放、全球边缘网络和内置服务网格和发现，无缝运行 Docker 容器、Web 应用程序和 API。免费实例可让您在德国法兰克福或美国华盛顿特区部署 Web 服务。免费的托管 PostgreSQL 数据库可在德国法兰克福（德国）、美国华盛顿特区（美国）和新加坡使用。512MB 内存、2GB 存储和 0.1 CPU。无需信用卡即可开始使用。
  ...
  * [WunderGraph](https://cloud.wundergraph.com)——一个开源平台，允许您快速构建、交付和管理现代 API。内置 CI/CD、GitHub 集成和自动 HTTPS。免费计划每月有 3 个项目、1GB 出口和 300 分钟构建时间。
  * [Zeabur](https://zeabur.com)——一键部署您的服务。免费提供三个服务，每月有 5 美元的免费信用额度。
  * [mogenius](https://mogenius.com)——轻松在 Kubernetes 上构建、部署和运行服务。免费层支持连接本地 Kubernetes 和 mogenius，使独立开发人员能够在其机器上创建类似生产的测试环境。
...
## 后端即服务（BaaS）
  ...
  * [Activepieces](https://www.activepieces.com) - 构建自动化流程，将多个应用在你的应用后端连接在一起。例如，当你的应用中发生事件时，发送一条 Slack 消息或在 Google 表格中添加一行。每月免费使用高达 5000 个任务。
  * [back4app.com](https://www.back4app.com) - Back4App 是一个易于使用、灵活且可扩展的后端，基于 Parse 平台。
  * [backendless.com](https://backendless.com/) — 移动和网络 Baas，免费提供 1GB 文件存储、每月 50,000 个推送通知和表中的 1000 个数据对象。
  * [bismuth.cloud](https://www.bismuth.cloud/) — 我们的人工智能将在我们的函数运行时和托管存储上引导你的 Python API，在我们的在线编辑器或本地使用你最喜欢的工具免费构建和托管。
  * [BMC Developer Program](https://developers.bmc.com/site/global/bmc_helix_platform/program/overview/index.gsp) — BMC Developer 计划提供文档和资源，帮助你为企业构建和部署数字创新。访问全面的个人沙盒，其中包括平台、SDK 和可用于构建和定制应用的组件库。
  * [connectycube.com](https://connectycube.com) - 无限聊天消息、p2p 语音和视频通话、文件附件和推送通知。对于最多 1000 个用户的应用免费。
  * [convex.dev](https://convex.dev/) - 反应式后端即服务，托管你的数据（具有关系和可序列化 ACID 事务的文档）、无服务器函数和 WebSockets，以将更新流式传输到各个客户端。对于小型项目免费 - 最多 1M 条记录，每月 5M 个函数调用。
  ...
  * [Flutter Flow](https://flutterflow.io) — 无需编写一行代码即可构建 Flutter App UI。还具有 Firebase 集成。免费计划包括对 UI 构建器和免费模板的完全访问。
  * [getstream.io](https://getstream.io/) - 在几个小时内而不是几周内构建可扩展的应用内聊天、消息传递、视频和音频以及源。
  * [hasura.io](https://hasura.io/) - Hasura 扩展了你所在位置的现有数据库，并提供了一个即时的 GraphQL API，可安全地用于网络、移动和数据集成工作负载。免费提供 1GB/月的数据传输。
  ...
  * [progress.com](https://www.progress.com/kinvey) - 移动后端，入门计划每秒无限制请求，1GB 数据存储。企业应用程序支持
  ...
  * [pushcrew.com](https://pushcrew.com/) - 推送通知服务。对于最多 2,000 个订阅者的无限制通知
  * [pusher.com](https://pusher.com/beams) - 免费、无限制的推送通知，适用于每月 2000 名活跃用户的 iOS 和 Android 设备。单一 API。
  ...
  * [Salesforce Developer Program](https://developer.salesforce.com/signup) — 使用拖放工具快速构建、测试和部署应用程序。自定义数据模型只需点击即可。使用 Apex 代码更进一步。使用强大的 API 与任何内容集成。使用企业级安全性进行保护。使用点击或任何前沿网络框架自定义 UI。免费开发者计划可访问完整的 Lightning 平台。
  * [ServiceNow Developer Program](https://developer.servicenow.com/) - 快速构建、测试和部署可改善组织工作的应用程序。免费实例和早期预览访问权限。
  * [simperium.com](https://simperium.com/) - 立即在各个平台上随时随地移动数据，多平台，无限制发送和存储结构化数据，每月最多 2500 名用户。
  * [Singlebase.cloud](https://singlebase.cloud) — SinglebaseCloud 是一个由人工智能驱动的一体化后端平台，可加速应用程序开发。它提供了向量数据库、关系文档数据库、身份验证、搜索和存储等工具，旨在简化后端开发。免费/入门计划提供关系文档数据库、身份验证、搜索、存储。
  ...
  * [zapier.com](https://zapier.com/) - 将你使用的应用程序连接起来以自动化任务。每 15 分钟五个 zaps 和每月 100 个任务
  * [IFTTT](https://ifttt.com) — 自动化你最喜欢的应用程序和设备。免费 2 个小程序
  * [Integrately](https://integrately.com) — 只需点击一下即可自动化繁琐的任务。免费 100 个任务，15 分钟更新时间，五个活动自动化，Webhooks。
...
## 低代码平台
  ...
  * [ILLA Cloud](https://www.illacloud.com/)——ILLA Cloud——一个强大的开源低代码平台，供开发人员构建内部工具。通过使用 ILLA 的组件和操作库，开发人员可以在构建工具时节省大量时间。免费提供给 5 个团队成员。
  ...
  * [Mendix](https://www.mendix.com/)——企业快速应用开发，无限可访问的沙盒环境支持总用户，每个应用程序 0.5GB 存储和 1GB RAM。免费层还允许使用 Studio 和 Studio Pro IDE。
  * [lil'bots](https://www.lilbots.io/)——在线编写和运行脚本，利用 OpenAI、Anthropic、Firecrawl 等免费内置 API。非常适合构建 AI 代理/内部工具并与团队共享。免费层包括对 API 的完全访问、AI 编码助手和每月 10,000 次执行信用。

**[⬆️ 返回顶部](#目录)**

## 虚拟主机服务

  * [Alwaysdata](https://www.alwaysdata.com/)—提供 100MB 免费网络托管，支持 MySQL、PostgreSQL、CouchDB、MongoDB、PHP、Python、Ruby、Node.js、Elixir、Java、Deno 等，可通过 FTP、WebDAV 和 SSH 访问；包含邮箱、邮件列表和应用程序安装器。
  * [Awardspace.com](https://www.awardspace.com)—免费网络托管+免费短域名，支持 PHP、MySQL、应用程序安装器、电子邮件发送和无广告。
  * [Bohr](https://bohr.io)—面向非商业项目的免费+开发者优先的部署和开发平台，可减少基础设施的麻烦并加快设置速度。
  ...
  * [HelioHost](https://heliohost.org) —非盈利免费网络托管，提供 Plesk 控制面板、PHP、Node.js、Python、Django、Flask、.NET、Perl、CGI、MySQL、PostgreSQL、SQLite、IMAP/POP3/SMTP 电子邮件、无限带宽、免费子域、1000 MB 存储空间，可选择升级。
  * [Kinsta Static Site Hosting](https://kinsta.com/static-site-hosting/) —免费部署多达 100 个静态站点，支持自定义域名和 SSL，100GB 月流量，260+Cloudflare CDN 位置。
  ...
  * [pantheon.io](https://pantheon.io/) —Drupal 和 WordPress 托管，自动化 DevOps 和可扩展基础设施。免费提供给开发人员和机构。不支持自定义域名。
  * [readthedocs.org](https://readthedocs.org/) —提供版本控制、PDF 生成等功能的免费文档托管。
  * [render.com](https://render.com) —统一的云，用于使用免费 SSL、全球 CDN、私有网络、从 Git 自动部署以及完全免费的网络服务、数据库和静态网页计划构建和运行应用程序和站点。
  ...
  * [tilda.cc](https://tilda.cc/) —一个站点，50 页，50MB 存储空间，170 多个可用的主要预定义块中，没有字体，没有 favicon，也没有自定义域名。
  ...
  * [FreeFlarum](https://freeflarum.com/) —社区驱动的免费 Flarum 托管，最多支持 250 个用户（捐赠可去除页脚的水印）。
  * [fleek.co](https://fleek.co/) —在开放网络及其协议上无缝构建现代网站和应用程序，免费提供无限网站和 50GB/月的带宽。
  ...
  * [Serv00.com](https://serv00.com/) —提供 3GB 免费网络托管，每天备份（7 天）。支持：Crontab 作业、SSH 访问、存储库（GIT、SVN 和 Mercurial），支持：MySQL、PostgreSQL、MongoDB、PHP、Node.js、Python、Ruby、Java、Perl、TCL/TK、Lua、Erlang、Rust、Pascal、C、C++、D、R 等。

**[⬆️ 返回顶部](#table-of-contents)**

## 域名系统（DNS）

  * [1.1.1.1](https://developers.cloudflare.com/1.1.1.1/) - 由 Cloudflare 提供的免费公共 DNS 解析器，快速且安全（加密 DNS 查询）。可用于绕过互联网提供商的 DNS 阻塞、防止 DNS 查询监控，以及[阻止成人和恶意软件内容](https://developers.cloudflare.com/1.1.1.1/1.1.1.1-for-families)。也可以通过[API 使用](https://developers.cloudflare.com/1.1.1.1/encrypted-dns/dns-over-https/make-api-requests)。注意：只是一个 DNS 解析器，不是 DNS 主机提供商。
  ...
  * [deSEC](https://desec.io)——带 API 支持的免费 DNS 托管，设计时考虑了安全性。运行在开源软件上，并得到[SSE](https://www.securesystems.de/)的支持。
  ...
  * [duckdns.org](https://www.duckdns.org/)——免费 DDNS，免费层最多 5 个域名。有各种设置的配置指南。
  * [Dynv6.com](https://dynv6.com/)——免费 DDNS 服务，支持[API](https://dynv6.com/docs/apis)和管理大量 DNS 记录类型（如 CNAME、MX、SPF、SRV、TXT 等）。
  * [freedns.afraid.org](https://freedns.afraid.org/)——免费 DNS 托管。还提供基于众多公共用户[贡献的域名](https://freedns.afraid.org/domain/registry/)的免费子域名。注册后从“子域名”菜单中获取免费子域名。
  * [luadns.com](https://www.luadns.com/)——免费 DNS 托管，三个域名，所有功能都有限制。
  ...
  * [noip.at](https://noip.at/)——免费 DDNS 服务，无注册、跟踪、日志记录或广告。对域名数量没有限制。
  * [noip](https://www.noip.com/)——动态 DNS 服务，免费提供最多 3 个主机名，每 30 天需要确认一次。
  * [sslip.io](https://sslip.io/)——免费 DNS 服务，用主机名查询时，如果主机名中嵌入了 IP 地址，则返回该 IP 地址。
  ...
  * [huaweicloud.com](https://www.huaweicloud.com/intl/en-us/product/dns.html)——华为的免费 DNS 托管。
  * [Hetzner](https://www.hetzner.com/dns-console)——带 API 支持的 Hetzner 免费 DNS 托管。
...
## 基础设施即服务（IaaS）
  ...
  * [4EVERLAND](https://www.4everland.org/)——与 AWS S3 兼容——API、接口操作、CLI 和其他上传方法，以安全、便捷和高效的方式从 IPFS 和 Arweave 网络上传和存储文件。注册用户可以免费获得 6GB 的 IPFS 存储和 300MB 的 Arweave 存储。任何小于 150KB 的 Arweave 文件上传都是免费的。
  * [backblaze.com](https://www.backblaze.com/b2/)——Backblaze B2 云存储。免费 10GB（类似亚马逊 S3）对象存储，无限期
  * [filebase.com](https://filebase.com/)——区块链驱动的 S3 兼容对象存储。5GB 免费存储，无限期
  * [Tebi](https://tebi.io/)——S3 兼容对象存储。免费 25GB 存储和 250GB 出站传输
  * [Idrive e2](https://www.idrive.com/e2/)——S3 兼容对象存储。10GB 免费存储和每月 10GB 下载带宽
  * [C2 Object Storage](https://c2.synology.com/en-us/pricing/object-storage)——S3 兼容对象存储。15GB 免费存储和每月 15GB 下载
  * [Spheron](https://spheron.network/)——从去中心化云存储和网络托管到应用程序和开发人员的去中心化计算，一个平台下的一切，免费计划提供 5GB 存储、100GB 带宽、无限域名和项目，以及价值 5 美元的计算资源（相对于 AWS 价值 50 美元）。
...
## 托管数据服务
  ...
  * [codehooks.io](https://codehooks.io/) — 易于使用的 JavaScript 无服务器 API/后端和 NoSQL 数据库服务，具有功能、类似于 Mongdb 的查询、键/值查找、作业系统、实时消息、工作队列、强大的 CLI 和基于网络的数据管理器。免费计划有 5GB 存储空间和每分钟 60/API 调用。包含 2 名开发人员。无需信用卡。
  ...
  * [Upstash](https://upstash.com/) — 无服务器 Redis，免费层每天高达 10,000 个请求、最大数据库大小 256MB 和 20 个并发连接
  ...
  * [Turso by ChiselStrike](https://chiselstrike.com/) - Turso 是边缘数据库中的 SQLite 开发人员体验。Turso 提供免费永久的入门计划，总存储量为 9GB，最多可容纳 500 个数据库，最多可容纳 3 个位置，每月读取 10 亿行，以及本地开发支持与 SQLite。
  * [InfluxDB](https://www.influxdata.com/) — 时间序列数据库，免费至 3MB/5 分钟写入、30MB/5 分钟读取和 10,000 个基数系列
  ...
  * [cockroachlabs.com](https://www.cockroachlabs.com/free-tier/) — 免费的 CockroachDB 最高可达 5GB 和 1vCPU（有限的[请求单位](https://www.cockroachlabs.com/docs/cockroachcloud/serverless-faqs.html#what-are-the-usage-limits-of-cockroachdb-serverless-beta))
  * [Neo4j Aura](https://neo4j.com/cloud/aura/) — 托管的本机图形数据库管理系统/分析平台，具有 Cypher 查询语言和 REST API。对图形大小的限制（50k 个节点，175k 个关系）。
  * [Neon](https://neon.tech/) — 托管的 PostgreSQL，总存储量为 0.5GB，1 个项目，10 个分支，无限制的数据库，始终可用的主分支（非主分支计算后 5 分钟自动暂停），每月 20 小时的活动时间
  * [Dgraph Cloud](https://cloud.dgraph.io/pricing?type=free) — 托管的本机图形数据库管理系统，具有 GraphQL API。每天限制 1MB 数据传输。
  ...
  * [TigerGraph Cloud](https://www.tigergraph.com/cloud/) — 托管的本机图形数据库管理系统/分析平台，具有类似于 SQL 的图形查询语言和 REST API。一个免费实例，两个 vCPU、8GB 内存和 50GB 存储，闲置 1 小时后睡眠。
  * [TerminusCMS](https://terminusdb.com/pricing) — 托管的免费服务，适用于基于 Prolog 和 Rust 编写的文档和图形数据库 TerminusDB。免费供开发使用，付费服务用于企业部署和支持。
  * [YugabyteDB](https://cloud.yugabyte.com) - YugabyteDB 是一个与 PostgreSQL 兼容的分布式 SQL 数据库。无云层包括两个 vCPU、4GB RAM 和 10GB 磁盘。
  * [filess.io](https://filess.io) - filess.io 是一个平台，您可以在其中免费创建以下任何一种 DBMS 的数据库：MySQL、MariaDB、MongoDB 和 PostgreSQL。
  * [xata.io](https://xata.io) - Xata 是一个无服务器数据库，内置强大的搜索和分析功能。一个 API，多个类型安全的客户端库，并且针对您的开发工作流程进行了优化。免费永久层足以满足业余开发人员的需求，其中包括三个 Xata 单位，请参考网站了解单位定义。
  * [8base.com](https://www.8base.com/) - 8base 是一个为 JavaScript 开发人员构建的全栈低代码开发平台，建立在 MySQL 和 GraphQL 之上，以及无服务器后端即服务之上。它允许您使用 UI 应用程序构建器快速构建 Web 应用程序并快速扩展，免费层包括行：2,500、存储：500、无服务器计算：1Gb/h 和客户端应用程序用户：5。
  * [Nile](https://www.thenile.dev/) — 一个用于 B2B 应用程序的 Postgres 平台。无限数据库，始终可用，无停机，1GB 存储（总），5000 万查询令牌，自动缩放，无限向量嵌入
...
## 隧道、WebRTC、WebSocket服务器和其他路由器
  ...
  * [Pinggy](https://pinggy.io)——只需一条命令，无需下载，即可为本地主机提供公共 URL。支持 HTTPS / TCP / TLS 隧道。免费计划的隧道生命周期为 60 分钟。
  * [conveyor.cloud](https://conveyor.cloud/)——适用于 Visual Studio 的扩展，可将 IIS Express 暴露到本地网络或通过隧道暴露到公共 URL。
  * [Hamachi](https://www.vpn.net/)——LogMeIn Hamachi 是一项托管式 VPN 服务，允许您使用免费计划创建无限个网络，每个网络最多可容纳 5 人，将类似 LAN 的网络安全扩展到分布式团队。
  ...
  * [Radmin VPN](https://www.radmin-vpn.com/)——通过启用类似 LAN 的网络将多台计算机连接在一起。无限对等方。（Hamachi 替代品）
  * [segment.com](https://segment.com/)——用于将事件翻译成并路由到其他第三方服务的中心。免费计划每月可处理 100,000 个事件和 100,000 次尝试，保留时间为三天。
  * Google STUN——[stun:stun.l.google.com:19302](stun:stun.l.google.com:19302)
  * Twilio STUN——[stun:global.stun.twilio.com:3478?transport=udp](stun:global.stun.twilio.com:3478?transport=udp)
  * [Tailscale](https://tailscale.com/)——使用开源的 WireGuard 协议的零配置 VPN。可安装在 MacOS、iOS、Windows、Linux 和 Android 设备上。个人使用的免费计划可容纳 100 个设备和 3 个用户。
  * [webhookrelay.com](https://webhookrelay.com)——管理、调试、分流和代理您所有的网络钩子到公共或内部（即 localhost）目的地。此外，通过获取公共 HTTP 终结点（`https://yoursubdomain.webrelay.io <----> http://localhost:8080`），可以将在私有网络中运行的服务器暴露到隧道中。
  * [Hookdeck](https://hookdeck.com/pricing)——从任何地方开发、测试和监控您的网络钩子。每月 100K 请求和 100K 尝试，保留三天。
  * [Xirsys](https://www.xirsys.com/pricing/)——无限的 STUN 使用+每月 500MB 的 TURN 带宽，限制带宽，单个地理区域。
  * [ZeroTier](https://www.zerotier.com)——免费的托管虚拟以太网即服务。免费计划可在一个网络中容纳 25 个客户端的端到端加密网络。客户端适用于桌面/移动/NA；Web 界面用于配置自定义路由规则和批准私有网络上的新客户端节点。
  * [LocalXpose](https://localxpose.io)——反向代理，可让您将本地服务器暴露到互联网。免费计划的隧道生命周期为 15 分钟。
  * [Traefik-Hub](https://traefik.io/traefik-hub/)——通过隧道将本地运行的服务发布到公共自定义 URL 并使用访问控制进行保护。一个集群中免费使用 5 个服务。
  * [Expose](https://expose.dev/)——通过安全隧道公开本地站点。免费计划包括欧盟服务器、随机子域名和单个用户。
  * [btunnel](https://www.btunnel.in/)——将 localhost 和本地 tcp 服务器暴露到互联网。免费计划包括文件服务器、自定义 http 请求和响应标头、基本身份验证保护和 1 小时隧道超时。
...
## 问题跟踪和项目管理
  ...
  * [asana.com](https://asana.com/)——私人项目免费，最多可与合作者协作
  * [Backlog](https://backlog.com)——您的团队在一个平台上发布出色项目所需的一切。免费计划提供 1 个项目，最多 10 名用户和 100MB 存储空间。
  * [Basecamp](https://basecamp.com/personal)——待办事项列表、里程碑管理、类似论坛的消息传递、文件共享和时间跟踪。最多 3 个项目、20 名用户和 1GB 存储空间。
  * [bitrix24.com](https://www.bitrix24.com/)——内部网和项目管理工具。免费计划对无限用户提供 5GB。
  * [cacoo.com](https://cacoo.com/)——在线实时图表：流程图、UML、网络。免费版最多 15 名用户/图表，25 张工作表
  * [Chpokify](https://chpokify.com/)——基于团队的规划扑克，可节省冲刺估计时间。免费版最多 5 名用户，免费 Jira 集成，无限视频通话，无限团队，无限会话。
  * [clickup.com](https://clickup.com/)——项目管理。免费，带有云存储的高级版。移动应用和 Git 集成可用。
  * [Clockify](https://clockify.me)——时间跟踪和时间表应用程序，可让您跨项目跟踪工作时间。无限用户，永久免费。
  * [Cloudcraft](https://cloudcraft.co/)——使用 Cloudcraft 可视化设计器在几分钟内设计专业架构图，针对 AWS 进行了优化，具有智能组件，可显示实时数据。免费计划为单个用户提供无限私有图表。
  ...
  * [Confluence](https://www.atlassian.com/software/confluence)—— Atlassian 的内容协作工具用于帮助团队高效协作和共享知识。免费计划最多供 10 名用户使用。
  ...
  * [diagrams.net](https://app.diagrams.net/)——存储在 Google Drive、OneDrive 或 Dropbox 中的在线图表。所有功能和存储级别均免费。
  * [freedcamp.com](https://freedcamp.com/)——任务、讨论、里程碑、时间跟踪、日历、文件和密码管理器。免费计划无限项目、用户和文件存储。
  * [easyretro.io](https://www.easyretro.io/)——简单直观的冲刺回顾工具。免费版每月每个板有三个公共板和一个调查。
  * [GForge](https://gforge.com)——用于复杂项目的项目管理和问题跟踪工具集，具有自托管和 SaaS 选项。SaaS 免费计划为前 5 个用户提供免费服务，开源项目也免费。
  ...
  * [Ilograph](https://www.ilograph.com/)——交互式图表，允许用户从多个角度和详细级别查看其基础架构。图表可以用代码表示。免费层提供无限的私有图表，最多可容纳 3 个查看器。
  * [Jira](https://www.atlassian.com/software/jira)——用于许多企业环境的高级软件开发项目管理工具。免费计划最多供 10 名用户使用。
  ...
  * [Kumu.io](https://kumu.io/)——具有动画、装饰、过滤器、聚类、电子表格导入等功能的关系图。免费层允许无限的公共项目。图形大小不受限制。免费的私人项目供学生使用。如果您不想将文件公开在线（上传、编辑、下载、丢弃），则提供沙盒模式。
  * [Linear](https://linear.app/)——具有简洁界面的问题跟踪器。免费，无限制成员，最大 10MB 文件上传大小，250 个问题（不包括存档）
  * [leiga.com](https://www.leiga.com/)——Leiga 是一款使用 AI 自动管理项目的 SaaS 产品，可帮助您的团队集中注意力并释放巨大潜力，确保项目按计划推进。免费供最多 10 名用户使用，20 个自定义字段，2GB 存储空间，AI 录制的视频限制为 5 分钟/视频，自动化运行速度为 20/用户/月。
  * [Lucidchart](https://www.lucidchart.com/)——具有协作功能的在线图表工具。免费计划包括三个可编辑文档、100 个专业模板和基本协作功能。
  * [MeisterTask](https://www.meistertask.com/)——面向团队的在线任务管理。免费最多可用于 3 个项目和无限项目成员。
  ...
  * [Ora](https://ora.pm/)——敏捷任务管理和团队协作。免费供最多 3 名用户使用，文件限制为 10MB。
  ...
  * [Plane](https://plane.so/)——Plane 是一个简单、可扩展、开源的项目和产品管理工具。免费供无限成员使用，最大文件上传大小为 5MB，问题数量为 1000。
  * [planitpoker.com](https://www.planitpoker.com/)——免费在线规划扑克（估算工具）
  * [point.poker](https://www.point.poker/)——在线规划扑克（基于共识的估算工具）。免费供无限用户、团队、会话、轮次和投票使用。无需注册。
  * [ScrumFast](https://www.scrumfast.com)——具有非常直观界面的 Scrum 板，免费供最多 5 名用户使用。

  * [Shortcut](https://shortcut.com/) - 项目管理平台。免费版最多可支持 10 个用户永久使用。
  * [SpeedBoard](https://speedboard.app) - 用于敏捷和 Scrum 回顾的看板-免费。
  * [Tadum](https://tadum.app) - 为定期会议设计的会议议程和会议纪要应用程序，团队最多可免费使用 10 个席位
  * [taiga.io](https://taiga.io/) — 适用于初创公司和敏捷开发者的项目管理平台，开源免费
  * [Tara AI](https://tara.ai/) — 简单的冲刺管理服务。免费版没有用户限制，任务、冲刺和工作区数量无限制。
  * [targetprocess.com](https://www.targetprocess.com/) — 可视化项目管理，从看板和 Scrum 到几乎任何运营流程。免费版无限用户，最多 1,000 个数据实体{[更多详细信息](https://www.targetprocess.com/pricing/)}
  * [taskade.com](https://www.taskade.com/) — 实时协作任务列表和团队大纲。免费版有一个工作区，任务和项目不限；1GB 文件存储；项目历史记录为 1 周；视频会议每次可容纳 5 名与会者。
  * [taskulu.com](https://taskulu.com/) — 基于角色的项目管理。免费版最多支持 5 个用户。与 GitHub/Trello/Dropbox/Google Drive 集成
  * [Teaminal](https://www.teaminal.com) - 适用于远程团队的站立会议、回顾和冲刺规划工具。免费版最多支持 15 个用户。
  ...
  * [todoist.com](https://todoist.com/) — 协作和个人任务管理。免费版：5 个活跃项目、项目中 5 个用户、上传文件最大 5MB、3 个过滤器和一周活动历史记录。
  ...
  * [vabotu](https://vabotu.com/) - 用于项目管理的协作工具。提供免费和其他计划。自由职业者计划供 10 个用户使用，包括消息传递、任务看板、5GB 在线存储、工作区和导出数据。
  * [Wikifactory](https://wikifactory.com/) — 具有项目、VCS 和问题的产品设计服务。免费版提供无限项目和协作者以及 3GB 存储。
  ...
  * [zenkit.com](https://zenkit.com) — 项目管理和协作工具。免费版最多 5 名成员，5GB 附件。
  * [Zube](https://zube.io) — 具有免费计划的项目管理，适用于 4 个项目和 4 个用户。支持 GitHub 集成
  * [Toggl](https://toggl.com/) — 提供两个免费的生产力工具。[Toggl Track](https://toggl.com/track/)用于时间管理和跟踪应用程序，免费版提供无缝时间跟踪和报告，专为自由职业者设计。它具有无限的跟踪记录、项目、客户、标签、报告等。以及[Toggl Plan](https://toggl.com/plan/)用于任务规划，免费版供独奏开发人员使用，任务、里程碑和时间线不限。
  * [Sflow](https://sflow.io) — sflow.io 是一款专为敏捷软件开发、营销、销售和客户支持而构建的项目管理工具，特别是对于外包和跨组织协作项目。免费版最多 3 个项目和 5 个成员。
  * [Pulse.red](https://pulse.red) — 用于项目的免费极简主义时间跟踪和时间表应用程序。

**[⬆️ 返回顶部](#table-of-contents)**

## 存储和媒体处理

  * [AndroidFileHost](https://androidfilehost.com/) - 免费文件共享平台，不限速度、带宽、文件数量、下载数量等。主要面向与 Android 开发相关的文件，如 APK 构建、自定义 ROM 和修改等，但似乎也接受任何其他文件。
  * [borgbase.com](https://www.borgbase.com/) — 用于 Borg Backup 的简单安全的离线备份主机。提供 10GB 免费备份空间和两个存储库。
  ...
  * [cloudimage.io](https://www.cloudimage.io/en/home) — 具有全球 1500 多个存在点的全面图像优化和 CDN 服务。多种图像缩放、压缩和水印功能。适用于响应式图像、360 度图像制作和图像编辑的开源插件。免费计划每月提供 25GB 的 CDN 流量、25GB 的缓存存储和无限转换。
  * [cloudinary.com](https://cloudinary.com/) — 用于网站和应用的图像上传、强大的操作、存储和交付，具有 Ruby、Python、Java、PHP、Objective-C 等更多库。免费层包括 25 个每月积分。一个积分等于 1000 次图像转换、1GB 存储空间或 1GB CDN 使用量。
  * [embed.ly](https://embed.ly/) — 提供在网页中嵌入媒体、响应式图像缩放和从网页中提取元素的 API。免费用于每月最多 5000 个 URL，每秒 15 个请求
  * [filestack.com](https://www.filestack.com/) — 文件选择、转换和交付，免费用于 250 个文件、500 个转换和 3GB 带宽
  ...
  * [GoFile.io](https://gofile.io/) - 免费文件共享和存储平台，可通过基于网络的 UI 和 API 使用。不限文件大小、带宽、下载数量等。但文件在不活动（10 天以上没有下载）时会被删除。
  ...
  * [Imgbot](https://github.com/marketplace/imgbot) — Imgbot 是一个友好的机器人，它可以优化你的图像并为你节省时间。优化后的图像意味着更小的文件大小，而不会牺牲质量。对于开源项目是免费的。
  ...
  * [kvstore.io](https://www.kvstore.io/) — 键值存储服务。免费层允许 100 个键、1KB/键、100 次/小时的调用
  * [npoint.io](https://www.npoint.io/) — 具有协作模式编辑的 JSON 存储
  * [nitropack.io](https://nitropack.io/) - 通过完整的前端优化（缓存、图像和代码优化、CDN）自动提高网站速度。免费计划适用于每月最多 5000 次页面浏览量
  ...
  * [Pinata IPFS](https://pinata.cloud) — Pinata 是上传和管理 IPFS 上文件的最简单方法。我们友好的用户界面和 IPFS API 使 Pinata 成为最容易使用的 IPFS 钉扎服务，适用于平台、创作者和收藏家。免费提供 1GB 存储空间和 API 访问权限。
  ...
  * [Shotstack](https://shotstack.io) - 用于按比例生成和编辑视频的 API。每月免费使用时间长达 20 分钟
  * [tinypng.com](https://tinypng.com/) — 用于压缩和调整 PNG 和 JPEG 图像大小的 API，每月免费提供 500 次压缩
  * [transloadit.com](https://transloadit.com/) — 处理文件上传以及视频、音频、图像、文档的编码。免费用于开源、慈善机构和学生，通过 GitHub 学生开发者包。商业应用在测试驱动时可获得 2GB 免费使用
  * [twicpics.com](https://www.twicpics.com) - 响应式图像即服务。它提供图像 CDN、媒体处理 API 和前端库，可自动优化图像。服务免费使用，每月流量上限为 3GB。
  * [uploadcare.com](https://uploadcare.com/hub/developers/) — Uploadcare 提供基于先进算法的媒体管道的终极工具包。所有功能对开发人员完全免费：文件上传 API 和 UI、图像 CDN 和源服务、自适应交付和智能压缩。免费层包括 3000 次上传、3GB 流量和 3GB 存储。
  * [imagekit.io](https://imagekit.io) - 具有自动优化、实时转换和存储的图像 CDN，您可以在几分钟内集成到现有设置中。免费计划每月包括高达 20GB 的带宽。
  * [internxt.com](https://internxt.com) - Internxt Drive 是基于绝对隐私和毫不妥协的安全性的零知识文件存储服务。注册并获得 10GB 免费，永远！
  * [degoo.com](https://degoo.com/) - 基于人工智能的云存储，免费提供高达 20GB、三个设备、5GB 推荐奖金（90 天账户不活动）。

  * [ImageEngine](https://imageengine.io/) - ImageEngine 是一个易于使用的全球图像 CDN。设置时间在 60 秒以下。支持 AVIF 和 JPEGXL，有 WordPress-、Magento-、React-、Vue- 等插件等。在此[申请](https://imageengine.io/developer-program/)免费开发者账号。
  * [DocsParse](https://docsparse.com/) - 使用 GPT 对 PDF、图像进行人工智能处理，将其转换为 JSON、CSV、EXCEL 格式的结构化数据。每月免费提供 30 个积分。
...
## 设计和用户界面（UI）
  ...
  * [Backlight](https://backlight.dev/)——Backlight 是一个完整的编码平台，它将开发人员和设计师的合作放在心中，团队可以在其中构建、记录、发布、扩展和维护设计系统。免费计划允许最多 3 个编辑器在一个设计系统上工作，无限制的查看者。
  ...
  * [CodeMyUI](https://codemyui.com/)——精选的 Web 设计和 UI 灵感资源，带有代码片段。
  ...
  * [css-gradient.com](https://www.css-gradient.com/)——免费的工具，用于快速生成自定义跨浏览器 CSS 渐变。以 RGB 和 HEX 格式。
  ...
  * [framer.com](https://www.framer.com/)——Framer 帮助您为下一个应用程序、网站或产品迭代和动画界面创意——从强大的布局开始。对于将 Framer 视为专业原型工具的任何人：无限制的查看者，最多 2 个编辑器和最多 3 个项目。
  ...
  * [Mindmup.com](https://www.mindmup.com/)——无限的思维导图免费，并存储在云中。您的思维导图随时随地，从任何设备上都可用。
  ...
  * [Octopus.do](https://octopus.do)——可视化的站点图构建器。实时构建您的网站结构，并迅速与您的团队或客户共享以进行协作。
  ...
  * [photopea.com](https://www.photopea.com/)——一款免费的高级在线设计编辑器，具有 Adobe Photoshop 用户界面，支持 PSD、XCF 和 Sketch 格式（Adobe Photoshop、Gimp 和 Sketch App）。
  * [pixlr.com](https://pixlr.com/)——免费的在线浏览器编辑器，可与商业编辑器媲美。
  * [Plasmic](https://www.plasmic.app/)——一个快速、易于使用、强大的网络设计工具和页面构建器，可集成到您的代码库中。构建响应式页面或复杂组件；可选地使用代码扩展；并发布到生产站点和应用程序。
  * [Pravatar](https://pravatar.cc/)——生成一个随机的占位符假头像，其 URL 可以直接在您的网络/应用程序中链接。
  * [Proto.io](https://www.proto.io)——无需编码即可创建完全交互的 UI 原型。免费层在免费试用结束后可用。免费层包括一个用户、一个项目、五个原型、100MB 的在线存储和 proto.io 应用程序的预览。
  * [resizeappicon.com](https://resizeappicon.com/)——一个简单的服务，用于调整和管理您的应用程序图标。
  * [Rive](https://rive.app)——为任何平台创建和交付美丽的动画。个人用户免费永久使用。该服务是一个编辑器，也在其服务器上托管所有图形。他们还为许多平台提供运行时，以运行使用 Rive 创建的表示形式。
  ...
  * [UI Avatars](https://ui-avatars.com/)——使用名称的首字母生成头像。URL 可以直接在您的网络/应用程序中链接。支持通过 URL 传递配置参数。
  ...
  * [Unicorn Platform](https://unicornplatform.com/) - 轻松的登录页面构建器和托管。一个免费网站。
  ...
  * [Superdesigner](https://superdesigner.co) - 一个免费的设计工具集合，可通过点击几下创建独特的背景、图案、形状、图像等。
  ...
  * [okso.app](https://okso.app) - 简单免费的在线绘图应用程序。允许创建快速草图和视觉笔记。将草图导出为 PNG、JPG、SVG 和 WEBP。也可作为 PWA 安装。免费供所有人使用（无需注册）。
  * [Wdrfree SVG](https://wdrfree.com/free-svg) - 黑白免费 SVG 切割文件。
  * [Lucide](https://lucide.dev) - 免费可定制且一致的 SVG 图标工具包。
  * [Logo.dev](https://www.logo.dev) - 拥有 4400 万多个品牌的公司徽标 API，使用 URL 调用非常简单。前 10,000 次 API 调用免费。
  ...
  * [css.glass](https://css.glass/) -- 使用 CSS 创建玻璃质感设计的免费网络应用程序。
...
## 设计灵感
  ...
  * [awwwards.](https://www.awwwards.com/) - [顶尖网站]一个展示所有最佳设计网站的平台（由设计师投票选出）。
  * [Behance](https://www.behance.net/) - [设计展示]设计师展示作品的地方。可以按 UI/UX 项目的类别进行筛选。
  * [dribbble](https://dribbble.com/) - [设计展示]独特的设计灵感，通常不是来自真实应用程序。
  * [Landings](https://landings.dev/) - [网页截图]根据您的偏好，找到最适合您设计灵感的最佳着陆页。
  * [Lapa Ninja](https://www.lapa.ninja/) - [着陆页/UI 套件/网页截图]Lapa Ninja 是一个画廊，展示了最佳的 6025 个着陆页示例，为设计师提供免费书籍和免费的 UI 套件来自网络。
  * [LovelyLanding.net](https://www.lovelylanding.net/) - [着陆页设计]定期更新的着陆页屏幕截图。包括桌面、平板电脑和移动屏幕截图。
  ...
  * [Mobile Patterns](https://www.mobile-patterns.com/) - [移动屏幕截图]一个设计灵感库，其中包含设计师、开发人员和产品制造商可参考的 iOS 和 Android 中最精美的 UI UX 模式。
  * [Page Flows](https://pageflows.com/) - [移动/网络视频和屏幕截图]许多移动和网络应用程序的完整流程视频。还包括屏幕截图。可高度搜索和索引。
  * [Screenlane](https://screenlane.com/) - [移动屏幕截图]获取灵感并跟上最新的网络和移动应用程序 UI 设计趋势。可按模式和应用程序进行过滤。
  ...
  * [Refero](https://refero.design/) - [网页截图]来自优秀网络应用程序的设计参考的标记和可搜索集合。
...
## 地图上的数据可视化
  ...
  * [developers.arcgis.com](https://developers.arcgis.com)——用于地图、地理空间数据存储、分析、地理编码、路由等的 API 和 SDK，适用于网络、桌面和移动设备。每月免费提供 200 万张基础地图瓦片、20,000 个未存储的地理编码、20,000 条简单路线、5,000 次驾车时间计算以及 5GB 免费的瓦片+数据存储。
  * [Foursquare](https://developer.foursquare.com/)——从 Places API 和 Pilgrim SDK 获得位置发现、场地搜索和上下文感知内容。
  * [geoapify.com](https://www.geoapify.com/)——提供矢量和光栅地图瓦片、地理编码、地点、路由、等距线 API。每天免费请求 3000 次。
  ...
  * [nextbillion.ai](https://nextbillion.ai/)——与地图相关的服务：地理编码、导航（方向、路由、路线优化、距离矩阵）、地图 SDK（矢量、静态、移动 SDK）。[每项服务都有免费的指定配额](https://nextbillion.ai/pricing)。
  ...
  * [Geokeo api](https://geokeo.com)——具有语言校正等功能的地理编码 API。全球覆盖。每天免费 2500 次查询
...
## 集成开发环境（IDE）和代码编辑
  ...
  * [BBEdit](https://www.barebones.com/) - BBEdit 是一款流行且可扩展的 macOS 编辑器。免费模式提供了强大的核心功能集[https://www.barebones.com/products/bbedit/comparison.html]和升级到高级功能的路径。
  * [Binder](https://mybinder.org/) - 将 Git 存储库转换为一组交互式笔记本。这是一项免费的公共服务。
  * [BlueJ](https://bluej.org) - 专为初学者设计的免费 Java 开发环境，全球数百万用户使用。由 Oracle 提供支持，具有简单的 GUI 可帮助初学者。
  ...
  * [cacher.io](https://www.cacher.io) - 具有标签的代码片段组织者，支持 100 多种编程语言。
  * [Code::Blocks](https://codeblocks.org) - 免费的 Fortran 和 C/C++ IDE。开源，可在 Windows、macOS 和 Linux 上运行。
  * [Cody](https://sourcegraph.com/cody) - 免费的人工智能编码助手，可以编写（代码块、自动完成、单元测试）、理解（整个代码库的知识）、修复和查找代码。可用于 VS Code、JetBrains 和在线使用。
  ...
  * [cocalc.com](https://cocalc.com/) - （以前在 cloud.sagemath.com 名为 SageMathCloud）- 云协作计算。浏览器访问完整的 Ubuntu，内置协作和大量免费软件，用于数学、科学、数据科学，预安装：Python、LaTeX、Jupyter Notebooks、SageMath、scikitlearn 等。
  * [code.cs50.io](https://code.cs50.io/) - 用于 CS50 的 Visual Studio Code 是 code.cs50.io 上的网络应用程序，它为学生和教师改编了 GitHub Codespaces。
  ...
  * [Eclipse Che](https://www.eclipse.org/che/) - 用于开发团队的基于 Web 和 Kubernetes 的 IDE，支持多种语言。开源且由社区驱动。Red Hat 提供的在线实例可在[https://workspaces.openshift.com](https://workspaces.openshift.com/)上获得。
  * [fakejson.com](https://fakejson.com/) - FakeJSON 通过其 API 帮助您快速生成虚假数据。发出描述您想要的内容和方式的 API 请求。API 以 JSON 格式返回所有内容。加快您的上市进程，并通过伪造来实现它。
  ...
  * [jsbin.com](https://jsbin.com) - 另一个前端网页（HTML、CSS 和 JavaScript）的游乐场和代码共享网站。它还支持 Markdown、Jade 和 Sass。
  * [jsfiddle.net](https://jsfiddle.net/) - 前端网页的游乐场和代码共享网站，支持协作。
  ...
  * [Mocklets](https://mocklets.com) - 基于 HTTP 的模拟 API 模拟器，可帮助您更快地进行平行开发和更全面的测试，并提供终身免费层。
  ...
  * [Prepros](https://prepros.io/) - Prepros 可以开箱即用地编译 Sass、Less、Stylus、Pug/Jade、Haml、Slim、CoffeeScript 和 TypeScript，重新加载您的浏览器，并使您易于开发和测试您的网站，因此您可以专注于使它们完美。您还可以通过点击几下添加自己的工具。
  ...
  * [VSCodium](https://vscodium.com/) - 一个由社区驱动的、没有遥测/跟踪功能且自由许可的微软编辑器 VSCode 的二进制分发版本
  * [wakatime.com](https://wakatime.com/) - 使用文本编辑器插件对你的编码活动进行自我量化，免费计划有限。
  ...
  * [Zed](https://zed.dev/) - Zed 是一个高性能的、多人游戏代码编辑器，由 Atom 和 Tree-sitter 的创建者开发。
...
## 分析、事件和统计
  ...
  * [Clicky](https://clicky.com) — 网站分析平台。免费计划适用于一个网站，具有 3000 次访问分析。
  ...
  * [GoatCounter](https://www.goatcounter.com/) — GoatCounter 是一个开源网络分析平台，可作为托管服务（免费供非商业使用）或自托管应用程序使用。它旨在提供易于使用和有意义的隐私友好型网络分析，作为 Google Analytics 或 Matomo 的替代方案。免费层适用于非商业用途，包括无限数量的网站、六个月的数据保留和每月 10 万次页面浏览量。
  ...
  * [Hotjar](https://hotjar.com) — 网站分析和报告。免费计划允许每天 2000 次页面浏览量。每天一百个快照（最大容量：300）。可以存储三个快照热图 365 天。无限团队成员。还包括应用内调查和反馈小部件以及屏幕截图。免费层允许创建 3 个调查和 3 个反馈小部件，每月收集 20 个响应。
  ...
  * [Mixpanel](https://mixpanel.com/) — 每月 10 万跟踪用户，无限数据历史记录和席位，美国或欧盟数据驻留
  * [Moesif](https://www.moesif.com) — API 分析用于 REST 和 GraphQL。（免费最多 500 万 API 调用/月）
  * [optimizely.com](https://www.optimizely.com) — A/B 测试解决方案，免费入门计划，一个网站，1 个 iOS 和 1 个 Android 应用
  * [Microsoft PowerBI](https://powerbi.com) — Microsoft 的业务洞察和分析。免费计划提供有限使用，用户许可证为 100 万。
  * [quantcast.com](https://www.quantcast.com/products/measure-audience-insights/) — 无限免费分析
  * [Row Zero](https://rowzero.io) - 快速连接的电子表格。直接连接到数据数据库、S3 和 API。导入、分析、绘制和共享数百万行数据瞬间。三个免费（永久）工作簿。
  ...
  * [Statsig](https://statsig.com) - 跨分析、功能标志和 A/B 测试的一体化平台。免费提供每月最多 100 万计量事件。
  ...
  * [usabilityhub.com](https://usabilityhub.com/) — 在真实用户上测试设计和模型并跟踪访问者。免费提供给一个用户，无限制的测试
  * [woopra.com](https://www.woopra.com/) — 免费的用户分析平台，适用于 500K 次操作，90 天数据保留，30 多个一键集成。
  * [counter.dev](https://counter.dev) — 简单易用的网络分析，隐私友好。免费或按您的意愿通过捐赠支付。
  ...
  * [Microsoft Clarity](https://clarity.microsoft.com/) - Clarity 是一个免费、易于使用的工具，用于捕获真实用户如何使用您的网站。
  * [Beampipe.io](https://beampipe.io) - 简单、专注于隐私的网络分析。免费适用于最多 5 个域和每月 10k 次页面浏览量。
  ...
  * [Trackingplan](https://www.trackingplan.com/) - 自动检测数字分析、营销数据和像素问题，维护最新的跟踪计划，并促进无缝协作。可以将其部署到您的生产环境中，也可以在不编写代码的情况下将分析覆盖添加到回归测试中。
  * [LogSpot](https://logspot.io) - 完整统一的网络和产品分析平台，包括可嵌入的分析小部件和自动化机器人（slack、telegram 和 webhooks）。免费计划包括每月 10,000 次事件。
  * [Umami](https://umami.is/) - 简单、快速、专注于隐私的开源替代方案，可替代 Google Analytics。
  * [TrackWith Dicloud](https://dicloud.net/trackwith-privacy-focused-analytics/) - 免费的轻量级隐私关注的 Google Analytics 替代方案。无限制的页面浏览量，无限制的访问者，无限制的页面热点图和目标跟踪。免费适用于最多 3 个域和每个域 600 次会话回放。
...
## 访问者会话记录
  ...
  * [OpenReplay.com](https://www.openreplay.com)——开源会话回放，带有用于错误重现的开发工具，实时会话支持，以及产品分析套件。每月一千个会话，访问所有功能，保留七天。
  ...
  * [hotjar.com](https://www.hotjar.com/)——按站点：每月一千零五十页浏览量，无限制热图，数据存储三个月
...
## 支付和计费集成
  ...
  * [Glassfy](https://glassfy.io/) – 应用内订阅基础架构，实时订阅事件和现成的货币化工具，适用于 iOS、Android、Stripe 和 Paddle。免费使用，每月收入最高可达$10k。
  * [Adapty.io](https://adapty.io/) – 用于移动应用内订阅集成的一站式解决方案，具有开源 SDK，适用于 iOS、Android、React Native、Flutter、Unity 或网络应用。免费使用，每月收入最高可达$10k。
  ...
  * [CoinGecko](https://www.coingecko.com/en/api) — 提供加密货币市场数据，包括最新的加密货币汇率和历史数据。演示 API 稳定的调用速率限制为每分钟 30 次，每月上限为 10,000 次。
  ...
  * [MailPopin](https://mailpop.in) - 利用上下文信息充分利用您的 Stripe 通知。
  ...
  * [Currencyapi](https://currencyapi.com) — 免费的货币转换和汇率数据 API。每月免费 300 次请求，私人使用每分钟 10 次请求。

**[⬆️ 返回顶部](#table-of-contents)**

## Docker相关

  * [canister.io](https://canister.io/) - 开发者提供 20 个免费私有存储库，团队可构建和存储 Docker 镜像的免费私有存储库为 30 个
  * [Container Registry Service](https://container-registry.com/) - 基于 Harbor 的容器管理解决方案。免费层为私有存储库提供 1GB 存储空间。
  * [Docker Hub](https://hub.docker.com) - 一个免费私有存储库和无限制的公共存储库，用于构建和存储 Docker 镜像
...
## 评论平台
  ...
  * [Disqus](https://disqus.com/) - Disqus 是一个网络社区平台，被数十万网站在全球范围内使用。
  * [Remarkbox](https://www.remarkbox.com/) - 开源托管评论平台，你可以为“在几个域上有一个管理员，完全控制行为和外观”支付你能支付的费用
...
## 屏幕截图API
  ...
  * [ScreenshotAPI.net](https://screenshotapi.net/) - 截图 API 使用简单的 API 调用生成任何网站的屏幕截图。可扩展构建并托管在 Google Cloud 上。每月提供 100 次免费截图。
  * [screenshotlayer.com](https://screenshotlayer.com/) - 捕获任何网站的高度可定制的快照。免费每月 100 次快照。
  * [screenshotmachine.com](https://www.screenshotmachine.com/) - 捕获每月 100 次快照，包括 png、gif 和 jpg，包括全长捕获，不仅仅是主页
  ...
  * [Httpic.com](https://httpic.com) - 将任何网站转换为 jpg、png 或 pdf。捕获整页屏幕截图，调整viewport，并注入自定义代码。免费层每月 150 张图片。
  * [Screenshots](https://screenshotson.click) - 你的截图 API。具有高度可定制的捕获选项。免费每月 100 次截图。

**[⬆️ 返回顶部](#table-of-contents)**

## 与Flutter相关以及无需Mac即可构建IOS应用

  * [FlutLab](https://flutlab.io/) - FlutLab 是一个现代的 Flutter 在线集成开发环境，也是创建、调试和构建跨平台项目的最佳场所。使用 Flutter 构建 iOS（无需 Mac）和 Android 应用。
  * [CodeMagic](https://codemagic.io/) - CodeMagic 是一个完全托管和管理的移动应用持续集成/持续部署（CI/CD）服务。您可以使用基于图形用户界面的 CI/CD 工具进行构建、测试和部署。免费套餐提供 500 分钟/月的免费使用时间和一个配备 2.3GHz 处理器和 8GB 内存的 Mac Mini 实例。
...
## 杂项
  ...
  * [BackgroundStyler.com](https://backgroundstyler.com) - 为你的代码、文本或图像创建美观的截图，以便在社交媒体上分享。
  ...
  * [Carbon.now.sh](https://carbon.now.sh) - 创建和分享代码片段的美观截图图像格式。通常用于在 Twitter 或博客文章上美观地分享/展示代码片段。
  ...
  * [Cronhooks](https://cronhooks.io/) - 按计划或定期安排网络钩子。免费计划允许 5 个临时计划。
  ...
  * [Format Express](https://www.format-express.dev) - 即时在线 JSON / XML / SQL 格式。
  * [FOSSA](https://fossa.com/) - 第三方代码、许可证合规性和漏洞的可扩展端到端管理。
  * [fullcontact.com](https://www.fullcontact.com/developer/pricing/) - 通过在应用程序中添加社交资料，帮助用户更多地了解他们的联系人。每月 500 个免费个人 API 匹配。
  * [Hook Relay](https://www.hookrelay.dev/) - 为您的应用程序添加网络钩子支持，无需烦恼：为您完成排队、带有退避的重试和日志记录。免费计划每天有 100 次交付，14 天保留期和 3 个钩子端点。
  ...
  * [QuickType.io](https://quicktype.io/) - 从 JSON、模式和 GraphQL 快速自动生成数据模型/类/类型/接口和序列化程序，以便在任何编程语言中快速安全地处理数据。将 JSON 转换为任何语言的美观、类型安全的代码。
  * [RandomKeygen](https://randomkeygen.com/) - 一个免费的移动友好工具，提供多种随机生成的密钥和密码，可用于保护任何应用程序、服务或设备。
  ...
  * [superfeedr.com](https://superfeedr.com/) - 实时符合 PubSubHubbub 的馈送、导出、分析。免费，定制较少。
  ...
  * [Versionfeeds](https://versionfeeds.com) - 您喜爱的软件版本的自定义 RSS 馈送。在一个馈送中拥有您的编程语言、库或喜爱的工具的最新版本。（前 3 个馈送是免费的）
...
## 游戏开发
  ...
  * [itch.io](https://itch.io/game-assets) - 免费/付费资产，如精灵、平铺集和角色包。
  ...
  * [CraftPix](https://craftpix.net) - 免费/付费资产，如 2D、3D、音频、GUI、背景、图标、平铺集、游戏工具包。
...
## 其他免费资源
  ...
  * [Wikimint 开发者](https://developer.wikimint.com/p/tools.html) - 面向网络开发者的免费工具，包括 CSS 压缩/解压、图像优化、图像大小调整、大小写转换、CSS 验证器、JavaScript 编译器、HTML 编辑器等。
  * [ElevateAI](https://www.elevateai.com) - 每月可免费获得长达 200 小时的音频转录服务。
  * [get.localhost.direct](https://get.localhost.direct)——更好的`*.localhost.direct`通配符公共 CA 签名 SSL 证书，适用于支持子域的本地开发。
  ...
  * [SmallDev.tools](https://smalldev.tools/)——开发者的免费工具，可让您对各种格式进行编码/解码、HTML/CSS/Javascript 压缩、美化、以 JSON/CSV 及多种其他格式生成虚假/测试数据集等。具有令人愉悦的界面。
  * [UseCSV by Layercode](https://layercode.com/usecsv)——在您的网络应用程序中几分钟内添加 CSV 和 Excel 导入。为您的用户提供愉快且强大的数据导入体验。免费开始，无需任何信用卡详细信息，立即开始集成 UseCSV。您可以创建无限的导入器，并上传高达 100Mb 的文件。
  ...
  * [regex101](https://regex101.com/)——免费网站，允许您测试和调试正则表达式(regex)。它提供了一个正则表达式编辑器和测试器，以及学习正则表达式的有用文档和资源。
  ...
  * [Glob tester](https://globster.xyz/)——允许您设计和测试 glob 模式的网站。它还提供了学习 glob 模式的资源。
  * [OpenUtils](https://openutils.org/) - 为开发者提供各种免费工具，如 HTML/CSS/JavaScript 格式化程序、压缩程序、转换器、编码器/解码器等。
  * [SimpleRestore](https://simplerestore.io) - 无 Hassle MySQL 备份还原。无需代码或服务器即可将 MySQL 备份还原到任何远程数据库。
  * [360Converter](https://www.360converter.com/) - 免费 tier 有用网站，可用于转换：视频转文本&&音频转文本&&语音转文本&&实时音频转文本&&YouTube 视频转文本&&添加视频字幕。在短视频转换或简短的 youtube 教程中可能会有所帮助:)
  * [QRCodeBest](https://qrcode.best/) - 使用 13 种模板创建自定义 QR 码，具有完全隐私和个人品牌。功能包括跟踪像素、项目分类和 QRCode.Best 上的无限团队席位。
  * [PostPulse](https://post-pulse.io) - 通过每月 AI 精心制作的帖子提升在线形象，增强 SEO 和网站排名，优化到 SEO 优化的域。免费计划允许您每月在我们的网站上手动发布一篇帖子。
  ...
  * [Killer Coda](https://killercoda.com/) - 在浏览器中进行互动式学习，学习 Linux、Kubernetes、容器、编程、DevOps、网络
