...
这是一个包含免费开发人员层的软件（SaaS、PaaS、IaaS 等）和其他产品的列表。

本特别列表的范围仅限于基础架构开发人员（系统管理员、DevOps 从业者等）可能会发现有用的东西。我们喜欢那里所有的免费服务，但保持主题相关性会很好。有时这是一条灰色的线，所以这是有意见的；如果我不接受您的贡献，请不要感到冒犯。

此列表是通过拉取请求、评论、想法和 1600 多人的工作得出的。您也可以通过发送[拉取请求](https://github.com/ripienaar/free-for-dev)来添加更多服务或删除那些提供的服务已经更改或已退役的服务来提供帮助。

[![跟踪真棒列表](https://www.trackawesomelist.com/badge.svg)](https://www.trackawesomelist.com/ripienaar/free-for-dev)

**注意**：此列表仅适用于即服务产品，不适用于自托管软件。要符合条件，服务必须提供免费层，而不仅仅是免费试用。如果是时间桶，则免费层必须至少为一年。我们还从安全角度考虑免费层，因此 SSO 可以，但我不会接受将 TLS 限制为仅付费层的服务。
...
41. **隐私管理**
42. **屏幕截图 API**
43. **与 Flutter 相关和在没有 Mac 的情况下构建 IOS 应用**
44. **搜索**
45. **安全和公钥基础设施（PKI）**
46. **身份验证、授权和用户管理**
47. **源代码存储库**
48. **存储和媒体处理**
49. **隧道、WebRTC、WebSocket 服务器和其他路由器**
50. **测试**
51. **团队和协作工具**
52. **翻译管理**
53. **与 Vagrant 相关**
...
55. **网络托管**
56. **评论平台**
57. **基于浏览器的硬件仿真（使用 JavaScript 编写）**
58. **远程桌面工具**
59. **游戏开发**
60. **其他免费资源**
...
## Major Cloud Providers
  ...
  * [Google Cloud Platform](https://cloud.google.com)
    ...
    * Cloud Run - 每月 200 万次请求，360,000GB 秒内存，180,000vCPU 秒计算时间，每月从北美到所有地区的 1GB 网络出站流量
  ...
  * [Microsoft Azure](https://azure.microsoft.com)
    ...
    * [Azure Pipelines](https://azure.microsoft.com/services/devops/pipelines/) - 对于 Linux、macOS 和 Windows 的开源，免费提供 10 个并行作业，不限时间
...
## Cloud management solutions
  ...
  * [scalr.com](https://scalr.com/) - Scalr 是一个 Terraform 自动化和协作（TACO）产品，用于更好地协作和自动化由 Terraform 管理的基础架构和配置。完全支持 Terraform CLI，OPA 集成和分层配置模型。无单点登录税。所有功能都包含在内。免费使用高达 50 次/月的运行次数。
  * [deployment.io](https://deployment.io) - Deployment.io 帮助开发人员在 AWS 上自动化部署。在我们的免费层中，开发人员（单个用户）可以无限部署静态站点、Web 服务和环境。我们在免费层中提供每月 20 次免费的作业执行，其中包括预览和自动部署。

**[⬆️ 返回顶部](#目录)**

## Source Code Repos

  * [Bitbucket](https://bitbucket.org/)——无限数量的公共和私人 Git 存储库，最多可供 5 名用户使用，带有 CI/CD 管道
  * [chiselapp.com](https://chiselapp.com/)——无限数量的公共和私人 Fossil 存储库
  * [codebasehq.com](https://www.codebasehq.com/)——一个免费项目，提供 100MB 空间和两个用户
  * [Codeberg](https://codeberg.org/)——无限数量的公共和私人 Git 存储库，适用于免费和开源项目（无限制协作者）。由 [Forgejo](https://forgejo.org/) 提供支持。静态网站托管与 [Codeberg Pages](https://codeberg.page/)。CI/CD 托管与 [Codeberg 的 CI](https://docs.codeberg.org/ci/)。翻译托管与 [Codeberg Translate](https://translate.codeberg.org/)。包括包和容器托管、项目管理和问题跟踪
  * [GitGud](https://gitgud.io)——无限数量的私人和公共存储库。永久免费。由 GitLab 和 Sapphire 提供支持。不提供 CI/CD。
  * [GitHub](https://github.com/)——无限数量的公共存储库和无限数量的私人存储库（无限制协作者）。包括 CI/CD、开发环境、静态托管、包和容器托管、项目管理和 AI Copilot
  * [gitlab.com](https://about.gitlab.com/)——最多 5 个协作者的无限数量的公共和私人 Git 存储库。包括 CI/CD、静态托管、容器注册表、项目管理和问题跟踪
  ...
  * [Pagure.io](https://pagure.io)——Pagure.io 是一个免费和开源的软件代码协作平台，适用于基于 Git 的 FOSS 许可项目
...
## APIs, Data, and ML
  ...
  * [JSONGrid](https://jsongrid.com) - 免费工具，用于可视化、编辑、过滤复杂的 JSON 数据到漂亮的表格网格。通过链接共享 JSON 数据并保存。
  ...
  * [Apify](https://www.apify.com/) - 网络抓取和自动化平台，用于为任何网站创建 API 并提取数据。提供即用型抓取程序、集成代理和自定义解决方案。免费计划每月包含 5 美元的平台信用额度。
  ...
  * [APIVerve](https://apiverve.com) - 免费获得超过 120 个 API 的即时访问权限，这些 API 经过精心设计，具有质量、一致性和可靠性。免费计划每月涵盖多达 50 个 API 令牌。
  ...
  * [bigml.com](https://bigml.com/) - 托管机器学习算法。开发免费任务不限，数据/任务限制为 16MB。
  ...
  * [Cloudmersive](https://cloudmersive.com/) - 实用 API 平台，可全面访问包括文档转换、病毒扫描等在内的广泛 API 库，每月有 800 次调用。
  ...
  * [Commerce Layer](https://commercelayer.io) - 可组合的商务 API，可从任何前端构建、放置和管理订单。开发人员计划每月免费提供 100 个订单和最多 1,000 个 SKU。
  ...
  * [Country-State-City Microservice API](https://country-state-city.rebuscando.info/) - 提供广泛信息的 API 和微服务，包括国家、地区、省份、城市、邮政编码等。免费层每天包括最多 100 个请求。
  ...
  * [Cube](https://cube.dev/) - Cube 帮助数据工程师和应用程序开发人员从现代数据存储中访问数据，将其组织成一致的定义，并将其交付到每个应用程序。使用 Cube 的最快方法是使用 Cube Cloud，它具有免费层，每月有 1GB 的数据通过。
  ...
  * [Dataimporter.io](https://www.dataimporter.io) - 将数据连接、清理和导入 Salesforce 的工具。免费计划每月最多包含 20,000 条记录。
  ...
  * [DB-IP](https://db-ip.com/api/free) - 免费的 IP 地理位置定位 API，每个 IP 每天限制 1k 请求。免费的 lite 数据库也在 CC-BY 4.0 许可证下提供。
  ...
  * [Deepnote](https://deepnote.com) - 新的数据科学笔记本。Jupyter 兼容实时协作和云运行。免费层包括无限个人项目、高达 750 小时的标准硬件和最多 3 个编辑器的团队。
  ...
  * [Invantive Cloud](https://cloud.invantive.com/)——访问超过 70 个（云）平台，如 Exact Online、Twinfield、ActiveCampaign 或 Visma，使用 Invantive SQL 或 OData4（通常为 Power BI 或 Power Query）。包括数据复制和交换。开发人员和实施顾问的免费计划。对于特定平台，免费使用，但数据量有限制。
  ...
  * [Lightly](https://www.lightly.ai/)——通过使用正确的数据来改进您的机器学习模型。免费使用高达 1000 个样本的数据集。
  ...
  * [Meteosource Weather API](https://www.meteosource.com/)——全球天气 API，用于获取当前和预测的天气数据。预测是基于机器学习对多个天气模型的组合，以提高准确性。免费计划每天提供 400 次调用。
  * [microlink.io](https://microlink.io/)——将任何网站转换为数据，例如元标记规范化、美化链接预览、抓取功能或屏幕截图服务。每天一百次请求免费。
  * [Mindee](https://developers.mindee.com/docs)——Mindee 是一款强大的 OCR 软件和 API 优先平台，通过使用计算机视觉和机器学习对关键信息进行数据识别，使文档处理层标准化，从而帮助开发人员自动化应用程序的工作流程。免费层提供每月 250 页。
  ...
  * [MockAPI](https://www.mockapi.io/)——MockAPI 是一个简单的工具，允许您快速模拟 API，生成自定义数据，并使用 RESTful 接口执行操作。MockAPI 旨在作为原型制作/测试/学习工具。免费提供一个项目/每个项目 4 个资源。
  ...
  * [Nordigen](https://nordigen.com)——免费的开放银行数据 API。PSD2。在欧盟+英国，连接 2300 多家银行与您的应用/软件。
  ...
  * [Duply.co](https://duply.co)——根据 API 和 URL 创建动态图像，设计模板一次并重复使用。免费层提供每月从 API 和 URL 创建 70 张图像，通过表单创建最多 100 张图像。
  ...
  * [pdfEndpoint.com](https://pdfendpoint.com)——使用简单 API 将 HTML 或 URL 轻松转换为 PDF。每月免费进行一百次转换。
  * [PDF-API.io](https://pdf-api.io)——PDF 自动化 API，具有可视化模板编辑器或 HTML 到 PDF、动态数据集成和 PDF 渲染功能的 API。免费计划提供一个模板，每月 100 份 PDF。
  ...
  * [Postman](https://postman.com)——用于设计和测试 API 的协作平台，可简化工作流程并创建更好的 API。免费使用 Postman App 永远。Postman 云功能也免费提供，有一定的限制。
  ...
  * [Preset Cloud](https://preset.io/)——托管的 Apache Superset 服务。对于最多 5 个用户的团队，永久免费，提供无限的仪表板和图表、无代码图表构建器和协作式 SQL 编辑器。
  ...
  * [QuickMocker](https://quickmocker.com/)——管理自己子域下的在线虚假 API 端点，将请求转发到本地主机 URL 进行 Webhooks 开发和测试，使用 RegExp 和多种 HTTP 方法进行 URL 路径，对端点进行优先级排序，每个端点超过 100 个短代码（动态或虚假响应值）进行响应模板化，从 OpenAPI（Swagger）规范以 JSON 格式导入，代理请求，根据 IP 地址和授权标头限制端点。免费帐户提供一个随机子域、十个端点、五个 RegExp URL 路径、每个端点 50 个短代码、每天 100 个请求和请求日志中的 50 个历史记录。
  * [Rapidapi](https://rapidapi.com/)——世界上最大的 API 中心，数百万开发人员发现并连接到数千个 API，使用有趣的挑战（和解决方案！）和交互式示例进行 API 开发。
  ...
  * [SaturnCloud](https://saturncloud.io/) - 数据科学云环境，允许运行 Jupyter 笔记本和 Dask 集群。每月有三十小时的免费计算时间和三小时的 Dask 时间。
  * [Scraper's Proxy](https://scrapersproxy.com)——简单的 HTTP 代理 API，用于抓取。抓取时无需担心限制、阻止或验证码。每月前 100 次成功抓取免费，包括 JavaScript 渲染（如果您联系支持人员，更多可用）。
  ...
  * [ScrapingDog](https://scrapingdog.com/)——Scrapingdog 处理数百万个代理、浏览器和验证码，为您提供任何网页的 HTML，只需一次 API 调用。它还包括适用于 Chrome 和 Firefox 的 Web Scraper 和用于即时抓取需求的软件。提供免费计划。
  ...
  * [Select Star](https://www.selectstar.com/)——是一个智能数据发现平台，它自动分析和记录您的数据。免费的轻量级套餐，包括 1 个数据源、最多 100 个表和 10 个用户。
  ...
  * [Sofodata](https://www.sofodata.com/)——从 CSV 文件创建安全的 RESTful API。上传 CSV 文件，然后通过其 API 立即访问数据，从而加快应用程序开发速度。免费计划包括 2 个 API 和每月 2500 个 API 调用。您无需信用卡。
  ...
  * [Treblle](https://www.treblle.com)——Treblle 帮助团队构建、交付和管理 API。具有高级 API 日志聚合、可观测性、文档和调试功能。您可以免费获得所有功能，但免费层每月的请求限制为最多 250k。
  ...
  * [Weights & Biases](https://wandb.ai)——开发者优先的 MLOps 平台。通过实验跟踪、数据集版本控制和模型管理更快地构建更好的模型。个人项目的免费层，其中包括 100GB 的存储空间。
  ...
  * [huggingface.co](https://huggingface.co)——用于 Pytorch、TensorFlow 和 JAX 的 NLP 模型的构建、训练和部署。免费使用，最高可达 30k 输入字符/月。
  ...
  * [Zuplo](https://zuplo.com/)——在几分钟内为任何 API 添加 API 密钥身份验证、速率限制和开发人员文档。免费计划提供最多 10 个项目、无限的生产边缘环境、250 个 API 密钥、每月 100 万次请求和 1GB 出口。
  * [OpenWeb Ninja](https://www.openwebninja.com/)——极其全面的实时 SERP 和公共数据 API：谷歌搜索、购物、工作、图片、镜头、新闻、谷歌地图企业/地点、评论、照片、网站电子邮件和社交联系人抓取器、亚马逊、Yelp 等。所有 API 都包含免费层，每月有 100 到 200 次免费请求。
...
## Artifact Repos
  ...
  * [Artifactory](https://jfrog.com/start-free/) - 一个支持多种软件包格式的工件存储库，如 Maven、Docker、Cargo、Helm、PyPI、CocoaPods 和 GitLFS。包括包扫描工具 XRay 和 CI/CD 工具 Pipelines（以前称为 Shippable），每月有 2000 分钟的免费 CI/CD 分钟。
...
## Tools for Teams and Collaboration
  ...
  * [Braid](https://www.braidchat.com/)——专为团队设计的聊天应用程序。免费提供公共访问组、无限制用户、历史记录和集成。它还提供自托管的开源版本。
  * [cally.com](https://cally.com/)——找到会议的完美时间和日期。简单易用，适用于大小群组。
  * [Calendly](https://calendly.com)——Calendly 是连接和安排会议的工具。免费计划为每个用户提供 1 个日历连接和无限制的会话。还提供桌面和移动应用程序。
  ...
  * [Hackmd.io](https://hackmd.io/)——实时协作和编写工具，用于 markdown 格式的文档/文件。就像 Google Docs 一样，但适用于 markdown 文件。免费无限数量的“笔记”，但私人笔记和模板的协作者（邀请者）数量将受到限制[将受到限制](https://hackmd.io/pricing)。
  ...
  * [/meet for Slack](https://meetslack.com)——在 Slack 中直接使用/meet 启动 Google 会议，无需任何限制。免费。
  * [Livecycle](https://www.livecycle.io/)——Livecycle 是一个包容的协作平台，使跨职能产品团队和开源项目的工作流程无摩擦。
  * [Lockitbot](https://www.lockitbot.com/)——在 Slack 中保留和锁定共享资源，如房间、开发环境、服务器等。免费最多 2 个资源
  ...
  * [Notion](https://www.notion.so/)——具有 markdown 支持的笔记记录和协作应用程序。该公司将该应用程序描述为笔记记录、项目管理和任务管理的一体化工作区。除了跨平台应用程序，还可以通过大多数网络浏览器访问。
  * [Nuclino](https://www.nuclino.com)——适用于团队的轻量级协作 wiki，可用于所有知识、文档和笔记。免费计划提供所有基本功能，最多 50 个项目和 5GB 存储空间。
  ...
  * [PageShare.dev](https://www.pageshare.dev)——为 GitHub Pull 请求添加视觉审查功能，无需部署网站。免费最多每月 10 页和总共 100MB 存储。
  ...
  * [Slab](https://slab.com/) — 一个面向团队的现代知识管理服务。最多可供 10 名用户免费使用。
  ...
  * [Stickies](https://stickies.app/) - 用于头脑风暴、内容策展和笔记的可视化协作应用程序。免费提供最多 3 个墙、无限制用户和 1GB 存储空间。
  ...
  * [Teamplify](https://teamplify.com) - 通过团队分析和智能日常站立会议提高团队开发流程。包括适用于远程优先团队的全功能请假管理。免费供最多 5 名用户的小团体使用。
  * [Tefter](https://tefter.io) - 具有强大 Slack 集成的书签应用程序。开源团队免费使用。
  * [TeleType](https://teletype.oorja.io/) — 共享终端、语音、代码、白板等。开发人员无需登录即可进行端到端加密协作。
  * [TimeCamp](https://www.timecamp.com/) - 无限制用户的免费时间跟踪软件。轻松与 Jira、Trello、Asana 等 PM 工具集成。
  * [Teamcamp](https://www.teamcamp.app) - 面向软件开发公司的一体化项目管理应用程序。
  * [twist.com](https://twist.com/) — 一个异步友好的团队通信应用程序，其中对话保持组织有序且主题相关。提供免费和无限计划。为符合条件的团队提供折扣。
  * [tldraw.com](https://tldraw.com/) - 具有智能箭头、捕捉、便签和 SVG 导出功能的免费开源白板和绘图工具。具有多人协作编辑模式。还提供免费的官方 VS Code 扩展。
  * [BookmarkOS.com](https://bookmarkos.com) - 免费的一体化书签管理器、选项卡管理器和任务管理器，可在可自定义的在线桌面上使用，支持文件夹协作。
  * [typetalk.com](https://www.typetalk.com/) — 通过网络或移动设备上的即时消息与团队共享和讨论想法。
  * [Tugboat](https://tugboat.qa) - 自动和按需预览每个拉取请求。免费供所有人使用，非盈利组织提供免费的 Nano 层。
  * [whereby.com](https://whereby.com/) — 一键式视频对话，免费（以前称为 appear.in）
  * [windmill.dev](https://windmill.dev/) - 一个开源开发者平台，用于从最小的 Python 和 Typescript 脚本快速构建生产级多步骤自动化和内部应用程序。作为免费用户，您最多可以创建和成为三个非高级工作空间的成员。
  * [vadoo.tv](https://vadoo.tv/) — 简单的视频托管和营销。一键上传视频。记录、管理、共享等。免费层提供最多 10 个视频、1GB 存储空间和 10GB 带宽/每月
  * [userforge.com](https://userforge.com/) - 相互关联的在线个人资料、用户故事和上下文映射。有助于保持设计和开发的同步，最多可供 3 个个人资料和两个协作者免费使用。
  ...
  * [Wormhole](https://wormhole.app/) - 使用端到端加密共享最大 5GB 的文件，长达 24 小时。对于大于 5GB 的文件，它使用点对点传输将您的文件直接发送。
  * [zoom.us](https://zoom.us/) - 提供安全的视频和网络会议附加组件。免费计划限制为 40 分钟。
  * [Zulip](https://zulip.com/) - 具有独特电子邮件式线程模型的实时聊天。免费计划包括 10,000 条消息搜索历史和文件存储最多 5GB。它还提供自托管的开源版本。
  ...
  * [Revolt.chat](https://revolt.chat/) - [Discord](https://discord.com/) 的开源替代方案，尊重您的隐私。它还免费提供了 Discord 的大多数专有功能。Revolt 是一个安全、快速且 100%免费的一体化应用程序。所有功能都是免费的。与大多数主流聊天应用程序不同，它还支持（官方和非官方）插件支持。
  * [Tencent RTC](https://trtc.io/) — 腾讯实时通信 (TRTC) 提供小组音频/视频通话解决方案。第一年每月 10,000 分钟免费。
...
## CMS
  ...
  * [Crystallize](https://crystallize.com)——支持电子商务的无头 PIM。内置 GraphQL API。免费版本包括无限用户、1000 个目录项目、5GB/月带宽和 25k/月 API 调用。
  * [DatoCMS](https://www.datocms.com/)——为小项目提供免费层。DatoCMS 是一个基于 GraphQL 的 CMS。在较低的层级，你有 100k/月的调用。
  ...
  * [kontent.ai](https://www.kontent.ai)——内容即服务平台，为你提供无头 CMS 的所有好处，同时赋予营销人员权力。开发者计划为两个用户提供无限项目，每个项目有两个环境，500 个内容项目，两种语言的交付和管理 API，以及自定义元素支持。你可以使用更详细的计划来满足你的需求。
  ...
  * [Sanity.io](https://www.sanity.io/)——具有开源编辑环境和实时托管数据存储的结构化内容平台。无限项目。无限管理员用户，三个非管理员用户，两个数据集，500K API CDN 请求，10GB 带宽，每个项目免费包含 5GB 资产。
  ...
  * [GatsbyjsCMS](https://www.gatsbyjs.com/)——Gatsby 是快速灵活的框架，使使用任何 CMS、API 或数据库构建网站再次变得有趣。构建和部署无服务器网站，以获得更多流量、更好的转化率和更多收入！
  * [Hygraph](https://hygraph.com/)——为小项目提供免费层。GraphQL 优先 API。摆脱遗留解决方案，采用 GraphQL 原生无头 CMS——并首先提供多渠道内容 API。
  * [Squidex](https://squidex.io/)——为小项目提供免费层。API/GraphQL 优先。开源，基于事件源（自动处理每次更改）。
  * [InstaWP](https://instawp.com/)——在几秒钟内启动 WordPress 网站。免费层包括 5 个活动网站、500MB 空间、48 小时网站过期。
  * [Storyblok](https://www.storyblok.com)——适用于开发人员和营销人员的无头 CMS，与所有现代框架兼容。社区（免费）层提供管理 API、可视化编辑器、十个源、自定义字段类型、国际化（无限语言/地区）、资产管理器（最多 2500 个资产）、图像优化服务、搜索查询、Webhook + 250GB 流量/月。
  * [WPJack](https://wpjack.com)——在不到 5 分钟的时间内在任何云平台上设置 WordPress！免费层包括 1 个服务器、2 个站点、免费的 SSL 证书和无限的 cron 作业。没有时间限制或过期——你的网站，你的方式。

**[⬆️ 返回顶部](#table-of-contents)**

## Code Generation

  * [Appinvento](https://appinvento.io/)——AppInvento 是一款免费的无代码应用构建器。在自动生成的后端代码中，用户可以完全访问源代码以及无限的 API 和路由，实现广泛的集成。免费计划包括三个项目、五个表格和一个 Google 附加组件。
  * [Cody AI](https://sourcegraph.com/cody)——Cody 是一个编码 AI 助手，它使用 AI 和对代码库的深入理解来帮助你更快地编写和理解代码。Cody 为开发人员提供了 LLMs 的选择（包括本地推理），支持各种 IDE，支持所有流行的编程语言，并提供免费计划。免费计划每月为开发人员提供 20 条聊天消息（使用 Claude 3 Sonnet 作为 LLM）和 500 次自动完成（使用 Starcoder 16b）。
  ...
  * [Fern](https://buildwithfern.com)——编写 API 定义并使用它们在 TypeScript、Python、Java、Go 等流行语言中生成 SDK/client 库。完全支持 OpenAPI。免费层最多可生成 20 个端点的代码。
  * [Metalama](https://www.postsharp.net/metalama)——仅适用于 C#。Metalama 在编译时动态生成代码的样板，使您的源代码保持整洁。它对开源项目是免费的，其对商业友好的免费层包括三个方面。
  ...
  * [v0.dev](https://v0.dev/)——v0 使用 AI 模型根据简单的文本提示生成代码。它根据 shadcn/ui 和 Tailwind CSS 生成可复制粘贴的友好 React 代码，人们可以在项目中使用。每次生成至少需要 30 个信用点。您一开始有 1200 个信用点，并且每月可以获得 200 个免费信用点。
...
## Code Quality
  ...
  * [browserling.com](https://www.browserling.com/)——实时交互式跨浏览器测试，免费仅 3 分钟会话，在 Vista 下分辨率为 1024 x 768 时支持 MS IE 9
  * [codacy.com](https://www.codacy.com/)——针对 PHP、Python、Ruby、Java、JavaScript、Scala、CSS 和 CoffeeScript 的自动化代码审查，免费供无限公共和私有存储库使用
  * [Codeac.io](https://www.codeac.io/infrastructure-as-code.html?ref=free-for-dev)——用于 DevOps 的自动化基础设施即代码审查工具，与 GitHub、Bitbucket 和 GitLab 集成（甚至是自托管的）。除了标准语言，它还分析 Ansible、Terraform、CloudFormation、Kubernetes 等。（开源免费）
  * [CodeBeat](https://codebeat.co)——适用于多种语言的自动化代码审查平台。免费版永久适用于带有 Slack 和电子邮件集成的公共存储库。
  * [codeclimate.com](https://codeclimate.com/)——自动化代码审查，开源免费，组织拥有的私有存储库无限（最多 4 个协作者）。学生和机构也免费。
  * [codecov.io](https://codecov.io/)——代码覆盖率工具（SaaS），开源免费，一个免费私有存储库
  * [CodeFactor](https://www.codefactor.io)——针对 Git 的自动化代码审查。免费版本包括无限用户、公共存储库和一个私有存储库。
  * [coderabbit.ai](https://coderabbit.ai)——基于人工智能的代码审查工具，与 GitHub/GitLab 集成。免费层包括每小时 200 个文件、每小时 3 次审查和每小时 50 次对话。开源项目永久免费。
  * [codescene.io](https://codescene.io/)——CodeScene 根据开发人员使用代码的方式优先处理技术债务，并可视化组织因素，如团队耦合和系统掌握。开源免费。
  * [CodSpeed](https://codspeed.io)——在 CI 管道中自动跟踪性能。通过精确和一致的指标，在部署前捕捉性能回归。开源项目永久免费。
  * [coveralls.io](https://coveralls.io/)——显示测试覆盖报告，开源免费
  * [dareboost](https://dareboost.com)——每月免费提供 5 份针对网络性能、可访问性和安全性的分析报告
  * [deepcode.ai](https://www.deepcode.ai)——DeepCode 基于人工智能发现错误、安全漏洞、性能和 API 问题。DeepCode 的分析速度允许我们在您的 IDE 中保存按钮时实时分析您的代码并提供结果。支持的语言是 Java、C/C++、JavaScript、Python 和 TypeScript。与 GitHub、BitBucket 和 GitLab 集成。开源和私有存储库以及最多 30 名开发人员免费。
  * [deepscan.io](https://deepscan.io/)——针对 JavaScript 代码自动查找运行时错误的高级静态分析，开源免费
  ...
  * [Moderne.io](https://app.moderne.io)——自动源代码重构。Moderne 提供框架迁移、代码分析和修复以及无与伦比的大规模代码转换，因此开发人员可以将时间花在构建新事物上，而不是维护旧事物上。开源免费。
  * [reviewable.io](https://reviewable.io/)——针对 GitHub 存储库的代码审查，公共或个人存储库免费。
  ...
  * [SourceLevel](https://sourcelevel.io/)——自动化代码审查和团队分析。开源和组织最多 5 个协作者免费。
...
## Code Search and Browsing
  ...
  * [tickgit.com](https://www.tickgit.com/)——将`TODO`注释（和其他标记）表面化，以识别值得改进的代码区域。
...
## CI and CD
  ...
  * [appcircle.io](https://appcircle.io)——一款企业级移动 DevOps 平台，可自动构建、测试和发布移动应用程序，以实现更快、更高效的发布周期。免费使用 30 分钟的最大构建时间，20 个每月构建和 1 个并发构建。
  * [appveyor.com](https://www.appveyor.com/)——适用于 Windows 的 CD 服务，开源免费。
  * [Argonaut](https://argonaut.dev/)——在您的云平台上几分钟内部署应用程序和基础架构。支持在 Kubernetes 和 Lambda 环境中自定义和第三方应用程序的部署。免费层允许 5 个域和 2 个用户无限制的应用程序和部署。
  * [bitrise.io](https://www.bitrise.io/)——适用于移动应用程序的 CI/CD，无论是原生的还是混合的。每月有 200 次免费构建，10 分钟构建时间和两个团队成员。OSS 项目获得 45 分钟构建时间，+1 并发性和无限的团队规模。
  ...
  * [CircleCI](https://circleci.com/)——全面的免费计划，包含在托管的 CI/CD 服务中，适用于 GitHub、GitLab 和 BitBucket 存储库的所有功能。多个资源类、Docker、Windows、Mac OS、ARM 执行器、本地运行器、测试拆分、Docker 层缓存和其他高级 CI/CD 功能。免费使用长达 6000 分钟/月的执行时间，无限制的协作者，私有项目中的 30 个并行作业，以及开源项目的 80,000 个免费构建分钟。
  ...
  * [Spacelift](https://spacelift.io/)——基础架构即代码的管理平台。免费计划功能：IaC 协作、Terraform 模块注册表、ChatOps 集成、与 Open Policy Agent 的持续资源合规性、SSO 与 SAML 2.0，以及对公共工作者池的访问：长达 200 分钟/月。
  ...
  * [Terramate](https://terramate.io/)——Terramate 是适用于基础设施即代码（IaC）工具（如 Terraform、OpenTofu 和 Terragrunt）的编排和管理平台。免费使用，最多可包括 2 个用户，包括所有功能。

**[⬆️ 返回顶部](#table-of-contents)**

## Testing

  * [Applitools.com](https://applitools.com/) - 智能视觉验证工具，适用于网络、原生移动和桌面应用程序。与几乎所有自动化解决方案（如 Selenium 和 Karma）以及远程运行器（Sauce Labs、Browser Stack）集成。开源免费。为每周有一定数量检查点限制的单个用户提供免费层。
  * [Appetize](https://appetize.io) - 在这个基于云的 Android 手机/平板电脑模拟器和 iPhone/iPad 模拟器上直接在您的浏览器中测试您的 Android 和 iOS 应用程序。免费层包括两个并发会话，每月使用时间为 30 分钟。无应用程序大小限制。
  ...
  * [BugBug](https://bugbug.io/) - 用于 Web 应用程序的轻量级测试自动化工具。易于学习，无需编码。您可以在自己的计算机上免费运行无限数量的测试。您还可以获得云监控和 CI/CD 集成，但需要额外支付月费。
  ...
  * [cypress.io](https://www.cypress.io/) - 快速、简单和可靠的浏览器内测试工具。Cypress Test Runner 始终免费且开源，无任何限制和限制。Cypress Dashboard 对开源项目的免费使用限制为最多 5 个用户。
  * [Cypress Recorder by Preflight](https://cypress.preflight.com/) - 在您的浏览器上创建基于人工智能的 Cypress 测试/POM 模型。它是开源的，人工智能部分除外。它的免费计划包括五个月的测试创建，具有自我修复脚本、电子邮件和视觉测试。
  ...
  * [preflight.com](https://preflight.com) - 无代码自动化网页测试。在您的浏览器上录制对 UI 更改有弹性的测试，并在 Windows 计算机上运行它们。可以与您的 CI/CD 集成吗？免费计划包括 50 次每月测试运行，带有视频、HTML 会话等。
  ...
  * [seotest.me](https://seotest.me/) - 免费的页面内 SEO 网站测试工具。每天 10 次免费网站抓取。有关如何提高任何网站的页面内 SEO 结果的有用 SEO 学习资源和建议，无论技术如何。
  * [snippets.uilicious.com](https://snippets.uilicious.com) - 它就像 CodePen 但用于跨浏览器测试。UI-licious 让您像用户故事一样编写测试，并提供一个免费平台 - UI-licious Snippets - 允许您在 Chrome 上无限制测试，无需注册，每次测试运行最长为 3 分钟。发现错误了吗？您可以复制测试的唯一 URL 并将其提供给开发人员，以确切地向他们展示如何重现该错误。
  ...
  * [qase.io](https://qase.io) - Dev 和 QA 团队的测试管理系统。管理测试用例、组成测试运行、执行测试、跟踪缺陷并衡量影响。免费层包括所有核心功能，可用于附件的可用空间为 500MB，最多可容纳 3 个用户。
...
## Security and PKI
  ...
  * [aikido.dev](https://www.aikido.dev) - 一站式应用安全平台，涵盖 SCA、SAST、CSPM、DAST、Secrets、IaC、恶意软件、容器扫描、EOL 等。免费计划包括两个用户，10 个存储库、1 个云、2 个容器和 1 个域的扫描。
  ...
  * [Bridgecrew](https://bridgecrew.io/) - 由开源工具 [Checkov](https://github.com/bridgecrewio/checkov) 提供支持的基础设施即代码 (IaC) 安全。Bridgecrew 核心平台对于最多 50 个 IaC 资源是免费的。
  ...
  * [CyberChef](https://gchq.github.io/CyberChef/) - 一个简单、直观的网络应用程序，用于分析和解码/编码数据，无需处理复杂的工具或编程语言。就像一把瑞士军刀的密码学和加密。所有功能均可免费使用，无限制。如果您希望自行托管，可以使用开源版本。
  ...
  * [Dotenv](https://dotenv.org/) - 快速、安全地同步您的.env 文件。停止通过不安全的渠道（如 Slack 和电子邮件）共享您的.env 文件，再也不会丢失重要的.env 文件。免费供最多 3 个队友使用。
  * [GitGuardian](https://www.gitguardian.com) - 使用自动化的秘密检测和修复功能，防止秘密进入您的源代码。扫描您的 git 存储库以查找 350 多种类型的秘密和敏感文件-免费供个人和团队使用，团队成员不超过 25 人。
  * [Have I been pwned?](https://haveibeenpwned.com) - 用于获取有关违规信息的 REST API。
  * [hostedscan.com](https://hostedscan.com) - 在线漏洞扫描程序，适用于 Web 应用程序、服务器和网络。每月可免费进行 10 次扫描。
  ...
  * [pixee.ai](https://pixee.ai) - 作为免费的 GitHub 机器人，自动提交 PR 以自动解决您的 Java 代码库中的漏洞。其他语言即将推出！
  ...
  * [seclookup.com](https://www.seclookup.com/) - Seclookup APIs 可以丰富 SIEM 中的域名威胁指标，提供域名的全面信息，并提高威胁检测和响应能力。在此处（https://account.seclookup.com/）免费获得 50K 次查找。
  * [snyk.io](https://snyk.io) - 可以找到并修复您的开源依赖项中的已知安全漏洞。对开源项目的测试和修复无限制。对于您的私有项目，每月限制为 200 次测试。
  ...
  * [StackHawk](https://www.stackhawk.com/) 在整个管道中自动扫描应用程序，以在安全漏洞进入生产环境之前找到并修复它们。一个应用程序的无限制扫描和环境。
  ...
  * [Virgil Security](https://virgilsecurity.com/) - 在您的数字解决方案中实现端到端加密、数据库保护、物联网安全等工具和服务。免费供最多 25 个用户的应用程序使用。
  * [Vulert](https://vulert.com) - Vulert 持续监控您的开源依赖项以发现新漏洞，并提供修复建议，无需安装或访问您的代码库。免费供开源项目使用。
...
## Authentication, Authorization, and User Management
  ...
  * [Authgear](https://www.authgear.com) - 在数分钟内为您的应用带来无密码、一次性密码、双因素身份验证、单点登录等功能。所有前端功能均包含在内。免费使用额度为 5000 MAUs。
  ...
  * [Corbado](https://www.corbado.com/) — 在新应用或现有应用中添加密码优先身份验证。无限 MAUs 免费。
  ...
  * [Kinde](https://kinde.com/) - 简单、强大的身份验证，您可以在几分钟内集成到您的产品中。免费使用额度为 7,500 MAU。
  ...
  * [Ory](https://ory.sh/) - 身份验证/授权/OAuth2.0/零信任管理安全平台。具有所有安全功能的永久免费开发者帐户，无限团队成员，每天 200 个活跃用户，每月 25k/权限检查。
  ...
  * [Stytch](https://www.stytch.com/) - 具有强大密码的灵活身份验证 API 和 SDK，无密码登录流程，MFA，SSO 等。内置电子邮件/短信发送+故障转移和欺诈保护。提供 5,000 个每月活跃用户免费（B2C）或 1,000 个每月活跃用户和 25 个组织免费（B2B）。
  * [Stack Auth](https://stack-auth.com) — 开源身份验证，不会让人失望。最适合开发人员的解决方案，只需五分钟即可开始使用。可免费自托管，或提供带有 10k 免费每月活跃用户的托管 SaaS 版本。
  * [SuperTokens](https://supertokens.com/) - 开源用户身份验证，可本机集成到您的应用程序中-使您能够快速入门，同时控制用户和开发人员体验。免费使用额度为每月最多 5000 MAUs。
  * [Warrant](https://warrant.dev/) — 为您的应用提供托管企业级授权和访问控制服务。免费层包括每月 100 万次 API 请求和 1000 个授权规则。
  * [ZITADEL Cloud](https://zitadel.com) — 即开即用的用户和访问管理，适用于您和支持多租户（B2B）用例。免费使用额度为每月最多 25,000 次经过身份验证的请求，具有所有安全功能（OTP、无密码、策略等无付费墙）。
  * [PropelAuth](https://propelauth.com) — 立即向任何规模的公司销售，只需几行代码，免费使用额度为最多 200 个用户和 10k 事务性电子邮件（带有水印品牌：“由 PropelAuth 提供支持”）。
  * [Logto](https://logto.io/) - 开发、保护和管理您产品的用户身份（用于身份验证和授权）。免费使用额度为每月最多 5,000 MAUs，提供开源自托管选项。
...
## Mobile App Distribution and Feedback
  ...
  * [TestApp.io](https://testapp.io) - 确保移动应用正常运行的首选平台。免费计划：一个应用、分析、无限制版本和安装次数，以及反馈收集。
  * [Loadly](https://loadly.io) - iOS 和 Android 测试版应用分发服务提供完全免费的服务，包括无限制下载、高速下载和无限制上传。
...
## Messaging and Streaming
  ...
  * [Ably](https://www.ably.com/) - 具有在线状态、持久化和消息保证交付的实时消息服务。免费计划包括每月 300 万条消息、100 个峰值连接和 100 个峰值通道。
  * [cloudamqp.com](https://www.cloudamqp.com/) — RabbitMQ 即服务。小狐猴计划：最大每月 100 万条消息，最大 20 个并发连接，最大 100 个队列，最大 10,000 条排队消息，不同 AZ 中的多个节点
  * [courier.com](https://www.courier.com/) — 用于推送、应用内、电子邮件、聊天、短信和其他消息渠道的单一 API，具有模板管理等功能。免费计划包括每月 10,000 条消息。
  ...
  * [NotificationAPI.com](https://www.notificationapi.com/) — 在 5 分钟内将用户通知添加到任何软件中。免费计划包括每月 10,000 条通知+100 SMS 和自动呼叫。
  ...
  * [synadia.com](https://synadia.com/ngs) - [NATS.io](https://nats.io) 即服务。全球、AWS、GCP 和 Azure。免费使用，无限数据量，每月 4k 消息大小，50 个活动连接和 5GB 数据。
...
## Log Management
  ...
  * [logzab.com](https://logzab.com/)——审计跟踪管理系统。每月最多 1000 个用户活动日志，保留时间为 1 个月，最多可用于 5 个项目
...
## Translation Management
  ...
  * [gitlocalize.com](https://gitlocalize.com)——免费，私有和公共存储库均无限制
  ...
  * [localazy.com](https://localazy.com/)——免费 1000 个源语言字符串，不限语言，不限贡献者，适用于初创公司和开源交易
  * [Localeum](https://localeum.com)——免费最多 1000 个字符串，一个用户，不限语言，不限项目
  * [localizely.com](https://localizely.com/)——开源免费
  * [Loco](https://localise.biz/)——免费最多 2000 个翻译，无限制翻译人员，每个项目 10 种语言，1000 个可翻译资产/项目
  * [oneskyapp.com](https://www.oneskyapp.com/)——最多 5 个用户的有限免费版，开源免费
  * [POEditor](https://poeditor.com/)——免费最多 1000 个字符串
  * [SimpleLocalize](https://simplelocalize.io/)——免费最多 100 个翻译键，不限字符串，不限语言，初创公司交易
...
## Monitoring
  ...
  * [alerty.ai](https://www.alerty.ai) - 为你的 FE、BE、DB 等提供免费的 APM 和监控+免费代理运行。
  * [appdynamics.com](https://www.appdynamics.com/) - 免费提供 24 小时指标，应用性能管理代理限制为一个 Java、一个.NET、一个 PHP 和一个 Node.js
  * [appneta.com](https://www.appneta.com/) - 免费保留 1 小时数据
  ...
  * [bleemeo.com](https://bleemeo.com) - 免费 3 台服务器、5 个监控器、无限用户、无限仪表板、无限告警规则。
  * [checklyhq.com](https://checklyhq.com) - 面向开发人员的开源端到端/合成监控和深度 API 监控。免费计划有 5 个用户和 50k+的检查运行。
  ...
  * [deadmanssnitch.com](https://deadmanssnitch.com/) - 对 cron 作业的监控。一个免费的监控器（监视器），如果其他人注册，还可以获得更多。
  * [downtimemonkey.com](https://downtimemonkey.com/) - 60 个监控器，间隔 5 分钟。电子邮件、Slack 提醒。
  * [economize.cloud](https://economize.cloud) - 通过组织云资源进行优化和报告来帮助用户了解云基础设施成本。免费适用于每月在 Google Cloud Platform 上花费最多 5,000 美元的情况。
  * [elastic.co](https://www.elastic.co/solutions/apm) - 为 JavaScript 开发人员提供即时性能洞察。免费保留 24 小时数据。
  ...
  * [newrelic.com](https://www.newrelic.com) - 为帮助工程师创建更完美的软件而构建的 New Relic 观测平台。从单体到无服务器，您可以对所有内容进行仪器化，然后分析、排除故障和优化整个软件堆栈。免费层提供 100GB/月的免费数据摄入，一个免费的全访问用户和无限免费的主要用户。
  * [Middleware.io](https://middleware.io/) - 中间件观测平台提供对应用程序和堆栈的完整可见性，以便您可以大规模监控和诊断问题。他们为开发人员社区提供免费的永久计划，允许 Log 监控最多 1M 日志事件，基础设施监控和 APM 最多 2 个主机。
  * [nixstats.com](https://nixstats.com) - 免费 1 台服务器。电子邮件通知、公共状态页面、60 秒间隔等。
  * [OnlineOrNot.com](https://onlineornot.com/) - 提供网站和 API 的监控，监控 cron 作业和计划任务。还提供状态页面。前五个间隔 3 分钟的检查免费。免费层通过 Slack、Discord 和电子邮件发送警报。
  ...
  * [pingbreak.com](https://pingbreak.com/) - 现代的监控服务。通过 Discord、Slack 或电子邮件检查无限数量的 URL 并获取停机通知。
  ...
  * [sematext.com](https://sematext.com/) - 24 小时指标免费，服务器无限，自定义指标 10 个，自定义指标数据点 500,000 个，仪表板、用户等无限。
  ...
  * [stathat.com](https://www.stathat.com/) - 免费开始，有 10 个统计数据，无过期限制。
  * [statuscake.com](https://www.statuscake.com/) - 网站监控，无限制免费测试，但有限制。
  ...
  * [Pulsetic](https://pulsetic.com) - 10 个监视器，6 个月历史正常运行时间/日志，无限状态页面和自定义域名！免费无限时间和无限电子邮件警报。您不需要信用卡
...
## Crash and Exception Handling
  ...
  * [sentry.io](https://sentry.io/) - Sentry 实时跟踪应用程序异常，有一个小的免费计划。每月免费 5000 个错误/1 个用户，自行托管则无限制使用。
  ...
  * [Jam](https://jam.dev) - 一键式开发者友好的错误报告。免费计划无限 Jam。
  * [Whitespace](https://whitespace.dev) - 在浏览器中一键式错误报告。个人使用的免费计划无限录制。
...
## Education and Career Development
  ...
  * [免费专业简历模板和编辑器](https://www.overleaf.com/latex/templates/tagged/cv) - 免费平台，提供大量经验丰富的专业人士的简历模板，可完全克隆、编辑和下载，ATS 优化。
  ...
  * [MIT 开放式课程](https://ocw.mit.edu/) - MIT 开放式课程是来自 2500 多门 MIT 课程的材料的在线出版物，与世界各地的学习者和教育工作者免费分享知识。Youtube 频道可以在[@mitocw](https://www.youtube.com/@mitocw/featured)找到。
...
## Email
  ...
  * [Buttondown](https://buttondown.email/) - 免费时事通讯服务。最多 100 个订阅者。
  * [CloudMailin](https://www.cloudmailin.com/) - 通过 HTTP POST 接收电子邮件和事务性出站 - 每月免费 10,000 封电子邮件。
  * [Contact.do](https://contact.do/) - 链接中的联系表格（bitly 用于联系表格）。
  ...
  * [forwardemail.net](https://forwardemail.net) - 免费自定义域名电子邮件转发。创建并转发您域名下的无限数量的电子邮件地址（**注意**：如果您使用.casa、.cf、.click、.email、.fit、.ga、.gdn、.gq、.lat、.loan、.london、.men、.ml、.pl、.rest、.ru、.tk、.top、.work TLD，则必须付费）。
  ...
  * [inboxkitten.com](https://inboxkitten.com/) - 免费临时/一次性电子邮件收件箱，最多可自动删除 3 天的电子邮件。开源且可自托管。
  ...
  * [MailerSend.com](https://www.mailersend.com) - 电子邮件 API、SMTP，免费提供 3000 封交易电子邮件。
  ...
  * [Mail7.io](https://www.mail7.io/) - 免费的临时电子邮件地址，供 QA 开发人员使用。使用 Web 界面或 API 立即创建电子邮件地址。
  ...
  * [QuickEmailVerification](https://quickemailverification.com) - 在免费层上每天免费验证 100 封电子邮件，以及其他免费 API，如 DEA 检测器、DNS 查询、SPF 检测器等。
  ...
  * [Sendpulse](https://sendpulse.com) - 每月 500 个订阅者，15,000 封电子邮件免费。
  * [SimpleLogin](https://simplelogin.io/) - 开源、可自托管的电子邮件别名/转发解决方案。免费 5 个别名，无限带宽，无限回复/发送。免费供教育工作者（学生、研究人员等）使用。
  * [Substack](https://substack.com) - 无限免费时事通讯服务。当您开始收费时开始支付。
  ...
  * [Tutanota](https://tutanota.com/)——免费的安全电子邮件帐户服务提供商，内置端到端加密，无广告，无跟踪。免费 1GB 存储空间。部分开源（[github.com/tutao/tutanota](https://github.com/tutao/tutanota)），因此您可以自行托管
  ...
  * [Zoho](https://www.zoho.com)——最初是作为电子邮件提供商，但现在提供一系列服务，其中一些提供免费计划。具有免费计划的服务列表：
     ...
     - [Subscriptions](https://zoho.com/subscriptions)——免费供 20 个客户/订阅和 1 个用户使用，所有支付托管由 Zoho 完成。存储最后 40 个订阅指标
     - [Checkout](https://zoho.com/checkout)——产品计费管理，有 3 个页面和最多 50 笔付款
     - [Desk](https://zoho.com/desk)——客户支持管理，有 3 个代理、私有知识库和电子邮件票务。与[Assist](https://zoho.com/assist)集成，用于 1 个远程技术人员和 5 个无人参与的计算机
...
## Feature Toggles Management Platforms
  ...
  * [Hypertune](https://www.hypertune.com) - 类型安全的功能标志、A/B 测试、分析和应用程序配置，具有 Git 风格的版本控制和同步、内存中、本地标志评估。免费最多供 5 名团队成员使用，无限制的功能标志和 A/B 测试。
  * [Molasses](https://www.molasses.app) - 强大的功能标志和 A/B 测试。免费最多可支持 3 个环境，每个环境有五个功能标志。
  * [Toggled.dev](https://www.toggled.dev) - 企业级、可扩展的多区域功能切换管理平台。免费计划最多可支持 10 个标志、两个环境、无限请求。SDK、分析仪表板、发布日历、Slack 通知和其他所有功能都包含在无尽的免费计划中。
...
## Font
  ...
  * [dafont](https://www.dafont.com/) - 该网站上展示的字体是其作者的财产，它们是免费软件、共享软件、演示版或公共领域的。
  ...
  * [Fontshare](https://www.fontshare.com/) - 是一项免费字体服务。它是一个不断增长的专业级字体集合，可免费用于个人和商业用途。
...
## Forms
  ...
  * [Form.taxi](https://form.taxi/) — HTML 表单提交的端点。具有通知、垃圾邮件阻止器和符合 GDPR 的数据处理。免费计划用于基本使用。
  * [Formcarry.com](https://formcarry.com) - HTTP POST 表单端点，免费计划允许每月 100 次提交。
  * [formingo.co](https://www.formingo.co/) - 适用于静态网站的简单 HTML 表单。无需注册即可免费开始。免费计划允许每月 500 次提交和可自定义的回复电子邮件地址。
  ...
  * [HeyForm.net](https://heyform.net/) - 拖放在线表单构建器。免费层允许创建无限表单和收集无限提交。带有预构建模板、反垃圾邮件和 100MB 文件存储。
  * [Tally.so](https://tally.so/) - 99%的功能都是免费的。免费层允许：无限表单、无限提交、电子邮件通知、表单逻辑、收款、文件上传、自定义感谢页面等。
  * [Hyperforms.app](https://hyperforms.app/) — 在几秒钟内无需后端代码即可创建表单到电子邮件等。个人帐户每月免费提供最多 50 次表单提交。
  ...
  * [Survicate](https://survicate.com/) — 从所有来源收集反馈并使用一个工具发送后续调查。使用 AI 自动分析反馈并提取见解。免费电子邮件、网站、应用内或移动调查、AI 调查创建者和 25 个每月响应。
  * [staticforms.xyz](https://www.staticforms.xyz/) - 免费集成 HTML 表单，无需任何服务器端代码。用户提交表单后，将发送一封包含表单内容的电子邮件到您注册的地址。
  * [stepFORM.io](https://stepform.io) - 测验和表单构建器。免费计划有五个表单，每个表单最多可设置 3 个步骤，每月有 50 次响应。
  * [Typeform.com](https://www.typeform.com/) — 在网站上包含设计精美的表单。免费计划允许每个表单最多 10 个字段，每月最多 100 次响应。
  ...
  * [Wufoo](https://www.wufoo.com/) - 在网站上快速使用的表单。免费计划每月有 100 次提交的限制。
...
## Generative AI
  ...
  * [Langfuse](https://langfuse.com/) - 开源 LLM 工程平台，可帮助团队协作调试、分析和迭代他们的 LLM 应用程序。免费永久计划包括每月 50k 次观察和所有平台功能。[#开源](https://github.com/langfuse/langfuse)
  * [Pollinations.AI](https://pollinations.ai/) - 易于使用的免费图像生成 AI，提供免费 API。无需注册或 API 密钥，并且有几种集成到网站或工作流程的选项。[#开源](https://github.com/pollinations/pollinations)

**[⬆️ 返回顶部](#目录)**

## CDN and Protection

  * [bootstrapcdn.com](https://www.bootstrapcdn.com/) - 引导程序的 CDN，包括 bootswatch 和 fontawesome.io
  ...
  * [Stellate](https://stellate.co/) - Stellate 是您 GraphQL API 的快速、可靠的 CDN，并且对于两个服务是免费的。
...
## PaaS
  ...
  * [anvil.works](https://anvil.works) - 使用 Python 开发 Web 应用程序，无需任何其他工具。免费层提供无限数量的应用程序和 30 秒超时。
  * [appwrite](https://appwrite.io) - 无限项目，无项目暂停（支持 websockets）和身份验证服务。免费层每个项目有 1 个数据库、3 个存储桶、5 个函数。
  * [configure.it](https://www.configure.it/)——移动应用程序开发平台，两个项目免费，功能有限，但无资源限制。
  * [codenameone.com](https://www.codenameone.com/)——开源、跨平台的移动应用程序开发工具链，适用于 Java/Kotlin 开发人员。免费用于商业用途，项目数量不限。
  ...
  * [gigalixir.com](https://gigalixir.com/)——Gigalixir 提供一个永不休眠的免费实例和一个免费的 PostgreSQL 数据库，限制为 2 个连接、10,000 行和不备份，适用于 Elixir/Phoenix 应用程序。
  * [Glitch](https://glitch.com/)——免费公共托管，具有代码共享和实时协作功能。免费计划每月有 1000 小时的限制。
  * [Lade](https://www.lade.io/)——Lade 是一个为开发人员构建的云平台。免费层包括三个服务，可以是应用程序或数据库。每个服务包括 128MB RAM 和 1GB 存储。
  * [pipedream.com](https://pipedream.com)——一个集成平台，用于开发人员。基于任何触发器开发任何工作流程。工作流程是您可以免费运行的代码[免费](https://docs.pipedream.com/pricing/)。无需管理服务器或云资源。
  ...
  * [YepCode](https://yepcode.io)——一个无服务器环境中连接 API 和服务的一体化平台。它带来了 NoCode 工具的所有敏捷性和优势，但具有使用编程语言的所有功能。免费层包括[1.000 yeps](https://yepcode.io/pricing/)。
  ...
  * [Zeabur](https://zeabur.com)——一键部署您的服务。免费提供三个服务，每月有 US$ 5 免费信用额度。
  * [mogenius](https://mogenius.com)——轻松在 Kubernetes 上构建、部署和运行服务。免费层支持连接本地 Kubernetes 和 mogenius，使个人开发人员能够在自己的机器上创建类似生产的测试环境。
  * [genezio](https://genezio.com/)——一个无服务器功能提供程序，提供 100 万个功能调用、100GB 带宽和每月 10 个 cron 作业免费，仅限非商业或学术用途。

**[⬆️ 返回顶部](#table-of-contents)**

## BaaS

  * [Activepieces](https://www.activepieces.com) - 构建自动化流程，将多个应用程序连接到您的应用程序的后端。例如，当您的应用程序中发生事件时，发送一条 Slack 消息或向 Google 表格添加一行。每月免费使用 5000 个任务。
  ...
  * [convex.dev](https://convex.dev/) - 作为服务的反应式后端，托管您的数据（具有关系和可序列化 ACID 事务的文档）、无服务器函数和 WebSockets，以将更新流式传输到各个客户端。小型项目免费-最多 100 万条记录，每月 500 万次函数调用。
  ...
  * [Flutter Flow](https://flutterflow.io) - 无需编写一行代码即可构建您的 Flutter App UI。还与 Firebase 集成。免费计划包括对 UI 构建器和免费模板的完全访问。
  ...
  * [onesignal.com](https://onesignal.com/) - 无限免费推送通知。每月 10,000 封电子邮件发送，无限联系人并可访问自动预热。
  * [paraio.com](https://paraio.com) - 具有灵活身份验证、全文搜索和缓存的后端服务 API。免费适用于一个应用程序，1GB 应用程序数据。
  ...
  * [pushcrew.com](https://pushcrew.com/) - 推送通知服务。无限通知适用于最多 2,000 个订阅者
  ...
  * [Salesforce Developer Program](https://developer.salesforce.com/signup) - 使用拖放工具快速构建、测试和部署应用程序。自定义数据模型只需点击即可。使用 Apex 代码更进一步。使用强大的 API 与任何内容集成。使用企业级安全性保护自己。使用点击或任何领先的 Web 框架自定义 UI。免费开发者计划可访问完整的 Lightning 平台。
  * [ServiceNow Developer Program](https://developer.servicenow.com/) - 快速构建、测试和部署使您的组织工作更轻松的应用程序。免费实例和早期预览访问。
  * [simperium.com](https://simperium.com/) - 立即在各个平台上随时随地移动数据，无限发送和存储结构化数据，最大。每月 2,500 个用户
...
## Low-code Platform
  ...
  * [ReTool](https://retool.com/)——用于构建内部应用程序的低代码平台。Retool 非常易于定制。如果您可以使用 JavaScript 和 API 编写它，那么您可以在 Retool 中实现它。免费套餐允许每月每个用户最多五个用户，无限制的应用程序和 API 连接。
  ...
  * [lil'bots](https://www.lilbots.io/)——在线编写和运行脚本，利用 OpenAI、Anthropic、Firecrawl 等免费内置 API。非常适合构建 AI 代理/内部工具并与团队共享。免费套餐包括对 API 的完全访问、AI 编码助手和每月 10,000 次执行信用。
...
## Web Hosting
  ...
  * [HelioHost](https://heliohost.org)——非盈利免费网络托管，带有 Plesk 控制面板，支持 PHP、Node.js、Python、Django、Flask、.NET、Perl、CGI、MySQL、PostgreSQL、SQLite、IMAP/POP3/SMTP 电子邮件，无限带宽，免费子域，1000MB 存储空间免费，可选择升级。
...
## DNS
  ...
  * [1.1.1.1](https://developers.cloudflare.com/1.1.1.1/) - 由 Cloudflare 提供的免费公共 DNS 解析器，速度快且安全（加密 DNS 查询）。可用于绕过互联网提供商的 DNS 阻塞、防止 DNS 查询监控以及[阻止成人和恶意软件内容](https://developers.cloudflare.com/1.1.1.1/1.1.1.1-for-families)。也可以通过[API](https://developers.cloudflare.com/1.1.1.1/encrypted-dns/dns-over-https/make-api-requests) 使用。注意：只是一个 DNS 解析器，不是 DNS 主机。
  ...
  * [deSEC](https://desec.io)——具有 API 支持的免费 DNS 托管，旨在考虑安全性。运行在开源软件上，并得到[SSE](https://www.securesystems.de/)的支持。
  ...
  * [duckdns.org](https://www.duckdns.org/)——免费 DDNS，免费层最多 5 个域。提供各种设置的配置指南。
  ...
  * [luadns.com](https://www.luadns.com/)——免费 DNS 托管，三个域，所有功能都有限制
  ...
  * [noip.at](https://noip.at/)——免费 DDNS 服务，无需注册、跟踪、日志记录或广告。对域没有限制。
  ...
  * [zilore.com](https://zilore.com/en/dns)——免费 DNS 托管，最多 5 个域。
  ...
  * [Hetzner](https://www.hetzner.com/dns-console)——具有 API 支持的免费 DNS 托管来自 Hetzner
...
## IaaS
  ...
  * [backblaze.com](https://www.backblaze.com/b2/)——Backblaze B2 云存储。免费的 10GB（类似亚马逊 S3）对象存储，无时间限制
  * [filebase.com](https://filebase.com/)——S3 兼容对象存储。无限期免费 5GB 存储
  ...
  * [Spheron](https://spheron.network/)——从去中心化云存储和网络托管到面向应用程序和开发人员的去中心化计算，一个平台下的所有功能，免费计划提供 5GB 存储、100GB 带宽、无限域名和项目，以及 5 美元的计算资源（相对于 AWS 价值 50 美元）。
...
## Managed Data Services
  ...
  * [Astra](https://www.datastax.com/products/datastax-astra/) — 云原生 Cassandra 即服务，有 [80GB 的免费层](https://www.datastax.com/products/datastax-astra/pricing)
  * [codehooks.io](https://codehooks.io/) — 使用 JavaScript 无服务器 API/后端和 NoSQL 数据库服务，具有功能、类似 Mongdb 的查询、键/值查找、作业系统、实时消息、工作队列、强大的 CLI 和基于网络的数据管理器。免费计划有 5GB 存储空间和每分钟 60/API 调用。包含 2 名开发人员。无需信用卡。
  ...
  * [Turso by ChiselStrike](https://chiselstrike.com/) - Turso 是边缘数据库中的 SQLite 开发人员体验。Turso 提供免费永久入门计划，总存储量为 9GB，最多 500 个数据库，最多 3 个位置，每月读取 10 亿行数据，以及本地开发支持与 SQLite。
  * [InfluxDB](https://www.influxdata.com/) — 时间序列数据库，免费层最多每 5 分钟写入 3MB/读取 30MB，最多 10,000 个基数系列
  * [restdb.io](https://restdb.io/) - 快速且直接的 NoSQL 云数据库服务。使用 restdb.io，您可以获得模式、关系、自动 REST API（具有类似 MongoDB 的查询）以及用于处理数据的高效多用户管理 UI。免费计划允许 3 个用户、2500 条记录和每秒 1 个 API 请求。
  ...
  * [Neon](https://neon.tech/) — 管理 PostgreSQL，0.5GB 存储（总计），1 个项目，10 个分支，无限制的数据库，始终可用的主分支（非主分支计算后 5 分钟自动暂停），每月 20 小时的活动时间
  ...
  * [8base.com](https://www.8base.com/) - 8base 是一个为 JavaScript 开发人员构建的全栈低代码开发平台，构建在 MySQL 和 GraphQL 之上以及无服务器后端即服务之上。它允许您使用 UI 应用程序构建器快速构建 Web 应用程序并快速扩展，免费层包括行：2,500，存储：500，无服务器计算：1Gb/h，客户端应用程序用户：5。
  * [Nile](https://www.thenile.dev/) — 一个用于 B2B 应用程序的 Postgres 平台。无限制的数据库，始终可用，无停机，1GB 存储（总计），5000 万查询令牌，自动缩放，无限向量嵌入
...
## Tunneling, WebRTC, Web Socket Servers and Other Routers
  ...
  * [Hamachi](https://www.vpn.net/)——LogMeIn Hamachi 是一种托管的 VPN 服务，允许你使用免费计划（最多可容纳 5 人）创建无限个网络，将分布式团队连接到类似 LAN 的网络。
  * [Mirna Sockets](https://mirna.cloud/)——免费的套接字即服务平台，在部署 Web 套接字服务器代码时为你提供 wss://URL，还允许你监控其性能。
  ...
  * [segment.com](https://segment.com/)——将事件转换和路由到其他第三方服务的中心。免费计划每月可处理 100,000 个事件和 100,000 次尝试，保留时间为 3 天。
  * Google STUN——[stun:stun.l.google.com:19302](stun:stun.l.google.com:19302)
  * Twilio STUN——[stun:global.stun.twilio.com:3478?transport=udp](stun:global.stun.twilio.com:3478?transport=udp)
  * [Tailscale](https://tailscale.com/)——使用开源 WireGuard 协议的零配置 VPN。可安装在 MacOS、iOS、Windows、Linux 和 Android 设备上。个人免费计划可使用 100 个设备和 3 个用户。
  * [webhookrelay.com](https://webhookrelay.com)——管理、调试、分流和代理所有你的 Webhook 到公共或内部（即 localhost）目的地。此外，还可以通过获取公共 HTTP 终结点（`https://yoursubdomain.webrelay.io <----> http://localhost:8080`）将运行在私有网络中的服务器暴露出来。
  * [Hookdeck](https://hookdeck.com/pricing)——从任何地方开发、测试和监控你的 Webhook。每月 100K 请求和 100K 尝试，保留时间为 3 天。
  * [Xirsys](https://www.xirsys.com/pricing/)——无限 STUN 使用+每月 500MB TURN 带宽，限制带宽，单个地理区域。
  * [ZeroTier](https://www.zerotier.com)——免费的托管虚拟以太网即服务。免费计划可在一个集群中为 25 个客户端提供无限端到端加密网络。客户端适用于桌面/移动/NA；Web 界面用于配置自定义路由规则和批准私有网络上的新客户端节点。
  * [LocalXpose](https://localxpose.io)——反向代理，使你能够将本地服务器暴露到互联网。免费计划的隧道生存期为 15 分钟。
  * [Traefik-Hub](https://traefik.io/traefik-hub/)——通过隧道将本地运行的服务发布到公共自定义 URL 并使用访问控制进行保护。免费计划在一个集群中可用于 5 个服务。
...
## Issue Tracking and Project Management
  ...
  * [asana.com](https://asana.com/)——私人项目免费，最多可与协作者一起使用
  ...
  * [Cloudcraft](https://cloudcraft.co/)——使用 Cloudcraft 可视化设计器在几分钟内设计专业的架构图，该设计器针对 AWS 进行了优化，具有智能组件，可显示实时数据。免费计划为单个用户提供无限私人图表。
  * [Codegiant](https://codegiant.io)——项目管理和存储库托管与 CI/CD。免费计划提供无限存储库、项目和文档，最多 5 个团队成员。每月 500 分钟的 CI/CD 分钟数。每月 30000 个无服务器代码运行分钟数 1GB 存储库存储。
  ...
  * [GraphQL Inspector](https://github.com/marketplace/graphql-inspector)——GraphQL Inspector 输出两个 GraphQL 模式之间的更改列表。每个差异都被准确解释，并标记为中断、非中断或危险。
  ...
  * [WishKit](https://wishkit.io)——为您的 iOS/macOS 应用收集应用内用户反馈，并根据用户投票确定功能的优先级。免费计划最多可用于 1 个应用程序。
  * [Ilograph](https://www.ilograph.com/)——交互式图表，允许用户从多个角度和详细级别查看其基础架构。图表可以用代码表示。免费层最多有 3 个查看者的无限私人图表。
  ...
  * [Kumu.io](https://kumu.io/)——具有动画、装饰、过滤器、聚类、电子表格导入等功能的关系图。免费层允许无限公开项目。图形大小不受限制。免费的私人项目供学生使用。如果您不想将文件公开在线（上传、编辑、下载、丢弃），则可以使用沙盒模式。
  * [Linear](https://linear.app/)——界面简洁的问题跟踪器。免费供无限成员使用，文件上传大小最多为 10MB，问题数量最多为 250 个（不包括存档）
  * [leiga.com](https://www.leiga.com/)——Leiga 是一款使用 AI 自动管理项目的 SaaS 产品，可帮助您的团队集中注意力并释放巨大潜力，确保项目按计划推进。免费供最多 10 个用户使用，20 个自定义字段，2GB 存储空间，AI 录制的视频限制为 5 分钟/视频，自动化运行速度为 20/用户/月。
  ...
  * [nTask](https://www.ntaskmanager.com/)——项目管理软件，可让您的团队协作、计划、分析和管理日常任务。Essential 计划永久免费，存储空间为 100MB，用户/团队数量不限。无限工作空间、会议、任务、时间表和问题跟踪。
  * [Ora](https://ora.pm/)——敏捷任务管理和团队协作。免费供最多 3 个用户使用，文件大小限制为 10MB。
  * [pivotaltracker.com](https://www.pivotaltracker.com/)——免费供无限公开项目和两个私人项目使用，每个项目有三个总活动用户（读写）和无限被动用户（只读）。
  ...
  * [point.poker](https://www.point.poker/)——在线规划扑克（基于共识的估计工具）。免费供无限用户、团队、会话、轮次和投票使用。无需注册。
  * [ScrumFast](https://www.scrumfast.com)——具有非常直观界面的 Scrum 板，免费供最多 5 个用户使用。

  * [快捷方式](https://shortcut.com/) - 项目管理平台。免费版最多可容纳 10 名用户，永久免费。
  ...
  * [Tara AI](https://tara.ai/) — 简单的冲刺管理服务。免费版无限制任务、冲刺和工作区，且无用户数量限制。
  * [targetprocess.com](https://www.targetprocess.com/) — 从看板和 Scrum 到几乎任何运营流程的可视化项目管理。免费版无限用户，最多 1,000 个数据实体 [{更多详情](https://www.targetprocess.com/pricing/) }
  * [taskade.com](https://www.taskade.com/) — 实时协作任务列表和团队大纲。免费版有一个工作区，可无限使用任务和项目；1GB 文件存储；项目历史记录为 1 周；每次视频会议可容纳 5 名与会者。
  ...
  * [todoist.com](https://todoist.com/) — 协作和个人任务管理。免费版：5 个活跃项目、项目中 5 个用户、上传文件最大 5MB、3 个过滤器和一周的活动历史。
  ...
  * [vabotu](https://vabotu.com/) - 用于项目管理的协作工具。免费版和其他计划。自由职业者计划适用于 10 名用户，包括消息传递、任务板、5GB 在线存储、工作区和导出数据。
  ...
  * [Toggl](https://toggl.com/) — 提供两个免费的生产力工具。[Toggl Track](https://toggl.com/track/)用于时间管理和跟踪应用程序，具有免费计划，提供与自由职业者设计的无缝时间跟踪和报告。它具有无限的跟踪记录、项目、客户端、标签、报告等。以及 [Toggl Plan](https://toggl.com/plan/) 用于任务规划的免费计划，适用于 solo 开发人员，无限任务、里程碑和时间表。
  * [Sflow](https://sflow.io) — sflow.io 是一个为敏捷软件开发、营销、销售和客户支持构建的项目管理工具，特别是对于外包和跨组织协作项目。免费版最多可容纳 3 个项目和 5 名成员。
...
## Storage and Media Processing
  ...
  * [borgbase.com](https://www.borgbase.com/)——用于 Borg Backup 的简单安全的异地备份托管。10GB 免费备份空间和两个存储库。
  * [icedrive.net](https://www.icedrive.net/)——简单的云存储服务。10GB 免费存储空间
  * [sync.com](https://www.sync.com/)——端到端云存储服务。5GB 免费存储空间
  * [pcloud.com](https://www.pcloud.com/)——云存储服务。高达 10GB 的免费存储空间
  * [sirv.com](https://sirv.com/)——具有即时图像优化和调整大小功能的智能图像 CDN。免费层包括 500MB 存储空间和 2GB 带宽。
  * [cloudimage.io](https://www.cloudimage.io/en/home)——全面的图像优化和 CDN 服务，在全球拥有 1500 多个接入点。多种图像缩放、压缩和水印功能。适用于响应式图像、360 度图像制作和图像编辑的开源插件。免费计划每月提供 25GB 的 CDN 流量、25GB 的缓存存储和无限转换。
  ...
  * [file.io](https://www.file.io)——2GB 文件存储。文件下载一次后自动删除。提供与存储的 REST API。每分钟限制一个请求。
  ...
  * [Imgbot](https://github.com/marketplace/imgbot)——Imgbot 是一个友好的机器人，可优化您的图像并节省您的时间。优化后的图像意味着更小的文件大小，而不会牺牲质量。对于开源项目是免费的。
  ...
  * [nitropack.io](https://nitropack.io/)——自动进行全面的前端优化（缓存、图像和代码优化、CDN）以加速您的网站速度。免费计划适用于每月最多 5000 次页面浏览量
  ...
  * [podio.com](https://podio.com/)——您可以与最多五个人的团队一起使用 Podio，并试用基本计划的功能，但不包括用户管理
  * [QuickChart](https://quickchart.io)——生成可嵌入的图像图表、图形和 QR 码
  * [redbooth.com](https://redbooth.com)——P2P 文件同步，最多可供 2 个用户免费使用
  * [resmush.it](https://resmush.it)——reSmush.it 是一个免费的 API，提供图像优化。reSmush.it 已在 WordPress、Drupal 或 Magento 等最常见的 CMS 上实现。reSmush.it 是使用最广泛的图像优化 API，已经处理了超过 70 亿张图像，而且仍然是免费的。
  * [Shotstack](https://shotstack.io)——按比例生成和编辑视频的 API。每月免费高达 20 分钟的渲染视频
  * [tinypng.com](https://tinypng.com/)——压缩和调整 PNG 和 JPEG 图像大小的 API，每月免费提供 500 次压缩
  * [transloadit.com](https://transloadit.com/)——处理文件上传以及视频、音频、图像、文档的编码。免费供开源、慈善机构和学生使用，通过 GitHub 学生开发者包。商业应用程序可免费获得 2GB 用于测试
  * [twicpics.com](https://www.twicpics.com)——响应式图像即服务。它提供图像 CDN、媒体处理 API 和前端库，可自动优化图像。服务免费使用，每月最多 3GB 流量。
  * [uploadcare.com](https://uploadcare.com/hub/developers/)——Uploadcare 为开发人员提供基于最先进算法的媒体管道和终极工具包：文件上传 API 和 UI、图像 CDN 和源服务、自适应交付和智能压缩。免费层包括 3000 次上传、3GB 流量和 3GB 存储。
  * [imagekit.io](https://imagekit.io)——图像 CDN，具有自动优化、实时转换和存储功能，可在几分钟内集成到现有设置中。免费计划每月包括高达 20GB 的带宽。
  * [internxt.com](https://internxt.com)——Internxt Drive 是基于绝对隐私和坚如磐石的安全性的零知识文件存储服务。注册即免费获得 10GB，永久有效！
  * [degoo.com](https://degoo.com/)——基于人工智能的云存储，免费提供高达 20GB、三个设备、5GB 推荐奖金（90 天账户不活动）。

  * [ImageEngine](https://imageengine.io/)——ImageEngine 是一个易于使用的全球图像 CDN。设置时间少于 60 秒。支持 AVIF 和 JPEGXL，WordPress、Magento、React、Vue 插件等。在此[申请](https://imageengine.io/developer-program/)免费开发者账户。
...
## Design and UI
  ...
  * [Backlight](https://backlight.dev/) - 专注于开发人员和设计师之间的协作，Backlight 是一个完整的编码平台，团队可以在其中构建、记录、发布、扩展和维护设计系统。免费计划允许最多 3 个编辑器在一个设计系统上工作，无限观众。
  ...
  * [ColorKit](https://colorkit.co/) - 在网上创建颜色调色板或从顶级调色板中获得灵感。
  * [coolors](https://coolors.co/) - 颜色调色板生成器。免费的。
  * [Branition](https://branition.com/colors) - 为品牌精心挑选的颜色调色板。
  * [css-gradient.com](https://www.css-gradient.com/) - 快速生成自定义跨浏览器 CSS 渐变的免费工具。以 RGB 和 HEX 格式。
  * [easyvectors.com](https://easyvectors.com/) - 免费 SVG 向量艺术库存。免费下载最佳向量图形。
  ...
  * [framer.com](https://www.framer.com/) - Framer 帮助您为下一个应用、网站或产品迭代和动画界面创意-从强大的布局开始。对于将 Framer 用作专业原型工具的任何人：无限的查看者，最多 2 个编辑器和最多 3 个项目。
  ...
  * [LottieFiles](https://lottiefiles.com/) - 世界上最大的设计师、开发人员等使用的最微型动画格式的在线平台。访问用于 Android、iOS 和 Web 的 Lottie 动画工具和插件。
  ...
  * [Mindmup.com](https://www.mindmup.com/) - 免费的无限思维导图，并将其存储在云中。您的思维导图随时随地都可用，从任何设备都可以立即访问。
  ...
  * [tabler-icons.io](https://tabler-icons.io/) - 1500 多个免费可复制和粘贴的可编辑 SVG 图标。
  ...
  * [walkme.com](https://www.walkme.com/)——企业级指导和参与平台，免费计划可进行三个最多五步/步的引导。
  ...
  * [SceneLab](https://scenelab.io)——在线模型图形编辑器，带有不断扩展的免费设计模板集合。
  * [xLayers](https://xlayers.dev)——预览和将 Sketch 设计文件转换为 Angular、React、Vue、LitElement、Stencil、Xamarin 等（免费和开源，可在[https://github.com/xlayers/xlayers](https://github.com/xlayers/xlayers)获得）
  ...
  * [Unicorn Platform](https://unicornplatform.com/)——具有托管功能的轻松着陆页构建器。免费提供一个网站。
  ...
  * [Logo.dev](https://www.logo.dev)——具有 4400 多万个品牌的公司徽标 API，就像调用 URL 一样简单。前 10,000 次 API 调用是免费的。
...
## Design Inspiration
  ...
  * [awwwards.](https://www.awwwards.com/) - [最佳网站]设计师投票选出的最佳设计网站展示平台。
  ...
  * [Mobile Patterns](https://www.mobile-patterns.com/) - [移动设备屏幕截图]一个设计灵感库，其中包含设计师、开发人员和产品制造商可以参考的 iOS 和 Android 上最好的 UI UX 模式。
  * [Page Flows](https://pageflows.com/) - [移动/网络视频和屏幕截图]许多移动和网络应用的完整流程视频。还包括屏幕截图。可高度搜索和索引。
  * [Screenlane](https://screenlane.com/) - [移动设备屏幕截图]获取灵感并跟上最新的网络和移动应用程序 UI 设计趋势。可以按模式和应用程序进行过滤。
...
## Data Visualization on Maps
  ...
  * [Clockwork Micro](https://clockworkmicro.com/)——像时钟一样精确的地图工具。每月免费查询五十万次（地图瓦片、db2vector、高程）。
  * [developers.arcgis.com](https://developers.arcgis.com)——用于地图、地理空间数据存储、分析、地理编码、路由等的 API 和 SDK，适用于网络、桌面和移动设备。每月免费提供二百万个基本地图瓦片、20,000 个未存储的地理编码、20,000 条简单路线、5,000 次驾车时间计算以及 5GB 的免费图块+数据存储。
  * [Foursquare](https://developer.foursquare.com/)——通过 Places API 和 Pilgrim SDK 进行位置发现、场地搜索和上下文感知内容。
  * [geoapify.com](https://www.geoapify.com/)——提供矢量和光栅地图瓦片、地理编码、地点、路由、等压线 API。每天免费请求三千次。
  ...
  * [opencagedata.com](https://opencagedata.com)——地理编码 API，聚合了 OpenStreetMap 和其他开放地理源。每天免费查询两千五百次。
  * [osmnames](https://osmnames.org/)——地理编码，按相关维基百科页面的受欢迎程度对搜索结果进行排名。
...
## IDE and Code Editing
  ...
  * [apiary.io](https://apiary.io/) - 具有即时 API 模拟和生成文档的协作式设计 API（免费提供无限的 API 蓝图和一个管理员帐户下的无限用户，以及托管文档）。
  ...
  * [cocalc.com](https://cocalc.com/) - （以前在 cloud.sagemath.com 名为 SageMathCloud）- 云协作计算。浏览器访问带有内置协作功能的完整 Ubuntu，以及大量免费的数学、科学、数据科学软件，预安装：Python、LaTeX、Jupyter Notebooks、SageMath、scikitlearn 等。
  ...
  * [Eclipse Che](https://www.eclipse.org/che/) - 用于开发团队的基于 Web 和 Kubernetes 的多语言 IDE。开源且由社区驱动。红帽提供了一个在线实例，可在[workspaces.openshift.com](https://workspaces.openshift.com/)访问。
  * [fakejson.com](https://fakejson.com/) - FakeJSON 帮助您使用其 API 快速生成虚假数据。发送一个描述您想要的内容和方式的 API 请求。API 以 JSON 格式返回所有内容。加快创意的推向市场过程，先假后真。
  ...
  * [jsbin.com](https://jsbin.com) - 另一个前端网络（HTML、CSS 和 JavaScript）的游乐场和代码共享网站。它还支持 Markdown、Jade 和 Sass。
  * [jsfiddle.net](https://jsfiddle.net/) - 前端网络的游乐场和代码共享网站，支持协作。
  ...
  * [Prepros](https://prepros.io/) - Prepros 可以开箱即用地编译 Sass、Less、Stylus、Pug/Jade、Haml、Slim、CoffeeScript 和 TypeScript，刷新您的浏览器，使您易于开发和测试您的网站，因此您可以专注于使它们完美。您还可以通过点击几下添加自己的工具。
  ...
  * [VSCodium](https://vscodium.com/)——社区驱动的、无遥测/跟踪功能且自由许可的微软编辑器 VSCode 的二进制分发版本
  * [wakatime.com](https://wakatime.com/)——使用文本编辑器插件对您的编码活动进行自我量化度量，免费计划有限。
  * [Wave Terminal](https://waveterm.dev/)——Wave 是一个开源的跨平台终端，用于无缝工作流程。内联呈现任何内容。保存会话和历史记录。由开放的网络标准提供支持。适用于 MacOS 和 Linux。
  * [WebComponents.dev](https://webcomponents.dev/)——在浏览器中隔离编写 Web 组件的集成开发环境，提供 58 个可用模板，支持故事和测试。
...
## Analytics, Events and Statistics
  ...
  * [Hightouch](https://hightouch.com/) - Hightouch 是一个反向 ETL 平台，可帮助用户将客户数据从数据仓库同步到 CRM、营销和支持工具。免费层提供一个目的地同步数据。
  ...
  * [Census](https://www.getcensus.com/) — 反向 ETL 和运营分析平台。从您的数据仓库同步 10 个字段到 Salesforce、Zendesk 或 Amplitude 等 60 多个 SaaS。
  ...
  * [GoatCounter](https://www.goatcounter.com/) — GoatCounter 是一个开源网络分析平台，可作为托管服务（非商业用途免费）或自托管应用程序使用。它旨在提供易于使用和有意义的隐私友好型网络分析，以替代 Google Analytics 或 Matomo。免费层适用于非商业用途，包括无限站点、6 个月的数据保留和每月 10 万次页面浏览量。
  * [Google Analytics](https://analytics.google.com/) — Google Analytics
  * [MetricsWave](https://metricswave.com) — 适合开发人员的隐私友好型 Google Analytics 替代方案。免费计划允许每月 20k 页面浏览量，无需信用卡。
  * [Expensify](https://www.expensify.com/) — 费用报告，免费个人报告审批工作流程
  * [getinsights.io](https://getinsights.io) - 关注隐私的分析，每月免费 3k 事件。
  * [heap.io](https://heap.io) — 在 iOS 或网络应用中自动捕获每个用户操作。免费提供高达 10K 每月会话。
  * [Hotjar](https://hotjar.com) — 网站分析和报告。免费计划允许每天 2000 次页面浏览量。每天一百个快照（最大容量：300）。可以存储三个快照热图 365 天。无限团队成员。还包括应用内和独立调查、带屏幕截图的反馈小部件。免费层允许创建 3 个调查和 3 个反馈小部件，每月收集 20 个响应。
  ...
  * [Moesif](https://www.moesif.com) — API 分析用于 REST 和 GraphQL。（免费最多 500,000 次 API 调用/月）
  * [optimizely.com](https://www.optimizely.com) — A/B 测试解决方案，免费入门计划，一个网站，1 个 iOS 和 1 个 Android 应用
  * [Microsoft PowerBI](https://powerbi.com) — Microsoft 业务洞察和分析。免费计划提供有限使用，用户许可证为 100 万。
  ...
  * [Statsig](https://statsig.com) - 涵盖分析、功能标志和 A/B 测试的一体化平台。免费提供每月最多 100 万次度量事件。
  * [Tableau Developer Program](https://www.tableau.com/developer) — 创新、创建并使 Tableau 完全适合您的组织。免费开发者计划为 Tableau Online 提供个人开发沙盒许可证。该版本是最新的预发行版本，因此数据开发人员可以测试该出色平台的每个功能。
  * [usabilityhub.com](https://usabilityhub.com/) — 在真实用户身上测试设计和模型并跟踪访问者。免费提供给一个用户，无限制的测试
  * [woopra.com](https://www.woopra.com/) — 免费的用户分析平台，适用于 500K 次操作，90 天数据保留，30 多个一键集成。
  * [counter.dev](https://counter.dev) — 简单易用的网络分析，注重隐私。免费或根据需要通过捐赠支付。
  * [PostHog](https://posthog.com) - 完整的产品分析套件，每月最多可跟踪 100 万次事件免费。还提供每月 250 次响应的无限量应用内调查。
  * [Uptrace](https://uptrace.dev) - 分布式跟踪工具，可帮助开发人员找出故障并找到性能瓶颈。有免费计划，为开源项目提供免费的个人订阅，并且有开源版本。
  * [Microsoft Clarity](https://clarity.microsoft.com/) - 清晰是一款免费、易于使用的工具，可捕获真实用户如何使用您的网站。
  * [Beampipe.io](https://beampipe.io) - 简单、注重隐私的网络分析。免费提供最多 5 个域和 10k 每月页面浏览量。
  ...
  * [Trackingplan](https://www.trackingplan.com/) - 自动检测数字分析、营销数据和像素问题，维护最新的跟踪计划，并促进无缝协作。可以将其部署到生产环境中，也可以在回归测试中添加分析覆盖，而无需编写代码。
  * [LogSpot](https://logspot.io) - 完整的统一网络和产品分析平台，包括可嵌入的分析小部件和自动化机器人（slack、telegram 和 webhooks）。免费计划包括每月 10,000 次事件。
  * [Umami](https://umami.is/) - 简单、快速、注重隐私的开源替代方案，替代 Google Analytics。
  * [TrackWith Dicloud](https://dicloud.net/trackwith-privacy-focused-analytics/) - 免费的轻量级注重隐私的 Google Analytics 替代方案。无限制的页面浏览量、访问者、页面热点图和目标跟踪。免费提供最多 3 个域和每个域 600 次会话回放。
  * [AppFit](https://appfit.io) - AppFit 是一个全面的分析和产品管理工具，旨在促进跨平台的分析和产品更新的无缝管理。免费计划包括每月 10,000 次事件、产品日记和每周洞察。

**[⬆️ 返回顶部](# 目录 )**

## Visitor Session Recording

  * [Reactflow.com](https://www.reactflow.com/) - 每个站点：每天 1000 页浏览量，三个热图，三个小部件，免费错误跟踪
  ...
  * [hotjar.com](https://www.hotjar.com/) - 每个站点：每月 1050 页浏览量，不限热图，数据存储三个月
  ...
  * [UXtweak.com](https://www.uxtweak.com/) - 记录并观看访问者如何使用您的网站或应用程序。小型项目免费无限期使用
...
## Payment and Billing Integration
  ...
  * [Qonversion](http://qonversion.io/)——一体化跨平台订阅管理平台，提供分析、A/B 测试、Apple Search Ads、远程配置和增长工具，用于优化应用内购买和货币化。与 iOS、Android、React Native、Flutter、Unity、Cordova、Stripe 和网络兼容。免费使用，每月最高可追踪收入为$10k。
  * [ParityVend](https://www.ambeteco.com/ParityVend/)——根据访问者位置自动调整价格，以扩大您的全球业务并进入新市场（购买力平价）。免费计划包括每月 7,500 次 API 请求。
  * [Glassfy](https://glassfy.io/)——应用内订阅基础架构，实时订阅事件和即开即用的货币化工具，适用于 iOS、Android、Stripe 和 Paddle。免费使用，每月最高可达到$10k 收入。
  * [Adapty.io](https://adapty.io/)——用于移动应用内订阅集成的开源 SDK 一站式解决方案，适用于 iOS、Android、React Native、Flutter、Unity 或网络应用。免费使用，每月最高可达到$10k 收入。
  ...
  * [RevenueCat](https://www.revenuecat.com/)——应用内购买和订阅的托管后端（iOS 和 Android）。免费使用，每月最高可追踪收入为$2.5k。
...
## Commenting Platforms
  ...
  * [Utterances](https://utteranc.es/) - 一个基于 GitHub 问题构建的轻量级评论小部件。将 GitHub 问题用于博客评论、wiki 页面等！
  * [Disqus](https://disqus.com/) - Disqus 是一个网络社区平台，被数十万网站在网络上使用。
  * [Remarkbox](https://www.remarkbox.com/) - 开源托管评论平台，可按需支付“几个域上的一位管理员，完全控制行为和外观”
...
## Screenshot APIs
  ...
  * [microlink.io](https://microlink.io/)——它将任何网站转换为数据，例如元标记规范化、美观的链接预览、抓取功能或截图服务。每天免费提供 250 个请求。
  ...
  * [screenshotmachine.com](https://www.screenshotmachine.com/)——每月捕获 100 张快照，包括 png、gif 和 jpg，包括全长捕获，不仅是主页
  * [PhantomJsCloud](https://PhantomJsCloud.com)——浏览器自动化和页面渲染。免费层提供每天最多 500 页。自 2017 年以来一直提供免费层。
...
## Flutter Related and Building IOS Apps without Mac
  ...
  * [CodeMagic](https://codemagic.io/) - CodeMagic 是一个完全托管和管理的移动应用持续集成/持续部署服务。您可以使用基于 GUI 的持续集成/持续部署工具进行构建、测试和部署。免费套餐提供 500 分钟/月的免费时长和一个具有 2.3GHz 处理器和 8GB 内存的 Mac Mini 实例。
...
## Miscellaneous
  ...
  * [Carbon.now.sh](https://carbon.now.sh) - 创建和分享代码片段的美观截图，类似于图像格式。通常用于在 Twitter 或博客文章上美观地分享/展示代码片段。
  ...
  * [fullcontact.com](https://www.fullcontact.com/developer/pricing/) - 通过在应用程序中添加社交个人资料，帮助用户更多地了解他们的联系人。每月 500 个免费个人 API 匹配。
  ...
  * [kandi](https://kandi.openweaver.com/) - 快速启动应用程序开发：通过代码片段和开源库的重用，更快地构建自定义函数、用例和完整的应用程序。
  ...
  * [readme.com](https://readme.com/) - 轻松实现美观的文档，开源免费。
  ...
  * [superfeedr.com](https://superfeedr.com/) - 实时 PubSubHubbub 兼容的提要、导出、分析。免费提供较少的自定义功能
  * [SurveyMonkey.com](https://www.surveymonkey.com) - 创建在线调查。在线分析结果。免费计划允许每个调查最多 10 个问题和 100 个响应。
  * [Tiledesk](https://tiledesk.com) - 创建聊天机器人和对话应用程序。将它们带入全渠道：从您的网站（实时聊天小部件）到 WhatsApp。免费计划无限量聊天机器人。
  * [Versionfeeds](https://versionfeeds.com) - 您喜爱的软件版本的自定义 RSS 饲料。将您最喜欢的编程语言、库或工具的最新版本集中在一个饲料中。（前 3 个饲料是免费的）
...
## Game Development
  ...
  * [ArtStation](https://www.artstation.com/) - 免费/付费的 2D、3D 资产和音频、图标、平铺集、游戏工具包的市场。也可以用于展示你的艺术作品集。
...
## Other Free Resources
  ...
  * [Wikimint 开发者](https://developer.wikimint.com/p/tools.html)——适用于网络开发者的免费工具，包括 CSS 压缩/解压、图像优化、图像大小调整、大小写转换、CSS 验证器、JavaScript 编译器、HTML 编辑器等。
  * [ElevateAI](https://www.elevateai.com)——每月可免费获得长达 200 小时的音频转录。
  * [get.localhost.direct](https://get.localhost.direct)——更好的`*.localhost.direct`通配符公共 CA 签名的 SSL 证书，适用于具有子域名支持的本地主机开发。
  * [Framacloud](https://degooglisons-internet.org/en/)——法国非营利组织 Framasoft（[framasoft.org/en/](https://framasoft.org/en/))提供的免费/自由开源软件和 SaaS 列表。
  ...
  * [SimpleBackups.com](https://simplebackups.com/)——服务器和数据库（MySQL、PostgreSQL、MongoDB）的备份自动化服务，直接存储到云存储提供商（AWS、DigitalOcean、Backblaze）。提供 1 个备份的免费计划。
  ...
  * [SmallDev.tools](https://smalldev.tools/)——一款免费的开发者工具，可让您对各种格式进行编码/解码、最小化 HTML/CSS/Javascript、美化、以 JSON/CSV 及多种其他格式生成虚假/测试数据集等功能。具有令人愉快的界面。
  * [UseCSV by Layercode](https://layercode.com/usecsv)——在您的网络应用程序中几分钟内添加 CSV 和 Excel 导入。为您的用户提供愉快而强大的数据导入体验。免费开始，无需任何信用卡详细信息，今天就开始集成 UseCSV。您可以创建无限的导入器，上传的文件最大为 100Mb。
  ...
  * [regex101](https://regex101.com/)——免费网站，可让您测试和调试正则表达式（regex）。它提供了一个正则表达式编辑器和测试器，以及学习 regex 的有用文档和资源。
  * [Kody Tools](https://www.kodytools.com/dev-tools)——100 多个开发工具，包括格式化程序、最小化程序和转换器。
  ...
  * [QRCodeBest](https://qrcode.best/)——使用 13 个模板创建自定义 QR 码，具有完全隐私和个人品牌。功能包括跟踪像素、项目分类和无限团队席位的 QRCode.Best。
