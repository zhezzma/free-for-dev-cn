...
开发者与开源作者如今拥有众多提供免费层级的服务，但要全面了解这些服务以便做出明智选择仍需耗费时间。

这里整理了一系列提供开发者免费层级的软件（SaaS、PaaS、IaaS等）及其他服务。

本清单的范围限定为基础设施开发者（系统管理员、DevOps从业者等）可能认为有用的内容。我们虽喜爱所有免费服务，但仍需保持主题相关性。有时界限较为模糊，因此本清单带有主观判断；若未采纳您的贡献，敬请谅解。

这份清单汇集了1600多位人士提交的Pull Request、评审、建议及工作成果。您也可以通过提交[Pull Requests](https://github.com/ripienaar/free-for-dev)来协助添加更多服务，或移除已变更或停服的项目。

[![追踪Awesome清单](https://www.trackawesomelist.com/badge.svg)](https://www.trackawesomelist.com/ripienaar/free-for-dev)

**注意**：本清单仅适用于服务型产品（as-a-Service），不包含自托管软件。入选服务必须提供免费层级（而非仅限免费试用）。若采用时间分段模式，免费层级至少需持续一年。我们也会从安全角度评估免费层级——支持SSO，但拒绝将TLS功能限制在付费层级的服务。
...
# 目录
  ...
  * [源代码仓库](#源代码仓库)
  * [存储与媒体处理](#存储与媒体处理)
  * [隧道技术、WebRTC、WebSocket服务器及其他路由](#隧道技术、WebRTC、WebSocket服务器及其他路由)
  * [测试工具](#测试工具)
  * [团队协作工具](#团队协作工具)
  * [翻译管理](#翻译管理)
  * [Vagrant相关](#Vagrant相关)
...
* [Microsoft Azure](https://azure.microsoft.com)
    ...
    * [Azure Pipelines](https://azure.microsoft.com/services/devops/pipelines/) — 10个免费并行任务，Linux、macOS和Windows开源项目无时间限制
...
* [Cloudflare](https://www.cloudflare.com/)
    * [应用服务](https://www.cloudflare.com/plans/) - 无限域名免费DNS、DDoS防护、CDN及免费SSL、防火墙规则和页面规则、WAF、机器人缓解、免费不限流量速率限制（每域名1条规则）、分析、邮件转发
    * [零信任与SASE](https://www.cloudflare.com/plans/zero-trust-services/) - 最多50个用户，24小时活动日志，3个网络位置
...
* [Cloud 66](https://www.cloud66.com/) - 个人项目免费（包含一个部署服务器和一个静态站点），提供在任意云平台构建、部署和扩展应用所需的一切功能，无需操心"服务器事务"
...
* [scalr.com](https://scalr.com/) - Terraform自动化协作平台(TACO)，用于优化Terraform管理的基础设施和配置的协作与自动化。完整支持Terraform CLI、OPA集成和分层配置模型。无SSO费用，包含所有功能。每月免费提供50次运行
* [deployment.io](https://deployment.io) - 帮助开发者在AWS上实现自动化部署。免费版支持单个开发者无限制部署静态站点、Web服务和环境，每月提供20次包含预览和自动部署的免费任务执行
...
* [Codeberg](https://codeberg.org/) — 免费开源项目的无限量公共/私有Git仓库（协作者不限）。基于[Forgejo](https://forgejo.org/)构建。通过[Codeberg Pages](https://codeberg.page/)提供静态网站托管，[Codeberg CI](https://docs.codeberg.org/ci/)实现CI/CD托管，[Codeberg Translate](https://translate.codeberg.org/)提供翻译托管。包含软件包/容器托管、项目管理和问题追踪功能  
...
* [pijul.com](https://pijul.com/) — 无限量免费开源的分布式版本控制系统。基于补丁理论设计，解决git/hg/svn/darcs的诸多问题  
...
* [RocketGit](https://rocketgit.com) — 基于Git的代码托管服务，无限量公共/私有仓库  
...
## API、数据与机器学习
  ...
  * [JSONGrid](https://jsongrid.com) - 免费工具，将复杂JSON数据可视化、编辑、筛选成美观的表格网格。通过链接保存和分享JSON数据。
* [Zerosheets](https://zerosheets.com) - 将Google表格转换为强大的API，快速开发原型、网站、应用等。每月免费提供500次请求。
...
* [APIToolkit.io](https://apitoolkit.io) - 提供全面了解API和后端运行情况所需的所有工具，包括自动API合约验证和监控。免费计划每月支持最多20,000次请求的服务器。
* [APIVerve](https://apiverve.com) - 免费即时访问120多个API，注重质量、一致性和可靠性。免费计划每月提供最多50个API令牌。
* [Arize AI](https://arize.com/) - 机器学习可观测性平台，用于模型监控和数据质量、性能漂移等问题的根源分析。免费支持最多两个模型。
* [Beeceptor](https://beeceptor.com) - 秒级模拟REST API，伪造API响应等。每天免费50次请求，公开仪表板，开放端点（任何拥有仪表板链接的人均可查看提交和响应）。
* [bigml.com](https://bigml.com/) — 托管的机器学习算法。开发阶段任务无限免费，每任务数据限制为16 MB。
* [Browse AI](https://www.browse.ai) — 网络数据提取和监控。免费每月50个积分。
...
* [Cloudmersive](https://cloudmersive.com/) — 多功能API平台，提供文档转换、病毒扫描等广泛API库的完全访问权限，每月800次调用。
...
* [CometML](https://www.comet.com/site/) - MLOps平台，涵盖实验跟踪、模型生产管理、模型注册和完整数据沿袭，从训练到生产全流程覆盖。个人和学术用户免费。
* [Commerce Layer](https://commercelayer.io) - 可组合商务API，可从任何前端构建、下单和管理订单。开发者计划每月允许100个订单和最多1,000个SKU免费。
* [Conversion Tools](https://conversiontools.io/) - 在线文件转换器，支持文档、图像、视频、音频和电子书。提供REST API及Node.js、PHP、Python库。付费计划支持最大50 GB文件。免费层受限于文件大小和每日转换次数。
...
* [Coupler](https://www.coupler.io/) - 在应用间同步的数据集成工具，可创建实时仪表板和报告，转换和操作值，收集和备份洞察。免费计划包含无限用户、每月100次运行（1000行数据）和无限集成。
...
* [Cube](https://cube.dev/) - Cube帮助数据工程师和应用开发者从现代数据存储访问数据，组织成一致定义并交付到每个应用。最快方式是使用Cube Cloud，其免费层每月允许1GB数据通过。
...
* [Datalore](https://datalore.jetbrains.com) - Jetbrains出品的Python笔记本。包含10 GB存储和每月120小时运行时间。
...
* [DB-IP](https://db-ip.com/api/free) - 免费IP地理定位API，每个IP每天1,000次请求。CC-BY 4.0许可的精简数据库同样免费。
* [DB Designer](https://www.dbdesigner.net/) — 基于云的数据库模式设计和建模工具，免费入门计划包含2个数据库模型，每个模型10个表。
* [DeepAR](https://developer.deepar.ai) — 适用于任何平台的增强现实人脸滤镜SDK。免费计划每月最多10个活跃用户(MAU)，跟踪最多4张人脸。
* [Deepnote](https://deepnote.com) - 新型数据科学笔记本，兼容Jupyter，支持实时协作和云端运行。免费层包含无限个人项目、最多750小时标准硬件和最多3名编辑的团队。
* [Diggernaut](https://www.diggernaut.com/) — 基于云的网络抓取和数据提取平台，可将任何网站转为数据集或作为API使用。免费计划每月5,000次页面请求。
* [Disease.sh](https://disease.sh/) — 免费API，为构建新冠疫情相关应用提供准确数据。
* [Doczilla](https://www.doczilla.app/) — SaaS API，直接从HTML/CSS/JS代码生成截图或PDF。免费计划每月250份文档。

  * [drawDB](https://drawdb.app/) — 免费开源的在线数据库图表编辑器，无需注册
* [dreamfactory.com](https://dreamfactory.com/) — 为移动、Web和IoT应用提供的开源REST API后端。连接任何SQL/NoSQL数据库、文件存储系统或外部服务，即可立即生成包含实时文档和用户管理的完整REST API平台
* [DynamicDocs](https://advicement.io) - 基于LaTeX模板的JSON转PDF API文档生成服务。免费计划每月允许50次API调用并提供模板库访问权限
* [Efemarai](https://efemarai.com) - 机器学习模型与数据的测试调试平台。可视化任何计算图。开发者每月可免费进行30次调试会话
* [ExtendsClass](https://extendsclass.com/rest-client-online.html) - 基于网页的免费HTTP客户端，用于发送HTTP请求
* [Export SDK](https://exportsdk.com) - 提供拖拽式模板编辑器的PDF生成API，包含SDK和无代码集成方案。免费计划每月250页，支持无限用户和三个模板
* [Fern](https://buildwithfern.com) - 通过API定义生成主流语言SDK，并创建API参考文档网页。支持添加Markdown页面并通过Fern托管完整文档解决方案。全面兼容OpenAPI规范
...
* [FraudLabs Pro](https://www.fraudlabspro.com) — 信用卡交易欺诈检测REST API，根据订单参数识别所有潜在欺诈特征。Micro免费计划每月处理500笔交易
* [Geekflare API](https://geekflare.com/api) - 提供网页截图、站点审计、TLS扫描、DNS查询、TTFB测试等功能。免费计划包含3,000次API调用
* [GeoCod](https://geocod.xyz) — 免费地理编码API：将邮政地址转换为地理坐标，或反向解析坐标到地址（逆地理编码）
* [GeoDataSource](https://www.geodatasource.com) — 通过经纬度坐标查询城市名称的位置服务。免费API每月可调用500次
* [Geolocated.io](https://geolocated.io) — 多大陆服务器的IP地理定位API，为爱好者提供永久免费计划（每月60,000次请求）
* [Glitterly](https://glitterly.app/) - 通过基础模板编程生成动态图像。提供RESTful API和无代码集成。免费层包含50张图片/月和5个模板
* [GoodData](https://www.gooddata.com/) - 数据即服务 - 创建交互式洞察仪表板。免费版包含5个工作区和100MB/工作区
* [Hex](https://hex.tech/) - 面向笔记本、数据应用和知识库的协作式数据平台。社区免费版支持3名作者和5个项目，每位作者享有4GB内存的计算配置
* [Hook0](https://www.hook0.com/) - 开源Webhooks即服务(WaaS)，帮助在线产品轻松实现Webhook功能。免费版每月可发送3,000个事件并保留7天历史记录
* [Hoppscotch](https://hoppscotch.io) - 免费、快速、美观的API请求构建工具
* [Hybiscus](https://hybiscus.dev/) - 使用声明式API构建PDF报告。免费层包含每月100份单页报告，支持调色板和字体自定义
* [Invantive Cloud](https://cloud.invantive.com/) — 通过Invantive SQL或OData4（通常用于Power BI/Power Query）访问70多个(云)平台如Exact Online、Twinfield等。包含数据复制与交换功能。开发者与实施顾问可享免费计划，特定平台有数据量限制
...
* [IPLocate](https://www.iplocate.io) — IP地理定位API，每日免费1,000次请求。包含代理/VPN/主机检测、ASN数据、IP归属企业查询等功能。同时提供可下载的IP与国家/ASN对应数据库（CSV或GeoIP兼容MMDB格式）
* [IP2Location](https://www.ip2location.com) — 免费增值IP地理定位服务。LITE数据库可免费下载，支持服务器本地查询城市坐标和ISP信息
* [IP2Location.io](https://www.ip2location.io/) — 快速可靠的免费增值IP地理定位API。获取城市、坐标、ISP等数据。免费计划含每月30k额度。同时提供免费WHOIS和托管域名查询服务，付费计划解锁更多功能
* [ipapi](https://ipapi.co/) - Kloudend公司出品的IP地址定位API - 基于AWS构建的可靠地理定位服务，财富500强企业信赖。免注册免费层每月30k次查询（每日1k次）
* [ipapi.is](https://ipapi.is/) - 开发者打造的可靠IP地址API，具备顶级主机检测能力。免注册免费提供1,000次查询
* [IPinfo](https://ipinfo.io/) — 快速精准的免费IP数据API（每月50k次上限）。提供地理位置、企业信息、运营商、IP范围、域名、滥用联系人等详细API。所有付费API均可免费试用
* [BigDataCloud](https://www.bigdatacloud.com/) - 为现代Web提供快速精准的免费API（无限或每月10K-50K次），涵盖IP地理定位、逆地理编码、网络洞察、邮箱/手机验证、客户端信息等功能
* [IPTrace](https://iptrace.io) — 极简API，为企业提供可靠的IP地理定位数据
* [JSON2Video](https://json2video.com) - 通过编程或无代码方式自动化生成营销视频和社交媒体视频的编辑API
* [JSON IP](https://getjsonip.com) — 返回请求客户端的公网IP地址。免费层无需注册，支持CORS可通过浏览器JS直接调用。适用于监控客户端与服务器IP变更的服务。不限请求次数
...
* [konghq.com](https://konghq.com/) — API市场与强大的公私API工具。免费版在监控、告警和支持等功能上有限制
* [Kreya](https://kreya.app) — 免费gRPC GUI客户端，用于调用和测试gRPC API。支持通过服务反射导入gRPC API
* [Lightly](https://www.lightly.ai/) — 通过正确数据提升机器学习模型性能。免费支持最多1,000个样本的数据集

  * [MailboxValidator](https://www.mailboxvalidator.com) — 通过真实邮件服务器连接验证邮箱有效性的服务。免费API计划每月提供300次验证。
* [Market Data API](https://www.marketdata.app) - 提供股票、期权、共同基金等金融产品的实时和历史数据。永久免费套餐允许每天100次免费API请求。
* [Tomorrow.io天气API](https://www.tomorrow.io/weather-api/) - 提供免费天气API方案。覆盖全球的精准天气预报服务，包含历史数据和天气监测解决方案。
* [Meteosource天气API](https://www.meteosource.com/) — 全球天气API，提供当前和预测天气数据。预报采用多气象模型机器学习组合以实现更高精度。免费计划每天400次调用。
* [microlink.io](https://microlink.io/) – 将任何网站转化为结构化数据，包括元标签标准化、精美链接预览、网页抓取或截图服务。每天100次免费请求。
* [Mindee](https://developers.mindee.com/docs) – 基于OCR技术和API优先平台的智能文档处理软件，通过计算机视觉和机器学习识别关键信息，自动化应用工作流。免费层每月250页额度。
* [Mintlify](https://mintlify.com) — 现代化API文档标准。提供美观易维护的UI组件、站内搜索和交互式演练场。免费支持1位编辑者。
* [monkeylearn.com](https://monkeylearn.com/) — 机器学习文本分析工具，每月300次免费查询。
* [MockAPI](https://www.mockapi.io/) — 快速创建模拟API、生成自定义数据并通过RESTful接口操作的简易工具。适用于原型设计/测试/学习场景。免费版支持1个项目（每个项目4个资源）。
* [Mockfly](https://www.mockfly.dev/) — 可信赖的API模拟和功能标志管理开发工具。通过直观界面快速生成控制模拟API。免费层每日500次请求。
* [Mocki](https://mocki.io) - 可创建与GitHub仓库同步的GraphQL和REST API模拟工具。简易REST API无需注册即可免费开发使用。
* [Mocko.dev](https://mocko.dev/) — 云端代理您的API，自由选择要模拟的端点并检查流量。加速开发和集成测试。
...
* [microenv.com](https://microenv.com) — 为开发者创建虚拟REST API，支持生成代码并打包为Docker容器应用。
* [Multi-Exit IP地址检查器](https://ip.alstra.ca/) — 免费简易工具，可跨多节点检查出口IP地址，了解IP在不同区域的呈现方式。适用于测试Control D等基于规则的DNS分流工具。
* [Namekit](https://namekit.app/) - AI驱动的域名发现工具——即时查找可用标准价域名。每日免费查询额度。
* [neptune.ai](https://neptune.ai/) - MLOps元数据记录、存储、展示、组织、对比和查询平台。个人免费版包含：1成员、100GB元数据存储、200小时/月监控。
* [News API](https://newsapi.org) — 通过代码搜索网络新闻并获取JSON结果。开发者每月3,000次免费查询。
* [GoCardless](https://gocardless.com/) — 免费开放银行数据API（PSD2合规）。连接欧盟和英国2300+银行与应用/软件。
* [Nyckel](https://www.nyckel.com) — 训练、部署和调用图像文本ML模型。免费支持最多5,000条训练数据。每月1,000次模型调用免费。
* [Observable](https://observablehq.com/) — 数据创作、协作学习平台。免费版：无限笔记本、无限发布、每个笔记本5位编辑者。
* [OCR.Space](https://ocr.space/) — 解析图像和PDF文件并返回JSON格式文本的OCR API。每月2.5万次免费请求。
* [Duply.co](https://duply.co) — 通过API和URL创建动态图像，设计模板可重复使用。免费层每月支持70次API/URL生成和100次表单生成。
...
* [Parseur](https://parseur.com) — 每月20免费额度：从PDF和邮件提取数据（AI驱动）。完整API访问权限。
...
* [PDF-API.io](https://pdf-api.io) - PDF自动化API，含可视化模板编辑器、HTML转PDF及动态数据集成。免费版包含1个模板，每月100次PDF生成。
* [Pixela](https://pixe.la/) - 免费日流数据库服务。所有操作通过API完成。支持热力图和折线图可视化。
* [Postbacks](https://postbacks.io/) - 延迟HTTP回调请求服务。注册即享8,000次免费请求。
* [Postman](https://postman.com) — API开发协作平台，简化工作流。Postman应用永久免费，云功能在限额内永久免费。
* [Insomnia](https://insomnia.rest) - 开源API客户端，支持REST和GraphQL设计与测试。
* [PrefectCloud](https://www.prefect.io/cloud/) — 数据流自动化全功能平台。所有方案每月包含2万次免费运行，满足多数小型企业ETL需求。
* [Preset Cloud](https://preset.io/) - Apache Superset托管服务。5人以下团队永久免费，包含无限仪表盘/图表、无代码图表构建器和协作SQL编辑器。
* [PromptLeo](https://promptleo.com/) - 面向创作者和开发者的提示工程平台，提供提示工程库、表单和API。免费版包含1个提示表单、1个API端点和每月30次生成。
* [PromptLoop](https://www.promptloop.com/) - AI网页抓取工具，节省85%以上工作时间，比人工研究快4倍，含REST API端点。每月首1,000积分免费。
* [Proxed AI](https://proxed.ai/) - iOS安全AI API解决方案（无需SDK，仅需修改API URL）。每月1,000次免费API调用。
* [Crawlbase](https://crawlbase.com/) — 无代理、无基础设施、无浏览器的网站抓取服务。自动处理验证码和防封禁。首1,000次调用免费。
* [Public-Apis Github仓库](https://github.com/public-apis/public-apis) — 免费公共API列表。

  * [QuickMocker](https://quickmocker.com/) — 在自有子域名下管理在线模拟API端点，将请求转发至本地URL用于webhook开发和测试，支持URL路径的正则表达式匹配和多种HTTP方法，可设置端点优先级，提供100多种短代码（动态或模拟响应值）用于响应模板，支持从JSON格式的OpenAPI（Swagger）规范导入，可代理请求并按IP地址和授权头限制端点访问。免费账户提供一个随机子域名、十个端点、5个正则表达式URL路径、每个端点50个短代码、每日100次请求以及请求日志中保留50条历史记录。
  
* [Rapidapi](https://rapidapi.com/) - 全球最大API中心，数百万开发者通过趣味挑战（含解决方案！）和交互示例连接数千个API进行开发。
  
* [RequestBin.com](https://requestbin.com) — 创建免费端点用于接收HTTP请求。发送至该端点的所有HTTP请求及其负载和头部信息将被记录，便于观察webhook等服务的调用情况。
  
* [Roboflow](https://roboflow.com) - 无需机器学习经验即可创建和部署定制计算机视觉模型。免费版包含最多1,000张源图像。
  
* [ROBOHASH](https://robohash.org/) - 通过任意文本生成独特炫酷图像的在线服务。
  
* [SaturnCloud](https://saturncloud.io/) - 数据科学云环境，支持运行Jupyter笔记本和Dask集群。每月免费提供30小时计算资源和3小时Dask集群。
  
* [Scraper's Proxy](https://scrapersproxy.com) — 简易HTTP代理API，实现匿名爬取数据，无需担心限制、封锁或验证码。每月前100次成功抓取免费（含JavaScript渲染，联系支持可获更多额度）。
  
* [ScrapingAnt](https://scrapingant.com/) — 无头Chrome爬取API及免费代理检测服务，支持JavaScript渲染、高级轮换代理和验证码规避，提供免费套餐。
  
* [ScraperBox](https://scraperbox.com/) — 使用真实Chrome浏览器和代理轮换的隐形网页抓取API，通过简单API调用抓取任意页面。免费版每月1,000次请求。
  
* [ScrapingDog](https://scrapingdog.com/) — 该服务通过数百万代理、浏览器和验证码处理，单次API调用即可获取任何网页HTML。包含Chrome & Firefox网页抓取工具及即时抓取软件，提供免费套餐。
  
* [scrapinghub.com](https://scrapinghub.com) — 带可视化界面和插件的数据抓取平台，免费版支持共享服务器上无限抓取。
  
* [Simplescraper](https://simplescraper.io) — 每次操作后触发webhook。免费版包含100次云端抓取额度。
  
* [Select Star](https://www.selectstar.com/) - 智能数据发现平台，自动分析与归档数据。免费轻量版支持1个数据源、最多100张表和10个用户。
  
* [Sheetson](https://sheetson.com) - 即时将Google表格转为RESTful API，提供免费方案。
  
* [Shipyard](https://www.shipyardapp.com) — 云端低代码数据编排平台，混合使用低代码模板和自定义代码（Python/Node.js/Bash/SQL）。开发者免费版每月提供10小时运行时，足够自动化多个工作流。
  
* [shrtcode API](https://shrtco.de/docs) - 无需认证且无请求限制的免费URL缩短API。
  
* [SerpApi](https://serpapi.com/) - 实时搜索引擎抓取API，返回Google/YouTube/Bing/百度/Walmart等平台的结构化JSON结果。免费版每月100次成功API调用。
  
* [SmartParse](https://smartparse.io) - 数据迁移和CSV转API平台，提供省时省力的开发工具。免费版含每月300处理单元、浏览器上传、数据隔离、熔断机制和任务提醒。
  
* [Sofodata](https://www.sofodata.com/) - 通过CSV文件创建安全RESTful API，快速接入数据加速应用开发。免费版含2个API和每月2,500次调用，无需信用卡。
  
* [Sqlable](https://sqlable.com/) - 免费在线SQL工具集，包含SQL格式化/验证器、SQL正则测试器、虚拟数据生成器和交互式数据库沙盒。
  
* [Stoplight](https://stoplight.io/) - 协作式API设计与文档SaaS平台，免费版提供设计、模拟和文档工具。
  
* [Svix](https://www.svix.com/) - Webhook即服务，免费发送50,000条消息/月。
  
* [TemplateTo](https://templateto.com) - 通过拖拽编辑器和简单API从可复用模板自动生成PDF/TXT文档。免费版每月450份PDF和3个模板。
  
* [TinyMCE](https://www.tiny.cloud) - 富文本编辑API，核心功能无限免费使用。
  
* [Treblle](https://www.treblle.com) - 帮助团队构建、发布和管理API，提供高级日志聚合、可观测性、文档和调试功能。免费版包含全部功能，每月限250,000次请求。
  
* [UniRateAPI](https://unirateapi.com) – 590+种法币与加密货币的实时汇率API，免费版无限调用，适合开发者和金融应用。
...
* [Weights & Biases](https://wandb.ai) — 开发者优先的MLOps平台，通过实验跟踪、数据集版本控制和模型管理更快构建模型。个人项目免费版含100GB存储。
  
* [wit.ai](https://wit.ai/) — 面向开发者的自然语言处理平台。
  
* [wolfram.com](https://wolfram.com/language/) — 云端内置基于知识的算法库。
  
* [wrapapi.com](https://wrapapi.com/) — 将任意网站转为参数化API，每月30,000次调用。
  
* [ZenRows](https://www.zenrows.com/) — 绕过任何反机器人方案的网页抓取API及代理服务器，支持JavaScript渲染、轮换代理和地理定位。免费版1,000次API调用。
  
* [Zenscrape](https://zenscrape.com/web-scraping-api) — 配备无头浏览器和住宅IP的网页抓取API，定价透明。每月1,000次免费调用，学生和非营利组织可获额外额度。
  
* [IPQuery](https://ipquery.io) — 开发者无限制IP查询API，无速率限制或费用。
  
* [ip-api](https://ip-api.com) — IP地理定位API，非商业用途免费，无需API密钥，免费版同一IP限45次请求/分钟。
  
* [WebScraping.AI](https://webscraping.ai) - 内置解析、Chrome渲染和代理的简易网页抓取API，每月2,000次免费调用。
  
* [Zipcodebase](https://zipcodebase.com) - 免费邮政编码API，提供全球邮政编码数据，每月10,000次免费请求。
...
* [numlookupapi.com](https://numlookupapi.com) - 免费电话号码验证API，每月10万次免费请求。
  ...
  * [Query.me](https://query.me) - 支持协作的数据笔记本工具，可执行类脚本操作，并通过SQL、API及多种自定义模块（如Slack和邮件）获取和发送数据。小团队免费使用。
* [ERD Lab](https://www.erdlab.io) — 专为开发者打造的免费云端实体关系图(ERD)工具。
...
* [OpenWeb Ninja](https://www.openwebninja.com/) - 功能全面的实时搜索引擎结果页(SERP)与公共数据API：涵盖Google搜索、购物、招聘、图片、Lens、新闻、Google地图商家/地点、评价、照片，以及网站邮箱和社交联系人爬虫，Amazon、Yelp等。所有API均提供含100至200次免费月请求的免费层级。
* [Tavily AI](https://tavily.com/) - 在线搜索与快速洞察API，支持研究成果结构化整理。免费版每月1000次请求，无需信用卡。
* [The IP API](https://theipapi.com/) - IP地理定位API，每日1000次免费请求。提供IP地址所属国家、城市、区域等信息。
* [Composio](https://composio.dev/) - AI代理与LLM集成平台，支持连接200+个智能互联网工具。
* [WeatherXu](https://weatherxu.com/) — 全球天气数据API，含实时天气、逐小时/每日预报及天气警报。通过AI模型与机器学习系统分析多源气象数据提升预报精度。免费版每月1万次API调用。
* [Zuplo](https://zuplo.com/) - 免费API管理平台，可快速设计、构建并部署边缘API。支持API密钥认证、速率限制、开发者文档和商业化功能。基于OpenAPI标准，完全支持Web标准API与Typescript编程。免费版提供10个项目、不限量生产环境、250个API密钥、10万次月请求和1GB出口流量。
* [Amazon SageMaker](https://studiolab.sagemaker.aws/) — 云端机器学习平台，提供JupyterLab和终端访问。免费层包含每日8小时CPU（单次最长4小时）或4小时GPU资源。
* [FormatJSONOnline.com](https://formatjsononline.com) - 基于浏览器的免费JSON数据格式化、验证、比较与压缩工具。

**[⬆️ 返回顶部](#目录)**

## 制品仓库

  * [Artifactory](https://jfrog.com/start-free/) - 支持多种包格式的制品仓库，包括Maven、Docker、Cargo、Helm、PyPI、CocoaPods和GitLFS。包含软件包扫描工具XRay和CI/CD工具Pipelines（原Shippable），免费版每月提供2,000分钟CI/CD时长。
  ...
  * [RepoForge](https://repoforge.io) - 托管Python、Debian、NPM包和Docker注册表的私有云仓库。开源/公共项目可免费使用。
...
## 团队协作工具
  ...
  * [3Cols](https://3cols.com/) - 免费的云端代码片段管理器，支持个人及协作编码
* [Bitwarden](https://bitwarden.com) — 为个人、团队和企业组织提供最简便安全的方式存储、共享和同步敏感数据
* [Braid](https://www.braidchat.com/) — 专为团队设计的聊天应用。公开群组免费使用，支持无限用户、历史记录和集成，并提供可自托管的开源版本
* [cally.com](https://cally.com/) — 快速找到会议最佳时间。操作简单，适用于小型和大型团队
* [Calendly](https://calendly.com) — 会议连接与日程安排工具。免费版提供每位用户1个日历连接和无限会话，支持桌面端和移动端应用
* [Discord](https://discord.com/) — 支持公开/私密频道的聊天平台。具备Markdown文本、语音、视频和屏幕共享功能，用户数量无限制
* [Telegram](https://telegram.org/) — 为需要快速可靠消息和通话的用户打造。大容量群组、用户名系统、桌面应用和强大文件共享功能适合商业用户和小型团队
* [DevToolLab](https://devtoollab.com) — 在线开发者工具集，免费使用所有基础工具，支持每个工具自动保存一条记录，提供标准处理速度和社区支持
* [Dubble](https://dubble.so/) — 免费的分步指南创建工具。支持截图、流程文档编写和团队协作，同时提供异步屏幕录制功能
...
* [flock.com](https://flock.com) — 高效的团队沟通工具。免费版支持无限消息、频道、用户及应用集成
* [Gather](https://www.gather.town/) - 创新的在线会议方式。通过可定制空间设计，让社群互动如同现实般自然。免费支持10人同时在线
* [gokanban.io](https://gokanban.io) - 基于语法的免注册看板工具，无限制免费使用
* [flat.social](https://flat.social) - 可定制的团队会议与社交互动空间。会议数量无限制，免费支持8人同时在线
* [GitDailies](https://gitdailies.com) - GitHub提交和Pull Request活动的每日报告工具。包含推送可视化、同伴认可系统和自定义提醒功能。免费版支持无限用户、3个仓库和3个提醒配置
* [gitter.im](https://gitter.im/) — GitHub专属聊天工具。公开和私密房间无限制，25人以下团队免费
* [Hackmd.io](https://hackmd.io/) - 实时协作的Markdown文档工具。类似Google Docs但专为Markdown设计，免费版笔记数量无限制，但私密笔记和模板的[协作者人数受限](https://hackmd.io/pricing)
...
* [Livecycle](https://www.livecycle.io/) — 跨职能产品团队和开源项目的无障碍协作平台
...
* [meet.jit.si](https://meet.jit.si/) — 一键发起视频通话和屏幕共享，完全免费
* [Microsoft Teams](https://products.office.com/microsoft-teams/free) — 整合对话、内容和应用的数字化协作中心，免费支持50万用户
* [Miro](https://miro.com/) - 可扩展、安全、跨设备的企业级协作白板，提供免费增值方案
* [nootiz](https://www.nootiz.com/) - 网站视觉反馈收集与管理工具
* [Notion](https://www.notion.so/) - 集成任务、维基和数据库的Markdown笔记协作应用，被描述为集笔记记录、项目管理和任务管理于一体的全能工作区
* [Nuclino](https://www.nuclino.com) - 轻量级团队知识协作维基，免费版包含核心功能、50个条目和5GB存储
* [OnlineInterview.io](https://onlineinterview.io/) - 嵌入式视频聊天、画板和在线代码编辑器的免费编程面试平台，支持浏览器端编译运行代码，一键创建远程面试房间
* [Quidlo Timesheets](https://www.quidlo.com/timesheets) - 简易团队时间追踪应用，免费版支持10人以下的时间记录和报告生成
* [PageShare.dev](https://www.pageshare.dev) - 无需部署即可为GitHub Pull Requests添加可视化评审功能，免费版每月10个页面和100MB总存储
* [Pendulums](https://pendulums.io/) - 通过直观界面和实用统计数据提升时间管理效率的免费时间追踪工具
* [Pumble](https://pumble.com) - 永久免费的团队聊天应用，支持无限用户和完整消息历史
* [Raindrop.io](https://raindrop.io) - 支持macOS、Windows、Android、iOS和网页端的私密书签工具，免费提供无限书签和协作功能
...
* [Slab](https://slab.com/) — 面向团队的现代知识管理服务，最多支持10名免费用户
...
* [Stickies](https://stickies.app/) - 可视化协作应用，用于头脑风暴、内容管理和笔记。免费版支持3个看板墙、不限用户和1GB存储
...
* [tldraw.com](https://tldraw.com) — 免费开源白板和绘图工具，支持智能箭头、吸附对齐、便利贴和SVG导出。多人协作模式，官方VS Code扩展免费
* [BookmarkOS.com](https://bookmarkos.com) - 多功能书签/标签/任务管理器，支持文件夹协作的自定义在线桌面
* [typetalk.com](https://www.typetalk.com/) — 通过网页或移动端即时消息与团队分享创意
* [Tugboat](https://tugboat.qa) - 自动按需预览每个pull request。非营利组织可享免费Nano套餐
* [whereby.com](https://whereby.com/) — 一键视频会议（原appear.in）
* [windmill.dev](https://windmill.dev/) - 开源开发者平台，用Python/Typescript快速构建多步骤自动化和内部应用。免费用户最多创建/加入3个非高级工作区
* [vadoo.tv](https://vadoo.tv/) — 极简视频托管营销平台。免费版支持10个视频/1GB存储/10GB月流量
* [userforge.com](https://userforge.com/) - 互联在线角色/用户故事/情境映射工具。免费支持3个角色和2名协作者
* [wistia.com](https://wistia.com/) — 带观看分析的高清视频托管，25个视频容量（含品牌水印播放器）
...
* [zoom.us](https://zoom.us/) — 安全视频/网络会议，免费版限时40分钟
* [Zulip](https://zulip.com/) — 独特邮件式线程模型的实时聊天。免费版含1万条历史消息和5GB存储，提供开源自托管版本
* [robocorp.com](https://robocorp.com) - 自动化运维开源套件。免费版含240分钟/月机器人时长、10次助手运行和100MB存储
* [Fleep.io](https://fleep.io/) — Slack替代品。小团队免费版含完整消息历史、无限私聊、1个群组和1GB存储
* [Chanty.com](https://chanty.com/) — 另一款Slack替代品。10人以下团队永久免费，含无限公私对话、可搜索历史、无限语音通话/留言、10个集成和20GB团队存储
* [ruttl.com](https://ruttl.com/) — 全能数字反馈工具，支持网页/PDF/图片审阅
* [Mattermost](https://mattermost.com/) — 技术团队安全协作平台。免费版含无限频道/剧本/看板/用户和10GB存储
* [Webvizio](https://webvizio.com) — 网站反馈/审阅/缺陷报告工具，支持直接在线上网站/应用/图片/PDF/设计文件上标注任务
* [Pullflow](https://pullflow.com) — 集成GitHub/Slack/VS Code的AI增强代码评审平台
* [Webex](https://www.webex.com/) — 视频会议，免费版支持100人40分钟会议
* [RingCentral](https://www.ringcentral.com/) — 视频会议，免费版支持100人50分钟会议
* [GitBook](https://www.gitbook.com/) — 技术知识管理平台（产品文档/内部知识库/API文档），开发者个人免费
...
* [SiteDots](https://sitedots.com/) - 直接在您的网站上收集用户反馈，无需模拟界面、画布或变通方案。免费版功能完整可用。
...
* [kontent.ai](https://www.kontent.ai) - 内容即服务平台，兼具无头 CMS 优势与营销人员友好特性。开发者计划提供 2 个用户、无限项目（每个项目含 2 个环境）、500 个内容项、2 种语言的交付与管理 API，以及自定义元素支持。可按需选择更高级套餐。
* [Prismic](https://www.prismic.io/) — 无头 CMS。提供全托管可扩展 API 的内容管理界面。社区计划为单个用户提供无限 API 调用、文档、自定义类型、资源和多语言支持。开源/开放内容项目可申请更大免费套餐。
* [Sanity.io](https://www.sanity.io/) - 结构化内容平台，含开源编辑环境和实时托管数据库。每个项目免费包含无限项目、无限管理员用户、3 个普通用户、2 个数据集、50 万次 API CDN 请求、10GB 带宽和 5GB 资源。
* [sensenet](https://sensenet.com) - API 优先的无头 CMS，为企业级需求提供解决方案。开发者计划包含 3 个用户、500 个内容项、3 种内置角色、25+5 种内容类型、完整 REST API 访问、文档预览生成和 Office Online 编辑功能。
* [TinaCMS](https://tina.io/) — Forestry.io 替代方案。开源 Git 托管的无头 CMS，支持 Markdown、MDX 和 JSON。基础套餐免费支持 2 个用户。
* [GatsbyjsCMS](https://www.gatsbyjs.com/) - 快速灵活的框架，让使用任意 CMS/API/数据库构建网站重获乐趣。构建并部署能提升流量、转化率和收益的无头网站！
* [Hygraph](https://hygraph.com/) - 为小型项目提供免费套餐。GraphQL 优先 API。从传统方案迁移至原生的 GraphQL 无头 CMS，实现全渠道内容 API 优先交付。
* [Squidex](https://squidex.io/) - 为小型项目提供免费套餐。API/GraphQL 优先。基于事件溯源（自动记录每次变更）的开源方案。
* [InstaWP](https://instawp.com/) - 数秒内启动 WordPress 站点。免费套餐含 5 个活跃站点、500MB 空间、48 小时站点有效期。
* [Storyblok](https://www.storyblok.com) - 面向开发者和营销人员的无头 CMS，兼容所有现代框架。社区（免费）套餐包含管理 API、可视化编辑器、10 个数据源、自定义字段类型、国际化支持（无限语言/地区）、资源管理器（最多 2500 个资源）、图片优化服务、搜索查询、Webhook 及 250GB/月流量。
* [WPJack](https://wpjack.com) - 5 分钟内在任意云端部署 WordPress！免费套餐含 1 台服务器、2 个站点、免费 SSL 证书和无限定时任务。无时间限制或强制过期——完全自主掌控网站。

**[⬆️ 返回顶部](#目录)**

## 代码生成

  * [Appinvento](https://appinvento.io/) — AppInvento 是一款免费的无代码应用构建工具。其自动生成的后端代码完全开放源代码，提供无限API和路由支持，可实现深度集成。免费方案包含3个项目、5个数据表和Google插件支持。
* [Cody AI](https://sourcegraph.com/cody) - Cody 是一款智能编程助手，通过AI技术深度理解代码库，帮助开发者更快编写和理解代码。支持多款LLM选择（含本地推理）、兼容主流IDE、覆盖所有热门编程语言，并提供免费方案。免费用户每月可获得20条聊天消息（使用Claude 3 Sonnet模型）和500次自动补全（基于Starcoder 16b模型）。
* [DhiWise](https://www.dhiwise.com/) — 通过DhiWise创新的代码生成技术，将Figma设计稿无缝转换为动态Flutter和React应用，优化工作流程，助您以前所未有的速度打造卓越的移动端和网页体验。
* [Codeium](https://www.codeium.com/) — Codeium 是免费的AI代码补全工具。支持20+编程语言（Python、JavaScript、Java、TypeScript、PHP、C/C++、Go等），兼容所有主流独立IDE和网页版IDE。
* [Fern](https://buildwithfern.com) - 编写API定义并自动生成多种语言的SDK/客户端库（TypeScript、Python、Java、Go等）。完整支持OpenAPI。免费版最多可为20个端点生成代码。
* [Metalama](https://www.postsharp.net/metalama) - 仅支持C#。Metalama在编译时动态生成样板代码，保持源代码整洁。开源项目免费使用，商业友好型免费方案包含3个切面功能。
* [Supermaven](https://www.supermaven.com/) — Supermaven是面向VSCode、JetBrains和Neovim的快速AI代码补全插件。免费版提供无限行内补全功能。
...
* [browserling.com](https://www.browserling.com/) — 实时跨浏览器测试服务，免费版仅支持Vista系统下1024x768分辨率的MS IE 9浏览器进行3分钟会话
* [codacy.com](https://www.codacy.com/) — 支持PHP/Python/Ruby/Java/JavaScript/Scala/CSS/CoffeeScript的自动化代码审查，无限公有与私有仓库免费
* [Codeac.io](https://www.codeac.io/infrastructure-as-code.html?ref=free-for-dev) — 面向DevOps的基础设施即代码自动化审查工具，集成GitHub/Bitbucket/GitLab（含自托管版本），除常规语言外还支持分析Ansible/Terraform/CloudFormation/Kubernetes等（开源免费）
* [CodeBeat](https://codebeat.co) — 多语言支持的自动化代码审查平台，公有仓库永久免费（含Slack和邮件集成）
...
* [CodeFactor](https://www.codefactor.io) — Git自动化代码审查，免费版支持无限用户、公有仓库及1个私有仓库
...
* [deepcode.ai](https://www.deepcode.ai) — 基于AI检测代码缺陷、安全漏洞和API问题的工具，支持Java/C/C++/JavaScript/Python/TypeScript，集成GitHub/BitBucket/GitLab，开源项目及30人以下团队私有仓库免费
...
* [eversql.com](https://www.eversql.com/) — 数据库优化平台，自动提供数据库与SQL查询的关键洞察
* [gerrithub.io](https://review.gerrithub.io/) — GitHub仓库的Gerrit代码审查服务
* [gocover.io](https://gocover.io/) — Go语言包的代码覆盖率分析
...
* [Moderne.io](https://app.moderne.io) — 自动化源代码重构工具，支持框架迁移/带修复的代码分析/大规模代码转换，开源免费
...
* [semanticdiff.com](https://app.semanticdiff.com/) — GitHub拉取请求与提交的编程语言感知差异分析，公开仓库免费
* [shields.io](https://shields.io) — 开源项目的元数据徽章生成服务
...
* [CodeKeep](https://codekeep.io) - 代码片段的Google Keep。提供代码片段管理、发现与分享功能，内置强大的代码截图工具（含预设模板和链接功能）  
...
* [Argonaut](https://argonaut.dev/) - 数分钟内完成应用和基础设施的云端部署。支持Kubernetes和Lambda环境下的自定义及第三方应用部署。免费版允许5个域名和2个用户无限制部署应用。
...
* [Buildkite](https://buildkite.com) CI流水线免费支持3个用户和每月5000分钟任务时长。测试分析免费开发者层级包含每月10万次测试执行，开源项目可获得更多免费额度。
* [bytebase.com](https://www.bytebase.com/) — 数据库CI/CD和DevOps工具。20个用户和10个数据库实例内免费
* [CircleCI](https://circleci.com/) — 功能全面的免费计划，包含GitHub、GitLab和BitBucket仓库托管CI/CD服务。多资源类型、Docker、Windows、Mac OS、ARM执行器、本地运行器、测试分割、Docker层缓存等高级功能。每月6000分钟免费执行时长，不限协作者，私有项目30个并行任务，开源项目最高8万构建分钟。
* [cirrus-ci.org](https://cirrus-ci.org) - 公开GitHub仓库免费
* [cirun.io](https://cirun.io) - 公开GitHub仓库免费
* [codefresh.io](https://codefresh.io) — 终身免费计划：1个构建、1个环境、共享服务器、不限量公开仓库
...
* [LayerCI](https://layerci.com) — 全栈项目CI平台。提供5GB内存和3核CPU的全栈预览环境。
* [semaphoreci.com](https://semaphoreci.com/) — 开源项目免费，每月100次私有构建
* [Squash Labs](https://www.squash.io/) — 为每个分支创建VM并通过唯一URL访问应用，不限公开/私有仓库，最高2GB VM规格。
* [styleci.io](https://styleci.io/) — 仅限公开GitHub仓库
* [Mergify](https://mergify.io) — GitHub工作流自动化和合并队列 — 公开GitHub仓库免费
* [Make](https://www.make.com/en) — 通过可视化界面连接应用并自动化工作流的工具。支持众多应用和主流API。公开GitHub仓库免费，免费版含100MB空间、1000次操作和15分钟最小间隔。
...
* [microtica.com](https://microtica.com/) - 提供预置基础设施组件的初创环境，免费在AWS部署应用并支持生产负载。免费版包含1个环境（在您的AWS账户）、2个Kubernetes服务、每月100构建分钟和20次部署。
...
* [Terramate](https://terramate.io/) - Terraform、OpenTofu和Terragrunt等基础设施即代码工具的编排管理平台。2个用户内免费包含全部功能。
* [Terrateam](https://terrateam.io) - 采用GitOps优先的Terraform自动化方案，支持拉取请求驱动的工作流、通过自托管运行器实现项目隔离，以及有序操作的分层运行。3个用户内免费。
...
* [Appetize](https://appetize.io) — 在基于云的Android手机/平板模拟器和iPhone/iPad模拟器上直接测试您的Android和iOS应用。免费套餐每月包含两个并发会话，每次使用30分钟。应用大小无限制。
...
* [BugBug](https://bugbug.io/) - 轻量级Web应用测试自动化工具。易于学习且无需编码。可在自有计算机上免费无限次运行测试。额外付费可获得云监控和CI/CD集成。
* [Checkly](https://checklyhq.com) - 面向现代DevOps的代码优先合成监控。以传统提供商的一小部分价格监控API和应用。基于Monitoring as Code工作流和Playwright。开发者可享慷慨的免费套餐。
* [checkbot.io](https://www.checkbot.io/) — 浏览器扩展，测试网站是否符合50+ SEO、速度和安全性最佳实践。小型网站可享免费套餐。
* [CORS-Tester](https://cors-error.dev/cors-tester/) - 开发者和API测试人员的免费工具，用于检查API是否针对特定域名启用CORS并识别差距。获取可操作的见解。
* [cypress.io](https://www.cypress.io/) - 快速、简单且可靠的浏览器测试工具。Cypress Test Runner始终免费开源，无任何限制。Cypress Dashboard对开源项目免费，最多支持5名用户。
* [Cypress Recorder by Preflight](https://cypress.preflight.com/) - 在浏览器中创建AI驱动的Cypress测试/POM模型。除AI部分外开源。每月免费创建5次测试，包含自愈脚本、邮件和视觉测试。
* [everystep-automation.com](https://www.everystep-automation.com/) — 记录并回放Web浏览器中的所有操作并生成脚本，免费版功能较少
* [Gremlin](https://www.gremlin.com/gremlin-free-software) — Gremlin的混沌工程工具可安全地向系统注入故障，在问题影响用户前发现弱点。Gremlin Free支持在最多5台主机或容器上执行关机和CPU攻击。
...
* [TestSpace.com](https://testspace.com/) - 发布自动化测试结果的仪表板和通过GitHub将手动测试实现为代码的框架。服务对[开源项目免费](https://github.com/marketplace/testspace-com)，每月450次结果。
  ...
  * [webhook-test.com](https://webhook-test.com) - 在集成过程中通过专属URL调试和检查webhook及HTTP请求。完全免费，可创建无限数量的URL并获取建议。
...
## 安全与公钥基础设施
  ...
  * [aikido.dev](https://www.aikido.dev) — 一体化应用安全平台，涵盖SCA、SAST、CSPM、DAST、密钥管理、IaC、恶意软件、容器扫描、生命周期终止(EOL)等。免费计划包含2个用户、10个代码库扫描、1个云账户、2个容器和1个域名扫描
...
* [DJ Checkup](https://djcheckup.com) — 免费自动化检测工具，扫描Django网站安全漏洞。基于Pony Checkup项目分支开发
* [Doppler](https://doppler.com/) — 通用密钥管理器，支持同步至各类云服务商。免费版支持5个用户及基础访问控制
* [Dotenv](https://dotenv.org/) — 安全快捷地同步.env文件。不再通过Slack或邮件等不安全渠道共享.env文件，永久避免重要.env文件丢失。最多支持3名团队成员免费使用
* [GitGuardian](https://www.gitguardian.com) — 通过自动化密钥检测与修复，防止密钥泄露至源代码。扫描Git仓库中350+种密钥和敏感文件——个人及25人以下团队免费
...
* [Infisical](https://infisical.com/) — 开源平台，集中管理开发团队及基础设施的密钥：从本地开发到生产环境第三方服务全覆盖。最多5名开发者免费
...
* [pixee.ai](https://pixee.ai) - 免费GitHub机器人形式的自动化产品安全工程师，通过提交PR自动修复Java代码库漏洞。即将支持更多语言！
...
* [SOOS](https://soos.io) - 开源项目免费无限次SCA扫描。在发布前检测并修复安全威胁，通过简洁高效的方案保护项目
...
* [Protectumus](https://protectumus.com) - 免费网站安全检查、站点杀毒及PHP服务器防火墙(WAF)。免费版用户可获取邮件通知
...
## 认证授权与用户管理
  ...
  * [Aserto](https://www.aserto.com) - 为应用和API提供的精细化授权服务。免费额度包含1000名月活跃用户(MAU)和100个授权实例。
...
* [Cerbos Hub](https://www.cerbos.dev/product-cerbos-hub) - 完整的授权管理系统，支持编写、测试和部署访问策略。精细化权限控制，免费额度100名月活跃主体。
...
* [Corbado](https://www.corbado.com/) - 为新老应用添加优先使用通行密钥的认证方案。支持无限MAU免费使用。
...
* [Kinde](https://kinde.com/) - 简单强大的认证系统，数分钟即可集成。免费提供7,500名MAU的起步套餐。
...
* [Permit.io](https://permit.io) - 授权即服务平台，支持RBAC、ABAC和ReBAC，适用于需要实时更新的微服务架构，提供无代码策略UI。免费额度1000名MAU。
* [Phase Two](https://phasetwo.io) - Keycloak开源身份与访问管理方案。免费额度1000名用户和10个SSO连接，基于包含[组织扩展](https://phasetwo.io/product/organizations/)的增强版Keycloak容器。
* [SSOJet](https://ssojet.com/) - 无需重构现有认证系统即可添加企业SSO。免费计划包含无限MAU、无限组织和2个SSO/SCIM连接。
...
* [SuperTokens](https://supertokens.com/) - 原生集成到应用中的开源用户认证方案，在保持用户体验的同时快速实现认证功能。免费额度5000名MAU。
* [Warrant](https://warrant.dev/) - 为企业级应用提供的托管式授权与访问控制服务。免费套餐包含100万次月API请求和1000条授权规则。
* [ZITADEL Cloud](https://zitadel.com) - 开箱即用的用户与访问管理系统，支持多租户(B2B)场景。免费额度25,000次认证请求，包含OTP/无密码认证/策略等全部安全功能。
* [PropelAuth](https://propelauth.com) - 通过几行代码即可服务任意规模企业客户。免费额度200名用户和1万封事务邮件（含"Powered by PropelAuth"水印标识）。
* [Logto](https://logto.io/) - 开发、保护和管理产品用户身份，涵盖认证与授权功能。免费额度5,000名MAU，提供开源自托管选项。
...
## 移动应用分发与反馈
  ...
  * [TestApp.io](https://testapp.io) - 移动应用质量保障首选平台。免费版支持：1个应用、分析功能、无限版本与安装、用户反馈收集。
* [Loadly](https://loadly.io) - iOS和Android测试版应用分发服务，提供完全免费的无限下载、高速下载和无限上传。
* [Diawi](https://www.diawi.com) - 直接向设备部署iOS和Android应用。免费版包含：应用上传、密码保护链接、1天有效期、10次安装。
* [InstallOnAir](https://www.installonair.com) - 无线分发iOS和Android应用。免费版支持：无限上传、私有链接（访客2天有效期/注册用户60天）。
...
* [Esper](https://esper.io) — 面向Android设备的MDM和MAM解决方案，含DevOps功能。免费提供100台设备配额、1个用户许可证及25MB应用存储空间  
...
* [xcloud.host](https://xcloud.host) — 提供友好界面的服务器管理与部署平台，含免费层级（1台服务器）  
...
* [cloudamqp.com](https://www.cloudamqp.com/) — RabbitMQ托管服务。Little Lemur套餐：每月最多100万条消息、20个并发连接、100个队列、1万条排队消息，支持跨可用区的多节点部署
* [courier.com](https://www.courier.com/) — 统一推送、应用内消息、邮件、聊天、短信等多渠道消息API，含模板管理等功能。免费套餐每月包含1万条消息
* [engagespot.co](https://engagespot.co/) — 开发者多渠道通知基础设施，提供预构建应用内收件箱和无代码模板编辑器。免费套餐每月包含1万条消息
* [HiveMQ](https://www.hivemq.com/mqtt-cloud-broker/) - 云端原生物联网消息代理服务，支持MQTT设备连接。永久免费连接100台设备（无需信用卡）
* [knock.app](https://knock.app) – 开发者通知基础设施。通过单次API调用实现应用内消息、邮件、短信、Slack和推送等多渠道发送。免费套餐每月包含1万条消息
* [NotificationAPI.com](https://www.notificationapi.com/) — 5分钟为任何软件添加用户通知功能。免费套餐包含每月1万条通知+100条短信和自动语音呼叫
* [Novu.co](https://novu.co) — 开源开发者通知基础设施。统一管理邮件、短信、站内信、应用内消息和推送的组件与API。免费套餐含每月3万条通知（保留90天）
* [pusher.com](https://pusher.com/) — 实时消息服务。免费支持最多100个同时连接和每日20万条消息
* [scaledrone.com](https://www.scaledrone.com/) — 实时消息服务。免费支持最多20个同时连接和每日10万次事件
* [synadia.com](https://synadia.com/ngs) — [NATS.io](https://nats.io)托管服务。覆盖全球及AWS/GCP/Azure平台。永久免费套餐支持4KB消息大小、50个活跃连接和每月5GB数据
...
* [webpushr](https://www.webpushr.com/) - 网页推送通知服务。免费支持1万订阅用户，推送次数不限，支持浏览器内消息
* [httpSMS](https://httpsms.com) - 将安卓手机作为短信网关收发短信。免费套餐每月可收发200条消息
* [EMQX Serverless](https://www.emqx.com/en/cloud/serverless-mqtt) - 秒级部署的可扩展安全MQTT代理服务。永久免费套餐含每月100万会话分钟数（无需信用卡）
* [Pocket Alert](https://pocketalert.app) - 向iOS/Android设备发送推送通知。支持API/Webhook无缝集成，完全掌控提醒设置。免费套餐：每日50条消息（限1设备1应用）
* [SuprSend](https://www.suprsend.com/) - 采用API优先设计的通知基础设施，通过单一API实现交易通知、定时任务和互动消息的多渠道发送。免费套餐每月1万条通知，支持摘要、批量、多渠道、偏好设置、租户隔离、广播等工作流节点
...
* [Lecto](https://lecto.ai/) - 含免费额度的机器翻译API（30次免费请求/次限1000字符），已集成至Loco Translate WordPress插件  
* [lingohub.com](https://lingohub.com/) — 最多3名用户免费，开源项目永久免费  
* [localazy.com](https://localazy.com) - 免费支持1000条源语言字符串，无限语言与贡献者，提供初创企业和开源优惠  
* [Localeum](https://localeum.com) - 免费额度：1000条字符串、1名用户，支持无限语言与项目  
* [localizely.com](https://localizely.com/) — 开源项目免费  
* [Loco](https://localise.biz/) — 免费2000条翻译，无限译者，支持10种语言/项目、1000个可翻译资源/项目  
* [oneskyapp.com](https://www.oneskyapp.com/) — 限5人免费版，开源项目免费  
* [POEditor](https://poeditor.com/) — 免费支持1000条字符串  
* [SimpleLocalize](https://simplelocalize.io/) - 免费100个翻译键值，无限字符串与语言，提供初创优惠  
* [Texterify](https://texterify.com/) - 单人用户免费  
...
* [Translized](https://translized.com) - 免费1000条字符串，1名用户，无限语言与项目  
* [webtranslateit.com](https://webtranslateit.com/) — 免费500条字符串  
* [weblate.org](https://weblate.org/) — 免费支持自由项目（限1万条字符串），自托管版本无限制  
* [Free PO editor](https://pofile.net/free-po-editor) — 全民免费  
* [Lingo.dev](https://lingo.dev) – 开源AI驱动的CLI工具，支持网页与移动端本地化。可自备LLM，或通过Lingo托管引擎每月免费使用1万词  

**[⬆️ 返回顶部](#目录)**

## 监控服务

  * [UptimeObserver.com](https://uptimeobserver.com) - 提供20个5分钟间隔的可用性监控器和可自定义状态页面（支持商业用途）。通过邮件和Telegram享受无限实时通知。无需信用卡即可开始使用。
...
* [bleemeo.com](https://bleemeo.com) - 免费支持3台服务器、5个可用性监控器，无限制用户、仪表盘和告警规则。
* [Core Web Vitals History](https://punits.dev/core-web-vitals-historical/) - 查询URL或网站的核心Web指标历史数据。
* [checklyhq.com](https://checklyhq.com) - 面向开发者的开源端到端/合成监控与深度API监控。免费计划包含1个用户、1万次API/网络检查和1.5千次浏览器检查运行。
...
* [economize.cloud](https://economize.cloud) — Economize通过组织云资源来优化和报告成本，帮助厘清云基础设施支出。每月Google Cloud Platform消费不超过5,000美元免费。
...
* [Grafana Cloud](https://grafana.com/products/cloud/) - Grafana Cloud是可组合的观测平台，集成指标与日志。免费版：3用户、10个仪表盘、100条告警，Prometheus和Graphite指标存储（1万序列，14天保留），Loki日志存储（50GB日志，14天保留）。
* [healthchecks.io](https://healthchecks.io) — 监控定时任务与后台任务。免费支持最多20个检查。
* [Hydrozen.io](https://hydrozen.io) — 可用性监控与状态页面，免费计划：10个可用性监控器、5个心跳监控器、1个域名监控器和1个状态页面。
* [incidenthub.cloud](https://incidenthub.cloud/) — 云与SaaS状态页面聚合器——永久免费提供20个监控器和2个通知渠道（Slack和Discord）。
...
* [netdata.cloud](https://www.netdata.cloud/) — Netdata是实时指标收集的开源工具。该产品持续发展，也可在GitHub找到！
* [newrelic.com](https://www.newrelic.com) — New Relic观测平台帮助工程师打造更完美的软件。从单体到无服务器架构，可检测所有组件并分析、排查和优化整个软件栈。免费层每月提供100GB数据摄入、1个全权限用户和无限基础用户。
* [Middleware.io](https://middleware.io/) - Middleware观测平台全面展示应用与技术栈，助您大规模监控与诊断问题。开发者社区永久免费计划支持100万日志事件监控、2台主机的基础设施监控与应用性能监控（APM）。
* [nixstats.com](https://nixstats.com) - 单台服务器免费。支持邮件通知、公开状态页面、60秒间隔等。
* [OnlineOrNot.com](https://onlineornot.com/) - OnlineOrNot提供网站与API的可用性监控、定时任务监控及状态页面。免费层包含5个3分钟间隔的检查，通过Slack、Discord和邮件发送告警。
...
* [paessler.com](https://www.paessler.com/) — 强大的基础设施与网络监控方案，含告警、可视化功能和基础报告。免费支持最多100个传感器。
...
* [phare.io](https://phare.io/) - 可用性监控免费支持10万次事件，项目与状态页面数量不限。
* [pingbreak.com](https://pingbreak.com/) — 现代可用性监控服务。无限量URL检查，通过Discord、Slack或邮件接收宕机通知。
* [pingpong.one](https://pingpong.one/) — 带监控的高级状态页面平台。免费层包含1个可自定义的公开状态页面（SSL子域名）。开源项目与非营利组织免费提供专业版。
...
* [sematext.com](https://sematext.com/) — 免费提供24小时指标监控，服务器数量不限，10个自定义指标，50万自定义指标数据点，无限仪表盘和用户等。
* [sitemonki.com](https://sitemonki.com/) — 网站、域名、定时任务与SSL监控，每类5个免费监控器。
...
* [SweetUptime](https://dicloud.net/sweetuptime-server-uptime-monitoring/) — 服务器监控、在线时长监控、DNS及域名监控。免费监控10台服务器、10个在线状态和10个域名
...
* [Squadcast.com](https://squadcast.com) - 端到端事件管理软件，帮助实践SRE最佳方案。永久免费版支持最多10名用户
* [RoboMiri.com](https://robomiri.com/) - 稳定运行时长监控，支持多种监测类型：定时任务、关键词、网站、端口、Ping。免费提供25个监测点，3分钟检测间隔。支持电话、短信、邮件和Webhook告警
* [Better Stack](https://betterstack.com/better-uptime) - 集运行时长监控、事件管理、值班排班/告警和状态页于一体。免费版包含10个监测点（3分钟检测频率）和状态页
* [Pulsetic](https://pulsetic.com) - 永久免费提供10个监测器、6个月历史运行日志、无限状态页和自定义域名！无限次邮件告警，无需信用卡
* [Wachete](https://www.wachete.com) - 监控5个页面，每24小时检测一次
* [Xitoring.com](https://xitoring.com/) — 运行时长监控：20个免费；Linux/Windows服务器监控：5个免费；状态页：1个免费 - 含移动应用、多通知渠道等功能
* [Servervana](https://servervana.com) - 支持大型项目和团队的高级运行时长监控。提供HTTP监控、浏览器监控、DNS监控、域名监控、状态页等。免费版包含10个HTTP监控器、1个DNS监控器和1个状态页
...
* [Embrace](https://embrace.io/) — 移动应用监控服务。小团队每年100万用户会话内免费。
* [exceptionless](https://exceptionless.com) — 实时错误、功能、日志报告等。每月免费3,000次事件/1用户。开源且支持无限量自托管。
* [GlitchTip](https://glitchtip.com/) — 简洁开源错误追踪系统。兼容开源Sentry SDK。每月免费1,000次事件，或无限量自托管。
...
* [Jam](https://jam.dev) - 开发者友好型一键错误报告工具。免费方案不限报告数量。
* [Whitespace](https://whitespace.dev) – 浏览器内一键提交错误报告。个人用户免费方案不限录制次数。
...
* [可汗学院](https://www.khanacademy.org/computing/computer-programming) - 包含HTML/CSS基础与进阶、JavaScript及SQL的免费在线教程
* [全栈公开课](https://fullstackopen.com/en/) - 大学级现代Web开发课程，涵盖React、Node.js、GraphQL和TypeScript，支持在线自主学习
* [Django教程站](https://django-tutorial.dev) - 面向初学者的免费Django框架教程，并为用户文章提供dofollow外链
...
* [EmailJS](https://www.emailjs.com/) – 非完整邮件服务器，客户端邮件发送工具（无需暴露凭证），免费层包含200次月请求、2个邮件模板、50KB请求限制、有限联系人历史记录
...
* [Inboxes App](https://inboxesapp.com) — 每天最多创建3个临时邮箱（可通过Chrome扩展管理），适合测试注册流程
...
* [Mailjet](https://www.mailjet.com/) — 每月6000封免费邮件（每日发送限制200封）
...
* [Resend](https://resend.com) - 开发者事务性邮件API，每月3000封（每天100封）免费，支持1个自定义域名
* [Sender](https://www.sender.net) 每月15000封邮件，最多2500名订阅者
* [Sendpulse](https://sendpulse.com) — 每月500名订阅者，15000封免费邮件
* [SimpleLogin](https://simplelogin.io/) – 开源可自托管邮箱别名/转发方案，免费5个别名，无限流量和收发，教育工作者（学生/研究人员等）免费
* [Substack](https://substack.com) — 无限免费邮件订阅服务，收费时开始支付
* [Sweego](https://www.sweego.io/) - 欧洲事务性邮件API，每天500封免费邮件

  * [temp-mail.io](https://temp-mail.io) — 免费一次性临时邮箱服务，支持同时使用多个邮箱地址及邮件转发功能  
* [tinyletter.com](https://tinyletter.com/) — 每月免费支持5,000名订阅者  
* [Touchlead](https://touchlead.app) - 多功能营销自动化工具，包含潜在客户管理、表单构建器和自动化流程。免费版支持有限数量的潜在客户和自动化任务  
* [trashmail.com](https://www.trashmail.com) - 提供带转发功能和自动过期机制的免费一次性邮箱地址  
* [Tuta](https://tuta.com/) - 免费安全邮箱服务商，内置端到端加密，无广告无追踪。提供1GB免费存储空间和一个日历功能（Tuta也提供[付费方案](https://tuta.com/pricing)）。该平台部分[开源](https://github.com/tutao/tutanota)，支持自托管  
...
* [Zoho](https://www.zoho.com) — 最初作为邮箱服务商，现发展为提供多款含免费方案的服务套件：  
   ...
   - [会议](https://zoho.com/meeting) — 免费支持最多3名参会者及10名网络研讨会观众  
   ...
   - [Subscriptions](https://zoho.com/subscriptions) — 周期性账单管理免费支持20个客户/订阅和1用户，支付托管由Zoho完成，保留最近40条订阅记录  
   - [Checkout](https://zoho.com/checkout) — 产品计费管理免费支持3个页面和50笔交易  
   - [Desk](https://zoho.com/desk) — 客户支持管理免费支持3个客服，含私有知识库和邮件工单，可集成[Assist](https://zoho.com/assist)实现1名远程技术支持+5台无人值守计算机  
   - [Cliq](https://zoho.com/cliq) — 团队聊天软件免费提供100GB存储，无用户数限制，单频道支持100人，含SSO功能  
...
## 功能开关管理平台
  ...
  * [ConfigCat](https://configcat.com) - ConfigCat 是一款以开发者为核心的功能开关服务，支持无限团队规模，提供卓越的技术支持且价格合理。免费版支持10个功能开关、2个环境、1个产品和每月500万次请求。
  * [Flagsmith](https://flagsmith.com) - 自信地发布功能，跨网页、移动端和服务器端应用管理功能开关。可使用托管API、部署到私有云或本地运行。
  * [GrowthBook](https://growthbook.io) - 开源功能开关与A/B测试平台，内置贝叶斯统计分析引擎。免费版支持3名用户，功能开关和实验数量不限。
  * [Hypertune](https://www.hypertune.com) - 类型安全的功能开关、A/B测试、分析与应用配置，具备Git式版本控制和同步内存本地开关评估。免费版支持5名团队成员，功能开关和A/B测试数量不限。
  * [Molasses](https://www.molasses.app) - 强大的功能开关与A/B测试工具。免费版支持3个环境，每个环境5个功能开关。
  * [Toggled.dev](https://www.toggled.dev) - 企业级可扩展多区域功能开关管理平台。免费版支持10个开关、2个环境且请求次数不限。SDK、分析仪表盘、发布日历、Slack通知等功能均包含在永久免费计划中。
  * [Statsig](https://www.statsig.com) - 功能管理、A/B测试与分析的一站式平台。其免费版提供不限席位、开关数量、实验次数和动态配置，每月支持100万次事件。
...
* [Google Fonts](https://fonts.google.com/) - 海量免费字体，可通过下载或链接Google CDN快速部署到网站
* [FontGet](https://www.fontget.com/) - 提供分类标签清晰、方便下载的多样化字体库
...
* [Form-Data](https://form-data.com) - 无代码表单后端。提供垃圾邮件过滤、邮件通知与自动回复、Webhooks、Zapier集成、重定向、AJAX/POST支持等功能。免费版包含无限表单和20次提交/月，展示Form-Data标识可额外获得2000次提交。
* [FabForm](https://fabform.io/) - 面向智能开发者的表单后端平台。免费版每月允许250次表单提交，提供现代化友好界面，支持Google Sheets、Airtable、Slack、邮件等集成。
* [Form.taxi](https://form.taxi/) — HTML表单提交端点服务。含通知、垃圾信息拦截和GDPR合规数据处理。基础使用免费。
...
* [Hyperforms.app](https://hyperforms.app/) — 无需后端代码，秒级创建表单转邮件服务。个人账户每月免费50次提交。
...
* [smartforms.dev](https://smartforms.dev/) - 强大易用的网站表单后端，永久免费版含每月50次提交、250MB存储、Zapier集成、CSV/JSON导出、自定义跳转/响应页、Telegram & Slack机器人、单邮件通知。
* [Survicate](https://survicate.com/) — 多渠道收集反馈并发送跟进调研。AI自动分析提取洞察。免费版含邮件/网站/产品内/移动端调研、AI问卷生成器和25次月回复。
* [staticforms.xyz](https://www.staticforms.xyz/) - 无需服务端代码即可免费集成HTML表单。用户提交后，表单内容将发送至注册邮箱。
* [stepFORM.io](https://stepform.io) - 测验和表单构建工具。免费版支持5个表单、每表单最多3个步骤和50次月回复。
* [Typeform.com](https://www.typeform.com/) — 在网站嵌入精美设计的表单。免费版限制每个表单10个字段和100次月回复。
* [WaiverStevie.com](https://waiverstevie.com) - 带REST API的电子签名平台，支持Webhook通知。免费版文档添加水印，但允许无限信封+签名。
* [Web3Forms](https://web3forms.com) - 静态&JAMStack网站联系表单解决方案，无需编写后端代码。免费版支持无限表单、无限域名和每月250次提交。
* [WebAsk](https://webask.io) - 调研与表单构建工具。免费版含3个调研/账户、100次月回复和每个调研10个元素。
* [Wufoo](https://www.wufoo.com/) - 快速创建网站表单。免费版每月限100次提交。
* [formpost.app](https://formpost.app) - 免费无限次表单转邮件服务。可设置自定义跳转、自动回复、Webhooks等。
* [Formester.com](https://formester.com) - 在网站分享和嵌入独特表单。不限制表单数量和功能，免费版每月100次提交。
* [SimplePDF.eu](https://simplepdf.eu/embed) - 在网站嵌入PDF编辑器，将任意PDF转为可填写表单。免费版支持无限PDF文档，每个PDF限3次提交。
* [forms.app](https://forms.app/) — 创建带条件逻辑、自动计分器和AI等强大功能的在线表单。免费版可收集100次回复，支持网站嵌入或链接分享。
...
* [Braintrust](https://www.braintrustdata.com/) - 生成式AI的评估、提示词游乐场和数据管理。免费计划每周提供1,000条私有评估数据行。
* [Findr](https://www.usefindr.com/) - 跨应用统一搜索工具，可同时搜索所有应用。智能搜索助手能基于您的信息回答问题。免费版提供无限统一搜索和每日5次协同查询。
...
* [Langtrace](https://langtrace.ai) - 帮助开发者追踪、评估、管理提示词和数据集，调试LLM应用性能问题。为所有LLM生成开放遥测标准追踪数据，提升可观测性，兼容任何观测客户端。免费计划每月5万次追踪。
* [LangWatch](https://langwatch.ai) - LLMOps平台，助力AI团队衡量、监控和优化LLM应用的可靠性、成本效益及性能。通过强大的DSPy组件，促进工程师与非技术团队协作，微调并投产生成式AI产品。免费计划包含全平台功能、每月1k次追踪和1个工作流DSPy优化器。[#开源](https://github.com/langwatch/langwatch)
* [Comet Opik](https://www.comet.com/site/products/opik/) - 在开发和产品生命周期中评估、测试和部署LLM应用。[#开源](https://github.com/comet-ml/opik/)
* [Langfuse](https://langfuse.com/) - 开源LLM工程平台，协助团队协作调试、分析和迭代LLM应用。永久免费计划包含每月5万次观测和全平台功能。[#开源](https://github.com/langfuse/langfuse)
...
* [Othor AI](https://othor.ai/) - 面向Tableau/Power BI/Looker的AI原生替代方案，快速、简单且安全。利用大语言模型(LLM)在数分钟内交付定制商业智能方案。永久免费计划提供1个工作区、5个数据源连接和1个用户，分析功能无限制。[#开源](https://github.com/othorai/othor.ai)
* [OpenRouter](https://openrouter.ai/models?q=free) - 提供多种免费AI模型包括DeepSeek R1/V3、Llama和Moonshot AI，擅长自然语言处理，适合多样化开发需求。注意免费模型存在速率限制。平台还提供Claude/OpenAI/Grok/Gemini/Nova等付费模型满足进阶需求。
...
* [Skypack](https://www.skypack.dev/) — 100%原生ES模块JavaScript CDN。每月每域名免费100万次请求  
* [raw.githack.com](https://raw.githack.com/) — **rawgit.com**的现代替代方案，使用Cloudflare托管文件  
* [section.io](https://www.section.io/) — 快速部署和管理完整Varnish缓存解决方案的简易平台，宣称单个站点永久免费  
* [statically.io](https://statically.io/) — 为Git仓库（GitHub/GitLab/Bitbucket）、WordPress资源和图片提供的CDN  
* [toranproxy.com](https://toranproxy.com/) — Packagist和GitHub的代理服务，保障持续交付。个人开发者免费（无技术支持）  
* [UNPKG](https://unpkg.com/) — npm全库CDN服务  
* [weserv](https://images.weserv.nl/) — 全球缓存的图片处理服务，支持实时图像裁剪与转换  
...
* [encore.dev](https://encore.dev/) — 通过静态分析实现自动基础设施的框架，提供无样板代码等功能。包含业余项目的免费云托管。
...
* [leapcell](https://leapcell.io/) - 可靠的分布式应用平台，为快速增长提供全面支持。免费版包含10万次服务调用、1万异步任务和10万Redis命令。
* [pipedream.com](https://pipedream.com) - 开发者专属集成平台，基于任意触发器构建工作流。[免费运行代码](https://docs.pipedream.com/pricing/)，无需管理服务器或云资源。
* [pythonanywhere.com](https://www.pythonanywhere.com/) — Python云应用托管。免费基础账户包含1个your-username.pythonanywhere.com域名下的Python应用、512MB私有存储和1个MySQL数据库。
* [ampt.dev](https://getampt.com/) - 无需复杂配置即可在AWS上构建部署JavaScript应用。免费预览版每小时500次调用，每日2,500次，每月50,000次。自定义域名仅限付费计划。
* [Koyeb](https://www.koyeb.com) - 面向开发者的全球无服务器平台，支持Docker容器/Web应用/API的Git部署。免费实例可部署至德国法兰克福或美国华盛顿，含512MB内存/2GB存储/0.1CPU。免费托管PostgreSQL数据库位于法兰克福、华盛顿和新加坡。
* [Napkin](https://www.napkin.io/) - FaaS服务提供500MB内存、15秒默认超时，每月5,000次API调用（限速5次/秒）。
* [Meteor Cloud](https://www.meteor.com/cloud) — Meteor应用PaaS服务Galaxy托管，含免费MongoDB共享主机和自动SSL。
* [Northflank](https://northflank.com) — 通过UI/API/CLI构建部署微服务、任务和托管数据库。免费层含2个服务、2个定时任务和1个数据库。
...
* [Firebase](https://firebase.com) — 助力应用开发与运营。免费Spark计划包含身份验证、托管、Firebase ML、实时数据库、云存储、Testlab。A/B测试、分析、应用分发等16项功能永久免费。
* [Flutter Flow](https://flutterflow.io) — 无需代码即可构建Flutter应用界面，集成Firebase。免费计划包含UI构建器和模板。
* [getstream.io](https://getstream.io/) — 数小时内实现可扩展的聊天、音视频及信息流功能，替代数周开发。
...
* [onesignal.com](https://onesignal.com/) — 无限免费推送通知。每月10,000封邮件，联系人无限制且支持自动预热。
* [paraio.com](https://paraio.com) — 带灵活身份验证、全文搜索和缓存的后端API。单个应用免费，含1GB数据。
...
* [pusher.com](https://pusher.com/beams) — 2,000月活用户的免费无限推送通知。iOS/Android统一API。
...
* [simperium.com](https://simperium.com/) — 实时跨平台数据传输，结构化数据存储与发送无限制，每月最多2,500用户。
* [Supabase](https://supabase.com) — 开源Firebase替代方案。免费计划含身份验证、实时数据库和对象存储。
* [tyk.io](https://tyk.io/) — 带身份验证、配额和监控的API管理。提供免费云服务。
...
* [LeanCloud](https://leancloud.app/) — 移动后端。免费1GB数据存储/256MB实例/每日3,000次API请求/10,000次推送。（API与Parse Platform高度相似）
* [Claw.cloud](https://run.claw.cloud) - PaaS平台为GitHub账户注册超过180天的用户提供5美元/月免费额度，适合托管应用和数据库等。（[免费额度注册链接](https://ap-southeast-1.run.claw.cloud/signin)）。
...
* [Clappia](https://www.clappia.com) — 专为构建业务流程应用设计的低代码平台，提供可定制的移动端和网页应用。具有拖放界面、离线支持、实时位置追踪及第三方服务集成等功能。
* [DronaHQ](https://www.dronahq.com/) — DronaHQ是一个低代码平台，帮助工程团队和产品经理快速构建内部工具、自定义用户流程、数字体验、自动化、管理面板和运营应用，效率提升10倍。
* [ILLA Cloud](https://www.illacloud.com/) — ILLA Cloud是一个强大的开源低代码平台，供开发者构建内部工具。通过使用ILLA的组件和动作库，开发者可大幅节省开发时间。5人以下团队免费。
* [lil'bots](https://www.lilbots.io/) - 在线编写和运行脚本，内置OpenAI、Anthropic、Firecrawl等免费API。适合构建AI代理/内部工具并与团队共享。免费版包含完整API访问权限、AI编程助手及每月10,000次执行额度。
* [Mendix](https://www.mendix.com/) — 企业级快速应用开发平台，提供无限访问的沙盒环境，支持总用户数不限，每应用0.5 GB存储和1 GB内存。免费版还包含Studio和Studio Pro IDE。
* [outsystems.com](https://www.outsystems.com/) — 企业级Web开发PaaS，支持本地或云端部署。免费"个人环境"提供无限代码和最高1 GB数据库。
* [ReTool](https://retool.com/) — 构建内部应用的低代码平台。ReTool高度可定制，任何能用JavaScript和API实现的功能均可完成。免费版每月支持最多5名用户，不限应用数量和API连接。
...
* [UI Bakery](https://uibakery.io) — 低代码平台，可快速构建自定义Web应用。支持通过拖放构建UI，并通过JavaScript、Python和SQL实现高度定制。提供云端和自托管方案，5人以下免费。
...
## 网站托管
  ...
  * [Alwaysdata](https://www.alwaysdata.com/) — 提供100MB免费网络托管，支持MySQL、PostgreSQL、CouchDB、MongoDB、PHP、Python、Ruby、Node.js、Elixir、Java、Deno，可自定义Web服务器，支持FTP、WebDAV和SSH访问；包含邮箱、邮件列表和应用安装器。
...
* [Bubble](https://bubble.io/) — 无需代码即可构建网页和移动应用的可视化编程工具，免费版带有Bubble品牌标识。
* [dAppling Network](https://www.dappling.network/) — 专注于Web3前端的去中心化网络托管平台，旨在提高正常运行时间和安全性，并为用户提供额外的访问入口。
...
* [Fenix Web Server](https://preview.fenixwebserver.com) — 一款开发者桌面应用，用于本地托管站点并实时公开分享。可通过其美观的用户界面、API和/或CLI按需操作。
* [Fern](https://buildwithfern.com) — 在Fern的免费计划中构建和托管基于Markdown的文档站点。甚至可以从API定义文件自动生成站点的API参考。站点托管在_yoursite_.docs.buildwithfern.com。
* [Free Hosting](https://freehostingnoads.net/) — 免费托管支持PHP 5、Perl、CGI、MySQL、FTP、文件管理器、POP电子邮件、免费子域名、免费域名托管、DNS区域编辑器、网站统计、免费在线支持以及许多其他免费主机未提供的功能。
* [Freehostia](https://www.freehostia.com) — FreeHostia提供免费托管服务，包括行业最佳的控制面板和50多个免费应用的一键安装。即时设置，无强制广告。
* [HelioHost](https://heliohost.org) — 非营利性免费网络托管，配备Plesk控制面板，支持PHP、Node.js、Python、Django、Flask、.NET、Perl、CGI、MySQL、PostgreSQL、SQLite、IMAP/POP3/SMTP邮件，无限带宽，免费子域名，1000 MB免费存储空间并可升级。
* [Kinsta Static Site Hosting](https://kinsta.com/static-site-hosting/) — 免费部署最多100个静态站点，支持自定义域名和SSL，每月100 GB带宽，260多个Cloudflare CDN节点。
* [Lecturify](https://www.lecturify.net/index.en.html) — 支持SFPT访问上传和下载文件的网络托管，提供PHP支持。
* [Neocities](https://neocities.org) — 静态托管，1 GB免费存储和200 GB带宽。
* [Netlify](https://www.netlify.com/) — 免费构建、部署和托管静态站点/应用，支持100 GB数据和100 GB/月带宽。
...
* [render.com](https://render.com) — 统一云平台，构建和运行应用及站点，提供免费SSL、全球CDN、私有网络、Git自动部署，并为Web服务、数据库和静态网页提供完全免费的计划。
* [SourceForge](https://sourceforge.net/) — 免费查找、创建和发布开源软件。
* [surge.sh](https://surge.sh/) — 面向前端开发者的静态网页发布工具。支持无限站点和自定义域名。
* [telegra.ph](https://telegra.ph/) — 使用Quill轻松创建网页。
* [tilda.cc](https://tilda.cc/) — 一个站点，50个页面，50 MB存储，仅限170多个预定义主块，不支持字体、网站图标和自定义域名。
* [Vercel](https://vercel.com/) — 构建、部署和托管Web应用，提供免费SSL、全球CDN，每次`git push`生成唯一预览URL。非常适合Next.js和其他静态站点生成器。
* [Versoly](https://versoly.com/) — 专注于SaaS的网站构建器，支持无限网站、70多个区块、五个模板、自定义CSS、网站图标、SEO和表单。不支持自定义域名。
...
* [MDB GO](https://mdbgo.com/) — 免费托管一个项目，容器TTL两周，每个项目500 MB RAM，SFTP - 1G磁盘空间。
* [Patr Cloud](https://patr.cloud/) — 易用的云平台，在其付费服务中提供三个静态站点的免费托管。
* [Serv00.com](https://serv00.com/) — 3 GB免费网络托管，每日备份（保留7天）。支持：Crontab任务、SSH访问、仓库（GIT、SVN和Mercurial），支持MySQL、PostgreSQL、MongoDB、PHP、Node.js、Python、Ruby、Java、Perl、TCL/TK、Lua、Erlang、Rust、Pascal、C、C++、D、R等多种语言。
* [Sevalla](https://sevalla.com/) — 简化应用、数据库和静态网站部署与管理的托管平台，限制1GB站点空间、100GB免费带宽、600分钟免费构建时间，每个账户100个站点。
...
* [1984.is](https://www.1984.is/product/freedns/) — 提供API支持的免费DNS服务，包含多种免费DNS功能。
* [cloudns.net](https://www.cloudns.net/) — 免费DNS托管服务，支持1个域名和50条记录。
...
* [Zonomi](https://zonomi.com/) — 提供即时DNS传播的免费DNS托管服务。免费方案支持1个DNS区域（域名）和最多10条DNS记录。
* [dnspod.com](https://www.dnspod.com/) — 免费DNS托管服务。
* [duckdns.org](https://www.duckdns.org/) — 免费动态DNS服务，免费版支持最多5个域名，提供多种配置指南。
* [Dynv6.com](https://dynv6.com/) — 免费动态DNS服务，支持[API调用](https://dynv6.com/docs/apis)，可管理多种DNS记录类型（如CNAME、MX、SPF、SRV、TXT等）。
* [freedns.afraid.org](https://freedns.afraid.org/) — 免费DNS托管服务，同时提供基于大量用户[贡献域名](https://freedns.afraid.org/domain/registry/)的免费子域名。注册后可在"Subdomains"菜单中获取免费子域名。
* [luadns.com](https://www.luadns.com/) — 免费DNS托管服务，支持3个域名，所有功能均有合理限制。
* [namecheap.com](https://www.namecheap.com/domains/freedns/) — 免费DNS服务，不限制域名数量。
* [nextdns.io](https://nextdns.io) — 基于DNS的防火墙，每月30万次免费查询。
* [noip.at](https://noip.at/) — 无需注册、无跟踪、无日志记录、无广告的免费动态DNS服务，不限制域名数量。
* [noip](https://www.noip.com/) — 动态DNS服务，免费版支持3个主机名，需每30天确认一次。
* [sslip.io](https://sslip.io/) — 免费DNS服务，当查询包含嵌入IP地址的主机名时，会返回该IP地址。
* [zilore.com](https://zilore.com/en/dns) — 免费DNS托管服务，支持5个域名。
* [zoneedit.com](https://www.zoneedit.com/free-dns/) — 支持动态DNS的免费DNS托管服务。
* [zonewatcher.com](https://zonewatcher.com) — 自动备份和DNS变更监控服务，免费版支持1个域名。
...
* [Glauca](https://docs.glauca.digital/hexdns/) — 免费DNS托管服务，支持最多3个域名和DNSSEC。
...
## 基础设施即服务
  ...
  * [4EVERLAND](https://www.4everland.org/) — 兼容AWS S3的API、界面操作、CLI等多种上传方式，可安全便捷高效地上传存储来自IPFS和Arweave网络的文件。注册用户可免费获得6GB IPFS存储空间和300MB Arweave存储空间。任何小于150KB的Arweave文件上传均免费。
...
* [CrateDB](https://crate.io/) - 用于实时分析的开源分布式 SQL 数据库。[免费层 CRFREE](https://crate.io/lp-crfree) 提供单节点配置：2 CPU、2 GiB 内存、8 GiB 存储。每个组织一个集群，无需支付方式。
...
* [MemCachier](https://www.memcachier.com/) — 托管 Memcache 服务。免费方案支持最多 25MB、1 个代理服务器和基础分析
* [scalingo.com](https://scalingo.com/) — 主要提供 PaaS，但包含 128MB 至 192MB 的免费 MySQL、PostgreSQL 或 MongoDB 层
* [SeaTable](https://seatable.io/) — Seafile 团队开发的灵活电子表格式数据库。无限表格、2,000 行、1 个月版本历史，最多 25 名团队成员。
* [skyvia.com](https://skyvia.com/) — 云端数据平台提供免费层，测试期间所有方案完全免费
* [StackBy](https://stackby.com/) — 融合电子表格灵活性、数据库功能及常用业务应用集成的工具。免费方案包含无限用户、10 个堆栈和每个堆栈 2GB 附件。
...
* [Turso by ChiselStrike](https://chiselstrike.com/) - Turso 是边缘数据库中的 SQLite 开发体验。永久免费入门方案提供 9GB 总存储、最多 500 个数据库、3 个位置、每月 10 亿行读取及 SQLite 本地开发支持。
* [InfluxDB](https://www.influxdata.com/) — 时序数据库，免费层支持每 5 分钟 3MB 写入、30MB 读取和 10,000 基数序列
* [restdb.io](https://restdb.io/) - 快速简洁的 NoSQL 云数据库服务。提供模式、关系、自动 REST API（支持类 MongoDB 查询）及高效的多用户数据管理界面。免费方案允许 3 用户、2,500 条记录和每秒 1 次 API 请求。
...
* [Neon](https://neon.tech/) — 托管 PostgreSQL，0.5 GB 总存储，1 个项目，10 个分支，无限数据库，主分支持续可用（非活跃 5 分钟后暂停），非主分支计算每月 20 小时活跃时间。
* [Dgraph Cloud](https://cloud.dgraph.io/pricing?type=free) — 托管原生图数据库管理系统，提供 GraphQL API。限制每日 1 MB 数据传输。
* [Tinybird](https://tinybird.co) - 无服务器托管 ClickHouse，支持 HTTP 无连接数据摄取，可将 SQL 查询发布为托管 HTTP API。免费层无时间限制，10GB 存储 + 每日 1,000 次 API 请求。
...
* [xata.io](https://xata.io) - Xata 是无服务器数据库，内置强大搜索和分析功能。单一 API、多类型安全客户端库，并针对开发工作流优化。永久免费层满足业余开发者需求，包含三个 Xata 单位（单位定义详见官网）。
* [8base.com](https://www.8base.com/) - 8base 是基于 MySQL 和 GraphQL 的全栈低代码开发平台，为 JavaScript 开发者提供无服务器后端即服务。通过 UI 应用构建器快速开发 Web 应用并轻松扩展，免费层包含：2,500 行、500 存储、1Gb/h 无服务器计算和 5 个客户端应用用户。
* [Nile](https://www.thenile.dev/) — 面向 B2B 应用的 Postgres 平台。无限数据库、持续可用不关闭、1GB 总存储、5,000 万查询令牌、自动扩展、无限向量嵌入

**[⬆️ 返回顶部](#目录)**

## 隧道技术、WebRTC、WebSocket服务器及其他路由
...
* [ZeroTier](https://www.zerotier.com) — 开源自托管虚拟以太网服务。免费方案支持25个客户端的端到端加密网络。支持桌面/移动/NA客户端；可通过网页界面配置自定义路由规则和审批私有网络新节点。
* [LocalXpose](https://localxpose.io) — 反向代理，可将本地服务器暴露到互联网。免费方案提供15分钟隧道有效期。
* [Traefik-Hub](https://traefik.io/traefik-hub/) - 通过隧道发布本地运行的服务到自定义公共URL，并通过访问控制进行保护。免费支持单个集群中的5个服务。
...
* [Basecamp](https://basecamp.com/personal) - 待办清单、里程碑管理、论坛式消息、文件共享和时间追踪。免费版支持3个项目、20名用户和1GB存储空间
...
* [clickup.com](https://clickup.com/) — 项目管理平台，含免费版与云存储高级版，提供移动应用和Git集成
...
* [gleek.io](https://www.gleek.io) — 开发者专属的免费文本转图表工具，支持非正式UML类图/对象图/实体关系图
* [GraphQL Inspector](https://github.com/marketplace/graphql-inspector) - 可视化对比两个GraphQL模式差异，精确标记破坏性/非破坏性/危险变更
...
* [pivotaltracker.com](https://www.pivotaltracker.com/) — 不限公开项目，免费版提供2个私有项目（3名活跃用户+不限只读用户）
* [plan.io](https://plan.io/) — 集成代码仓库的项目管理平台，免费版支持2名用户+10个客户+500MB存储
* [Plane](https://plane.so/) - 开源项目/产品管理工具，免费版不限成员数（文件上限5MB，支持1000个问题）
...
* [teleretro.com](https://www.teleretro.com/) — 趣味性回顾工具，包含破冰活动、GIF和表情符号。免费版支持3次回顾会议，成员人数不限
...
* [todoist.com](https://todoist.com/) — 协作与个人任务管理工具。免费版功能：5个活跃项目、每个项目5名成员、5MB文件上传、3个筛选器、1周活动记录
...
* [YouTrack](https://www.jetbrains.com/youtrack/buy/#edition=incloud) — 为开源项目及私有项目提供的免费托管版YouTrack（InCloud），支持3名用户。包含时间追踪和敏捷看板功能
* [zenhub.com](https://www.zenhub.com) — GitHub内嵌的独家项目管理方案，公共仓库、开源组织及非营利机构免费
* [zenkit.com](https://zenkit.com) — 项目管理与协作工具，免费版支持最多5名成员，5GB附件
* [Zube](https://zube.io) — 项目管理工具，免费版支持4个项目4名用户，提供GitHub集成
* [Toggl](https://toggl.com/) — 提供两款免费效率工具：[Toggl Track](https://toggl.com/track/)时间追踪应用（免费版含无限记录/项目/客户/标签/报告等功能，专为自由职业者设计）；[Toggl Plan](https://toggl.com/plan/)任务规划工具（免费版为独立开发者提供无限任务/里程碑/时间线）
...
* [icedrive.net](https://www.icedrive.net/) - 简洁的云存储服务，提供10GB免费空间
* [sync.com](https://www.sync.com/) - 端到端加密云存储服务，提供5GB免费空间
* [pcloud.com](https://www.pcloud.com/) - 云存储服务，最高可获10GB免费空间
* [sirv.com](https://sirv.com/) — 智能图片CDN服务，支持实时图像优化和尺寸调整。免费套餐包含500MB存储和2GB带宽
* [cloudimage.io](https://www.cloudimage.io/en/home) — 全球1500+节点的全功能图像优化CDN服务，提供多种尺寸调整、压缩和水印功能。开源插件支持响应式图片、360度图像制作和编辑。免费套餐每月含25GB CDN流量、25GB缓存存储和无限次转换
* [cloudinary.com](https://cloudinary.com/) — 为网站和应用提供图像上传、强大编辑、存储和分发服务，支持Ruby/Python/Java/PHP/Objective-C等语言库。免费套餐每月25积分（1积分=1000次图像转换/1GB存储/1GB CDN用量）
* [embed.ly](https://embed.ly/) — 提供网页媒体嵌入API、响应式图像缩放和网页元素提取服务。免费套餐支持每月5000个URL（15请求/秒）
...
* [gumlet.com](https://www.gumlet.com/) — 图像视频托管与CDN处理流媒体服务，免费套餐每月含250GB视频流量和30GB图片流量
* [image-charts.com](https://www.image-charts.com/) — 带水印的无限次图像图表生成服务
* [Imgbot](https://github.com/marketplace/imgbot) — 自动优化图像的开源机器人，在不损失质量的前提下减小文件体积（开源项目免费使用）
* [ImgBB](https://imgbb.com/) — 无限容量的图像托管服务，支持拖拽上传（单图上限32MB），提供直链、BBCode和HTML缩略图，登录后可查看上传历史
* [imgen](https://www.jitbit.com/imgen/) - 无水印的无限量社交媒体封面图生成API
* [imgix](https://www.imgix.com/) - 图像缓存管理和CDN服务，免费套餐含1000张原始图片、无限转换和100GB带宽
* [kraken.io](https://kraken.io/) — 网站性能优化服务，免费套餐支持1MB以内文件
* [kvstore.io](https://www.kvstore.io/) — 键值存储服务，免费套餐支持100个键（每个1KB）、每小时100次调用
* [npoint.io](https://www.npoint.io/) — 支持协同编辑模式的JSON存储服务
* [nitropack.io](https://nitropack.io/) - 自动前端优化加速服务（缓存/图像代码优化/CDN），免费支持每月5000次页面浏览
...
* [podio.com](https://podio.com/) — 支持5人以下团队免费使用基础功能（用户管理除外）
* [QRME.SH](https://qrme.sh) - 无需登录的批量二维码生成器（无水印无广告），单次最多支持100个URL
* [QuickChart](https://quickchart.io) — 可嵌入的图表/图形/二维码生成服务
...
* [Shotstack](https://shotstack.io) - 视频批量生成编辑API，免费套餐每月可渲染20分钟视频
* [tinypng.com](https://tinypng.com/) — PNG/JPEG压缩API，每月免费500次压缩
* [transloadit.com](https://transloadit.com/) — 文件上传及音视频/图像/文档转码服务，开源项目/慈善机构/学生可通过GitHub学生包免费使用，商业应用可试用2GB额度
* [twicpics.com](https://www.twicpics.com) - 响应式图像服务，提供CDN和前端自动化优化库，免费套餐每月3GB流量
* [uploadcare.com](https://uploadcare.com/hub/developers/) — 包含文件上传API/UI、智能压缩等功能的媒体管道服务，开发者免费套餐含3000次上传、3GB流量和3GB存储
* [imagekit.io](https://imagekit.io) – 分钟级集成的图像CDN服务，支持自动优化和实时转换，免费套餐每月20GB带宽
* [internxt.com](https://internxt.com) – 基于零知识证明的隐私云存储服务，注册即获永久10GB空间
* [degoo.com](https://degoo.com/) – AI云存储服务，免费提供20GB空间（支持3台设备），推荐可再获5GB（账户90天不活跃失效）
* [MConverter.eu](https://mconverter.eu/) – 支持[AVIF](https://mconverter.eu/convert/to/avif/)等格式的批量文件转换工具，可从视频提取所有帧。每日免费处理10个100MB以内文件（每次批量处理2个）
  ...
  * [DocsParse](https://docsparse.com/) – 基于GPT的AI文档处理工具，可将PDF、图片转换为JSON/CSV/EXCEL结构化数据。每月免费提供30次额度。
...
* [Backlight](https://backlight.dev/) - 专为开发设计协作打造的完整编码平台，支持设计系统的构建、文档化、发布和维护。免费版允许3名编辑协作1个设计系统，观众数量不限
...
* [framer.com](https://www.framer.com/) - 专业原型设计工具，支持界面动效设计，免费版允许无限查看者，最多2名编辑和3个项目
...
* [landen.co](https://www.landen.co) - 无代码建站平台，免费版支持完全自定义并发布一个网站
...
* [LottieFiles](https://lottiefiles.com/) - 全球最大轻量动画格式平台，提供Android/iOS/Web的Lottie动画工具和插件
...
* [Mindmup.com](https://www.mindmup.com/) - 无限量免费思维导图云端存储，支持全设备实时同步
* [Mockplus iDoc](https://www.mockplus.com/idoc) - 强大的设计协作与交付工具，免费版支持3用户5项目（全功能可用）
* [mockupmark.com](https://mockupmark.com/create/free) - 社交媒体和电商用T恤/服装模型生成器，提供40个免费模板
...
* [Penpot](https://penpot.app) - 基于网页的开源设计与原型工具，支持SVG格式
...
* [Plasmic](https://www.plasmic.app/) - 可集成至代码库的网页设计与页面构建工具，支持响应式页面/复杂组件开发，支持代码扩展并发布至生产环境
...
* [Shadcn Studio](https://shadcnstudio.com/theme-editor) - 实时预览主题在不同组件和布局中的效果
...
* [unsplash.com](https://unsplash.com/) - 商用和非商用免费图库（无限制使用许可）
...
* [svgrepo.com](https://www.svgrepo.com/) - 探索海量矢量库，为项目寻找最合适的免费商用SVG矢量素材
* [haikei.app](https://www.haikei.app/) - 生成独特SVG形状、背景和图案的在线工具，可直接用于设计工作流
* [Canva](https://canva.com) - 创建视觉内容的免费在线设计工具
* [Superdesigner](https://superdesigner.co) - 通过点击即可生成独特背景、图案、形状等设计的免费工具集
* [TeleportHQ](https://teleporthq.io/) - 低代码前端设计与开发平台，支持无头静态网站快速创建发布，免费3个项目且支持代码导出
* [vector.express](https://vector.express) — 快速转换AI/CDR/DWG等矢量文件格式
* [Vertopal](https://www.vertopal.com) - 免费在线文件转换平台，支持JPG转SVG等开发者常用格式转换
* [okso.app](https://okso.app) - 极简在线绘图工具，可导出PNG/SVG等格式，无需注册
* [Wdrfree SVG](https://wdrfree.com/free-svg) - 黑白风格免费SVG切割文件
* [Lucide](https://lucide.dev) - 免费可定制的统一风格SVG图标工具包
* [Logo.dev](https://www.logo.dev) - 包含4400万+品牌标志的API服务，前1万次调用免费
* [MDBootstrap](https://mdbootstrap.com/) - 免费个人商用Bootstrap/Angular/React/Vue UI套件，含700+组件和精美模板
...
* [iconify.design](https://icon-sets.iconify.design/) -- 100+图标库统一搜索平台，支持SVG导出
* [NextUI](https://nextui.org/) -- 免费精美的React & Next.js UI库
* [Glyphs](https://glyphs.fyi/) -- 免费可编辑的开源图标设计系统
* [ShadcnUI](https://ui.shadcn.com/) -- 可复制粘贴的优雅组件库，开源可定制
...
* [Lunacy](https://icons8.com/lunacy) -- 内置素材的免费图形设计工具，免费版支持10个云文档
...
* [Lapa Ninja](https://www.lapa.ninja/) - [着陆页/UI套件/网页截图] 展示6025个最佳着陆页案例，提供免费设计书籍和全球优质UI套件资源。
* [LovelyLanding.net](https://www.lovelylanding.net/) - [着陆页设计] 持续更新的着陆页截图集，包含桌面端、平板和移动端视图。
* [Mobbin](https://mobbin.design/) - [移动端截图] 通过5万+可全检索的移动应用截图库，节省UI/UX研究时间。
* [Uiland Design](https://uiland.design/) - [移动端截图] 探索非洲及全球顶尖企业的移动端与网页UI设计。
* [Mobile Patterns](https://www.mobile-patterns.com/) - [移动端截图] 为设计师、开发者和产品人员提供精选UI/UX模式库（iOS与Android）。
* [Page Flows](https://pageflows.com/) - [移动端/网页视频与截图] 多款应用的完整流程视频记录，含可高度检索的截图资源。
* [Screenlane](https://screenlane.com/) - [移动端截图] 获取最新网页和移动应用UI设计趋势，支持按模式和应用筛选。
* [scrnshts](https://scrnshts.club/) - [移动端截图] 精选应用商店设计截图合集。
* [UI Garage](https://uigarage.net/) - [移动端/网页截图] 每日UI灵感库，为设计师和开发者提供项目所需的灵感、工具和资源。
* [Refero](https://refero.design/) - [网页截图] 来自优秀网页应用的可标记、可检索设计参考合集。
...
* [maptiler.com](https://www.maptiler.com/cloud/) — 矢量地图、地图服务与可视化SDK。免费矢量瓦片每周更新，提供四种地图样式
...
* [cacher.io](https://www.cacher.io) — 代码片段管理工具，支持标签和100多种编程语言
...
* [cocalc.com](https://cocalc.com/) — （原cloud.sagemath.com）云端协作计算平台。通过浏览器访问完整Ubuntu系统，内置协作功能，预装大量数学、科学、数据科学免费软件：Python、LaTeX、Jupyter Notebooks、SageMath、scikitlearn等
...
* [Eclipse Che](https://www.eclipse.org/che/) - 面向开发团队的基于Web和Kubernetes的IDE，支持多语言。开源且社区驱动。Red Hat托管实例：[workspaces.openshift.com](https://workspaces.openshift.com/)
* [fakejson.com](https://fakejson.com/) — 通过API快速生成虚假数据。描述需求后，API将以JSON格式返回结果。加速创意落地过程
* [GetVM](https://getvm.io) — 即时免费的Linux和IDE Chrome侧边栏。免费版每天可使用5个VM
* [GitPod](https://www.gitpod.io) — 为GitHub项目提供即时可用的开发环境。免费版每月50小时
* [ide.goorm.io](https://ide.goorm.io) goormIDE是云端全功能IDE。支持多语言，基于Linux容器的完整网页终端，端口转发，自定义URL，实时协作与聊天，分享链接，Git/Subversion支持。免费版包含1GB内存和10GB存储/容器，5个容器槽位
...
* [jsbin.com](https://jsbin.com) — 前端网页开发 playground（HTML/CSS/JavaScript），同时支持Markdown、Jade和Sass
* [jsfiddle.net](https://jsfiddle.net/) — 支持协作的前端网页开发 playground
* [JSONPlaceholder](https://jsonplaceholder.typicode.com/) 提供返回虚假JSON数据的REST API端点。也可本地运行服务端源码
...
* [Mocklets](https://mocklets.com) - HTTP模拟API工具，支持终身免费套餐
* [Paiza](https://paiza.cloud/en/) — 无需配置即可在浏览器开发Web应用。免费版提供24小时有效期的服务器，每天4小时运行时间（2核CPU/2GB内存/1GB存储）
* [Prepros](https://prepros.io/) - 开箱即用支持Sass/Less/Stylus/Pug/Haml/Slim/CoffeeScript/TypeScript编译，实时重载浏览器，轻松开发测试网站。支持自定义工具扩展
* [Replit](https://replit.com/) — 多语言云端编程环境
* [SoloLearn](https://code.sololearn.com) — 适合运行代码片段的云端编程playground。运行代码无需注册，保存代码需登录。提供免费编程课程
* [stackblitz.com](https://stackblitz.com/) — 在线全栈应用开发IDE。支持主流NodeJs前后端框架。新建项目短链：[https://node.new](https://node.new)
* [Sublime Text](https://www.sublimetext.com/) - 流行、多功能、高度可定制的代码/文本编辑器
* [Visual Studio Code](https://code.visualstudio.com/) - 微软开发的现代化网页/云应用开发和调试优化编辑器

  * [VSCodium](https://vscodium.com/) - 社区驱动的 Microsoft 编辑器 VSCode 二进制发行版，无遥测/跟踪，采用自由许可证
* [wakatime.com](https://wakatime.com/) — 通过文本编辑器插件量化编程活动数据，提供免费基础版
* [Wave Terminal](https://waveterm.dev/) - 开源跨平台终端工具，支持内联渲染、会话保存与历史记录，基于开放网络标准，支持 MacOS 和 Linux
* [WebComponents.dev](https://webcomponents.dev/) — 浏览器内 Web Components 开发 IDE，提供 58 种模板，支持组件故事和测试
* [PHPSandbox](https://phpsandbox.io/) — 在线 PHP 开发环境
* [WebDB](https://webdb.app) - 免费高效数据库 IDE，支持服务器发现、ERD、数据生成器、AI、NoSQL 结构管理、数据库版本控制等功能
...
* [Avo](https://avo.app/) - 简化的分析发布流程。提供单一事实来源的跟踪方案、类型安全分析跟踪库、应用内调试器和数据可观测性。免费版支持2名工作区成员和1小时数据回溯
* [Branch](https://www.branch.io) - 移动分析平台。免费版支持最多1万移动应用用户，包含深度链接等服务
* [Cauldron](https://cauldron.io) - 开源分析解决方案，支持从Git/GitHub/Gitlab等协作平台聚合数据。免费版包含无限量报告
* [Census](https://www.getcensus.com/) - 反向ETL和运营分析平台。免费版支持从数据仓库同步10个字段到60+个SaaS应用
...
* [MetricsWave](https://metricswave.com) - 开发者友好的隐私保护型谷歌分析替代方案。免费版每月2万页面浏览且无需信用卡
...
* [Hotjar](https://hotjar.com) - 网站分析与报告。免费版每日2000页面浏览、100个快照（上限300）、存储3个热力图365天。同时提供应用内调查和反馈组件，免费版支持3个调查/组件及每月20条回复
...
* [Mixpanel](https://mixpanel.com/) - 每月10万追踪用户，无限数据历史和席位，支持美欧数据驻留
* [Moesif](https://www.moesif.com) - REST/GraphQL接口分析工具。免费版每月50万次API调用
...
* [sematext.com](https://sematext.com/cloud/) - 免费版每月5万次操作，1天数据保留，无限仪表板
...
* [Statsig](https://statsig.com) - 集成分析、功能标记和A/B测试的全能平台。免费版每月100万计量事件
* [Tableau Developer Program](https://www.tableau.com/developer) - Tableau开发者计划提供个人沙盒许可
...
* [PostHog](https://posthog.com) - 全功能产品分析套件，免费版每月100万追踪事件+250条应用内调查回复
* [Uptrace](https://uptrace.dev) - 分布式追踪工具，开源项目可申请个人订阅
* [Microsoft Clarity](https://clarity.microsoft.com/) - 免费易用的用户行为记录工具
* [Beampipe.io](https://beampipe.io) - 简洁的隐私优先网站分析，免费支持5个域名+1万月浏览量
* [Aptabase](https://aptabase.com) - 开源隐私友好的移动/桌面应用分析工具，免费版每月2万事件
* [Trackingplan](https://www.trackingplan.com/) - 自动化数字分析监测与协作平台
* [LogSpot](https://logspot.io) - 统一分析平台，免费版每月1万事件
* [Umami](https://umami.is/) - 简洁快速、注重隐私的谷歌分析替代方案
* [TrackWith Dicloud](https://dicloud.net/trackwith-privacy-focused-analytics/) - 轻量级隐私分析工具，免费支持3个域名+每域名600次会话回放
* [AppFit](https://appfit.io) - 跨平台分析与产品管理工具，免费版每月1万事件+产品日志+每周洞察

  * [Peasy](https://peasy.so) - Peasy是一款轻量级、注重隐私的网站与产品分析工具。免费套餐每月包含3,000次事件记录。
* [Rybbit](https://rybbit.io) - 开源的Google Analytics替代方案，无需Cookie且操作直观性提升10倍。免费套餐提供每月3,000次事件记录。
...
* [Adapty.io](https://adapty.io/) – 提供开源SDK的一站式解决方案，支持iOS、Android、React Native、Flutter、Unity或网页应用的应用内订阅集成。每月收入1万美元以下免费。
* [CoinMarketCap](https://coinmarketcap.com/api/) — 提供加密货币市场数据，包括最新加密与法币汇率。免费层每月提供1万次API调用额度。
...
* [CurrencyApi](https://currencyapi.net/) — 支持JSON和XML格式的实物货币与加密货币实时汇率。免费层每月提供1,250次API请求。
* [currencylayer](https://currencylayer.com/) — 为企业提供可靠的汇率与货币转换服务，免费每月100次API请求。
...
* [FxRatesAPI](https://fxratesapi.com) — 提供实时和历史汇率数据。免费层需标注数据来源。
* [Moesif API Monetization](https://www.moesif.com/) - 通过用量计费实现API变现。支持对接Stripe、Chargebee等平台。免费层每月3万次事件处理。
* [Nami ML](https://www.namiml.com/) - iOS和Android平台的应用内购与订阅全流程平台，含无代码付费墙、CRM和分析工具。基础功能永久免费。
* [RevenueCat](https://www.revenuecat.com/) — 应用内购与订阅托管后端（支持iOS和Android）。每月追踪收入2,500美元以下免费。
* [vatlayer](https://vatlayer.com/) — 即时增值税号验证与欧盟增值税率API，免费每月100次请求。
...
## Vagrant相关
  ...
  * [Vagrant Cloud](https://app.vagrantup.com) - HashiCorp Vagrant 云服务。提供 Vagrant 虚拟机镜像托管。
* [Vagrantbox.es](https://www.vagrantbox.es/) — 一个替代性的公共镜像索引站点
...
* [Remarkbox](https://www.remarkbox.com/) - 开源托管评论平台，采用"按需付费"模式，支持"单个管理员管理多个域名，并完全控制行为与外观"
...
* [screenshotlayer.com](https://screenshotlayer.com/) — 可高度自定义的网站快照服务。每月免费100次截取。
* [screenshotmachine.com](https://www.screenshotmachine.com/) — 每月100次截取，支持png/gif/jpg格式，包含完整页面滚动截图（不限于首页）。
* [PhantomJsCloud](https://PhantomJsCloud.com) — 浏览器自动化与页面渲染服务。免费层每日500页处理额度。自2017年持续提供免费方案。
* [Webshrinker.com](https://webshrinker.com) — 提供网站截图与域名情报API服务。每月免费100次请求。
* [Screenshots](https://screenshotson.click) — 多功能截图API，支持高度自定义截取参数。每月免费100张截图。
...
* [CodeMagic](https://codemagic.io/) - Codemagic 是一款全托管的移动应用持续集成/持续交付服务。通过图形化 CI/CD 工具实现构建、测试和部署。免费套餐每月提供 500 分钟构建时长，配备 2.3 GHz 处理器和 8GB 内存的 Mac Mini 实例。
...
* [Iubenda](https://www.iubenda.com/) - 隐私政策、Cookie政策及同意书管理工具。免费版提供有限的隐私/Cookie政策及横幅功能。
* [Cookiefirst](https://cookiefirst.com/) - 多语言Cookie横幅、审计及同意管理方案。免费版含单次扫描和单个横幅功能。
* [Ketch](https://www.ketch.com/) - 同意管理框架工具。免费版在访问量限制下开放多数功能。
* [Concord](https://www.concord.tech/) - 完整数据隐私平台，涵盖同意管理、隐私请求处理(DSARs)和数据映射。免费版包含核心同意管理功能，并为通过验证的开源项目提供高级免费方案。
...
* [Blynk](https://blynk.io) — 提供API控制的SaaS物联网开发平台，免费开发者计划支持5台设备，含云端存储。提供移动端应用
...
* [datelist.io](https://datelist.io) - 在线预约系统，免费版每月5次预订，包含1个日历
...
* [Hook Relay](https://www.hookrelay.dev/) - 为应用轻松添加webhook功能，含队列管理、退避重试和日志记录。免费版每日100次投递，14天留存，支持3个端点
* [http2.pro](https://http2.pro) — HTTP/2协议支持检测与就绪测试API
...
* [OnlineExifViewer](https://onlineexifviewer.com/) — 即时查看照片的EXIF元数据（含GPS定位信息）
...
* [redirect.ing](https://redirect.ing/) - 无需服务器的快速域名转发服务，免费版含10个主机名和每月10万次请求
* [redirect.pizza](https://redirect.pizza/) - 支持HTTPS的重定向管理工具，免费版含10个源地址和每月10万次点击
* [ReqBin](https://reqbin.com/) — 在线HTTP请求测试工具，支持GET/POST/PUT/DELETE/HEAD方法及令牌认证
* [Smartcar API](https://smartcar.com) - 车辆定位、油量电量查询、里程读取、车门锁控制等汽车API
* [snappify](https://snappify.com) - 开发者视觉化工具，免费版支持3个同时编辑的代码快照及每月5次AI代码解释
...
* [Tiledesk](https://tiledesk.com) - 全渠道聊天机器人构建平台，免费版支持无限量机器人
* [Versionfeeds](https://versionfeeds.com) — 为软件版本更新生成定制RSS订阅（前3个订阅免费）
...
* [RemSupp](https://remsupp.com) — 按需技术支持与设备永久访问权限（免费版每天2次会话）  
...
* [github.com — 开发者开源软件集](https://github.com/tvvocold/FOSS-for-Dev) — 开发者免费开源软件资源中心
...
* [临时短信接收](https://smsreceivefree.com/) — 提供免费临时 disposable 电话号码
* [SMS测试沙盒](https://sandbox.httpsms.com) — 免费收发测试短信
* [SimpleBackups备份服务](https://simplebackups.com/) — 支持服务器及数据库（MySQL/PostgreSQL/MongoDB）的自动化备份服务，直接存储至云存储（AWS/DigitalOcean/Backblaze），免费版提供1个备份额度
* [SnapShooter备份方案](https://snapshooter.com/) — 支持DigitalOcean/AWS/LightSail/Hetzner/Exoscale的备份服务，可直接备份数据库/文件系统/应用到s3存储，免费版提供单资源每日备份
* [主题精选](https://themeselection.com/) — 精选高质量、现代化、专业易用的免费管理后台模板/HTML主题/UI套件，加速应用开发
* [网站性能检测](https://web.dev/measure/) — 免费检测网站性能并优化SEO排名的工具
* [开发者小工具集](https://smalldev.tools/) — 支持编解码/代码压缩美化/生成测试数据等功能的开发者工具集，拥有友好界面
* [Layercode数据导入工具](https://layercode.com/usecsv) — 快速为网页应用添加CSV/Excel导入功能，免费版支持100MB文件大小且无需绑卡
...
* [Utils.fun工具集](https://utils.fun/en) — 基于浏览器算力的离线工具集（水印生成/录屏/编解码/加解密/代码格式化等），完全免费不上传云端
...
* [360格式转换](https://www.360converter.com/) - 免费音视频转文字工具（支持实时音频/YouTube视频/字幕生成）
* [QRCodeBest](https://qrcode.best/) - 提供13种模板的隐私保护型二维码生成器，支持追踪像素/项目分类/无限团队席位
* [PostPulseAI](https://PostPulseAI.com) - 通过每月AI生成SEO优化内容提升网站排名，免费版支持手动发布1篇/月
* [PageTools工具包](https://pagetools.co/) - 永久免费的AI工具集（网站政策生成/社交媒体文案/网页生成等）
