...
这是一份带有免费开发者层的软件（SaaS、PaaS、IaaS 等）和其他产品的列表。

此特定列表的范围仅限于基础架构开发人员（系统管理员、DevOps 从业者等）可能会发现有用的内容。我们喜欢那里所有的免费服务，但保持主题相关性会很好。有时候这是一条灰色的线，所以这是有倾向性的；如果我不接受你的贡献，请不要感到冒犯。
...
**注意**：此列表仅适用于即服务产品，不适用于自托管软件。要符合条件，服务必须提供免费层，而不仅仅是免费试用。如果是时间桶模式，则免费层必须至少为一年。我们还从安全角度考虑免费层，因此 SSO 是可以的，但我不会接受将 TLS 限制为仅付费层的服务。
...
# 目录
  ...
  * [持续集成/持续部署（CI/CD）](#持续集成/持续部署（CI/CD）)
...
## 主要云服务提供商的免费使用限制
  ...
  * [Google Cloud Platform](https://cloud.google.com)
    ...
    * Google Kubernetes Engine - 对于一个区域集群，无集群管理费用。每个用户节点按标准 Compute Engine 定价收费
  ...
  * [Oracle Cloud](https://www.oracle.com/cloud/)
    ...
    * 带宽 - 每月 10TB 出站流量，x64 架构 VM 上的速度限制为 50Mbps，基于 ARM 的 VM 上的速度限制为 500Mbps * 内核数
  ...
  * [Cloudflare](https://www.cloudflare.com/)
    ...
    * [Cloudflare Tunnel](https://www.cloudflare.com/products/tunnel/) - 您可以通过[Quick Tunnels](https://developers.cloudflare.com/cloudflare-one/connections/connect-apps/run-tunnel/trycloudflare)在隧道中暴露本地运行的 HTTP 端口到 trycloudflare.com 上的随机子域，无需帐户。更多功能（TCP 隧道、负载均衡、VPN）在[Zero Trust](https://www.cloudflare.com/products/zero-trust/)免费计划中。

    * [ Workers KV](https://developers.cloudflare.com/kv) - 每天 10 万次读请求，每天 1000 次写请求，每天 1000 次删除请求，每天 1000 次列表请求，1GB 存储数据
    * [R2](https://developers.cloudflare.com/r2/) - 每月 10GB，每月 100 万 A 级操作，每月 1000 万 B 级操作
...
## 源代码仓库
  ...
  * [gitlab.com](https://about.gitlab.com/)——最多 5 名协作者的无限数量的公共和私人 Git 存储库。包括 CI/CD、静态托管、容器注册表、项目管理和问题跟踪
  ...
  * [perforce.com](https://www.perforce.com/products/helix-teamhub)——免费 1GB 云存储以及 Git、Mercurial 或 SVN 存储库。
...
## API、数据和机器学习
  ...
  * [Apify](https://www.apify.com/) - 网络抓取和自动化平台，可创建任何网站的 API 并提取数据。提供现成的抓取器、集成的代理和自定义解决方案。免费计划每月包含 5 美元的平台信用额度。
  ...
  * [APIVerve](https://apiverve.com) - 立即访问超过 120 个 API，免费使用，质量、一致性和可靠性高。免费计划每月涵盖最多 50 个 API 令牌。
  ...
  * [Bruzu](https://bruzu.com/) - 自动生成图像。使用 API、集成或无代码工作表生成大量图像变体。API 免费使用，带有水印。
  ...
  * [Conversion Tools](https://conversiontools.io/) - 在线文件转换器，用于文档、图像、视频、音频和电子书。提供 REST API。适用于 Node.js、PHP、Python 的库。支持的文件最大为 50GB（付费计划）。免费层受文件大小和每天转换次数的限制。
  * [Country-State-City Microservice API](https://country-state-city.rebuscando.info/) - API 和 Microservice，提供广泛的信息，包括国家、地区、省份、城市、邮政编码等。免费层每天包括最多 100 个请求。
  * [Coupler](https://www.coupler.io/) - 用于在应用程序之间同步数据的工具。它可以创建实时仪表板和报告，转换和操作值，并收集和备份见解。免费计划具有无限用户，每月 100 次运行，每月 1000 行，以及无限集成。
  ...
  * [Data Dead Drop](https://datadeaddrop.com) - 简单、免费的文件共享。数据在访问后会自行销毁。通过浏览器或您喜欢的命令行客户端上传和下载数据。
  ...
  * [Deepnote](https://deepnote.com) - 一种新的数据科学笔记本。兼容 Jupyter，支持实时协作和在云端运行。免费层包括无限个人项目，最多 750 小时的标准硬件，以及最多 3 个编辑器的团队。
  ...
  * [dreamfactory.com](https://dreamfactory.com/)——用于移动、Web 和 IoT 应用程序的开源 REST API 后端。连接任何 SQL/NoSQL 数据库、文件存储系统或外部服务，它会立即创建一个具有实时文档和用户管理的全面 REST API 平台。
  ...
  * [file.coffee](https://file.coffee/)——一个平台，您可以在其中存储最多 15MB/file（使用帐户可存储 30MB/file）。
  * [FraudLabs Pro](https://www.fraudlabspro.com)——对信用卡支付欺诈进行订单交易筛查。此 REST API 将根据订单的输入参数检测所有可能的欺诈特征。免费的微计划每月有 500 笔交易。
  * [Geekflare API](https://geekflare.com/api)——Geekflare API 让您可以截图、审核网站、TLS 扫描、DNS 查找、测试 TTFB 等。免费计划提供 3,000 次 API 请求。
  ...
  * [Hybiscus](https://hybiscus.dev/)——使用简单的声明性 API 构建 pdf 报告。免费层包括每月最多 100 份单页报告的能力，并能够自定义调色板和字体。
  * [Invantive Cloud](https://cloud.invantive.com/)——访问超过 70 个（云）平台，如 Exact Online、Twinfield、ActiveCampaign 或 Visma，使用 Invantive SQL 或 OData4（通常是 Power BI 或 Power Query）。包括数据复制和交换。开发人员和实施顾问的免费计划。对于特定平台，免费计划有限制。
  ...
  * [IP2Location.io](https://www.ip2location.io/)——免费增值、快速可靠的 IP 地理定位 API，用于确定城市、坐标、ISP 等地理定位数据。免费计划每月提供 30k 信用额度。订阅付费计划以获得更多高级功能或联系我们以获得个性化计划。
  * [ipapi](https://ipapi.co/)——由 Kloudend, Inc 提供的 IP 地址位置 API——基于 AWS 构建的可靠地理定位 API，受到 Fortune 500 的信任。免费层提供每月 30k 次查找（每天 1k 次），无需注册。
  * [ipapi.is](https://ipapi.is/)——适用于开发人员的可靠 IP 地址 API，具有最佳的主机检测功能。免费计划提供 1,000 次无注册查找。
  ...
  * [GoCardless](https://gocardless.com/)——免费的开放银行数据 API。PSD2。在欧盟+英国，将 2300 多家银行与您的应用/软件连接。
  * [Nyckel](https://www.nyckel.com)——训练、部署和调用图像和文本 ML 模型。免费训练，最多可使用 5000 条训练数据。每月免费调用 1000 个模型。
  * [Observable](https://observablehq.com/)——一个用于创建、协作和学习数据的地方。免费：无限笔记本、无限发布、每个笔记本五个编辑器。
  * [OCR.Space](https://ocr.space/)——OCR API 解析图像和 pdf 文件，以 JSON 格式返回文本结果。每月免费 25000 次请求。
  * [Duply.co](https://duply.co)——从 API 和 URL 创建动态图像，设计模板一次并重复使用。免费层每月提供 70 个从 API 和 URL 创建的图像，以及最多 100 个通过表单创建的图像。
  ...
  * [pdfEndpoint.com](https://pdfendpoint.com)——使用简单的 API 将 HTML 或 URL 轻松转换为 PDF。每月免费 100 次转换。
  ...
  * [PromptLoop](https://www.promptloop.com/)——使用 AI 和大型语言模型（如 GPT-3），通过简单的电子表格公式在 Google Sheets 中转换、理解和分析文本。每月免费 2000 个信用额度。
  ...
  * [Scraper's Proxy](https://scrapersproxy.com) — 用于抓取的简单 HTTP 代理 API。无需担心限制、阻止或验证码即可匿名抓取。每月前 100 次成功抓取免费，包括 JavaScript 呈现（如果您联系支持人员，更多可用）。
  * [ScrapingAnt](https://scrapingant.com/) — 无头 Chrome 抓取 API 和免费检查代理服务。JavaScript 呈现，优质旋转代理，验证码回避。提供免费计划。
  ...
  * [Select Star](https://www.selectstar.com/) - 是一个智能数据发现平台，它自动分析和记录您的数据。免费的轻量级层，包括 1 个数据源、最多 100 个表和 10 个用户。
  ...
  * [shrtcode API](https://shrtco.de/docs) - 免费的无授权 URL 缩短 API，无请求限制。
  * [SerpApi](https://serpapi.com/) - 实时搜索引擎抓取 API。为 Google、YouTube、Bing、百度、沃尔玛等许多机器返回结构化 JSON 结果。免费计划包括每月 100 次成功的 API 调用。
  * [SmartParse](https://smartparse.io) - SmartParse 是一个数据迁移和 CSV 到 API 平台，提供节省时间和成本的开发人员工具。免费层包括每月 300 个处理单元、浏览器上传、数据隔离、断路器和作业警报。
  * [Sofodata](https://www.sofodata.com/) - 从 CSV 文件创建安全的 RESTful API。上传 CSV 文件并通过其 API 立即访问数据，从而加快应用程序开发速度。免费计划包括 2 个 API 和每月 2,500 个 API 调用。您不需要信用卡。
  ...
  * [TemplateTo](https://templateto.com) - 使用我们的拖放编辑器和简单 API，从可重复用的模板自动生成 PDF/TXT 文档。免费计划包括每月 450 份 PDF 和三个模板。
  * [TinyMCE](https://www.tiny.cloud) - 富文本编辑 API。核心功能免费，无使用限制。
  * [Treblle](https://www.treblle.com) - Treblle 帮助团队构建、交付和管理 API。具有高级 API 日志聚合、可观察性、文档和调试功能。所有功能都免费，但免费层每月的请求限制高达 250k。
  ...
  * [ZenRows](https://www.zenrows.com/) — Web Scraping API 和代理服务器，可绕过任何反机器人解决方案，同时提供 JavaScript 呈现、旋转代理和地理定位。免费层为 1000 次 API 调用。
  ...
  * [WebScraping.AI](https://webscraping.ai) - 具有内置解析、Chrome 呈现和代理的简单 Web 抓取 API。每月两千次免费 API 调用。
  ...
  * [Zuplo](https://zuplo.com/) - 在几分钟内为任何 API 添加 API 密钥身份验证、速率限制和开发者文档。免费计划提供最多 10 个项目、无限的生产边缘环境、250 个 API 密钥、100K 每月请求和 1GB 出口。
...
## 工件仓库
  ...
  * [Artifactory](https://jfrog.com/start-free/) - 一个支持多种软件包格式的工件存储库，如 Maven、Docker、Cargo、Helm、PyPI、CocoaPods 和 GitLFS。它包含包扫描工具 XRay 和 CI/CD 工具 Pipelines（以前称为 Shippable），每月有 2,000 分钟的免费 CI/CD 分钟数。
  ...
  * [jitpack.io](https://jitpack.io/) - GitHub 上用于 JVM 和 Android 项目的 Maven 存储库，公共项目免费。
...
## 团队和协作工具
  ...
  * [Braid](https://www.braidchat.com/)——专为团队设计的聊天应用程序。公共访问组免费，无限制的用户、历史记录和集成。它还提供自托管的开源版本。
  * [cally.com](https://cally.com/)——找到会议的完美时间和日期。简单易用，适用于大小团体。
  ...
  * [Notion](https://www.notion.so/)——具有 markdown 支持的笔记记录和协作应用程序，集成了任务、维基和数据库。该公司将该应用程序描述为笔记记录、项目管理和任务管理的一体化工作区。除了跨平台应用程序，它还可以通过大多数网络浏览器访问。
  * [Nuclino](https://www.nuclino.com)——适用于团队所有知识、文档和笔记的轻量级协作维基。免费计划包含所有基本功能，最多 50 个项目和 5GB 存储空间。
  ...
  * [PageShare.dev](https://www.pageshare.dev)——为 GitHub Pull 请求添加视觉审查功能，无需部署网站。免费最多每月 10 页和总存储量 100MB。
  ...
  * [Slab](https://slab.com/) - 一个现代知识管理服务，适用于团队。最多可供 10 名用户免费使用。
  * [slack.com](https://slack.com/) - 无限用户免费，但有一些功能限制
  * [Spectrum](https://spectrum.chat/) - 免费创建公共或私人社区。
  * [StatusPile](https://www.statuspile.com/) - 一个状态页面的状态页面。您能跟踪您的上游提供商的状态页面吗？
  ...
  * [Teamhood](https://teamhood.com/) - 免费的项目、任务和问题跟踪软件。支持带有泳道和完整 Scrum 实现的看板。集成了时间跟踪。免费供最多 5 个用户和 3 个项目组合使用。
  * [Teamplify](https://teamplify.com) - 通过团队分析和智能每日站立会议改进团队开发流程。包括适用于远程优先团队的全功能休假管理。免费供最多 5 个用户的小团体使用。
  ...
  * [twist.com](https://twist.com/) - 一种异步友好的团队通信应用程序，可保持对话的组织性和主题性。免费和无限计划可供选择。为符合条件的团队提供折扣。
  ...
  * [windmill.dev](https://windmill.dev/) - Windmill 是一个开源开发者平台，可从最少的 Python 和 Typescript 脚本快速构建生产级多步骤自动化和内部应用程序。作为免费用户，您最多可以创建并成为三个非高级工作空间的成员。
  * [vadoo.tv](https://vadoo.tv/) - 简单的视频托管和营销。只需点击一下即可上传视频。录制、管理、共享等。免费层提供多达 10 个视频、1GB 存储空间和 10GB 带宽/每月
  * [userforge.com](https://userforge.com/) - 相互关联的在线个人资料、用户故事和上下文映射。有助于保持设计和开发的同步，最多可免费使用 3 个个人资料和 2 个协作者。
  ...
  * [Zulip](https://zulip.com/) - 具有独特电子邮件式线程模型的实时聊天。免费计划包括 10,000 条消息搜索历史和高达 5GB 的文件存储。还提供自托管的开源版本。
  ...
  * [Chanty.com](https://chanty.com/) - Chanty 是 Slack 的另一种替代品。它为小型团队（最多 10 人）提供免费的永久计划，包括无限的公共和私人对话、可搜索的历史记录、无限的 1:1 音频通话、无限的语音消息、10 个集成和每个团队 20GB 存储。
  ...
  * [Webvizio](https://webvizio.com) - 用于在直接在实时网站和网络应用程序、图像、PDF 和设计文件上的任务上简化网络开发协作的网站反馈工具、网站评论软件和错误报告工具。
  * [Pullflow](https://pullflow.com) - 在 GitHub、Slack 和 VS Code 上提供 AI 增强的代码审查协作平台。
  * [Webex](https://www.webex.com/) - 提供免费计划的视频会议，每个会议提供 40 分钟，最多可容纳 100 名参会者。
  * [RingCentral](https://www.ringcentral.com/) - 提供免费计划的视频会议，每个会议提供 50 分钟，最多可容纳 100 名参与者。
  ...
  * [Revolt.chat](https://revolt.chat/) - [Discord](https://discord.com/)的开源替代品，尊重您的隐私。它还提供了 Discord 的大多数专有功能，而且是免费的。Revolt 是一个安全、快速且 100%免费的全功能应用程序。每个功能都是免费的。与大多数主流聊天应用程序不同，它还支持（官方和非官方）插件。
...
## 内容管理系统（CMS）
  ...
  * [Crystallize](https://crystallize.com) - 支持电子商务的无头 PIM。内置 GraphQL API。免费版本包括无限制的用户、1000 个目录项目、5GB/月带宽和 25k/月 API 调用。
  ...
  * [kontent.ai](https://www.kontent.ai) - 一个内容即服务平台，为您提供无头 CMS 的所有好处，同时赋予营销人员权力。开发者计划为两个用户提供无限项目，每个项目有两个环境，500 个内容项目，两种语言的交付和管理 API，以及自定义元素支持。您可以使用更详细的计划来满足您的需求。
  ...
  * [Sanity.io](https://www.sanity.io/) - 具有开源编辑环境和实时托管数据存储的结构化内容平台。无限项目。无限管理员用户、三个非管理员用户、两个数据集、500K API CDN 请求、10GB 带宽和每个项目免费 5GB 资产。
  ...
  * [TinaCMS](https://tina.io/) - 取代 Forestry.io。基于开源 Git 的无头 CMS，支持 Markdown、MDX 和 JSON。基本套餐免费，提供两个用户。
  ...
  * [Squidex](https://squidex.io/) - 为小型项目提供免费层级。API / GraphQL 优先。开源且基于事件源（自动跟踪每个更改）。
  * [InstaWP](https://instawp.com/) - 在几秒钟内启动 WordPress 网站。免费层级包括 5 个活动网站、500MB 空间、48 小时网站过期。
  * [Storyblok](https://www.storyblok.com) - 面向开发人员和营销人员的无头 CMS，与所有现代框架兼容。社区（免费）层级提供管理 API、可视化编辑器、十个源、自定义字段类型、国际化（无限语言/地区）、资产管理器（最多 2500 个资产）、图像优化服务、搜索查询、Webhook + 250GB 流量/月。
...
## 代码生成
  ...
  * [Metalama](https://www.postsharp.net/metalama)——仅适用于 C#。Metalama 在编译时动态生成代码的样板，从而保持您的源代码干净。它对开源项目免费，其商业友好的免费层包括三个方面。
  ...
  * [v0.dev](https://v0.dev/)——v0 使用 AI 模型根据简单的文本提示生成代码。它基于 shadcn/ui 和 Tailwind CSS 生成可复制粘贴的友好 React 代码，人们可以在他们的项目中使用。每次生成至少需要 30 个学分。您一开始有 1200 个学分，并且每月可以获得 200 个免费学分。
...
## 代码质量
  ...
  * [codescene.io](https://codescene.io/)——CodeScene 根据开发人员处理代码的方式优先处理技术债务，并可视化团队耦合和系统掌握等组织因素。对于开源免费。
  * [CodSpeed](https://codspeed.io)——在您的 CI 管道中自动跟踪性能。通过精确和一致的指标，在部署前捕获性能回归。对于开源项目免费永久。
  * [coveralls.io](https://coveralls.io/)——显示测试覆盖率报告，对于开源免费
  * [dareboost](https://dareboost.com)——每月免费提供 5 份针对网络性能、可访问性和安全性的分析报告
  * [deepcode.ai](https://www.deepcode.ai)——DeepCode 基于人工智能发现错误、安全漏洞、性能和 API 问题。DeepCode 的分析速度允许我们在您的 IDE 中保存按钮时实时分析您的代码并提供结果。支持的语言是 Java、C/C++、JavaScript、Python 和 TypeScript。与 GitHub、BitBucket 和 GitLab 的集成。对于开源和私有存储库以及最多 30 个开发人员免费。
  ...
  * [gerrithub.io](https://review.gerrithub.io/)——针对 GitHub 存储库的免费 Gerrit 代码审查
  ...
  * [Moderne.io](https://app.moderne.io)——自动源代码重构。Moderne 提供框架迁移、带修复的代码分析和无与伦比的大规模代码转换，因此开发人员可以将时间花在构建新事物上，而不是维护旧事物上。对于开源免费。
...
## 代码搜索和浏览
  ...
  * [libraries.io](https://libraries.io/) - 搜索 32 种不同包管理器的依赖更新通知，开源免费
  ...
  * [CodeKeep](https://codekeep.io) - 代码片段的 Google Keep。组织、发现和分享代码片段，具有强大的代码屏幕截图工具，带有预设模板和链接功能。
...
## 持续集成/持续部署（CI/CD）
  ...
  * [appcircle.io](https://appcircle.io) - 一个企业级移动 DevOps 平台，可自动构建、测试和发布移动应用程序，以实现更快、更高效的发布周期。免费使用，最长 30 分钟的构建时间，每月 20 次构建和 1 次并发构建。
  * [appveyor.com](https://www.appveyor.com/) - Windows 的持续交付服务，开源免费
  * [Argonaut](https://argonaut.dev/) - 在您的云平台上分钟级部署应用和基础架构。支持在 Kubernetes 和 Lambda 环境中自定义和第三方应用程序的部署。免费层允许 5 个域和 2 个用户无限制的应用和部署。
  * [bitrise.io](https://www.bitrise.io/) - 移动应用的持续集成/持续部署服务，每月有 200 次免费构建、10 分钟构建时间和两个团队成员。OSS 项目获得 45 分钟构建时间、+1 并发性和无限团队规模。
  * [buddy.works](https://buddy.works/) - 具有五个免费项目和一个并发运行（每月 120 次执行）的持续集成/持续部署服务
  * [Buildkite](https://buildkite.com) 持续集成管道免费使用 3 个用户和 5k 作业分钟/月。测试分析免费开发者层包括每月 100 万次测试执行，对于开源项目还有更多免费内容。
  * [bytebase.com](https://www.bytebase.com/) - 数据库持续集成/持续部署和 DevOps。免费使用，最多 20 个用户和 10 个数据库实例
  * [CircleCI](https://circleci.com/) - 全面的免费计划，包含在托管的持续集成/持续部署服务中，适用于 GitHub、GitLab 和 BitBucket 存储库的所有功能。多个资源类、Docker、Windows、Mac OS、ARM 执行器、本地运行器、测试拆分、Docker 层缓存和其他高级持续集成/持续部署功能。免费使用，最高可达 6000 分钟/月的执行时间，无限协作者，私有项目中的 30 个并行作业，以及开源项目的 80,000 个免费构建分钟。
  ...
  * [LayerCI](https://layerci.com) - 全栈项目的持续集成服务。一个具有 5GB 内存和 3 个 CPU 的全栈预览环境
  * [semaphoreci.com](https://semaphoreci.com/) - 开源免费，每月 100 个私有构建
  * [Squash Labs](https://www.squash.io/) - 为每个分支创建一个 VM，并使您的应用从一个独特的 URL 可用，无限公共和私有存储库，最多 2GB VM 大小。
  * [styleci.io](https://styleci.io/) - 仅限公共 GitHub 存储库
  * [Mergify](https://mergify.io) - GitHub 的工作流程自动化和合并队列 - 公共 GitHub 存储库免费
  * [Make](https://www.make.com/en) - 工作流程自动化工具，允许您使用 UI 连接应用程序并自动化工作流程。它支持许多应用程序和最流行的 API。公共 GitHub 存储库免费，免费层包含 100 Mb、1000 次操作和 15 分钟的最小间隔。
  * [Spacelift](https://spacelift.io/) - 基础设施即代码的管理平台。免费计划功能：IaC 协作、Terraform 模块注册表、ChatOps 集成、使用 Open Policy Agent 的持续资源合规性、SSO 与 SAML 2.0，以及对公共工作者池的访问：最多 200 分钟/月
  * [microtica.com](https://microtica.com/) - 具有即用型基础设施组件的启动环境，可在 AWS 上免费部署应用程序，并支持您的生产工作负载。免费层包括 1 个环境（在您的 AWS 账户上）、2 个 Kubernetes 服务、每月 100 个构建分钟和 20 个每月部署。
  * [Nx Cloud](https://nx.dev/ci) - Nx Cloud 通过远程缓存、任务在机器上的分布甚至自动拆分端到端测试运行等功能，加快了您的 monorepos 在 CI 上的速度。它提供了一个免费计划，最多可容纳 30 名贡献者，并包含慷慨的 150k 信用额度。
...
## 测试
  ...
  * [Applitools.com](https://applitools.com/) - 智能视觉验证工具，适用于网络、原生移动和桌面应用程序。与几乎所有自动化解决方案（如 Selenium 和 Karma）和远程运行器（Sauce Labs、Browser Stack）集成。开源免费。为每周检查点有限的单个用户提供免费层。
  ...
  * [BugBug](https://bugbug.io/) - 用于 Web 应用程序的轻量级测试自动化工具。易于学习，不需要编码。您可以在自己的计算机上免费运行无限次测试。您还可以获得云监控和 CI/CD 集成，额外每月收费。
  ...
  * [Checkly](https://checklyhq.com) - 用于现代 DevOps 的代码优先合成监控。以传统提供商一小部分的价格监控您的 API 和应用程序。开发人员的慷慨免费层。
  ...
  * [Gremlin](https://www.gremlin.com/gremlin-free-software) - Gremlin 的混沌工程工具允许您安全地将故障注入系统，以在出现问题之前发现弱点。Gremlin Free 提供对多达 5 个主机或容器的关闭和 CPU 攻击的访问权限。
  * [gridlastic.com](https://www.gridlastic.com/) - 具有免费计划的 Selenium 网格测试，最多可同时使用 4 个 Selenium 节点/10 个网格启动/4000 个测试分钟/月
  * [katalon.com](https://katalon.com) - 提供测试平台，可帮助不同测试成熟度级别的各个规模的团队，包括 Katalon Studio、TestOps（+视觉测试免费）、TestCloud 和 Katalon Recorder。
  * [Keploy](https://keploy.io/) - 用于开发人员的功能测试工具包。记录 API 调用为 API（KTests）生成端到端测试，并为 mocks 或存根（KMocks）。对于开源项目免费。
  ...
  * [preflight.com](https://preflight.com) - 无代码自动化 Web 测试。在您的浏览器上录制对 UI 更改有弹性的测试，并在 Windows 计算机上运行它们。可以与您的 CI/CD 集成吗？免费计划包括 50 个每月测试运行，带有视频、HTML 会话等。
  ...
  * [seotest.me](https://seotest.me/) - 免费的页面 SEO 网站测试工具。每天 10 次免费网站抓取。有关如何改善任何网站的页面 SEO 结果的有用 SEO 学习资源和建议，无论技术如何。
  * [snippets.uilicious.com](https://snippets.uilicious.com) - 它就像 CodePen 但用于跨浏览器测试。UI-licious 让您像用户故事一样编写测试，并提供一个免费平台 - UI-licious Snippets - 允许您在 Chrome 上无限制测试，无需注册，每次测试运行最多 3 分钟。发现错误？您可以复制测试的唯一 URL 并将其提供给开发人员，以确切地向他们展示如何重现该错误。
  ...
  * [Testspace.com](https://testspace.com/) - 使用 GitHub 作为代码的仪表板，用于发布自动化测试结果和框架，以实施手动测试。该服务[对开源免费](https://github.com/marketplace/testspace-com)，并为 450 个每月结果提供支持。
  ...
  * [UseWebhook.com](https://usewebhook.com) - 在浏览器中捕获和检查 Webhook。可转发到本地主机，或从历史记录中重放。免费使用。
...
## 安全和公钥基础设施（PKI）
  ...
  * [CyberChef](https://gchq.github.io/CyberChef/)——一个简单、直观的网络应用程序，用于分析和解码/编码数据，无需处理复杂的工具或编程语言。就像一个加密和加密的瑞士军刀。所有功能均可免费使用，无限制。如果您希望自行托管，可以使用开源版本。
  ...
  * [DJ Checkup](https://djcheckup.com)——使用此免费、自动化的检查工具扫描您的 Django 网站的安全漏洞。从 Pony Checkup 网站分叉。
  ...
  * [hostedscan.com](https://hostedscan.com)——在线 Web 应用程序、服务器和网络漏洞扫描程序。每月免费进行 10 次扫描。
  * [Infisical](https://infisical.com/)——开源平台，可让您在整个团队和基础架构中管理开发人员的秘密：从本地开发到登台/生产第三方服务。免费供最多 5 名开发人员使用。
  ...
  * [seclookup.com](https://www.seclookup.com/)——Seclookup API 可以丰富 SIEM 中的域名威胁指标，提供域名的全面信息，并提高威胁检测和响应能力。在此处获得 50K 次查找的免费访问权限[https://account.seclookup.com/]。
  ...
  * [StackHawk](https://www.stackhawk.com/)——在整个管道中自动扫描应用程序，以在安全漏洞进入生产环境之前找到并修复它们。单个应用程序的无限制扫描和环境。
  ...
  * [Vulert](https://vulert.com)——Vulert 持续监控您的开源依赖项以发现新的漏洞，推荐修复方法，无需安装或访问您的代码库。适用于开源项目。

  * [HasMySecretLeaked](https://gitguardian.com/hasmysecretleaked) - 在公共 GitHub 存储库、gist、问题和评论中搜索 2000 万个泄露的秘密，免费

**[⬆️ 返回顶部](# 目录 )**

## 身份验证、授权和用户管理

  * [Aserto](https://www.aserto.com) - 应用和 API 的细粒度授权即服务。免费使用额度为 1000 MAUs 和 100 个授权器实例。
  * [asgardeo.io](https://wso2.com/asgardeo) - 无缝集成单点登录、多因素身份验证、无密码身份验证等。包括适用于前端和后端应用的 SDK。免费使用额度为 1000 MAUs 和五个身份提供商。
  * [Auth0](https://auth0.com/) — 托管式单点登录。免费计划包括 25,000 MAUs、不限数量的社交连接、自定义域名等。
  * [Authgear](https://www.authgear.com) - 在数分钟内为您的应用带来无密码、一次性密码、双因素身份验证、单点登录等功能。所有前端均包含在内。免费使用额度为 5000 MAUs。
  ...
  * [Corbado](https://www.corbado.com/) — 为新应用或现有应用添加密码优先身份验证。免费使用额度为无限 MAUs。
  * [Descope](https://www.descope.com/) — 高度可定制的身份验证流程，同时提供无代码和 API/SDK 方法，免费使用额度为每月 7,500 个活跃用户，50 个租户（最多 5 个 SAML/SSO 租户）。
  * [duo.com](https://duo.com/) — 网站或应用的双因素身份验证（2FA）。免费使用额度为 10 个用户，所有身份验证方法、不限数量的集成、硬件令牌。
  * [Kinde](https://kinde.com/) - 简单、强大的身份验证，您可以在几分钟内集成到您的产品中。开始使用的一切您都需要 7,500 个免费 MAU。
  * [logintc.com](https://www.logintc.com/) — 通过推送通知进行双因素身份验证（2FA），免费使用额度为 10 个用户，VPN、网站和 SSH
  * [MojoAuth](https://mojoauth.com/) - MojoAuth 使您可以在几分钟内轻松实现您的网络、移动或任何应用的无密码身份验证。
  * [Okta](https://developer.okta.com/signup/) — 用户管理、身份验证和授权。免费使用额度为最多 100 个每月活跃用户。
  * [onelogin.com](https://www.onelogin.com/) — 身份即服务（IDaaS）、单点登录身份提供商、云 SSO IdP、三个公司应用和五个个人应用，无限制用户
  * [Ory](https://ory.sh/) - 身份验证/授权/OAuth2.0/零信任管理安全平台。具有所有安全功能的永久免费开发者帐户，无限团队成员，每天 200 个活跃用户，每月 25k 次权限检查。
  * [Phase Two](https://phasetwo.io) - Keycloak 开源身份和访问管理。免费领域最多 1000 个用户，最多 10 个单点登录连接，利用 Phase Two 的增强容器，其中包括[组织](https://phasetwo.io/product/organizations/)扩展。
  * [Stytch](https://www.stytch.com/) - 具有强大密码保护功能的灵活身份验证 API 和 SDK，无密码登录流程、MFA、单点登录等。内置电子邮件/短信发送+故障转移和欺诈保护。提供 5,000 个每月活跃用户免费（B2C）或 1,000 个每月活跃用户和 25 个组织免费（B2B）。
  * [Stack Auth](https://stack-auth.com) — 开源身份验证，不会让您失望。最适合开发者的解决方案，只需五分钟即可开始使用。免费自托管，或提供带有 10k 免费每月活跃用户的托管 SaaS 版本。
  * [SuperTokens](https://supertokens.com/) - 开源用户身份验证，可本机集成到您的应用中-使您能够快速启动，同时控制用户和开发人员体验。免费使用额度为最多 5000 MAUs。
  ...
  * [Logto](https://logto.io/) - 开发、保护和管理您产品的用户身份（用于身份验证和授权）。免费使用额度为最多 5,000 MAUs，提供开源自托管选项。
  * [WorkOS](https://workos.com/) - 免费用户管理和身份验证，最多可支持 100 万 MAUs。支持电子邮件+密码、社交身份验证、Magic Auth、MFA 等。
...
## 移动应用分发和反馈
  ...
  * [Loadly](https://loadly.io) - iOS 和 Android 测试版应用程序分发服务提供完全免费的服务，不限下载次数、高速下载和无限制上传。
  * [Diawi](https://www.diawi.com) - 直接将 iOS 和 Android 应用部署到设备上。免费计划：应用上传、密码保护链接、一天有效期、十次安装。
  * [InstallOnAir](https://www.installonair.com) - 通过空中分发 iOS 和 Android 应用。免费计划：无限制上传、私人链接、来宾两天有效期、注册用户六十天。
...
## 消息和流
  ...
  * [cloudamqp.com](https://www.cloudamqp.com/) - RabbitMQ 即服务。Little Lemur 计划：每月最多 100 万条消息，最多 20 个并发连接，最多 100 个队列，最多 10,000 条排队消息，不同可用区的多个节点
  ...
  * [pusher.com](https://pusher.com/) - 实时消息服务。免费用于最多 100 个并发连接和 200,000 条消息/天
  * [scaledrone.com](https://www.scaledrone.com/) - 实时消息服务。免费用于最多 20 个并发连接和 100,000 个事件/天
...
## 翻译管理
  ...
  * [crowdin.com](https://crowdin.com/) - 开源项目不限量，字符串不限量，协作不限量
  * [gitlocalize.com](https://gitlocalize.com) - 私有和公共存储库都免费且不限量
  ...
  * [Loco](https://localise.biz/) - 免费 2000 次翻译，不限译者，每个项目 10 种语言，1000 个可翻译资产/项目
  ...
  * [Tolgee](https://tolgee.io) - 免费 SaaS 产品，提供有限的翻译量，永久免费的自托管版本
  ...
  * [weblate.org](https://weblate.org/) - 对于免费层，自由项目是免费的，最多可有 10,000 个字符串源，对于自托管版本则不限
...
## 监控
  ...
  * [UptimeObserver.com](https://uptimeobserver.com/) - 获取 20 个具有 5 分钟间隔的监控器和可定制的状态页面-甚至可用于商业用途。通过电子邮件和 Telegram 享受无限的实时通知。无需信用卡即可开始使用。
  * [Pingmeter.com](https://pingmeter.com/) - 5 个具有 10 分钟间隔的监控器。监控 SSH、HTTP、HTTPS 和任何自定义 TCP 端口。
  * [alerty.ai](https://www.alerty.ai) - 为 FE、BE、DB 等提供免费的 APM 和监控+免费代理运行。
  ...
  * [economize.cloud](https://economize.cloud) - 通过组织云资源以优化和报告相同的内容来帮助您了解云基础设施成本。免费适用于每月在 Google Cloud Platform 上花费最多 5,000 美元的情况。
  * [elastic.co](https://www.elastic.co/solutions/apm) - 为 JavaScript 开发人员提供即时性能洞察。24 小时数据保留免费。
  ...
  * [healthchecks.io](https://healthchecks.io) - 监控您的 cron 作业和后台任务。免费最多 20 次检查。
  * [Hydrozen.io](https://hydrozen.io) - 监控 & 状态页面，免费计划：10 个监控器，5 个心跳监控器，1 个域名监控器和 1 个状态页面免费。
  ...
  * [linkok.com](https://linkok.com) - 在线断链检查，免费适用于最多 100 页的小型网站，完全免费适用于开源项目。
  ...
  * [Middleware.io](https://middleware.io/) - 中间件观测平台提供对您的应用程序和堆栈的完整可见性，因此您可以大规模监控和诊断问题。他们为开发人员社区提供免费的永久计划，允许 Log 监控最多 100 万日志事件，基础设施监控和 APM 最多 2 个主机。
  * [nixstats.com](https://nixstats.com) - 免费 1 台服务器。电子邮件通知、公共状态页面、60 秒间隔等。
  * [OnlineOrNot.com](https://onlineornot.com/) - OnlineOrNot 为网站和 API 提供监控，为 cron 作业和计划任务提供监控。还提供状态页面。前五个检查间隔 3 分钟免费。免费层通过 Slack、Discord 和电子邮件发送警报。
  ...
  * [sematext.com](https://sematext.com/) - 24 小时指标免费，无限制服务器，10 个自定义指标，500 万个自定义指标数据点，无限制仪表板，用户等。
  ...
  * [instatus.com](https://instatus.com) - 在 10 秒内获得漂亮的状态页面。无限子域名和无限团队永久免费
  * [Squadcast.com](https://squadcast.com) - Squadcast 是一款端到端事件管理软件，旨在帮助您推广 SRE 最佳实践。免费永久计划最多可供 10 个用户使用
  * [RoboMiri.com](https://robomiri.com/) - RoboMiri 是一个稳定的正常运行时间监视器，提供多种监视器：cronjob、关键字、网站、端口、ping。免费提供 25 次正常运行时间检查，间隔 3 分钟检查。通过电话呼叫、短信、电子邮件和 Webhooks 进行警报
...
## Crash and Exception Handling
  ...
  * [exceptionless](https://exceptionless.com)——实时错误、功能、日志报告等。每月免费 3k 事件/1 用户。开源且易于自托管，无限使用。
  * [GlitchTip](https://glitchtip.com/)——简单的开源错误跟踪。与开源 Sentry SDK 兼容。每月免费 1000 个事件，或可自托管，无限制。
  ...
  * [Axiom](https://axiom.co/)——存储最多 0.5 TB 的日志，保留 30 天。包括与 Vercel 等平台的集成以及通过电子邮件/Discord 通知器进行高级数据查询。
  * [Semaphr](https://semaphr.com)——适用于移动应用的免费一体化 Kill Switch。
  * [Jam](https://jam.dev)——一键式开发者友好的错误报告。免费计划无限 Jams。
  * [Whitespace](https://whitespace.dev)——一键式错误报告，直接在您的浏览器中。免费计划个人使用无限制录制。

**[⬆️ 返回顶部](# 目录)**

## 搜索

  * [algolia.com](https://www.algolia.com/)——具有容错性、相关性和用户界面库的托管搜索解决方案，可轻松创建搜索体验。免费的“Build”计划包括 100 万份文档和 10,000 次搜索/月。还提供免费的[开发者文档搜索](https://docsearch.algolia.com/)。
...
## 教育和职业发展
  ...
  * [The Odin Project](https://www.theodinproject.com/) - 免费、开源的平台，课程专注于 JavaScript 和 Ruby 用于 Web 开发。
  * [Free Professional Resume Templates & Editor](https://www.overleaf.com/latex/templates/tagged/cv) - 免费平台，有大量经验丰富的专业人士的简历模板，可完全克隆、编辑和下载，ATS 优化。
...
## 电子邮件
  ...
  * [AnonAddy](https://anonaddy.com) - 开源匿名邮件转发，可免费创建无限个邮件别名
  * [Antideo](https://www.antideo.com) - 免费版每小时提供 10 次 API 请求，用于电子邮件验证、IP 和电话号码验证。不需要信用卡。
  ...
  * [CloudMailin](https://www.cloudmailin.com/) - 通过 HTTP POST 接收邮件和事务性出站邮件 - 每月免费 10,000 封邮件
  ...
  * [EmailOctopus](https://emailoctopus.com) - 免费提供最多 2500 个订阅者和 10,000 封邮件
  * [EmailJS](https://www.emailjs.com/) - 这不是完整的电子邮件服务器；这只是一个电子邮件客户端，您可以使用它从客户端直接发送电子邮件，而无需暴露您的凭据，免费套餐每月有 200 次请求、2 个电子邮件模板、请求高达 50KB、有限的联系人历史记录。
  ...
  * [Temp-Mail.org](https://temp-mail.org/en/) - 利用多种域名的临时/一次性邮件生成实用程序。每次重新加载页面时，电子邮件地址都会刷新。完全免费，不包含任何服务定价。
  ...
  * [FakeMailGenerator.com](https://www.fakemailgenerator.com/) - 德国临时/一次性邮件生成器。支持 10 个域名，同时允许您自由创建无限个地址。（包含广告）但除此之外，该服务不包含任何定价，完全免费。
  * [forwardemail.net](https://forwardemail.net) - 免费自定义域邮件转发。使用您的域名创建和转发无限数量的电子邮件地址（**注意**：如果您使用.casa、.cf、.click、.email、.fit、.ga、.gdn、.gq、.lat、.loan、.london、.men、.ml、.pl、.rest、.ru、.tk、.top、.work TLD，则需要付费）
  ...
  * [EForw](https://www.eforw.com) - 免费为一个域转发邮件。从您的域接收和发送电子邮件。
  ...
  * [Mailchimp](https://mailchimp.com/) - 500 个订阅者和 1000 封邮件/月免费。
  * [Maildroppa](https://maildroppa.com) - 最多 100 个订阅者和无限封邮件以及免费的自动化功能。
  * [MailerLite.com](https://www.mailerlite.com) - 每月 1000 个订阅者，每月 12000 封邮件免费
  ...
  * [Mailtrap.io](https://mailtrap.io/) - 用于开发的虚假 SMTP 服务器，免费计划包括 1 个收件箱、100 条消息、没有团队成员、每秒 2 封电子邮件、没有转发规则。
  * [Mail7.io](https://www.mail7.io/) - 免费的临时电子邮件地址。通过 Web 界面或 API 即时创建电子邮件地址。
  * [Momentary Email](https://www.momentaryemail.com/) - 免费的临时电子邮件地址。在网站上或通过 RSS -feed 读取传入的电子邮件。
  * [Mutant Mail](https://www.mutantmail.com/) - 免费 10 个电子邮件 ID，1 个域，1 个邮箱。所有电子邮件 ID 使用单个邮箱。
  ...
  * [Resend](https://resend.com) - 开发人员的事务性电子邮件 API。每月 3000 封邮件，每天 100 封邮件免费，1 个自定义域。
  * [Sender](https://www.sender.net) - 每月最多 15000 封邮件，最多 2500 个订阅者
  * [SendGrid](https://sendgrid.com/) - 每天 100 封邮件和 2000 个联系人免费
  * [Sendpulse](https://sendpulse.com) - 每月 500 个订阅者，每月 15000 封邮件免费
  ...
  * [Zoho](https://www.zoho.com)——最初是作为电子邮件提供商，但现在提供一系列服务，其中一些提供免费计划。提供免费计划的服务列表：
     - [电子邮件](https://zoho.com/mail)免费供 5 个用户使用。每个用户 5GB 和 25MB 附件限制，一个域名
     ...
     - [Desk](https://zoho.com/desk)——具有 3 个代理、私人知识库和电子邮件票证的客户支持管理。与[Assist](https://zoho.com/assist)集成，用于 1 个远程技术人员和 5 台无人值守计算机
...
## 功能切换管理平台
  ...
  * [ConfigCat](https://configcat.com) - ConfigCat 是一款以开发者为中心的功能标志服务，具有无限的团队规模、出色的支持和合理的价格标签。免费计划可支持最多 10 个标志、两个环境、1 个产品和每月 500 万次请求。
  * [Flagsmith](https://flagsmith.com) - 自信地发布功能；跨网络、移动和服务器端应用程序管理功能标志。使用我们的托管 API，部署到您自己的私有云，或在本地运行。
  * [GrowthBook](https://growthbook.io) - 开源功能标志和 A/B 测试提供程序，内置贝叶斯统计分析引擎。免费最多可支持 3 个用户，无限制的功能标志和实验。
  * [Hypertune](https://www.hypertune.com) - 类型安全的功能标志、A/B 测试、分析和应用程序配置，具有 Git 风格的版本控制和同步、内存中、本地标志评估。免费最多可支持 5 名团队成员，无限制的功能标志和 A/B 测试。
  * [Molasses](https://www.molasses.app) - 强大的功能标志和 A/B 测试。免费最多可支持 3 个环境，每个环境有五个功能标志。
  * [Toggled.dev](https://www.toggled.dev) - 企业级、可扩展的多区域功能切换管理平台。免费计划最多可支持 10 个标志、两个环境、无限制的请求。SDK、分析仪表板、发布日历、Slack 通知和所有其他功能都包含在无尽的免费计划中。
  * [Statsig](https://www.statsig.com) - 一个功能管理、A/B 测试、分析等的强大平台。其慷慨的免费计划提供无限座位、标志、实验和动态配置，每月最多支持 100 万次事件。
...
## 字体
  ...
  * [fonts.xz.style](https://fonts.xz.style/) 使用 CSS 将字体家族提供给网站的免费和开源服务。
...
## 表单
  ...
  * [Feathery](https://feathery.io) - 强大且便于开发者使用的表单构建器。可构建注册与登录、用户引导、支付流程、复杂金融应用等。免费计划允许每月最多 250 次提交和五个活动表单。
  * [Form-Data](https://form-data.com) - 无代码表单后端。提供垃圾邮件过滤、电子邮件通知和自动回复、Webhooks、zapier、重定向、AJAX 或 POST 等功能。免费计划提供无限表单、每月 20 次提交，以及带有 Form-Data 徽章的额外 2000 次提交。
  ...
  * [FormKeep.com](https://www.formkeep.com/) - 无限表单，每月 50 次提交，垃圾保护，电子邮件通知，以及可导出 HTML 的拖放设计器。其他功能包括自定义字段规则、团队以及与 Google Sheets、Slack、ActiveCampaign 和 Zapier 的集成。
  ...
  * [Formlick.com](https://formlick.com) - Typeform 替代品，具有终身交易。用户可以创建一个免费表单并接收无限提交。在高级版中，用户可以创建无限表单和无限提交。
  ...
  * [WaiverStevie.com](https://waiverstevie.com) - 具有 REST API 的电子签名平台。您可以使用 Webhooks 接收通知。免费计划对已签署的文件加水印，但允许无限的信封+签名。
  ...
  * [SimplePDF.eu](https://simplepdf.eu/embed) - 在您的网站上嵌入 PDF 编辑器，并将任何 PDF 转换为可填写的表单。免费计划允许每月无限 PDF，每个 PDF 有三次提交。
  * [forms.app](https://forms.app/)——使用条件逻辑、自动评分计算器和 AI 等强大功能创建在线表单。免费计划可收集最多 100 次响应，可嵌入网站，或使用链接使用。
  * [Qualli](https://usequalli.com)——应用内调查，专为移动设备设计。使用 Qualli AI 打造完美问题。您可以在我们的免费计划中试用，最高可达 500 MAU，创建无限表单和触发器。
  * [Sprig](https://sprig.com/)——每月 1 次产品内调查或带有 Replay 的调查，具有 GPT 驱动的 AI 分析。
...
## 生成式人工智能（GenerativeAI）
  ...
  * [Portkey](https://portkey.ai/) - 面向 Gen AI 应用的控制面板，提供可观测性套件和 AI 网关。每月免费发送和记录高达 10,000 个请求。
  * [Braintrust](https://www.braintrustdata.com/) - Gen AI 的评估、提示符游乐场和数据管理。免费计划每周提供高达 1,000 条私有评估行。
  * [Findr](https://www.usefindr.com/) - 通用搜索，可一次搜索所有应用。搜索助手，可使用您的信息回答问题。免费计划提供无限统一搜索和 5 个每日合作驾驶查询。
  * [ReportGPT](https://report-gpt.io) - AI 写作助手。整个平台只要您提供自己的 API 密钥，就是免费的。
  * [Clair](https://askclair.ai/) - 临床 AI 参考。学生可以免费访问专业工具套件，包括 Open Search、临床摘要、Med Review、药物相互作用、ICD-10 代码和管理。此外，还提供专业套件的免费试用。
  ...
  * [Pollinations.AI](https://pollinations.ai/) - 易于使用的免费图像生成 AI，提供免费 API。无需注册或 API 密钥，并且有多种集成到网站或工作流程的选项。[#开源](https://github.com/pollinations/pollinations)
...
## 内容分发网络（CDN）和防护
  ...
  * [raw.githack.com](https://raw.githack.com/)——**rawgit.com**的现代替代品，它只是使用 Cloudflare 托管文件
  * [section.io](https://www.section.io/)——一种简单的方式来启动和管理完整的 Varnish 缓存解决方案。据称对于一个站点来说是免费的永久使用
  * [statically.io](https://statically.io/)——Git 存储库（GitHub、GitLab、Bitbucket）、WordPress 相关资产和图像的 CDN
  * [toranproxy.com](https://toranproxy.com/)——Packagist 和 GitHub 的代理。从不失败的 CD。免费供个人使用，一个开发者，不提供支持
...
## 平台即服务（PaaS）
  ...
  * [gigalixir.com](https://gigalixir.com/)——Gigalixir 提供一个永不休眠的免费实例和一个免费的 PostgreSQL 数据库，限制为 2 个连接、10,000 行且不备份 Elixir/Phoenix 应用。
  ...
  * [ampt.dev](https://getampt.com/)——Ampt 让团队能够在 AWS 上构建、部署和扩展 JavaScript 应用程序，无需复杂的配置或管理基础设施。免费预览计划包括每小时 500 次调用、每天 2500 次调用和每月 50,000 次调用。自定义域仅在付费计划中允许。
  * [Koyeb](https://www.koyeb.com)——Koyeb 是一个面向开发者的无服务器平台，可在全球范围内部署应用程序。通过基于 git 的部署、本机自动缩放、全球边缘网络和内置的服务网格和发现，无缝运行 Docker 容器、Web 应用程序和 API。免费实例可让您在德国法兰克福或美国华盛顿特区部署 Web 服务。免费托管的 Postgres 数据库可在德国法兰克福（德国）、美国华盛顿特区（美国）和新加坡使用。512MB 内存、2GB 存储和 0.1 CPU。无需信用卡即可开始使用。
  * [Napkin](https://www.napkin.io/)——具有 500Mb 内存、默认超时 15 秒和每月 5000 次免费 API 调用（速率限制为每秒 5 次调用）的 FaaS。
  * [Meteor Cloud](https://www.meteor.com/cloud)——Galaxy 托管。Meteor 的即服务平台，用于 Meteor 应用程序，包括免费的 MongoDB 共享托管和自动 SSL。
  ...
  * [WunderGraph](https://cloud.wundergraph.com)——一个开源平台，允许您快速构建、交付和管理现代 API。内置 CI/CD、GitHub 集成和自动 HTTPS。免费计划每月最多 3 个项目、1GB 出站、300 分钟构建时间。
...
## 后端即服务（BaaS）
  ...
  * [Activepieces](https://www.activepieces.com) - 构建自动化流程，将多个应用程序连接到您的应用程序后端。例如，当您的应用程序中发生事件时，发送一条 Slack 消息或在 Google Sheet 中添加一行。每月免费使用高达 5000 个任务。
  ...
  * [convex.dev](https://convex.dev/) - 作为服务的反应式后端，托管您的数据（具有关系和可序列化 ACID 事务的文档）、无服务器函数和 WebSockets，以将更新流式传输到各种客户端。小型项目免费 - 最多 1M 记录，每月 5M 函数调用。
  ...
  * [hasura.io](https://hasura.io/) - Hasura 扩展您在任何地方托管的现有数据库，并提供即时 GraphQL API，可安全访问网络、移动和数据集成工作负载。免费 1GB/月的数据通过量。
  ...
  * [onesignal.com](https://onesignal.com/) - 无限免费推送通知。每月 10,000 封电子邮件发送，无限制的联系人，并可访问自动预热。
  ...
  * [Salesforce Developer Program](https://developer.salesforce.com/signup) - 使用拖放工具快速构建、测试和部署应用程序。自定义数据模型只需点击一下。使用 Apex 代码更深入。使用强大的 API 与任何内容集成。使用企业级安全性保护自己。使用点击或任何前沿网络框架自定义 UI。免费开发者计划可访问完整的 Lightning 平台。
  ...
  * [Singlebase.cloud](https://singlebase.cloud) - SinglebaseCloud 是一个由人工智能驱动的一体化后端平台，可加速应用程序开发。它提供了工具，如 Vector DB、关系文档 DB、身份验证、搜索和存储，旨在简化后端开发。免费/入门计划提供关系文档 DB、身份验证、搜索、存储。
...
## 低代码平台
  ...
  * [ReTool](https://retool.com/)——用于构建内部应用程序的低代码平台。Retool 非常易于定制。如果您可以使用 JavaScript 和 API 编写它，那么您可以在 Retool 中实现它。免费套餐允许每月每个用户最多五个用户，无限应用程序和 API 连接。
  ...
  * [outsystems.com](https://www.outsystems.com/)——企业网络开发 PaaS，适用于内部部署或云，免费的“个人环境”提供无限代码和最多 1GB 数据库。
  ...
  * [Mendix](https://www.mendix.com/)——企业快速应用开发，无限可访问的沙盒环境支持总用户，每个应用程序 0.5GB 存储和 1GB RAM。免费套餐还允许使用 Studio 和 Studio Pro IDE。
...
## 虚拟主机服务
  ...
  * [Fenix Web Server](https://preview.fenixwebserver.com)——用于在本地托管站点并公开共享（实时）的开发者桌面应用程序。使用其美丽的用户界面、API 和/或 CLI，以您喜欢的方式工作。
  ...
  * [Freehostia](https://www.freehostia.com)——提供免费托管服务，包括业内最佳的控制面板和 50+免费应用程序的一键安装。即时设置。无强制广告。
  ...
  * [Kinsta Static Site Hosting](https://kinsta.com/static-site-hosting/)——免费部署多达 100 个静态站点，自定义域名带 SSL，100GB 月带宽，260+Cloudflare CDN 位置。
  * [Lecturify](https://www.lecturify.net/index.en.html)——具有 SFPT 访问权限的文件上传和下载的 Web 托管，支持 php。
  ...
  * [tilda.cc](https://tilda.cc/)——一个站点，50 页，50MB 存储空间，仅可用的 170 多个主要预定义块之一，无字体，无 favicon，无自定义域名。
...
## 域名系统（DNS）
  ...
  * [1.1.1.1](https://developers.cloudflare.com/1.1.1.1/) - 由 Cloudflare 提供的免费公共 DNS 解析器，速度快且安全（加密 DNS 查询）。可用于绕过互联网提供商的 DNS 阻止、防止 DNS 查询监控以及[阻止成人和恶意软件内容](https://developers.cloudflare.com/1.1.1.1/1.1.1.1-for-families)。还可以通过[API 使用](https://developers.cloudflare.com/1.1.1.1/encrypted-dns/dns-over-https/make-api-requests)。注意：只是 DNS 解析器，不是 DNS 主机提供商。
  ...
  * [sslip.io](https://sslip.io/)——免费 DNS 服务，当使用包含嵌入 IP 地址的主机名查询时，返回该 IP 地址。
...
## 基础设施即服务（IaaS）
  ...
  * [filebase.com](https://filebase.com/)——区块链驱动的 S3 兼容对象存储。5GB 免费存储空间，无限期使用
...
## 托管数据服务
  ...
  * [codehooks.io](https://codehooks.io/) — 使用 JavaScript 无服务器 API/后端和 NoSQL 数据库服务，具有功能、类似于 Mongdb 的查询、键/值查找、作业系统、实时消息、工作队列、强大的 CLI 和基于网络的数据管理器。免费计划有 5GB 存储空间和每分钟 60/API 调用。包含 2 名开发人员。无需信用卡。
  ...
  * [Upstash](https://upstash.com/) — 无服务器 Redis，免费层每天高达 10,000 次请求、256MB 最大数据库大小和 20 个并发连接
  ...
  * [SeaTable](https://seatable.io/) — 由 Seafile 团队构建的灵活、类似电子表格的数据库。无限制的表、2000 行、1 个月的版本控制，最多 25 个团队成员。
  ...
  * [Turso by ChiselStrike](https://chiselstrike.com/) - Turso 是边缘数据库中的 SQLite 开发体验。Turso 提供免费永久入门计划，总存储量为 9GB，最多可创建 500 个数据库，最多可设置 3 个位置，每月读取 10 亿行数据，并提供本地开发支持和 SQLite。
  ...
  * [Neon](https://neon.tech/) — 托管 PostgreSQL，总存储量为 0.5GB，1 个项目，10 个分支，无限制的数据库，始终可用的主分支（非主分支计算后 5 分钟自动暂停），每月 20 小时的活动时间
  ...
  * [TigerGraph Cloud](https://www.tigergraph.com/cloud/) — 管理原生图形数据库/分析平台，具有类似 SQL 的图形查询语言和 REST API。一个免费实例，两个 vCPU、8GB 内存和 50GB 存储，闲置 1 小时后睡眠。
  * [TerminusCMS](https://terminusdb.com/pricing) — TerminusDB 的托管免费服务，一种用 Prolog 和 Rust 编写的文档和图形数据库。免费供开发使用，付费服务用于企业部署和支持。
  ...
  * [8base.com](https://www.8base.com/) - 8base 是一个为 JavaScript 开发人员构建的全栈低代码开发平台，建立在 MySQL 和 GraphQL 之上以及无服务器后端即服务之上。它允许您使用 UI 应用程序构建器快速构建 Web 应用程序并快速扩展，免费层包括行数：2500，存储：500，无服务器计算：1Gb/h，客户端应用程序用户：5。
  * [Nile](https://www.thenile.dev/) — 一个用于 B2B 应用程序的 Postgres 平台。无限制的数据库，始终可用，无停机，总存储量为 1GB，5000 万个查询令牌，自动缩放，无限的向量嵌入
...
## 隧道、WebRTC、WebSocket服务器和其他路由器
  ...
  * [Hamachi](https://www.vpn.net/) - LogMeIn Hamachi 是一种托管 VPN 服务，允许您使用免费计划（最多 5 人）创建无限个网络，将分布在各地的团队安全地连接到类似 LAN 的网络。
  ...
  * [Tailscale](https://tailscale.com/) - 使用开放源代码的 WireGuard 协议的零配置 VPN。可安装在 MacOS、iOS、Windows、Linux 和 Android 设备上。个人使用的免费计划可容纳 100 个设备和 3 个用户。
  * [webhookrelay.com](https://webhookrelay.com) - 管理、调试、分流和代理您所有的 Webhook 到公共或内部（即 localhost）目的地。此外，通过获取公共 HTTP 终结点（`https://yoursubdomain.webrelay.io <----> http://localhost:8080`），可以将运行在私有网络中的服务器暴露到隧道中。
  * [Hookdeck](https://hookdeck.com/pricing) - 随时随地开发、测试和监控您的 Webhook。每月 100K 请求和 100K 尝试，保留三天。
  * [Xirsys](https://www.xirsys.com/pricing/) - 无限 STUN 使用量+每月 500MB TURN 带宽，限制带宽，单个地理区域。
  * [ZeroTier](https://www.zerotier.com) - 免费的托管虚拟以太网即服务。免费计划可在一个网络中容纳 25 个客户端的无限制端到端加密网络。客户端适用于桌面/移动/NA；Web 界面用于配置自定义路由规则和批准私有网络上的新客户端节点。
...
## 问题跟踪和项目管理
  ...
  * [Backlog](https://backlog.com)——您的团队在一个平台上发布出色项目所需的一切。免费计划提供 1 个项目，10 个用户和 100MB 存储空间。
  ...
  * [clickup.com](https://clickup.com/)——项目管理。免费，带云存储的高级版。提供移动应用程序和 Git 集成。
  ...
  * [diagrams.net](https://app.diagrams.net/)——本地存储在 Google Drive、OneDrive 或 Dropbox 中的在线图表。免费提供所有功能和存储级别
  ...
  * [WishKit](https://wishkit.io)——为您的 iOS/macOS 应用收集应用内用户反馈，并根据用户投票确定功能优先级。免费计划最多适用于 1 个应用程序。
  ...
  * [leiga.com](https://www.leiga.com/)——Leiga 是一款使用 AI 自动管理项目的 SaaS 产品，可帮助您的团队集中注意力并释放巨大潜力，确保项目按计划推进。免费供最多 10 个用户、20 个自定义字段、2GB 存储空间使用，视频录制与 AI 限制为 5 分钟/视频，自动化运行频率为 20/用户/月。
  * [Lucidchart](https://www.lucidchart.com/)——具有协作功能的在线图表工具。免费计划有三个可编辑文档、100 个专业模板和基本协作功能。
  * [MeisterTask](https://www.meistertask.com/)——面向团队的在线任务管理。免费供最多 3 个项目和无限数量的项目成员使用。
  * [MeuScrum](https://www.meuscrum.com/en)——免费的在线 Scrum 工具，带有看板
  * [nTask](https://www.ntaskmanager.com/)——项目管理软件，可让您的团队协作、计划、分析和管理日常任务。Essential 计划永久免费，提供 100MB 存储空间和 5 个用户/团队。无限工作空间、会议、任务分配、时间表和问题跟踪。
  ...
  * [Shortcut](https://shortcut.com/) - 项目管理平台。免费版最多可容纳 10 名用户，永久免费。
  * [SpeedBoard](https://speedboard.app) - 用于敏捷和 Scrum 回顾的看板 - 免费。
  * [Tadum](https://tadum.app) - 为定期会议设计的会议议程和纪要应用程序，团队规模达 10 人以内免费
  * [taiga.io](https://taiga.io/) — 适用于初创公司和敏捷开发者的项目管理平台，开源免费
  * [Tara AI](https://tara.ai/) — 简单的冲刺管理服务。免费版无用户限制，任务、冲刺和工作区数量无限制。
  * [targetprocess.com](https://www.targetprocess.com/) — 从看板和 Scrum 到几乎任何运营流程的可视化项目管理。免费版无用户限制，最多可容纳 1,000 个数据实体 [{更多详情](https://www.targetprocess.com/pricing/) }
  * [taskade.com](https://www.taskade.com/) — 实时协作任务列表和团队大纲。免费版有一个工作区，任务和项目数量不限；1GB 文件存储；项目历史记录保留一周；视频会议每次最多可容纳五名参会者。
  * [taskulu.com](https://taskulu.com/) — 基于角色的项目管理。免费版最多可容纳 5 名用户。与 GitHub/Trello/Dropbox/Google Drive 集成
  * [Teaminal](https://www.teaminal.com) - 适用于远程团队的站立会议、回顾和冲刺规划工具。免费版最多可容纳 15 名用户。
  ...
  * [terrastruct.com](https://terrastruct.com/) — 专门用于软件架构的在线图表制作工具。免费版每个图表最多可容纳 4 层。
  * [todoist.com](https://todoist.com/) — 协作和个人任务管理。免费版：5 个活跃项目、项目中 5 名用户、上传文件最大 5MB、3 个过滤器和一周活动历史记录。
  ...
  * [Yodiz](https://www.yodiz.com/) — 敏捷开发和问题跟踪。免费版最多可容纳 3 名用户，无限制项目。
  * [YouTrack](https://www.jetbrains.com/youtrack/buy/#edition=incloud) — 免费托管的 YouTrack（InCloud）适用于 FOSS 项目和私人项目（免费供三名用户使用）。包括时间跟踪和敏捷看板
  * [zenhub.com](https://www.zenhub.com) — GitHub 内部唯一的项目管理解决方案。免费版适用于公共存储库、OSS 和非营利组织
  * [zenkit.com](https://zenkit.com) — 项目管理和协作工具。免费版最多可容纳 5 名成员，5GB 附件。
  * [Zube](https://zube.io) — 具有免费计划的项目管理，适用于 4 个项目和 4 名用户。支持 GitHub 集成。
  * [Toggl](https://toggl.com/) — 提供两个免费的生产力工具。[Toggl Track](https://toggl.com/track/)用于时间管理和跟踪应用程序，免费版提供无缝时间跟踪和报告，专为自由职业者设计。它具有无限的跟踪记录、项目、客户、标签、报告等。以及[Toggl Plan](https://toggl.com/plan/)用于任务规划，免费版适用于独奏开发人员，任务、里程碑和时间线数量无限制。
...
## 存储和媒体处理
  ...
  * [cloudimage.io](https://www.cloudimage.io/en/home)——全面的图像优化和 CDN 服务，在全球拥有 1500 多个接入点。提供各种图像调整大小、压缩和水印功能。具有响应式图像、360 度图像制作和图像编辑的开源插件。免费计划包括每月 25GB 的 CDN 流量、25GB 的缓存存储和无限转换。
  ...
  * [file.io](https://www.file.io)——提供 2GB 文件存储空间。文件下载一次后自动删除。提供与存储交互的 REST API。请求速率限制为每分钟一次。
  ...
  * [ImgBB](https://imgbb.com/)——ImgBB 是一个无限制的图像托管服务。将图像拖放到屏幕上的任何位置。图像限制为 32MB。上传图像后，接收直接图像链接、BBCode 和 HTML 缩略图。登录以查看上传历史记录。
  ...
  * [nitropack.io](https://nitropack.io/)——自动进行全面的前端优化（缓存、图像和代码优化、CDN），从而加快网站速度。免费计划适用于每月最多 5000 次页面浏览量。
  ...
  * [Pinata IPFS](https://pinata.cloud)——Pinata 是上传和管理 IPFS 上文件的最简单方法。我们友好的用户界面和 IPFS API 使 Pinata 成为平台、创作者和收藏家最容易使用的 IPFS 钉扎服务。免费提供 1GB 存储空间和 API 访问权限。
  * [placekitten.com](https://placekitten.com/)——获取小猫图片的快速简单服务，可作为占位符使用
  * [plot.ly](https://plot.ly/)——图表和共享您的数据。免费层包括无限公共文件和十个私有文件
  * [podio.com](https://podio.com/)——您可以与最多五个人的团队一起使用 Podio，并尝试基本计划的功能，但不包括用户管理
  ...
  * [Shotstack](https://shotstack.io)——用于按比例生成和编辑视频的 API。免费使用，每月最多可生成 20 分钟的已渲染视频
  * [tinypng.com](https://tinypng.com/)——用于压缩和调整 PNG 和 JPEG 图像大小的 API，每月免费提供 500 次压缩
  * [transloadit.com](https://transloadit.com/)——处理文件上传以及视频、音频、图像、文档的编码。免费提供给开源、慈善机构和学生，通过 GitHub 学生开发者包。商业应用程序可免费获得 2GB 用于测试
  ...
  * [internxt.com](https://internxt.com)——Internxt 驱动器是基于绝对隐私和无可妥协的安全性的零知识文件存储服务。注册并获得 10GB 免费，永远！
  * [degoo.com](https://degoo.com/)——基于 AI 的云存储，免费提供高达 20GB、三个设备、5GB 推荐奖金（90 天账户不活动）。

  * [ImageEngine](https://imageengine.io/) - ImageEngine 是一个易于使用的全球图像 CDN。设置时间在 60 秒以内。支持 AVIF 和 JPEGXL，有 WordPress、Magento、React、Vue 等插件，还有更多。在此[申请](https://imageengine.io/developer-program/)免费开发者账号。
  * [DocsParse](https://docsparse.com/) - 使用 GPT 对 PDF、图像进行 AI 处理，将其转换为 JSON、CSV、EXCEL 格式的结构化数据。每月有 30 个免费积分。

**[⬆️ 返回顶部](#目录)**

## 设计和用户界面（UI）

  * [AllTheFreeStock](https://allthefreestock.com) - 一个免费图片、音频和视频资源的精选列表。
  ...
  * [Backlight](https://backlight.dev/) - 一个完整的编码平台，让开发人员和设计师能够协作，团队可以在其中构建、记录、发布、扩展和维护设计系统。免费计划允许最多 3 个编辑器在一个设计系统上工作，没有限制的查看者。
  ...
  * [framer.com](https://www.framer.com/) - Framer 帮助您为下一个应用程序、网站或产品迭代和动画界面创意-从强大的布局开始。对于将 Framer 作为专业原型工具进行验证的任何人：无限查看者，最多 2 个编辑器，最多 3 个项目。
  ...
  * [landen.co](https://www.landen.co) - 为您的初创公司生成、编辑和发布漂亮的网站和登录页面。无需代码。免费层允许您拥有一个完全可定制的网站，并在网络上发布。
  ...
  * [MagicPattern](https://www.magicpattern.design/tools) - CSS 和 SVG 背景生成器和工具的集合，用于渐变、图案和blob。
  * [marvelapp.com](https://marvelapp.com/) - 设计、原型制作和协作，免费计划仅限于一个用户和一个项目。
  * [Mindmup.com](https://www.mindmup.com/) - 无限的思维导图免费且可存储在云中。您的思维导图随处可用，从任何设备上立即可用。
  ...
  * [Plasmic](https://www.plasmic.app/) - 快速、易于使用、强大的网络设计工具和页面构建器，可集成到您的代码库中。构建响应式页面或复杂组件；可选地使用代码扩展；并发布到生产站点和应用程序。
  ...
  * [UI Avatars](https://ui-avatars.com/) - 根据名称的首字母生成头像。其 URL 可以直接在您的网络/应用程序中链接。支持通过 URL 配置参数。
  * [unDraw](https://undraw.co/) - 一个不断更新的美丽 SVG 图像集合，您可以完全免费使用，无需归属。
  * [unsplash.com](https://unsplash.com/) - 免费的商业和非商业用途库存照片（随意使用许可证）。

  * [walkme.com](https://www.walkme.com/)——企业级指导和参与平台，免费计划可进行三个最多 5 步/步的引导操作。
  ...
  * [Unicorn Platform](https://unicornplatform.com/)——无压力的着陆页构建器和托管。免费一个网站。
  * [react-favicon.com](https://react-favicon.com/)——使用 React 和 JSX 使用任何字体和图标库为您的网站生成 favicons。
  * [SVGmix.com](https://www.svgmix.com/)——拥有 30 多万个免费 SVG 图标、集合和品牌徽标的大型资源库。它在浏览器中提供了一个简单的向量编辑程序，用于快速文件编辑。
  ...
  * [TeleportHQ](https://teleporthq.io/)——低代码前端设计和开发平台。TeleportHQ 是一个协作式前端平台，可以立即创建和发布无后端静态网站。三个免费项目，无限协作者和免费代码导出。
  ...
  * [Logo.dev](https://www.logo.dev)——具有 4400 万多个品牌的公司徽标 API，使用起来就像调用 URL 一样简单。前 10,000 次 API 调用免费。
  ...
  * [DaisyUI](https://daisyui.com/)——免费。“使用 Tailwind CSS，但减少类名的使用”提供了按钮等组件。
  * [Scrollbar.app](https://scrollbar.app)——简单的免费网络应用程序，用于为网络设计自定义滚动条。
  * [css.glass](https://css.glass/)——免费的网络应用程序，用于使用 CSS 创建玻璃质感设计。
  * [hypercolor.dev](https://hypercolor.dev/)——Tailwind CSS 颜色渐变的精选集合，还提供了多种生成器来创建自己的渐变。
  * [iconify.design](https://icon-sets.iconify.design/)——超过 100 个图标的集合，具有统一的界面。允许您在包中搜索图标，并将单个图标导出为 SVGs 或用于流行的网络框架。
...
## 设计灵感
  ...
  * [Landings](https://landings.dev/) - [网页截图] 根据您的喜好，找到最佳的登录页面以获取设计灵感。
  * [Lapa Ninja](https://www.lapa.ninja/) - [登录页面/UI 套件/网页截图] Lapa Ninja 是一个画廊，展示了最好的 6025 个登录页面示例，为设计师提供免费书籍和免费的 UI 套件来自网络。
  * [LovelyLanding.net](https://www.lovelylanding.net/) - [登录页面设计] 定期更新的登录页面屏幕截图。包括桌面、平板电脑和移动屏幕截图。
  ...
  * [Mobile Patterns](https://www.mobile-patterns.com/) - [移动屏幕截图] 一个设计灵感库，其中包含设计师、开发人员和产品制造商可参考的 iOS 和 Android 上最好的 UI UX 模式。
...
## 地图上的数据可视化
  ...
  * [IP Geolocation](https://ipgeolocation.io/)——提供免费的开发者计划，每月有 30K 次请求。
  ...
  * [developers.arcgis.com](https://developers.arcgis.com)——用于地图、地理空间数据存储、分析、地理编码、路由等的 API 和 SDK。每月免费提供 200 万张基本地图瓦片、20,000 个未存储的地理编码、20,000 条简单路线、5,000 次驾车时间计算以及 5GB 的免费瓦片+数据存储。
  * [Foursquare](https://developer.foursquare.com/)——来自 Places API 和 Pilgrim SDK 的位置发现、场地搜索和上下文感知内容。
  * [geoapify.com](https://www.geoapify.com/)——矢量和光栅地图瓦片、地理编码、地点、路由、等距线 API。每天免费请求 3000 次。
...
## 集成开发环境（IDE）和代码编辑
  ...
  * [cocalc.com](https://cocalc.com/) - （以前是 cloud.sagemath.com 上的 SageMathCloud） - 云协作计算。浏览器访问完整的 Ubuntu，内置协作和大量免费软件，用于数学、科学、数据科学，预安装：Python、LaTeX、Jupyter Notebooks、SageMath、scikitlearn 等。
  * [code.cs50.io](https://code.cs50.io/) - 用于 CS50 的 Visual Studio Code 是 code.cs50.io 上的网络应用程序，它为学生和教师改编了 GitHub Codespaces。
  * [codepen.io](https://codpen.io/) - CodePen 是前端的游乐场。
  ...
  * [Eclipse Che](https://www.eclipse.org/che/) - 适用于开发团队的基于 Web 和 Kubernetes-Native 的 IDE，支持多种语言。开源且由社区驱动。红帽提供了一个在线实例，可在[workspaces.openshift.com](https://workspaces.openshift.com/)上访问。
  * [fakejson.com](https://fakejson.com/) - FakeJSON 通过其 API 帮助您快速生成虚假数据。发出描述您想要的内容和方式的 API 请求。API 以 JSON 格式返回所有内容。加快创意的推向市场过程，先假后真。
  * [GetVM](https://getvm.io) - 即时免费的 Linux 和 IDE 铬边栏。免费层每天包含 5 个 VM。
  * [GitPod](https://www.gitpod.io) - 适用于 GitHub 项目的即时、随时可用的开发环境。免费层每月包含 50 小时。
  * [ide.goorm.io](https://ide.goorm.io) goormIDE 是完整的云 IDE。多语言支持，基于 Linux 的容器通过功能齐全的基于网络的终端、端口转发、自定义 URL、实时协作和聊天、共享链接、Git/Subversion 支持。还有许多其他功能（免费层每个容器包含 1GB RAM 和 10GB 存储，5 个容器插槽）。
  ...
  * [jsbin.com](https://jsbin.com) - 另一个前端网页的游乐场和代码共享网站（HTML、CSS 和 JavaScript。它还支持 Markdown、Jade 和 Sass）。
  ...
  * [Mocklets](https://mocklets.com) - 基于 HTTP 的模拟 API 模拟器，有助于模拟 API 以实现更快的并行开发和更全面的测试，具有免费终身层。
  ...
  * [Prepros](https://prepros.io/) - Prepros 可以开箱即用编译 Sass、Less、Stylus、Pug/Jade、Haml、Slim、CoffeeScript 和 TypeScript，重新加载您的浏览器，使您易于开发和测试您的网站，以便您专注于使它们完美。您还可以通过点击几下添加自己的工具。
  ...
  * [VSCodium](https://vscodium.com/) - 由社区驱动，无遥测/跟踪，是 Microsoft 编辑器 VSCode 的自由许可二进制分发
...
## 分析、事件和统计
  ...
  * [Hightouch](https://hightouch.com/) - Hightouch 是一个反向 ETL 平台，可帮助您将客户数据从数据仓库同步到您的 CRM、营销和支持工具。免费套餐为您提供一个目的地来同步数据。
  ...
  * [GoatCounter](https://www.goatcounter.com/) — GoatCounter 是一个开源的网络分析平台，可作为托管服务（免费供非商业使用）或自托管应用程序使用。它旨在提供易于使用和有意义的隐私友好型网络分析，作为 Google Analytics 或 Matomo 的替代方案。免费套餐适用于非商业用途，包括无限网站、六个月的数据保留和每月 10 万次页面浏览量。
  ...
  * [Hotjar](https://hotjar.com) — 网站分析和报告。免费计划允许每天 2000 次页面浏览量。每天一百个快照（最大容量：300）。可以存储三个快照热图 365 天。无限团队成员。还包括应用内调查和带有屏幕截图的反馈小部件。免费套餐允许创建 3 个调查和 3 个反馈小部件，每月收集 20 个响应。
  ...
  * [Moesif](https://www.moesif.com) — API 分析，适用于 REST 和 GraphQL。（免费使用，每月最多 500 万 API 调用）
  ...
  * [Row Zero](https://rowzero.io) - 快速连接的电子表格。直接连接到数据库、S3 和 API。导入、分析、绘制和共享数百万行数据瞬间完成。三个免费（永久）工作簿。
  ...
  * [Statsig](https://statsig.com) - 涵盖分析、功能标志和 A/B 测试的一体化平台。免费提供每月最多 100 万次计量事件。
  ...
  * [counter.dev](https://counter.dev) — 简单易用的网络分析，注重隐私。免费或根据您的意愿通过捐赠支付。
  ...
  * [TrackWith Dicloud](https://dicloud.net/trackwith-privacy-focused-analytics/) - 免费的轻量级注重隐私的 Google Analytics 替代方案。无限制的页面浏览量，无限制的访问者，无限制的页面热点图和目标跟踪。免费提供最多 3 个域名和每个域名 600 次会话回放。
  * [AppFit](https://appfit.io) - AppFit 是一款全面的分析和产品管理工具，旨在促进跨平台管理分析和产品更新的无缝体验。免费计划包括每月 10,000 次事件、产品日记和每周洞察。
...
## 访问者会话记录
  ...
  * [OpenReplay.com](https://www.openreplay.com) - 开源会话重放，具有用于错误重现的开发工具、实时会话支持和产品分析套件。每月 1000 个会话，可访问所有功能，保留期为 7 天。
...
## 支付和计费集成
  ...
  * [Qonversion](http://qonversion.io/) - 一体化跨平台订阅管理平台，提供分析、A/B 测试、Apple Search Ads、远程配置和增长工具，用于优化应用内购买和货币化。与 iOS、Android、React Native、Flutter、Unity、Cordova、Stripe 和网络兼容。免费使用额度高达每月 10k 美元的跟踪收入。
  * [ParityVend](https://www.ambeteco.com/ParityVend/) – 根据访问者的位置自动调整价格，以扩大您的全球业务并进入新市场（购买力平价）。免费计划包括每月 7,500 次 API 请求。
  * [Glassfy](https://glassfy.io/) – 应用内订阅基础架构，实时订阅事件和现成的货币化工具，适用于 iOS、Android、Stripe 和 Paddle。免费使用额度高达每月 10k 美元的收入。
  * [Adapty.io](https://adapty.io/) – 用于移动应用内订阅集成到 iOS、Android、React Native、Flutter、Unity 或网络应用的开源 SDK 的一站式解决方案。免费使用额度高达每月 10k 美元的收入。
  ...
  * [CoinGecko](https://www.coingecko.com/en/api) — 提供加密货币市场数据，包括最新的加密货币汇率和历史数据。演示 API 提供稳定的每分钟 30 次调用限制和每月 10,000 次调用的上限。
  * [CurrencyApi](https://currencyapi.net/) — 用于物理和加密货币的实时汇率，以 JSON 和 XML 形式提供。免费层每月提供 1,250 次 API 请求。
  ...
  * [Nami ML](https://www.namiml.com/) - iOS 和 Android 上应用内购买和订阅的完整平台，包括无代码支付墙、CRM 和分析。免费使用所有基本功能来开展 IAP 业务。
  * [RevenueCat](https://www.revenuecat.com/) — 应用内购买和订阅的托管后端（iOS 和 Android）。免费使用额度高达每月 2.5k 美元的跟踪收入。
...
## 评论平台
  ...
  * [Remarkbox](https://www.remarkbox.com/) - 开源托管评论平台，为“几个域上的一位管理员，完全控制行为和外观”支付您所需要的费用
...
## 屏幕截图API
  ...
  * [ApiFlash](https://apiflash.com)——一个基于 Aws Lambda 和 Chrome 的截图 API。可处理整页截图、捕获时间和viewport 尺寸。
  * [microlink.io](https://microlink.io/)——将任何网站转化为数据，如元标记规范化、美化链接预览、抓取功能或截图即服务。每天免费 250 次请求。
  ...
  * [PhantomJsCloud](https://PhantomJsCloud.com)——浏览器自动化和页面渲染。免费层每天提供高达 500 页。自 2017 年以来一直是免费层。
...
## 与Flutter相关和在没有Mac的情况下构建IOS应用程序
  ...
  * [CodeMagic](https://codemagic.io/) - CodeMagic 是一个完全托管和管理的移动应用持续集成/持续部署（CI/CD）服务。您可以使用基于 GUI 的 CI/CD 工具进行构建、测试和部署。免费层提供 500 分钟/月的免费时长和一个配备 2.3GHz 处理器和 8GB 内存的 Mac Mini 实例。
...
## 杂项
  ...
  * [Code Time](https://www.software.com/code-time) - 在 VS Code、Atom、IntelliJ、Sublime Text 等中的扩展，用于跟踪时间和编码指标。
  ...
  * [Versionfeeds](https://versionfeeds.com) - 您喜爱的软件版本的自定义 RSS 提要。拥有您编程语言、库或喜爱工具的最新版本在一个馈送中。（前 3 个馈送是免费的）
...
## 远程桌面工具
  ...
  * [Getscreen.me](https://getscreen.me) - 免费供 2 台设备使用，对会话的数量和时长没有限制
...
## 游戏开发
  ...
  * [CraftPix](https://craftpix.net) - 免费/付费的资源，如 2D、3D、音频、GUI、背景、图标、平铺集、游戏工具包。
  * [Game Icons](https://game-icons.net/) - 免费的可定制的 SVG/PNG 图标，根据 CC-BY 许可证提供。
  ...
  * [Poliigon](https://www.poliigon.com/) - 免费和付费的纹理（具有可变分辨率）、模型、HDRIs 和画笔。提供免费的插件，可以导出到 Blender 等软件。
...
## 其他免费资源
  ...
  * [Wikimint 开发者](https://developer.wikimint.com/p/tools.html)——面向网络开发人员的免费工具，包括 CSS 压缩/解压缩、图像优化、图像大小调整、大小写转换、CSS 验证、JavaScript 编译器、HTML 编辑器等。
  * [ElevateAI](https://www.elevateai.com)——每月可免费获得长达 200 小时的音频转录。
  * [get.localhost.direct](https://get.localhost.direct)——支持子域的更好的`*.localhost.direct`通配符公共 CA 签名 SSL 证书，适用于本地开发。
  ...
  * [GitHub 教育](https://education.github.com/pack)——学生免费服务合集。需要注册。
  * [Markdown 工具](https://markdowntools.com)——将 HTML、CSVs、PDF、JSON 和 Excel 文件转换为和从 Markdown 的工具。
  * [微软 365 开发者计划](https://developer.microsoft.com/microsoft-365/dev-program)——获得免费沙盒、工具和其他资源，以构建适用于微软 365 平台的解决方案。订阅是 90 天的[微软 365 E5 订阅](https://www.microsoft.com/microsoft-365/enterprise/e5)(不包括 Windows)，可续订。如果您积极参与开发(通过遥测数据和算法衡量)，则会续订。
  * [Pyrexp](https://pythonium.net/regex)——免费的基于网络的正则表达式测试和可视化工具，用于调试正则表达式。
  * [RedHat for Developers](https://developers.redhat.com)——为开发人员提供免费访问 Red Hat 产品，包括 RHEL、OpenShift、CodeReady 等。仅限个人计划。还提供免费电子书供参考。
  * [smsreceivefree.com](https://smsreceivefree.com/)——提供免费的临时和一次性电话号码。
  * [sandbox.httpsms.com](https://sandbox.httpsms.com)——免费发送和接收测试短信。
  * [SimpleBackups.com](https://simplebackups.com/)——直接存储到云存储提供商(AWS、DigitalOcean 和 Backblaze)的服务器和数据库(MySQL、PostgreSQL、MongoDB)的备份自动化服务。提供免费计划，可进行 1 次备份。
  ...
  * [Themeselection](https://themeselection.com/)——精选高质量、现代设计、专业且易于使用的免费管理仪表板模板、HTML 主题和 UI 工具包，帮助您更快地创建应用程序！
  * [Web.Dev](https://web.dev/measure/)——这是一款免费工具，可让您查看网站性能并改善 SEO，以在搜索引擎中获得更高的排名。
  * [SmallDev.tools](https://smalldev.tools/)——一款免费的开发人员工具，允许您对各种格式进行编码/解码、最小化 HTML/CSS/JavaScript、美化、以 JSON/CSV 及多种其他格式生成虚假/测试数据集等功能。具有令人愉悦的界面。
  * [UseCSV by Layercode](https://layercode.com/usecsv)——在几分钟内将 CSV 和 Excel 导入您的网络应用程序。为您的用户提供愉快且强大的数据导入体验。免费开始，无需任何信用卡详细信息，立即开始集成 UseCSV。您可以创建无限的导入器，上传大小高达 100Mb 的文件。
  ...
  * [OpenUtils](https://openutils.org/)——为开发人员提供各种免费工具，如 HTML/CSS/JavaScript 格式化程序、压缩程序、转换器、编码器/解码器等。
  * [SimpleRestore](https://simplerestore.io)——轻松 MySQL 备份还原。无需代码或服务器即可将 MySQL 备份还原到任何远程数据库。
  ...
  * [QRCodeBest](https://qrcode.best/)——使用 13 种模板创建自定义 QR 码，提供完全隐私和个人品牌。功能包括跟踪像素、项目分类和无限团队席位的 QRCode.Best。
  * [PostPulse](https://post-pulse.io)——通过每月 AI 制作的文章提升在线存在，增强 SEO 和网站排名，这些文章针对 SEO 优化的域名进行了优化。免费计划允许您每月在我们的网站上手动发布一篇文章。
