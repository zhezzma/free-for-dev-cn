...
这是一份带有免费开发者层级的软件（SaaS、PaaS、IaaS 等）和其他产品的列表。

此特定列表的范围仅限于基础架构开发人员（系统管理员、DevOps 从业者等）可能会发现有用的东西。我们喜欢那里所有的免费服务，但保持主题相关会很好。有时候这是一条灰色地带，所以这是有意见的；如果我不接受您的贡献，请不要感到冒犯。

这份列表是通过拉取请求、评论、想法和 1600 多人的工作得出的。您也可以通过发送[拉取请求](https://github.com/ripienaar/free-for-dev)来添加更多服务或删除那些提供的服务已经更改或已停用的服务来提供帮助。

[![跟踪出色列表](https://www.trackawesomelist.com/badge.svg)](https://www.trackawesomelist.com/ripienaar/free-for-dev)

**注意**：此列表仅适用于即服务产品，不适用于自托管软件。要符合条件，服务必须提供免费层，而不仅仅是免费试用。如果是时间桶，则免费层必须至少为一年。我们还从安全角度考虑免费层，因此 SSO 是可以接受的，但我不会接受将 TLS 限制为仅付费层的服务。
...
42. 隐私管理
43. 屏幕截图 API
44. Flutter 相关和在没有 Mac 的情况下构建 IOS 应用
45. 搜索
46. 安全和 PKI
47. 身份验证、授权和用户管理
48. 源代码存储库
...
## Major Cloud Providers
  ...
  * [Google Cloud Platform](https://cloud.google.com)
    ...
    * Compute Engine - 1 个非抢占式 e2-micro，30GB HDD，5GB 快照存储（受某些地区限制），每月从北美到所有地区目的地（不包括中国和澳大利亚）的 1GB 网络出口
    * Cloud Storage - 5GB，1GB 网络出口
    * Cloud Shell - 基于 Web 的 Linux 外壳/主要 IDE，具有 5GB 持久存储。每周限制 60 小时
    * Cloud Pub/Sub - 每月 10GB 的消息
    * Cloud Functions - 每月 200 万次调用（包括后台和 HTTP 调用）
    * Cloud Run - 每月 200 万次请求，360,000GB 秒内存，180,000vCPU 秒计算时间，每月从北美到所有地区的 1GB 网络出口
  ...
  * [Cloudflare](https://www.cloudflare.com/)
    ...
    * [Pages](https://developers.cloudflare.com/pages/) - 在 Cloudflare 的快速、安全的全球网络上开发和部署您的 Web 应用程序。每月五百次构建，一百个自定义域，集成 SSL，无限可访问座位，无限预览部署，以及通过 Cloudflare Workers 集成的全栈功能。
...
## Cloud management solutions
  ...
  * [Brainboard](https://www.brainboard.co) - 一种协作式解决方案，用于从端到端可视化构建和管理云基础架构。
  ...
  * [deployment.io](https://deployment.io) - Deployment.io 帮助开发人员在 AWS 上自动化部署。在我们的免费层中，开发人员（单个用户）可以免费部署无限的静态站点、Web 服务和环境。我们提供每月 20 次免费的作业执行，其中包括预览和自动部署。

**[⬆️ 返回顶部](# 目录)**

## Source Code Repos

  * [Bitbucket](https://bitbucket.org/)——无限数量的公共和私有 Git 存储库，最多可供 5 个用户使用，并且具有用于 CI/CD 的 Pipelines
  * [chiselapp.com](https://chiselapp.com/)——无限数量的公共和私有 Fossil 存储库
  * [codebasehq.com](https://www.codebasehq.com/)——一个免费项目，提供 100MB 空间和两个用户
  * [Codeberg](https://codeberg.org/)——无限数量的公共和私有 Git 存储库，适用于免费和开源项目（无限制的协作者）。由 [Forgejo](https://forgejo.org/) 提供支持。具有 [Codeberg Pages](https://codeberg.page/) 的静态网站托管。具有 [Codeberg 的 CI](https://docs.codeberg.org/ci/) 的 CI/CD 托管。具有 [Codeberg Translate](https://translate.codeberg.org/) 的翻译托管。包括包和容器托管、项目管理和问题跟踪
  * [GitGud](https://gitgud.io)——无限数量的私有和公共存储库。永久免费。由 GitLab 和 Sapphire 提供支持。不提供 CI/CD。
  * [GitHub](https://github.com/)——无限数量的公共存储库和无限数量的私有存储库（无限制的协作者）。包括 CI/CD、开发环境、静态托管、包和容器托管、项目管理和 AI Copilot
  * [gitlab.com](https://about.gitlab.com/)——无限数量的公共和私有 Git 存储库，最多可与 5 个协作者一起使用。包括 CI/CD、静态托管、容器注册表、项目管理和问题跟踪
  ...
  * [perforce.com](https://www.perforce.com/products/helix-teamhub)——免费 1GB 云，以及 Git、Mercurial、或 SVN 存储库。
  ...
  * [RocketGit](https://rocketgit.com)——基于 Git 的存储库托管。无限数量的公共和私有存储库。
...
## APIs, Data, and ML
  ...
  * [JSONGrid](https://jsongrid.com) - 免费工具，用于可视化、编辑、过滤复杂的 JSON 数据为漂亮的表格网格。通过链接共享 JSON 数据。
  ...
  * [Apify](https://www.apify.com/) - 网络抓取和自动化平台，可创建任何网站的 API 并提取数据。提供现成的抓取程序、集成代理和自定义解决方案。免费计划每月包含 5 美元的平台信用额度。
  ...
  * [Browse AI](https://www.browse.ai) - 在网络上提取和监控数据。每月免费 50 个信用。
  ...
  * [Commerce Layer](https://commercelayer.io) - 可组合的商务 API，可以从任何前端构建、放置和管理订单。开发人员计划免费提供每月 100 个订单和最多 1,000 个 SKU。
  * [Conversion Tools](https://conversiontools.io/) - 在线文件转换器，用于文档、图像、视频、音频和电子书。提供 REST API。适用于 Node.js、PHP、Python 的库。支持高达 50GB 的文件（付费计划）。免费层受文件大小和每天转换次数的限制。
  * [Country-State-City Microservice API](https://country-state-city.rebuscando.info/) - API 和 Microservice，提供包括国家、地区、省份、城市、邮政编码等广泛信息。免费层包括每天最多 100 次请求。
  ...
  * [Cube](https://cube.dev/) - Cube 帮助数据工程师和应用程序开发人员从现代数据存储中访问数据，将其组织为一致的定义，并将其交付到每个应用程序。使用 Cube 最快的方法是使用 Cube Cloud，它有一个免费层，每月有 1GB 的数据通过。
  ...
  * [Deepnote](https://deepnote.com) - 一种新的数据科学笔记本。与 Jupyter 兼容，具有实时协作和在云端运行的功能。免费层包括无限个人项目、高达 750 小时的标准硬件和最多 3 个编辑器的团队。
  * [Diggernaut](https://www.diggernaut.com/) - 基于云的网络抓取和数据提取平台，用于将任何网站转换为数据集或作为 API 使用。免费计划每月包括 5K 页请求。

  * [Doczilla](https://www.doczilla.app/)——一款 SaaS API，可从 HTML/CSS/JS 代码直接生成屏幕截图或 PDF。免费计划允许每月生成 250 份文档。
  * [Doppio](https://doppio.sh/)——使用顶级渲染技术生成和私有存储 PDF 和屏幕截图的托管 API。免费计划允许每月生成 400 份 PDF 和屏幕截图。
  * [dreamfactory.com](https://dreamfactory.com/)——用于移动、Web 和 IoT 应用程序的开源 REST API 后端。连接任何 SQL/NoSQL 数据库、文件存储系统或外部服务，它立即创建一个具有实时文档和用户管理的综合 REST API 平台。
  ...
  * [Export SDK](https://exportsdk.com)——具有拖放模板编辑器的 PDF 生成 API，提供 SDK 和无代码集成。免费计划每月有 250 页，无限制用户和三个模板。
  ...
  * [file.coffee](https://file.coffee/)——一个平台，您可以在其中存储最多 15MB/file（注册后为 30/MB 文件）。
  * [FraudLabs Pro](https://www.fraudlabspro.com)——对信用卡支付欺诈进行订单交易筛查。此 REST API 将根据订单的输入参数检测所有可能的欺诈特征。免费的微计划每月有 500 笔交易。
  * [Geekflare API](https://geekflare.com/api)——Geekflare API 让您可以截取屏幕截图、审核网站、TLS 扫描、DNS 查找、测试 TTFB 等。免费计划提供 3,000 次 API 请求。
  ...
  * [Glitterly](https://glitterly.app/)——从基础模板动态生成图像的程序。Restful API 和无代码集成。免费层每月提供 50 张图像和 5 个模板。
  ...
  * [Hook0](https://www.hook0.com/)——Hook0 是一个开源的 Webhooks-as-a-service (WaaS)，它使在线产品提供 Webhooks 变得容易。免费提供 7 天历史记录，每月最多可调度 3,000 个事件。
  * [Hoppscotch](https://hoppscotch.io)——一个免费、快速和美丽的 API 请求构建器。
  * [Hybiscus](https://hybiscus.dev/)——使用简单的声明性 API 构建 pdf 报告。免费层包括每月最多 100 份单页报告的能力，以及自定义调色板和字体的能力。
  * [Invantive Cloud](https://cloud.invantive.com/)——访问超过 70 个（云）平台，如 Exact Online、Twinfield、ActiveCampaign 或 Visma 使用 Invantive SQL 或 OData4（通常是 Power BI 或 Power Query）。包括数据复制和交换。开发人员和实施顾问的免费计划。对于特定平台，免费但有数据量限制。
  ...
  * [IP2Location.io](https://www.ip2location.io/)——免费增值、快速可靠的 IP 地理定位 API，用于确定城市、坐标、ISP 等地理定位数据。免费计划每月提供 30k 信用。订阅付费计划以获得更多高级功能或联系我们获取个性化计划。
  * [ipapi](https://ipapi.co/)——由 Kloudend, Inc 提供的 IP 地址位置 API——基于 AWS 构建的可靠地理定位 API，受到 Fortune 500 的信任。免费层提供每月 30k 次查找（每天 1k 次），无需注册。
  * [ipapi.is](https://ipapi.is/)——为开发人员提供的可靠 IP 地址 API，具有最佳的主机检测功能。免费计划提供 1,000 次无注册查找。
  * [IPinfo](https://ipinfo.io/)——快速、准确和免费（每月最多 50k）的 IP 地址数据 API。提供有关地理位置、公司、运营商、IP 范围、域、滥用联系人等的 API。所有付费 API 均可免费试用。
  * [BigDataCloud](https://www.bigdatacloud.com/)——为现代网络提供快速、准确和免费（无限制或每月 10K-50K）的 API，如 IP 地理定位、反向地理编码、网络洞察、电子邮件和电话验证、客户端信息等。
  ...
  * [JSON IP](https://getjsonip.com)——返回从其请求的客户端的公共 IP 地址。免费层无需注册。使用 CORS，数据可以使用客户端 JavaScript 直接从浏览器请求。对于服务监控客户端和服务器 IP 的更改很有用。无限请求。
  ...
  * [Market Data API](https://www.marketdata.app) - 提供股票、期权、共同基金等的实时和历史金融数据。免费永久 API 层允许每天无限制地进行 100 次 API 请求。
  ...
  * [Mindee](https://developers.mindee.com/docs)——Mindee 是一款强大的 OCR 软件和 API 优先平台，通过使用计算机视觉和机器学习对关键信息进行数据识别，帮助开发人员通过标准化文档处理层来自动化应用程序的工作流程。免费层提供每月 250 页。
  ...
  * [Nyckel](https://www.nyckel.com)——训练、部署和调用图像和文本 ML 模型。免费培训，最多可使用 5000 个训练数据。每月免费调用 1000 个模型。
  * [Observable](https://observablehq.com/)——一个用于创建、协作和学习数据的地方。免费：无限制的笔记本、无限制的发布、每个笔记本的五个编辑器。
  * [OCR.Space](https://ocr.space/)——OCR API 解析图像和 pdf 文件，以 JSON 格式返回文本结果。每月免费 25000 次请求。
  * [Duply.co](https://duply.co)——从 API 和 URL 创建动态图像，设计模板一次并重复使用。免费层每月提供 70 个图像（来自 API 和 URL）的创建，最多 100 个通过表单。
  ...
  * [Pixela](https://pixe.la/)——免费的日常数据流数据库服务。所有操作均通过 API 执行。也可以使用热图和折线图进行可视化。
  * [Postbacks](https://postbacks.io/)——请求稍后的 HTTP 回调。注册时提供 8000 次免费请求。
  * [Postman](https://postman.com)——用于简化工作流程和更快创建更好的 API 的协作平台。免费使用 Postman 应用程序。Postman 云功能也免费提供，有一定的限制。
  ...
  * [Preset Cloud](https://preset.io/)——托管的 Apache Superset 服务。对于最多 5 个用户的团队，永久免费，功能包括无限的仪表板和图表、无代码图表构建器和协作式 SQL 编辑器。
  * [PromptLeo](https://promptleo.com/)——创作者和开发人员的提示工程平台。它提供提示工程库、表单和 API。免费计划提供一个提示形成、一个提示 API 端点和每月 30 次生成。
  * [PromptLoop](https://www.promptloop.com/)——使用 AI 和大型语言模型（如 GPT-3），通过简单的电子表格公式在 Google Sheets 中转换、理解和分析文本。每月前 2000 个信用额度免费。
  ...
  * [QuickMocker](https://quickmocker.com/)——管理自己子域下的在线虚假 API 终结点，将请求转发到本地主机 URL 进行 Webhooks 开发和测试，使用 RegExp 和多个 HTTP 方法进行 URL 路径，对端点进行优先级排序，每个端点超过 100 个短代码（动态或虚假响应值）进行响应模板化，从 OpenAPI（Swagger）规范以 JSON 格式导入，代理请求，根据 IP 地址和授权标头限制端点。免费帐户提供一个随机子域、十个端点、五个 RegExp URL 路径、每个端点 50 个短代码、每天 100 个请求和请求日志中的 50 条历史记录。
  * [Rapidapi](https://rapidapi.com/)——世界上最大的 API 中心，数百万开发人员查找并连接到数千个 API，使用有趣的挑战（和解决方案！）和交互示例进行 API 开发。
  ...
  * [Scraper's Proxy](https://scrapersproxy.com)——用于抓取的简单 HTTP 代理 API。无需担心限制、阻止或验证码即可匿名抓取。第一个月免费提供 100 次成功抓取（如果您联系支持人员，还可以获得更多）。
  ...
  * [Select Star](https://www.selectstar.com/) - 是一个智能数据发现平台，可自动分析和记录您的数据。免费轻量级套餐，包括 1 个数据源、最多 100 个表和 10 个用户。
  ...
  * [Sofodata](https://www.sofodata.com/)——从 CSV 文件创建安全的 RESTful API。上传 CSV 文件，然后通过其 API 立即访问数据，从而加快应用程序开发速度。免费计划包括 2 个 API 和每月 2,500 个 API 调用。您不需要信用卡。
  ...
  * [TemplateTo](https://templateto.com)——使用我们的拖放编辑器和简单 API，从可重复用模板自动生成 PDF/TXT 文档。免费计划每月提供 450 份 PDF 和三个模板。
  * [TinyMCE](https://www.tiny.cloud) - 富文本编辑 API。核心功能免费，无使用限制。
  * [Treblle](https://www.treblle.com)——Treblle 帮助团队构建、交付和管理 API。具有高级 API 日志聚合、可观察性、文档和调试功能。您可以免费获得所有功能，但免费层每月的请求限制为最多 250k。
  ...
  * [Weights & Biases](https://wandb.ai)——开发者优先的 MLOps 平台。通过实验跟踪、数据集版本控制和模型管理更快地构建更好的模型。免费层仅适用于个人项目，包括 100GB 存储空间。
  ...
  * [ZenRows](https://www.zenrows.com/)——网络爬虫 API 和代理服务器，可绕过任何反机器人解决方案，同时提供 JavaScript 渲染、旋转代理和地理定位。免费层为 1000 个 API 调用。
  * [Zenscrape](https://zenscrape.com/web-scraping-api)——具有无头浏览器、住宅 IP 和简单定价的网络爬虫 API。每月一千个免费 API 调用，学生和非营利组织有额外积分。
  * [ip-api](https://ip-api.com)——IP 地理位置 API，非商业用途免费，无需 API 密钥，免费计划每个 IP 地址每分钟限制为 45 个请求。
  * [WebScraping.AI](https://webscraping.ai) - 具有内置解析、Chrome 渲染和代理的简单 Web 爬虫 API。每月两千个免费 API 调用。
  * [Zipcodebase](https://zipcodebase.com) - 免费邮政编码 API，可访问全球邮政编码数据。每月十万个免费请求。
  ...
  * [What The Diff](https://whatthediff.ai) - AI 驱动的代码审查助手。免费计划每月有 25,000 个令牌限制（约 10 个 PR）。
  ...
  * [OpenWeb Ninja](https://www.openwebninja.com/)——极其全面的实时 SERP 和公共数据 API：Google 搜索、购物、工作、图片、镜头、新闻、Google 地图企业/地点、评论、照片、网站电子邮件和社交联系人抓取器、亚马逊、Yelp 等。所有 API 都包含免费层，每月有 100 到 200 个免费请求。
...
## Artifact Repos
  ...
  * [Artifactory](https://jfrog.com/start-free/) - 一个支持多种软件包格式的工件存储库，如 Maven、Docker、Cargo、Helm、PyPI、CocoaPods 和 GitLFS。它包含了一个包扫描工具 XRay 和一个 CI/CD 工具 Pipelines（以前称为 Shippable），每月有 2000 分钟的免费 CI/CD 分钟数。
...
## Tools for Teams and Collaboration
  ...
  * [Braid](https://www.braidchat.com/)——专为团队设计的聊天应用程序。免费供公众访问组使用，无限制用户、历史记录和集成。它还提供了一个自托管的开源版本。
  ...
  * [Duckly](https://duckly.com/)——与团队实时交谈和协作。与 IDE 进行配对编程，终端共享，语音、视频和屏幕共享。适用于小型团队。
  * [Dyte](https://dyte.io)——最适合开发人员的实时视频和音频 SDK，具有协作插件，可提高生产力和参与度。免费层包括每月 10,000 分钟的实时视频/音频使用。
  * [evernote.com](https://evernote.com/)——组织信息的工具。与他人分享您的笔记并共同协作。
  ...
  * [Gather](https://www.gather.town/)——在线见面的更好方式。围绕完全可定制的空间，Gather 使与您的社区共度时光变得与现实生活一样容易。免费供最多 10 个并发用户使用。
  ...
  * [GitDailies](https://gitdailies.com)——团队在 GitHub 上的提交和拉取请求活动的每日报告。包括推送可视化工具、同伴认可系统和自定义警报构建器。免费层有无限用户、三个存储库和 3 个警报配置。
  * [gitter.im](https://gitter.im/)——用于 GitHub 的聊天。无限数量的公共和私人房间，团队最多 25 人免费。
  * [Hackmd.io](https://hackmd.io/)——用于 markdown 格式文档/文件的实时协作和编写工具。就像 Google Docs 一样，但适用于 markdown 文件。免费无限数量的“笔记”，但私人笔记和模板的协作者（邀请者）数量[将受到限制](https://hackmd.io/pricing)。
  ...
  * [ideascale.com](https://ideascale.com/)——允许客户提交想法和投票，免费供 1 个社区中的 25 名成员使用
  ...
  * [/meet for Slack](https://meetslack.com)——在 Slack 中使用/meet 直接启动 Google 会议，无需任何限制。免费。
  ...
  * [Microsoft Teams](https://products.office.com/microsoft-teams/free)——Microsoft Teams 是一个基于聊天的数字中心，将对话、内容和应用程序集中在一个地方，来自单一体验。免费供最多 500k 用户使用。
  * [Miro](https://miro.com/)——可扩展、安全、跨设备且适用于分布式团队的企业就绪型协作白板。有免费增值计划。
  * [nootiz](https://www.nootiz.com/)——收集和管理任何网站上视觉反馈的首选工具。
  * [Notion](https://www.notion.so/)——具有 markdown 支持的笔记记录和协作应用程序。该公司将该应用程序描述为笔记记录、项目管理和任务管理的一体化工作区。除了跨平台应用程序，它还可以通过大多数网络浏览器访问。
  ...
  * [PageShare.dev](https://www.pageshare.dev)——为 GitHub 的拉取请求添加视觉审查功能，无需部署网站。免费供每月最多 10 页和总计 100MB 的存储空间。
  * [Pendulums](https://pendulums.io/)——Pendulums 是一款免费的时间跟踪工具，具有易于使用的界面和有价值的统计信息，可帮助您更好地管理时间。
  * [Pumble](https://pumble.com)——免费团队聊天应用程序。无限制的用户和消息历史记录，永久免费。
  ...
  * [Slab](https://slab.com/) - 一个面向团队的现代知识管理服务。免费供最多 10 个用户使用。
  ...
  * [Teamhood](https://teamhood.com/) - 免费的项目、任务和问题跟踪软件。支持带有泳道和完整 Scrum 实现的看板。具有集成的时间跟踪功能。免费供五名用户和三个项目组合使用。
  * [Teamplify](https://teamplify.com) - 通过团队分析和智能每日站立会议提高团队开发流程。包括适用于远程优先团队的全功能休假管理。免费供最多 5 名用户的小团体使用。
  * [Tefter](https://tefter.io) - 具有强大 Slack 集成的书签应用程序。免费供开源团队使用。
  ...
  * [tldraw.com](https://tldraw.com/) - 具有智能箭头、捕捉、粘性便签和 SVG 导出功能的免费开源白板和绘图工具。具有多人协作编辑模式。还提供免费的官方 VS Code 扩展。
  * [BookmarkOS.com](https://bookmarkos.com) - 免费的一体化书签管理器、选项卡管理器和任务管理器，具有可自定义的在线桌面和文件夹协作功能。
  ...
  * [windmill.dev](https://windmill.dev/) - 一个开源开发者平台，可从最小的 Python 和 Typescript 脚本快速构建生产级多步骤自动化和内部应用程序。作为免费用户，您最多可以创建和成为三个非高级工作空间的成员。
  ...
  * [Zulip](https://zulip.com/) - 具有独特的电子邮件式线程模型的实时聊天。免费计划包括 10,000 条消息搜索历史记录和文件存储最多 5GB。它还提供自托管的开源版本。
  * [robocorp.com](https://robocorp.com) - 用于推动自动化运营的开源堆栈。免费试用云功能并实现简单自动化。机器人工作 240 分钟/月，10 个助手运行，存储 100MB。
  * [Fleep.io](https://fleep.io/) - 替代 Slack 的一种选择。它为小型团队提供免费计划，其中包括完整的消息历史记录、无限的 1:1 对话、1 个组对话和 1GB 文件存储。
  * [Chanty.com](https://chanty.com/) - 替代 Slack 的另一种选择。它为小型团队（最多 10 人）提供免费的永久计划，包括无限的公共和私人对话、可搜索的历史记录、无限的 1:1 音频通话、无限的语音消息、10 个集成和每个团队 20GB 的存储空间。
  ...
  * [Webvizio](https://webvizio.com) - 用于简化网站开发协作的网站反馈工具、网站审查软件和错误报告工具，可直接在实时网站和网络应用程序、图像、PDF 和设计文件上执行任务。
  * [Pullflow](https://pullflow.com) - 在 GitHub、Slack 和 VS Code 上提供 AI 增强的代码审查协作平台。
  * [Webex](https://www.webex.com/) - 提供免费计划的视频会议，每次会议提供 40 分钟，最多可容纳 100 名参会者。
  * [RingCentral](https://www.ringcentral.com/) - 提供免费计划的视频会议，每次会议提供 50 分钟，最多可容纳 100 名参与者。
  ...
  * [Revolt.chat](https://revolt.chat/) - 一个替代[Discord](https://discord.com/)的开源选项，它尊重您的隐私。它还免费提供了 Discord 的大多数专有功能。Revolt 是一个安全、快速的一体化应用程序，同时完全免费。所有功能都是免费的。他们还支持（官方和非官方）插件，这与大多数主流聊天应用程序不同。
...
## CMS
  ...
  * [Contentful](https://www.contentful.com/) - 无头 CMS。云内的内容管理和交付 API。附带一个免费的社区空间，其中包括五个用户、25K 条记录、48 个内容类型、2 个地区。
  ...
  * [Directus](https://directus.io) - 无头 CMS。一个完全免费和开源的平台，用于管理内部或云托管的资产和数据库内容。没有限制或付费墙。
  * [FrontAid](https://frontaid.io/) - 无头 CMS，将 JSON 内容直接存储在你的 Git 存储库中。无限制。
  * [kontent.ai](https://www.kontent.ai) - 一个内容即服务平台，为你提供无头 CMS 的所有好处，同时赋予营销人员权力。开发者计划为两个用户提供无限的项目，每个项目有两个环境，500 个内容项目，两种语言的交付和管理 API，以及自定义元素支持。你可以使用更详细的计划来满足你的需求。
  ...
  * [sensenet](https://sensenet.com) - 提供企业级解决方案的 API 优先无头 CMS，适用于各种规模的企业。开发者计划提供三个用户、500 个内容项目、三个内置角色、25 个+5 个内容类型、完全可访问的 REST API、文档预览生成和 Office Online 编辑。
  ...
  * [WPJack](https://wpjack.com) - 在不到 5 分钟的时间内在任何云平台上设置 WordPress！免费层包括 1 个服务器、2 个站点、免费 SSL 证书和无限的 cron 作业。无时间限制或过期-你的网站，你的方式。
...
## Code Generation
  ...
  * [Cody AI](https://sourcegraph.com/cody) - Cody 是一款编码 AI 助手，它利用 AI 和对代码库的深入理解，帮助您更快地编写和理解代码。Cody 为开发人员提供了对各种 LLM（包括本地推理）的选择，支持各种 IDE，支持所有流行的编程语言，并提供免费计划。免费计划每月为开发人员提供 20 条聊天消息（使用 Claude 3 Sonnet 作为 LLM）和 500 次自动完成（使用 Starcoder 16b）。
  ...
  * [Fern](https://buildwithfern.com)——编写 API 定义，并使用它们以 TypeScript、Python、Java、Go 等流行语言生成 SDK/client 库。完全支持 OpenAPI。免费层最多为 20 个端点生成代码。
  ...
  * [tabnine.com](https://www.tabnine.com/)——Tabnine 通过利用世界上所有代码中学习到的见解，帮助开发人员更快地创建更好的软件。提供插件。
  * [v0.dev](https://v0.dev/)——v0 使用 AI 模型根据简单的文本提示生成代码。它根据 shadcn/ui 和 Tailwind CSS 生成可复制和粘贴的友好 React 代码，人们可以在项目中使用。每次生成至少需要 30 个信用点。您开始时拥有 1200 个信用点，并且每月可获得 200 个免费信用点。
...
## Code Quality
  ...
  * [browserling.com](https://www.browserling.com/)——实时交互式跨浏览器测试，免费，仅在 Vista 下以 1024 x 768 分辨率使用 MS IE 9 时有 3 分钟的会话
  ...
  * [codeclimate.com](https://codeclimate.com/)——自动化代码审查，对于开源和组织拥有的私有存储库（最多 4 个协作者）免费。也适用于学生和机构。
  ...
  * [codescene.io](https://codescene.io/)——CodeScene 根据开发人员使用代码的方式优先处理技术债务，并可视化组织因素，如团队耦合和系统掌握程度。开源免费。
  * [CodSpeed](https://codspeed.io)——在您的 CI 管道中自动跟踪性能。由于精确和一致的指标，在部署之前捕获性能回归。对于开源项目免费永久。
  ...
  * [deepscan.io](https://deepscan.io/)——针对 JavaScript 代码的高级静态分析，开源免费
  ...
  * [Moderne.io](https://app.moderne.io)——自动源代码重构。Moderne 提供框架迁移、代码分析和修复以及无与伦比的大规模代码转换，因此开发人员可以将时间花在构建新事物上，而不必维护旧事物。开源免费。
  ...
  * [sonarcloud.io](https://sonarcloud.io)——针对 Java、JavaScript、C/C++、C#、VB.NET、PHP、Objective-C、Swift、Python、Groovy 等语言的自动化源代码分析，开源免费
...
## CI and CD
  ...
  * [AccessLint](https://github.com/marketplace/accesslint)——AccessLint 将自动化的网页可访问性测试引入您的开发工作流程。它对开源和教育目的是免费的。
  * [appcircle.io](https://appcircle.io)——一个企业级移动 DevOps 平台，它可以自动构建、测试和发布移动应用程序，以实现更快、更高效的发布周期。免费使用 30 分钟的最大构建时间，每月 20 次构建和 1 次并发构建。
  * [appveyor.com](https://www.appveyor.com/)——适用于 Windows 的 CD 服务，对开源免费
  * [Argonaut](https://argonaut.dev/)——在您的云环境中几分钟内部署应用程序和基础架构。支持在 Kubernetes 和 Lambda 环境中自定义和第三方应用程序部署。免费层允许无限的应用程序和部署，最多 5 个域和 2 个用户。
  ...
  * [CircleCI](https://circleci.com/)——全面的免费计划，其中包含在托管 CI/CD 服务中针对 GitHub、GitLab 和 BitBucket 存储库的所有功能。多个资源类、Docker、Windows、Mac OS、ARM 执行器、本地运行器、测试拆分、Docker 层缓存和其他高级 CI/CD 功能。免费使用时间每月最多 6000 分钟，无限制的协作者，私有项目中最多 30 个并行作业，以及开源项目最多 80,000 次免费构建分钟。
  ...
  * [codeship.com](https://codeship.com/)——100 个私有构建/月，5 个私有项目，对开源免费
  * [deploybot.com](https://www.deploybot.com/)——1 个存储库，10 次部署，对开源免费
  ...
  * [semaphoreci.com](https://semaphoreci.com/)——对开源免费，每月 100 个私有构建
  * [Squash Labs](https://www.squash.io/)——为每个分支创建一个 VM，并使您的应用程序可从唯一的 URL 访问，无限的公共和私有存储库，最多 2GB VM 大小。
  * [styleci.io](https://styleci.io/)——仅限公共 GitHub 存储库
  * [Mergify](https://mergify.io)——用于 GitHub 的工作流程自动化和合并队列——对公共 GitHub 存储库免费
  * [Make](https://www.make.com/en)——工作流程自动化工具，允许您使用 UI 连接应用程序并自动化工作流程。它支持许多应用程序和最流行的 API。对公共 GitHub 存储库免费，免费层包含 100 Mb、1000 次操作和 15 分钟的最小间隔。
  * [Spacelift](https://spacelift.io/)——基础设施即代码的管理平台。免费计划功能：IaC 协作、Terraform 模块注册表、ChatOps 集成、使用 Open Policy Agent 进行持续资源合规性、使用 SAML 2.0 的单点登录以及访问公共工作者池：最多 200 分钟/月
  ...
  * [Nx Cloud](https://nx.dev/ci)——Nx Cloud 通过远程缓存、任务在机器上的分布甚至自动化的 e2e 测试运行拆分等功能，加快您的 CI 上的 monorepos 速度。它提供了一个免费计划，最多可容纳 30 名贡献者，并包含慷慨的 150k 信用额度。
...
## Testing
  ...
  * [Applitools.com](https://applitools.com/)——智能视觉验证工具，适用于网络、原生移动和桌面应用程序。几乎与所有自动化解决方案（如 Selenium 和 Karma）和远程运行器（Sauce Labs、Browser Stack）集成。开源免费。个人用户免费套餐每周有有限的检查点。
  * [Appetize](https://appetize.io)——在这个基于云的 Android 手机/平板电脑模拟器和 iPhone/iPad 模拟器上直接在浏览器中测试你的 Android 和 iOS 应用程序。免费套餐包括两个并发会话，每月使用 30 分钟。无应用大小限制。
  * [Apptim](https://apptim.com)——一款移动测试工具，使没有性能工程技能的人员能够评估应用程序的性能和用户体验（UX）。使用自己设备的桌面版本完全免费，iOS 和 Android 上的测试次数不限。
  * [Bencher](https://bencher.dev/)——一款持续基准测试工具套件，用于捕获 CI 性能回归。对所有公共项目免费。
  * [BugBug](https://bugbug.io/)——用于 Web 应用程序的轻量级测试自动化工具。易于学习，不需要编码。您可以在自己的计算机上免费运行无限的测试。您还可以获得云监控和 CI/CD 集成，但需要额外支付月费。
  ...
  * [checkbot.io](https://www.checkbot.io/)——浏览器扩展，用于测试您的网站是否遵循 50 多个 SEO、速度和安全最佳实践。免费套餐适用于较小的网站。
  * [Checkly](https://checklyhq.com)——现代 DevOps 的代码优先合成监控。以传统提供商的一小部分成本监控您的 API 和应用程序。开发人员有慷慨的免费套餐。
  * [cypress.io](https://www.cypress.io/)——快速、简单和可靠的浏览器测试工具。Cypress Test Runner 始终免费且开源，没有任何限制和限制。Cypress Dashboard 为开源项目提供免费的个人用户，最多 5 个用户。
  * [Cypress Recorder by Preflight](https://cypress.preflight.com/)——在浏览器上创建基于 AI 的 Cypress 测试/POM 模型。它是开源的，除了 AI 部分。它的免费套餐包括五个月的测试创建，具有自我修复脚本、电子邮件和视觉测试。
  ...
  * [katalon.com](https://katalon.com)——提供测试平台，可帮助不同测试成熟度级别的团队，包括 Katalon Studio、TestOps（+免费视觉测试）、TestCloud 和 Katalon Recorder。
  * [Keploy](https://keploy.io/)——Keploy 是开发人员的功能测试工具包。记录 API 调用为 API（KTests）生成端到端测试，并为 mocks 或 stubs（KMocks）生成测试。它对开源项目免费。
  ...
  * [preflight.com](https://preflight.com)——无代码自动网页测试。在您的浏览器上录制对 UI 更改有弹性的测试，并在 Windows 计算机上运行它们。可以与您的 CI/CD 集成吗？免费套餐包括 50 个每月测试运行，带有视频、HTML 会话等。
  ...
  * [Repeato](https://repeato.app/)——基于计算机视觉和 AI 的无代码移动应用程序测试自动化工具。适用于原生应用程序、Flutter、React Native、Web、Ionic 等众多应用程序框架。免费套餐限制为 iOS 上的 10 个测试和 Android 上的 10 个测试，但包括付费计划的大多数功能，包括无限的测试运行次数。
  * [Requestly](https://requestly.com/)——开源 Chrome 扩展程序，用于拦截、重定向和模拟 HTTP 请求。具有[调试器](https://requestly.com/products/web-debugger/)、[模拟服务器](https://requestly.com/products/mock-server/)、[API 客户端](https://requestly.com/products/api-client/)和[会话记录](https://requestly.com/products/session-book/)功能。重定向 URL、修改 HTTP 标头、模拟 API、注入自定义 JS、修改 GraphQL 请求、生成模拟 API 终结点、记录带有网络和控制台日志的会话。免费套餐可创建最多 10 个规则。开源免费。
  * [seotest.me](https://seotest.me/)——免费的页面 SEO 网站测试工具。每天 10 个免费网站抓取。有关如何改善任何网站页面 SEO 结果的有用 SEO 学习资源和建议，无论技术如何。
  * [snippets.uilicious.com](https://snippets.uilicious.com)——它就像 CodePen 但用于跨浏览器测试。UI-licious 允许您编写类似用户故事的测试，并提供一个免费平台——UI-licious Snippets——允许您在 Chrome 上无限次免费测试，无需注册，每次测试运行最长 3 分钟。发现错误了吗？您可以复制测试的唯一 URL 并将其提供给开发人员，以确切地向他们展示如何重现该错误。
  ...
  * [qase.io](https://qase.io)——Dev 和 QA 团队的测试管理系统。管理测试用例，组成测试运行，执行测试，跟踪缺陷，并衡量影响。免费套餐包括所有核心功能，附件可用 500MB，最多可容纳 3 个用户。
  ...
  * [Vaadin](https://vaadin.com) - 使用 Java 或 TypeScript 构建可扩展的用户界面，并使用集成的工具、组件和设计系统更快地迭代、更好地设计和简化开发过程。五年免费维护，无限项目。
...
## Security and PKI
  ...
  * [aikido.dev](https://www.aikido.dev)——一站式应用安全平台，涵盖 SCA、SAST、CSPM、DAST、Secrets、IaC、Malware、容器扫描、EOL 等。免费计划包括两个用户，10 个存储库、1 个云、2 个容器和 1 个域的扫描。
  * [alienvault.com](https://www.alienvault.com/open-threat-exchange/reputation-monitor)——揭示您网络中的被入侵系统
  * [Altcha.org](https://altcha.org/anti-spam)——由自然语言处理和机器学习提供支持的网站和 API 垃圾邮件过滤器。免费计划包括每个域每天 200 次请求。
  ...
  * [Public Cloud Threat Intelligence](https://cloudintel.himanshuanand.com/)——针对公共云基础设施的高置信度 IOC，一部分可在 github（https://github.com/unknownhad/AWSAttacks）上获得。完整列表可通过 API 获取
  ...
  * [CyberChef](https://gchq.github.io/CyberChef/)——一个简单、直观的网络应用程序，用于分析和解码/编码数据，无需使用复杂的工具或编程语言。就像一把瑞士军刀的密码学和加密工具。所有功能均可免费使用，没有限制。如果您希望自行托管，也可以使用开源版本。
  ...
  * [Dependabot](https://dependabot.com/)——自动进行 Ruby、JavaScript、Python、PHP、Elixir、Rust、Java（Maven 和 Gradle）、.NET、Go、Elm、Docker、Terraform、Git 子模块和 GitHub Actions 的依赖项更新。
  ...
  * [Infisical](https://infisical.com/)——开源平台，可让您在团队和基础架构中管理开发人员的秘密：从本地开发到登台/生产第三方服务的各个方面。最多可供 5 名开发人员免费使用。
  ...
  * [pyup.io](https://pyup.io)——监控 Python 依赖项的安全漏洞并自动更新它们。免费适用于一个私有项目，开源项目不限。
  ...
  * [seclookup.com](https://www.seclookup.com/)——Seclookup API 可以丰富 SIEM 中的域名威胁指标，提供有关域名的全面信息，并提高威胁检测和响应能力。在此处[https://account.seclookup.com/]获取 50K 次查找的免费使用权限。
  ...
  * [StackHawk](https://www.stackhawk.com/)——在整个管道中自动执行应用程序扫描，以在安全漏洞进入生产环境之前发现并修复它们。一个应用程序的无限扫描和环境。
  ...
  * [Virgil Security](https://virgilsecurity.com/)——在您的数字解决方案中实现端到端加密、数据库保护、物联网安全等的工具和服务。免费适用于最多 250 个用户的应用程序。
  * [Vulert](https://vulert.com)——Vulert 持续监控您的开源依赖项以获取新漏洞，并推荐修复方法，无需安装或访问您的代码库。开源项目免费。

  * [HasMySecretLeaked](https://gitguardian.com/hasmysecretleaked) - 免费在 2000 万个公开的 GitHub 存储库、gist、问题和评论中搜索泄露的秘密
...
## Authentication, Authorization, and User Management
  ...
  * [Authgear](https://www.authgear.com) - 在几分钟内为您的应用带来无密码、OTP、2FA、单点登录等。所有前端都包含在内。免费使用，最高可达 5000 MAUs。
  * [Authress](https://authress.io/) - 身份验证登录和访问控制，任何项目的无限身份提供商。Facebook、Google、Twitter 等。前 1000 次 API 调用免费。
  * [Authy](https://authy.com) - 在多个设备上进行双因素身份验证 (2FA)，并提供备份。可替代 Google Authenticator。免费使用，最多 100 次成功身份验证。
  * [Cerbos Hub](https://www.cerbos.dev/product-cerbos-hub) - 用于编写、测试和部署访问策略的完整授权管理系统。细粒度授权和访问控制，免费使用，每月活跃主体最高 100。
  * [Clerk](https://clerk.com) - 用户管理、身份验证、2FA/MFA、用于登录、注册、用户个人资料等的预构建 UI 组件。免费使用，每月活跃用户最高 10,000。
  * [Cloud-IAM](https://www.cloud-iam.com/) - Keycloak 身份和访问管理即服务。免费使用，最高可达 100 用户和一个领域。
  * [Corbado](https://www.corbado.com/) - 将无密码优先身份验证添加到新的或现有的应用程序中。免费使用，无限制的 MAUs。
  * [Descope](https://www.descope.com/) - 高度可定制的身份验证流程，同时提供无代码和 API/SDK 方法，免费使用，每月活跃用户 7,500，租户 50（最多 5 个 SAML/SSO 租户）。
  * [duo.com](https://duo.com/) - 网站或应用程序的双因素身份验证 (2FA)。免费使用，最多 10 个用户，所有身份验证方法，无限制，集成，硬件令牌。
  * [Kinde](https://kinde.com/) - 简单、强大的身份验证，您可以在几分钟内集成到您的产品中。开始使用所需的一切，免费提供 7,500 个 MAU。
  ...
  * [Phase Two](https://phasetwo.io) - Keycloak 开源身份和访问管理。免费使用，最多 1000 个用户，最多 10 个 SSO 连接，利用 Phase Two 的增强容器 Keycloak，其中包括 [Organization](https://phasetwo.io/product/organizations/) 扩展。
  * [Stytch](https://www.stytch.com/) - 具有强大密码的灵活身份验证 API 和 SDK、无密码登录流程、MFA、SSO 等。内置电子邮件/短信发送和故障转移以及欺诈保护。提供 5,000 个每月活跃用户免费（B2C）或 1,000 个每月活跃用户和 25 个组织免费（B2B）。
  * [Stack Auth](https://stack-auth.com) - 开源身份验证，不会让人失望。最适合开发人员的解决方案，只需五分钟即可入门。免费自托管，或提供带有 10k 免费每月活跃用户的托管 SaaS 版本。
  * [SuperTokens](https://supertokens.com/) - 开源用户身份验证，可本机集成到您的应用程序中 - 使您能够快速启动，同时控制用户和开发人员体验。免费使用，最多 5000 MAUs。
  * [Warrant](https://warrant.dev/) - 为您的应用提供托管企业级授权和访问控制服务。免费层包括 100 万每月 API 请求和 1000 个授权规则。
  * [ZITADEL Cloud](https://zitadel.com) - 即开即用的用户和访问管理，适用于您和支持多租户 (B2B) 用例。免费使用，最多 25,000 个经过身份验证的请求，具有所有安全功能（OTP、无密码、策略等无付费墙）。
  * [PropelAuth](https://propelauth.com) - 立即向任何规模的公司销售，只需几行代码，免费使用，最多 200 个用户和 10k 交易电子邮件（带有水印品牌：“由 PropelAuth 提供支持”）。
  * [Logto](https://logto.io/) - 开发、保护和管理您产品的用户身份（用于身份验证和授权）。免费使用，最多 5,000 MAUs，提供开源自托管选项。
  * [WorkOS](https://workos.com/) - 免费用户管理和身份验证，最多 100 万 MAUs。支持电子邮件+密码、社交身份验证、Magic Auth、MFA 等。
...
## Mobile App Distribution and Feedback
  ...
  * [Diawi](https://www.diawi.com) - 直接将 iOS 和 Android 应用部署到设备上。免费计划：应用程序上传、密码保护链接、1 天过期、10 次安装。
  * [InstallOnAir](https://www.installonair.com) - 通过空中分发 iOS 和 Android 应用程序。免费计划：无限制上传、私人链接、访客 2 天过期，注册用户 60 天。
  * [GetUpdraft](https://www.getupdraft.com) - 分发移动应用进行测试。免费计划包括一个应用项目、三个应用版本、500MB 存储空间和每月 100 次应用安装。
  * [Appho.st](https://appho.st) - 移动应用托管平台。免费计划包括五个应用程序、每月 50 次下载和最大 100MB 文件大小。
...
## Management System
  ...
  * [Esper](https://esper.io) - 适用于 Android 设备的 MDM 和 MAM，带有 DevOps。一个用户许可证和 25 MB 应用程序存储可免费使用 100 个设备。
  * [jamf.com](https://www.jamf.com/) - iPad、iPhone 和 Mac 的设备管理，免费提供 3 个设备
  * [Miradore](https://miradore.com) - 设备管理服务。保持设备车队的最新状态，并免费安全地无限期使用设备。免费计划提供基本功能。
  ...
  * [ploi.io](https://ploi.io/) - 服务器管理工具，可轻松管理和部署服务器和站点。免费提供 1 台服务器。

**[⬆️ 返回顶部](#目录)**

## Messaging and Streaming

  * [Ably](https://www.ably.com/) - 具有存在感知、持久化和消息送达保证的实时消息服务。免费计划包括每月 300 万条消息、100 个峰值连接和 100 个峰值通道。
  * [cloudamqp.com](https://www.cloudamqp.com/) - 即 RabbitMQ 服务。Little Lemur 计划：每月最多 100 万条消息，最多 20 个并发连接，最多 100 个队列，最多 10,000 条排队消息，不同 AZ 中的多个节点
...
## Log Management
  ...
  * [bugfender.com](https://bugfender.com/)——免费，每天最多可保留 100k 行日志
  * [logentries.com](https://logentries.com/)——免费，每月最多可保留 5GB，七天
  * [loggly.com](https://www.loggly.com/)——免费，单个用户，每天 200MB，七天
  * [logz.io](https://logz.io/)——免费，每天最多可保留 1GB，一天
  ...
  * [sematext.com](https://sematext.com/logsene)——每天最多可保留 500MB，七天
  * [sumologic.com](https://www.sumologic.com/)——每天最多可保留 500MB，七天
  * [logflare.app](https://logflare.app/)——每个应用每月最多可免费记录 12960000 条条目，保留时间为三天
  * [logtail.com](https://logtail.com/)——基于 ClickHouse 的 SQL 兼容日志管理。每月最多可保留 1GB，保留时间为三天
...
## Translation Management
  ...
  * [crowdin.com](https://crowdin.com/)——无限项目，无限字符串，以及开源项目的协作者
  ...
  * [Loco](https://localise.biz/)——免费最多 2000 次翻译，无限译者，每个项目 10 种语言/项目，每个项目 1000 个可翻译资产
  * [oneskyapp.com](https://www.oneskyapp.com/)——最多 5 个用户的有限免费版，开源免费
  * [POEditor](https://poeditor.com/)——免费最多 1000 个字符串
  * [SimpleLocalize](https://simplelocalize.io/)——免费最多 100 个翻译键，无限字符串，无限语言，创业交易
  ...
  * [weblate.org](https://weblate.org/)——对于自由项目免费，免费层的字符串源最多为 10,000，无限的自托管本地版本
...
## Monitoring
  ...
  * [UptimeObserver.com](https://uptimeobserver.com) - 获得 20 个监控器，间隔 5 分钟，可自定义状态页面——甚至可用于商业用途。通过电子邮件和 Telegram 享受无限的实时通知。无需信用卡即可开始使用。
  * [Pingmeter.com](https://pingmeter.com/) - 5 个监控器，间隔 10 分钟。监控 SSH、HTTP、HTTPS 和任何自定义 TCP 端口。
  * [alerty.ai](https://www.alerty.ai) - 免费的 FE、BE、DB 等的 APM 和监控+免费代理运行。
  ...
  * [checklyhq.com](https://checklyhq.com)——开源端到端/合成监控和深度 API 监控，供开发人员使用。免费计划有 5 个用户和 50k+次检查运行。
  ...
  * [downtimemonkey.com](https://downtimemonkey.com/)——60 个监控器，间隔 5 分钟。电子邮件、Slack 警报。
  * [economize.cloud](https://economize.cloud)——通过组织云资源进行优化并报告，帮助揭开云基础设施成本的神秘面纱。免费适用于每月在 Google Cloud Platform 上花费最多 5,000 美元的情况。
  ...
  * [inspector.dev](https://www.inspector.dev)——在不到一分钟的时间内使用免费永久层创建完整的实时监控仪表板。
  * [instrumentalapp.com](https://instrumentalapp.com)——具有高达 500 个指标和 3 小时数据可见性的免费应用程序和服务器监控，易于使用
  ...
  * [OnlineOrNot.com](https://onlineornot.com/)——在线或不在线提供网站和 API 的监控，以及 cron 作业和计划任务的监控。还提供状态页面。免费层的前五个检查间隔为 3 分钟。免费层通过 Slack、Discord 和电子邮件发送警报。
  ...
  * [pingbreak.com](https://pingbreak.com/)——现代的监控服务。检查无限的 URL，并通过 Discord、Slack 或电子邮件获得停机通知。
  ...
  * [stathat.com](https://www.stathat.com/)——免费开始，无过期限制
  * [statuscake.com](https://www.statuscake.com/)——网站监控，免费测试无限，有限制
  ...
  * [Squadcast.com](https://squadcast.com) - Squadcast 是一款端到端事件管理软件，旨在帮助您推广 SRE 最佳实践。免费提供最多 10 个用户的永久计划
  ...
  * [Better Stack](https://betterstack.com/better-uptime) - 单一产品中包含正常运行时间监控、事件管理、值班调度/警报和状态页面。免费计划包括 10 个监控器，检查频率为 3 分钟，状态页面
  * [Pulsetic](https://pulsetic.com) - 10 个监控器，6 个月的历史正常运行时间/日志，无限状态页面和自定义域名！免费无限时间和无限电子邮件警报。您不需要信用卡
...
## Crash and Exception Handling
  ...
  * [bugsnag.com](https://www.bugsnag.com/) - 初始试用后，每月免费 2,000 个错误
  * [elmah.io](https://elmah.io/) - 网络开发人员的错误日志记录和正常运行时间监控。开源项目的免费小型企业订阅。
  * [Embrace](https://embrace.io/) - 移动应用监控。小型团队免费，每年最多 100 万用户会话。
  ...
  * [Jam](https://jam.dev) - 一键式开发者友好的错误报告。免费计划，无限 Jams。
...
## Education and Career Development
  ...
  * [Free Professional Resume Templates & Editor](https://www.overleaf.com/latex/templates/tagged/cv) - 免费平台，提供大量经验丰富的专业人士的简历模板，可完全克隆、编辑和下载，符合 ATS 优化。
  * [DeepLearning.AI 短期课程](https://www.deeplearning.ai/short-courses/) - 行业领先专家提供的免费短期课程，在一小时或更短时间内获得最新生成式 AI 工具和技术的实践经验。
  * [LabEx](https://labex.io) - 通过互动实验室和真实项目，发展 Linux、DevOps、网络安全、编程、数据科学等技能。
  ...
  * [MIT OpenCourseWare](https://ocw.mit.edu/) - MIT OpenCourseWare 是来自 2500 多门 MIT 课程的材料的在线出版物，与世界各地的学习者和教育工作者免费分享知识。Youtube 频道可以在 [@mitocw](https://www.youtube.com/@mitocw/featured) 找到

**[⬆️ 返回顶部](#table-of-contents)**

## Email

  * [10minutemail](https://10minutemail.com) - 用于测试的免费、临时电子邮件。
  ...
  * [forwardemail.net](https://forwardemail.net) - 自定义域的免费电子邮件转发。使用您的域名创建和转发无限数量的电子邮件地址（**注意**：如果您使用.casa、.cf、.click、.email、.fit、.ga、.gdn、.gq、.lat、.loan、.london、.men、.ml、.pl、.rest、.ru、.tk、.top、.work TLD，则需要付费）
  ...
  * [MailerSend.com](https://www.mailersend.com) - 电子邮件 API、SMTP，免费提供 3000 封交易邮件
  ...
  * [Mailtrap.io](https://mailtrap.io/) - 用于开发的虚假 SMTP 服务器，免费计划包含一个收件箱，100 条消息，没有团队成员，每秒两封电子邮件，没有转发规则。
  * [Mail7.io](https://www.mail7.io/) - 免费的 Temp Email 地址，供 QA 开发人员使用。使用 Web 界面或 API 即时创建电子邮件地址。
  * [Momentary Email](https://www.momentaryemail.com/) - 免费的临时电子邮件地址。在网站上或通过 RSS 源读取传入的电子邮件。
  ...
  * [Plunk](https://useplunk.com) - 免费每月 3K 封电子邮件。
  ...
  * [Substack](https://substack.com) - 无限免费新闻通讯服务。开始收费时支付费用。
  * [Sweego](https://www.sweego.io/) - 适用于开发人员的欧洲交易电子邮件 API。每天免费 500 封电子邮件。
  ...
  * [Tutanota](https://tutanota.com/)——免费的安全电子邮件帐户服务提供商，内置端到端加密，无广告，无跟踪。免费 1GB 存储空间。部分[开源](https://github.com/tutao/tutanota)，因此您可以自行托管
  ...
  * [Zoho](https://www.zoho.com)——最初是作为电子邮件提供商，但现在提供一系列服务，其中一些提供免费计划。提供免费计划的服务列表：
     ...
     - [Zoho Assist](https://www.zoho.com/assist)——Zoho Assist 的永久免费计划包括一个并发远程支持许可证，并可无限期访问 5 个无人参与计算机许可证，供专业和个人使用
     ...
     - [Subscriptions](https://zoho.com/subscriptions)——无限制客户/订阅者和 1 个用户的经常性计费管理，所有支付托管由 Zoho 完成。最后 40 个订阅指标存储
     - [Checkout](https://zoho.com/checkout)——具有 3 页和最多 50 笔支付的产品计费管理
     - [Desk](https://zoho.com/desk)——具有 3 个代理、私人知识库和电子邮件票证的客户支持管理。与[Assist](https://zoho.com/assist)集成，用于 1 个远程技术人员和 5 个无人值守计算机
  ...
  * [Maileroo](https://maileroo.com)——开发人员的 SMTP 中继和电子邮件 API。每月 5,000 封电子邮件，无限域名，免费电子邮件验证，黑名单监控，邮件测试器等

**[⬆️ 返回顶部](#目录)**

## Feature Toggles Management Platforms

  * [ConfigCat](https://configcat.com) - ConfigCat 是一款以开发人员为中心的功能标志服务，具有无限的团队规模、出色的支持和合理的价格标签。免费计划最多可支持 10 个标志、两个环境、1 个产品和每月 500 万次请求。
  * [Flagsmith](https://flagsmith.com) - 自信地发布功能；跨网络、移动和服务器端应用程序管理功能标志。使用我们的托管 API，部署到您自己的私有云，或在本地运行。
  * [GrowthBook](https://growthbook.io) - 开源功能标志和 A/B 测试提供程序，内置贝叶斯统计分析引擎。免费最多支持 3 个用户，无限制的功能标志和实验。
  * [Hypertune](https://www.hypertune.com) - 类型安全的功能标志、A/B 测试、分析和应用程序配置，具有 Git 风格的版本控制和同步、内存中、本地标志评估。免费最多支持 5 个团队成员，无限制的功能标志和 A/B 测试。
  * [Molasses](https://www.molasses.app) - 强大的功能标志和 A/B 测试。免费最多支持 3 个环境，每个环境有五个功能标志。
  ...
  * [Statsig](https://www.statsig.com) - 一个强大的功能管理、A/B 测试、分析等平台。其慷慨的免费计划提供无限的席位、标志、实验和动态配置，每月最多支持 100 万个事件。
...
## Font
  ...
  * [dafont](https://www.dafont.com/) - 该网站提供的字体是其作者的财产，包括免费软件、共享软件、演示版或公共领域。
  * [Everything Fonts](https://everythingfonts.com/) - 提供多种工具；@font-face、单位转换器、字体黑客和字体提交器。
  * [Font Squirrel](https://www.fontsquirrel.com/) - 免费字体，可用于商业工作。精心挑选这些字体，并以易于使用的格式呈现。
  * [Google Fonts](https://fonts.google.com/) - 许多免费字体可以通过下载或链接到 Google 的 CDN 轻松安装在网站上。
  ...
  * [fonts.xz.style](https://fonts.xz.style/) 免费且开源的服务，用于使用 CSS 将字体家族提供给网站。
...
## Forms
  ...
  * [Feathery](https://feathery.io) - 功能强大、对开发者友好的表单构建器。可构建注册/登录、用户引导、支付流程、复杂金融应用等。免费计划允许每月最多 250 次提交和五个活动表单。
  ...
  * [Formspree.io](https://formspree.io/)——使用 HTTP POST 请求发送电子邮件。免费层每个表单每月限制 50 次提交。
  * [Formsubmit.co](https://formsubmit.co/)——您 HTML 表单的简单表单端点。免费永久。无需注册。
  ...
  * [Hyperforms.app](https://hyperforms.app/)——在几秒钟内创建表单到电子邮件等，无需后端代码。个人帐户每月为您提供最多 50 次表单提交的免费服务。
  ...
  * [Qualtrics Survey](https://qualtrics.com/free-account)——使用此一流工具创建专业表单和调查。50 多个专家设计的调查模板。免费帐户的限制为：1 个活跃调查、100 个响应/调查和 8 个响应类型。
  ...
  * [Survicate](https://survicate.com/)——从所有来源收集反馈并使用一个工具发送后续调查。自动使用 AI 分析反馈并提取见解。免费电子邮件、网站、应用内或移动调查、AI 调查创建者和 25 个每月响应。
  ...
  * [Typeform.com](https://www.typeform.com/)——在网站上包含设计精美的表单。免费计划允许每个表单最多 10 个字段和每月 100 次响应。
  * [WaiverStevie.com](https://waiverstevie.com) - 具有 REST API 的电子签名平台。您可以使用 Webhooks 接收通知。免费计划对签署的文件加水印，但允许无限数量的信封+签名。
  * [Web3Forms](https://web3forms.com) - 无需编写后端代码的静态和 JAMStack 网站的联系表单。免费计划允许每月无限表单、无限域名和 250 次提交。
  ...
  * [Formester.com](https://formester.com) - 在您的网站上分享和嵌入独特外观的表单——创建的表单数量或计划限制的功能不受限制。每月免费获得高达 100 次提交。
  * [SimplePDF.eu](https://simplepdf.eu/embed) - 在您的网站上嵌入 PDF 编辑器，并将任何 PDF 转换为可填写的表单。免费计划允许每个 PDF 无限使用三个提交。
  * [forms.app](https://forms.app/)——使用强大功能（如条件逻辑、自动评分计算器和 AI）创建在线表单。免费计划可收集最多 100 次响应，可嵌入网站，或使用链接使用。
  * [Qualli](https://usequalli.com)——应用内调查，专为移动设备设计。使用 Qualli AI 打造完美问题。您可以在我们的免费计划中试用，最高可达 500 MAU，创建无限表单和触发器。
  * [Sprig](https://sprig.com/)——每月一次的产品内调查或调查，带有 GPT 供电的 AI 分析。
...
## Generative AI
  ...
  * [Portkey](https://portkey.ai/) - Gen AI 应用的控制面板，具有可观测性套件和 AI 网关。每月免费发送和记录高达 10,000 个请求。
  * [Braintrust](https://www.braintrustdata.com/) - Gen AI 的评估、提示符游乐场和数据管理。免费计划每周提供多达 1,000 条私有评估行。
  * [Findr](https://www.usefindr.com/) - 通用搜索，可让您一次搜索所有应用程序。搜索助手，可让您使用您的信息回答问题。免费计划提供无限统一搜索和 5 个每日合作代理查询。
  ...
  * [Comet Opik](https://www.comet.com/site/products/opik/) - 在整个开发和生产生命周期中评估、测试和交付 LLM 应用程序。
...
## CDN and Protection
  ...
  * [cdnjs.com](https://cdnjs.com/)——简单。快速。可靠。cdnjs 是一个免费且开源的 CDN 服务，超过 11%的网站都信任它，它由 Cloudflare 提供支持。
  * [developers.google.com](https://developers.google.com/speed/libraries/)——Google 托管库是最流行的开源 JavaScript 库的内容分发网络
  * [Stellate](https://stellate.co/)——Stellate 是一个快速、可靠的 GraphQL API CDN，且对两个服务免费。
  ...
  * [section.io](https://www.section.io/)——一种简单的方法，可以启动和管理完整的 Varnish 缓存解决方案。据称，一个站点终身免费。
...
## PaaS
  ...
  * [anvil.works](https://anvil.works) - 使用 Python 开发 Web 应用，无需其他任何东西。免费层可无限创建应用，且超时时间为 30 秒。
  ...
  * [codenameone.com](https://www.codenameone.com/)——开源、跨平台的移动应用开发工具链，适用于 Java/Kotlin 开发者。免费供商业使用，无项目数量限制。
  ...
  * [Deno Deploy](https://deno.com/deploy)——分布式系统，可在全球边缘运行 JavaScript、TypeScript 和 WebAssembly。免费层包括每天 100,000 次请求和每月 100 GiB 数据传输。
  * [domcloud.co](https://domcloud.co)——提供 CI/CD 与 GitHub、SSH 和 MariaDB/Postgres 数据库的 Linux 托管服务。免费版有 1GB 存储和 1GB 网络/月限制，且仅限免费域名。
  * [encore.dev](https://encore.dev/)——使用静态分析提供自动基础设施、无样板代码等的后端框架。包括为业余项目提供免费云托管。
  * [flightcontrol.dev](https://flightcontrol.dev/)——使用 Git 推送风格工作流程在自己的 AWS 账户上部署 Web 服务、数据库等。免费层适用于个人 GitHub 存储库上有 1 个开发人员的用户。AWS 费用通过 AWS 计费，但您可以使用信用额度和 AWS 免费层。
  * [gigalixir.com](https://gigalixir.com/)——Gigalixir 提供一个永不休眠的免费实例和一个免费的 PostgreSQL 数据库，限制为 2 个连接、10,000 行且无备份，适用于 Elixir/Phoenix 应用。
  * [Glitch](https://glitch.com/)——具有代码共享和实时协作功能的免费公共托管。免费计划每月有 1000 小时的限制。
  * [Lade](https://www.lade.io/)——为开发人员构建的云平台。免费层包括三个服务，可选择应用或数据库。每个服务包括 128MB RAM 和 1GB 存储。
  ...
  * [ampt.dev](https://getampt.com/)——Ampt 让团队无需复杂的配置或管理基础设施即可在 AWS 上构建、部署和扩展 JavaScript 应用。免费预览计划包括每小时 500 次调用、每天 2500 次调用和每月 50,000 次调用。自定义域仅在付费计划中允许。
  * [Koyeb](https://www.koyeb.com)——Koyeb 是一个面向开发者的无服务器平台，可在全球范围内部署应用。通过基于 git 的部署、本机自动缩放、全球边缘网络和内置服务网格和发现，无缝运行 Docker 容器、Web 应用和 API。免费实例可让您在德国法兰克福或美国华盛顿特区部署 Web 服务。免费的托管 PostgreSQL 数据库可在德国法兰克福（德国）、美国华盛顿特区（美国）和新加坡使用。512MB 内存、2GB 存储和 0.1 CPU。无需信用卡即可开始使用。
  * [Napkin](https://www.napkin.io/)——具有 500Mb 内存、默认超时时间为 15 秒和每月 5000 次免费 API 调用的 FaaS，速率限制为每秒 5 次调用。
  * [Meteor Cloud](https://www.meteor.com/cloud)——Galaxy 托管。Meteor 的平台即服务，适用于 Meteor 应用，包括免费的 MongoDB 共享托管和自动 SSL。
  ...
  * [YepCode](https://yepcode.io)——无服务器环境中连接 API 和服务的一体化平台。它带来了 NoCode 工具的所有敏捷性和优势，但具有使用编程语言的所有功能。免费层包括[1.000 yeps](https://yepcode.io/pricing/)。
  * [WunderGraph](https://cloud.wundergraph.com)——一个开放源代码平台，允许您快速构建、交付和管理现代 API。内置 CI/CD、GitHub 集成和自动 HTTPS。免费计划包括 3 个项目、1GB 出口、每月 300 分钟构建时间。
  ...
  * [genezio.com](https://genezio.com/)——一个无服务器功能提供程序，每月免费提供 100 万个功能调用、100GB 带宽和 10 个 cron 作业，仅供非商业或学术用途。

**[⬆️ 返回顶部](#table-of-contents)**

## BaaS

  * [Activepieces](https://www.activepieces.com) - 使用自动化流程将多个应用程序连接在一起。例如，当应用程序中发生事件时，发送一条 Slack 消息或添加一个 Google Sheet 行。每月免费使用 5000 个任务。
  ...
  * [convex.dev](https://convex.dev/) - 作为服务的反应式后端，托管您的数据（具有关系和可序列化 ACID 事务的文档）、无服务器功能和 WebSockets，以将更新流式传输到各个客户端。对于小型项目免费 - 最多 1M 记录，每月 5M 功能调用。
  ...
  * [hasura.io](https://hasura.io/) - Hasura 扩展您在任何地方托管的现有数据库，并提供可安全访问的即时 GraphQL API，可用于网络、移动和数据集成工作负载。免费 1GB/月的数据通过量。
  ...
  * [onesignal.com](https://onesignal.com/) - 无限免费推送通知。每月 10,000 封电子邮件发送，无限制的联系人以及自动预热访问权限。
  ...
  * [pubnub.com](https://www.pubnub.com/) - 免费推送通知，每月最多 100 万条消息和 100 个活跃设备
  ...
  * [Salesforce Developer Program](https://developer.salesforce.com/signup) — 使用拖放工具快速构建、测试和部署应用程序。自定义数据模型只需点击即可。使用 Apex 代码进一步扩展。使用强大的 API 与任何内容集成。使用企业级安全性保护自己。使用点击或任何前沿网络框架自定义 UI。免费开发者计划可访问完整的 Lightning 平台。
  * [ServiceNow Developer Program](https://developer.servicenow.com/) — 快速构建、测试和部署可改善您组织工作的应用程序。免费实例和早期预览访问权限。
  * [simperium.com](https://simperium.com/) - 立即在各个平台上随时随地自动移动数据，多平台，结构化数据的无限制发送和存储，每月最多 2,500 个用户。
  ...
  * [stackstorm.com](https://stackstorm.com/) — 用于应用程序、服务和工作流程的事件驱动自动化，免费，无流程、访问控制、LDAP
...
## Low-code Platform
  ...
  * [outsystems.com](https://www.outsystems.com/) - 企业网络开发 PaaS，适用于内部部署或云，免费的“个人环境”提供允许无限代码和最多 1GB 数据库
...
## Web Hosting
  ...
  * [Fern](https://buildwithfern.com)——在 Fern 的免费计划上构建和托管基于 Markdown 的文档站点。您甚至可以从 API 定义文件为您的站点自动生成 API 参考。该站点托管在_yoursite_.docs.buildwithfern.com上。
  ...
  * [Kinsta Static Site Hosting](https://kinsta.com/static-site-hosting/)——免费部署多达 100 个静态站点，自定义域名和 SSL，100GB 每月带宽，260+Cloudflare CDN 位置。
  ...
  * [render.com](https://render.com)——统一的云，用于构建和运行应用程序和站点，提供免费 SSL、全球 CDN、私有网络、从 Git 自动部署以及完全免费的网络服务、数据库和静态网页计划。
  ...
  * [tilda.cc](https://tilda.cc/)——一个站点，50 页，50MB 存储，170 多个可用的主要预定义块，没有字体，没有 favicon，也没有自定义域名。
...
## DNS
  ...
  * [1.1.1.1](https://developers.cloudflare.com/1.1.1.1/) - 由 Cloudflare 提供的免费公共 DNS 解析器，快速且安全（加密 DNS 查询）。可用于绕过互联网提供商的 DNS 阻塞、防止 DNS 查询监视以及[阻止成人和恶意软件内容](https://developers.cloudflare.com/1.1.1.1/1.1.1.1-for-families)。还可以通过[API 使用](https://developers.cloudflare.com/1.1.1.1/encrypted-dns/dns-over-https/make-api-requests)。注意：只是 DNS 解析器，不是 DNS 主机。
  ...
  * [Dynv6.com](https://dynv6.com/)—具有[API 支持](https://dynv6.com/docs/apis)和大量 DNS 记录类型（如 CNAME、MX、SPF、SRV、TXT 等）管理的免费 DDNS 服务。
  ...
  * [noip.at](https://noip.at/)—免费 DDNS 服务，无需注册、跟踪、记录或广告。对域名数量没有限制
  ...
  * [Hetzner](https://www.hetzner.com/dns-console)–具有 API 支持的来自 Hetzner 的免费 DNS 托管。
...
## IaaS
  ...
  * [filebase.com](https://filebase.com/)——基于区块链的 S3 兼容对象存储。5GB 免费存储空间，无限期
...
## Managed Data Services
  ...
  * [Astra](https://www.datastax.com/products/datastax-astra/)——作为服务的云原生 Cassandra，提供[80GB 免费层](https://www.datastax.com/products/datastax-astra/pricing)
  ...
  * [Upstash](https://upstash.com/)——无服务器 Redis，免费层每天最多 10,000 次请求，最大数据库大小 256MB，最大 20 个并发连接
  ...
  * [SeaTable](https://seatable.io/)——由 Seafile 团队构建的灵活、类似于电子表格的数据库。无限制的表、2,000 行、1 个月的版本控制，最多 25 个团队成员。
  ...
  * [Turso by ChiselStrike](https://chiselstrike.com/) - Turso 是边缘数据库中的 SQLite 开发人员体验。Turso 提供免费永久的起始计划，总存储量为 9GB，最多 500 个数据库，最多 3 个位置，每月读取 10 亿行，以及本地开发支持与 SQLite。
  ...
  * [Neo4j Aura](https://neo4j.com/cloud/aura/)——管理本机图形 DBMS /分析平台，具有 Cypher 查询语言和 REST API。图形大小限制（50k 个节点，175k 个关系）。
  * [Neon](https://neon.tech/)——托管 PostgreSQL，总存储量为 0.5GB，1 个项目，10 个分支，无限制的数据库，始终可用的主分支（非主分支计算后 5 分钟自动暂停），每月活动时间为 20 小时
  * [Dgraph Cloud](https://cloud.dgraph.io/pricing?type=free)——管理本机图形 DBMS，具有 GraphQL API。每天限制 1MB 数据传输。
  ...
  * [TigerGraph Cloud](https://www.tigergraph.com/cloud/)——管理本机图形 DBMS /分析平台，具有类似于 SQL 的图形查询语言和 REST API。一个免费实例，两个 vCPU，8GB 内存和 50GB 存储，闲置 1 小时后睡眠。
  ...
  * [8base.com](https://www.8base.com/) - 8base 是一个为 JavaScript 开发人员构建的全栈低代码开发平台，构建在 MySQL 和 GraphQL 之上以及无服务器后端即服务之上。它允许您使用 UI 应用程序构建器快速构建 Web 应用程序并快速扩展，免费层包括行数：2,500，存储：500，无服务器计算：1Gb/h，客户端应用程序用户：5。
  * [Nile](https://www.thenile.dev/) ——一个用于 B2B 应用程序的 Postgres 平台。无限数据库，始终可用，无停机，1GB 存储（总），5000 万个查询令牌，自动缩放，无限向量嵌入

## Tunneling, WebRTC, Web Socket Servers and Other Routers

  * [Pinggy](https://pinggy.io)——只需一条命令，无需下载，即可为公网提供本地 URL。支持 HTTPS/TCP/TLS 隧道。免费计划的隧道生存期为 60 分钟。
  * [conveyor.cloud](https://conveyor.cloud/)——Visual Studio 扩展，可将 IIS Express 暴露到本地网络或通过隧道暴露到公共 URL。
  * [Hamachi](https://www.vpn.net/)——LogMeIn Hamachi 是一种托管 VPN 服务，允许您使用免费计划（最多 5 人）安全地将类似 LAN 的网络扩展到分布式团队，该计划允许无限数量的网络。
  ...
  * [Radmin VPN](https://www.radmin-vpn.com/)——通过启用类似 LAN 的网络将多台计算机连接在一起。无限对等点。（Hamachi 替代方案）
  * [segment.com](https://segment.com/)——将事件转换并路由到其他第三方服务的中心。免费计划每月 100,000 个事件和 100,000 次尝试，保留三天。
  * Google STUN——[stun:stun.l.google.com:19302](stun:stun.l.google.com:19302)
  * Twilio STUN——[stun:global.stun.twilio.com:3478?transport=udp](stun:global.stun.twilio.com:3478?transport=udp)
  * [Tailscale](https://tailscale.com/)——使用开放源代码的 WireGuard 协议的零配置 VPN。适用于 MacOS、iOS、Windows、Linux 和 Android 设备。个人免费计划可使用 100 个设备和 3 个用户。
  * [webhookrelay.com](https://webhookrelay.com)——管理、调试、分流和代理您所有的 Webhook 到公共或内部（即 localhost）目的地。此外，通过获取公共 HTTP 终结点（`https://yoursubdomain.webrelay.io <----> http://localhost:8080`），可以将在私有网络中运行的服务器暴露在隧道之外。
  ...
  * [ZeroTier](https://www.zerotier.com)——FOSS 管理的虚拟以太网即服务。免费计划可在一个网络中拥有 25 个客户端的无限端到端加密网络。客户端适用于桌面/移动/NA；Web 界面用于配置自定义路由规则和批准私有网络上的新客户端节点。
...
## Issue Tracking and Project Management
  ...
  * [asana.com](https://asana.com/)——与协作者一起使用私人项目免费
  ...
  * [Chpokify](https://chpokify.com/)——基于团队的 Planning Poker，可节省 Sprint 估计时间。免费最多 5 个用户，免费 Jira 集成，无限视频通话，无限团队，无限会话。
  ...
  * [Cloudcraft](https://cloudcraft.co/)——使用 Cloudcraft 可视化设计器在几分钟内设计专业的架构图，针对 AWS 进行了优化，具有智能组件，可显示实时数据。免费计划为单个用户提供无限私人图表。
  ...
  * [easyretro.io](https://www.easyretro.io/)——简单直观的 Sprint 回顾工具。免费计划每月有三个公开板和一个调查。
  * [GForge](https://gforge.com)——用于复杂项目的项目管理和问题跟踪工具集，具有自托管和 SaaS 选项。SaaS 免费计划为前五个用户提供免费服务，开源项目也免费。
  ...
  * [WishKit](https://wishkit.io)——为您的 iOS/macOS 应用收集应用内用户反馈，并根据用户投票确定功能的优先级。免费计划最多可用于 1 个应用。
  * [Ilograph](https://www.ilograph.com/)——允许用户从多个角度和详细级别查看其基础设施的交互式图表。图表可以用代码表示。免费层最多有 3 个查看者的无限私人图表。
  * [Jira](https://www.atlassian.com/software/jira)——用于许多企业环境的高级软件开发项目管理工具。免费计划最多供 10 名用户使用。
  * [kanbanflow.com](https://kanbanflow.com/)——基于看板的项目管理。免费，高级版有更多选项
  ...
  * [Kumu.io](https://kumu.io/)——具有动画、装饰、过滤器、聚类、电子表格导入等功能的关系图。免费层允许无限公共项目。图形大小不受限制。免费私人项目供学生使用。如果您不想将文件公开在线（上传、编辑、下载、丢弃），则提供沙盒模式。
  * [Linear](https://linear.app/)——界面简洁的问题跟踪器。免费供无限成员使用，文件上传大小最多为 10MB，问题数量最多为 250（不包括存档）
  * [leiga.com](https://www.leiga.com/)——Leiga 是一款使用 AI 自动管理项目的 SaaS 产品，可帮助您的团队集中注意力并释放巨大潜力，确保项目按计划推进。免费供最多 10 名用户使用，20 个自定义字段，2GB 存储空间，AI 视频录制限制为 5 分钟/视频，自动化运行速度为 20/用户/月。
  ...
  * [Ora](https://ora.pm/)——敏捷任务管理和团队协作。免费供最多 3 名用户使用，文件大小限制为 10MB。
  ...
  * [快捷键](https://shortcut.com/) - 项目管理平台。免费版最多可容纳 10 名用户，且终身免费。
  ...
  * [Tara AI](https://tara.ai/)——简单的冲刺管理服务。免费版计划无用户限制，任务、冲刺和工作区无限。
  ...
  * [taskade.com](https://www.taskade.com/)——实时协作任务列表和团队大纲。免费版计划一个工作区，无限任务和项目；1GB 文件存储；项目历史记录为 1 周；每次视频会议最多 5 名与会者。
  * [taskulu.com](https://taskulu.com/)——基于角色的项目管理。免费版最多供 5 名用户使用。与 GitHub/Trello/Dropbox/Google Drive 集成
  * [Teaminal](https://www.teaminal.com) - 适用于远程团队的站立会议、回顾和冲刺规划工具。免费版最多供 15 名用户使用。
  * [teamwork.com](https://teamwork.com/)——项目管理和团队聊天。免费版供五名用户和两个项目使用。提供高级计划。
  * [teleretro.com](https://www.teleretro.com/)——简单有趣的回顾工具，带有破冰游戏、动图和表情符号。免费版计划包括三个回顾和无限成员。
  * [testlio.com](https://testlio.com/)——问题跟踪、测试管理和测试版测试平台。免费供私人使用
  * [terrastruct.com](https://terrastruct.com/)——专门用于软件架构的在线图表制作工具。免费层每图表最多 4 层。
  * [todoist.com](https://todoist.com/)——协作和个人任务管理。免费版计划：5 个活跃项目、项目中的 5 个用户、上传文件最大 5MB、3 个过滤器和一周的活动历史。
  ...
  * [vabotu](https://vabotu.com/) - 用于项目管理的协作工具。免费版和其他计划可供选择。自由职业者计划适用于 10 名用户，包括消息传递、任务看板、5GB 在线存储、工作区和导出数据。
  * [Wikifactory](https://wikifactory.com/)——具有项目、VCS 和问题的产品设计服务。免费版提供无限项目和协作者以及 3GB 存储空间。
  * [Yodiz](https://www.yodiz.com/)——敏捷开发和问题跟踪。免费版最多供 3 名用户使用，无限项目。
  ...
  * [Zube](https://zube.io)——具有免费计划的项目管理，适用于 4 个项目和 4 名用户。支持 GitHub 集成。
  * [Toggl](https://toggl.com/)——提供两个免费的生产力工具。[Toggl Track](https://toggl.com/track/)用于时间管理和跟踪应用程序，具有免费计划，提供与自由职业者设计的无缝时间跟踪和报告。它具有无限的跟踪记录、项目、客户端、标签、报告等。和[Toggl Plan](https://toggl.com/plan/)用于任务规划，具有免费计划，适用于独立开发人员，任务、里程碑和时间线不限。
...
## Storage and Media Processing
  ...
  * [AndroidFileHost](https://androidfilehost.com/)——免费文件共享平台，速度、带宽、文件数量、下载数量等无限制。主要面向与 Android 开发相关的文件，如 APK 构建、自定义 ROM 和修改等。但似乎也接受其他任何文件。
  ...
  * [sirv.com](https://sirv.com/)——智能图像 CDN，具有即时图像优化和调整大小功能。免费层包括 500MB 存储空间和 2GB 带宽。
  ...
  * [cloudinary.com](https://cloudinary.com/)——图像上传、强大的操作、存储和交付服务，适用于网站和应用程序，提供 Ruby、Python、Java、PHP、Objective-C 等更多语言的库。免费层包括 25 个每月信用额度。一个信用额度等于 1000 次图像转换、1GB 存储空间或 1GB CDN 使用量。
  * [embed.ly](https://embed.ly/)——提供在网页中嵌入媒体、响应式图像缩放和从网页提取元素的 API。免费计划可在每月 5000 个 URL、15 次请求/秒的情况下使用。
  * [filestack.com](https://www.filestack.com/)——文件选择、转换和交付，免费使用 250 个文件、500 次转换和 3GB 带宽。
  * [file.io](https://www.file.io)——2GB 文件存储。文件下载一次后自动删除。提供与存储交互的 REST API。请求限制为每分钟一次。
  * [freetools.site](https://freetools.site/)——免费在线工具。可转换或编辑文档、图像、音频、视频等。
  * [GoFile.io](https://gofile.io/)——免费文件共享和存储平台，可通过基于网络的用户界面和 API 使用。无文件大小、带宽、下载数量等限制。但文件在不活动（10 天以上没有下载）时会被删除。
  ...
  * [kraken.io](https://kraken.io/)——网站性能优化服务，免费计划可处理高达 1MB 文件大小的文件。
  * [kvstore.io](https://www.kvstore.io/)——键值存储服务。免费层允许 100 个键、1KB/键、100 次调用/小时。
  * [npoint.io](https://www.npoint.io/)——具有协作模式编辑的 JSON 存储。
  * [nitropack.io](https://nitropack.io/)——自动进行前端优化（缓存、图像和代码优化、CDN），加速网站速度。免费计划适用于每月最多 5000 次页面浏览量。
  * [otixo.com](https://www.otixo.com/)——从一个地方加密、共享、复制和移动您所有云存储文件。基本计划提供无限文件传输，最大文件大小为 250MB，并允许五个加密文件。
  * [packagecloud.io](https://packagecloud.io/)——提供 YUM、APT、RubyGem 和 PyPI 的托管包存储库。可通过请求获得有限的免费计划和开源计划。
  * [getpantry.cloud](https://getpantry.cloud/)——适用于个人项目、黑客马拉松和移动应用程序的简单 JSON 数据存储 API！
  * [piio.co](https://piio.co/)——适用于每个网站的响应式图像优化和交付。免费计划适用于开发人员和个人网站。包括免费 CDN、WebP 和即时加载。
  * [Pinata IPFS](https://pinata.cloud)——Pinata 是上传和管理 IPFS 上文件的最简单方法。我们友好的用户界面和 IPFS API 使 Pinata 成为平台、创作者和收藏家最容易使用的 IPFS 钉扎服务。提供 1GB 免费存储空间和 API 访问权限。
  ...
  * [Shotstack](https://shotstack.io)——用于按比例生成和编辑视频的 API。免费计划每月最多可生成 20 分钟的渲染视频。
  * [tinypng.com](https://tinypng.com/)——用于压缩和调整 PNG 和 JPEG 图像大小的 API，每月免费提供 500 次压缩。
  * [transloadit.com](https://transloadit.com/)——处理文件上传以及视频、音频、图像、文档的编码。免费计划适用于开源、慈善机构和学生，通过 GitHub 学生开发人员包。商业应用程序可获得 2GB 免费试用期。
  ...
  * [imagekit.io](https://imagekit.io)——图像 CDN，具有自动优化、实时转换和存储功能，可在几分钟内集成到现有设置中。免费计划包括每月 20GB 的带宽。
  * [internxt.com](https://internxt.com)——Internxt Drive 是基于绝对隐私和毫不妥协的安全性的零知识文件存储服务。注册即免费获得 10GB，永久有效！
  * [degoo.com](https://degoo.com/)——基于 AI 的云存储，免费提供 20GB、三个设备、5GB 推荐奖金（90 天账户不活动）。

  * [ImageEngine](https://imageengine.io/)——ImageEngine 是一个易于使用的全球图像 CDN。设置时间在 60 秒以下。支持 AVIF 和 JPEGXL，有 WordPress、Magento、React、Vue 等插件，还有更多。在此[申请](https://imageengine.io/developer-program/)免费开发者账号。
  * [DocsParse](https://docsparse.com/)——使用 GPT 处理 PDF、图像等的 AI，将其转换为 JSON、CSV、EXCEL 格式的结构化数据。每月免费 30 个积分。
...
## Design and UI
  ...
  * [AllTheFreeStock](https://allthefreestock.com/) - 一个精心挑选的免费图片、音频和视频资源列表。
  * [Float UI](https://floatui.com/) - 一个免费的网络开发工具，用于快速创建具有现代感和响应式的网站，即使是非设计师也能使用。
  * [Ant Design Landing Page](https://landing.ant.design/) - Ant Design Landing Page 提供了一个由 Ant Motion 的动画组件构建的模板。它有一个丰富的主页模板，下载模板代码包，可以快速使用。你也可以使用编辑器快速构建自己的专用页面。
  * [Backlight](https://backlight.dev/) - 一个完整的编码平台，由开发人员和设计师共同打造，团队可以在其中构建、记录、发布、扩展和维护设计系统。免费计划允许最多 3 个编辑器在一个设计系统上工作，无限制的查看者。
  ...
  * [easyvectors.com](https://easyvectors.com/) - 免费的 SVG 向量艺术库存。下载最佳的向量图形，完全免费。
  * [figma.com](https://www.figma.com/) - 在线团队协作设计工具；免费层包括无限制的文件和查看器，最大有 2 个编辑器和 3 个项目。
  * [Flyon UI](https://flyonui.com/) - 用于 Tailwind CSS 的最简单组件库。
  * [framer.com](https://www.framer.com/) - Framer 帮助你为下一个应用、网站或产品迭代和动画界面创意——从强大的布局开始。对于将 Framer 视为专业原型工具的任何人：无限的查看者，最多 2 个编辑器，最多 3 个项目。
  ...
  * [landen.co](https://www.landen.co) - 为你的初创公司生成、编辑和发布漂亮的网站和登录页面。无需代码。免费层允许你拥有一个完全可定制的网站，并在网络上发布。
  * [Quant Ux](https://quant-ux.com/) - Quant Ux 是一个原型制作和设计工具。- 它完全免费，也是开源的。
  * [lensdump.com](https://lensdump.com/) - 免费的云图像托管。
  * [Lorem Picsum](https://picsum.photos/) - 一个免费的工具，易于使用，时尚的占位符。在我们的 URL 之后，添加你想要的图像大小（宽度和高度），你将得到一个随机图像。
  ...
  * [Mindmup.com](https://www.mindmup.com/) - 无限的思维导图免费使用，并存储在云中。你的思维导图随时随地，从任何设备都可以使用。
  ...
  * [Octopus.do](https://octopus.do) - 可视化站点图生成器。实时构建你的网站结构，并快速与你的团队或客户共享以进行协作。
  ...
  * [Plasmic](https://www.plasmic.app/) - 一个快速、易于使用、强大的网络设计工具和页面构建器，可集成到你的代码库中。构建响应式页面或复杂组件；可选地使用代码扩展；并发布到生产站点和应用程序。
  * [Pravatar](https://pravatar.cc/) - 生成一个随机的占位符假头像，其 URL 可以直接在你的网络/应用程序中链接。
  * [Proto.io](https://www.proto.io) - 创建无需编码的全交互式 UI 原型。免费层在免费试用期结束后可用。免费层包括一个用户、一个项目、五个原型、100MB 的在线存储和 proto.io 应用程序的预览。
  ...
  * [UI Avatars](https://ui-avatars.com/) - 根据名称的首字母生成头像。其 URL 可以直接在你的网络/应用程序中链接。支持通过 URL 配置参数。
  * [unDraw](https://undraw.co/) - 一个不断更新的美丽 SVG 图像集合，你可以完全免费使用，无需归属。
  * [unsplash.com](https://unsplash.com/) - 免费的商业和非商业用途库存照片（你可以随意使用的许可证）。

  * [walkme.com](https://www.walkme.com/)——企业级指导和参与平台，免费计划可进行三次最多 5 步/步的引导。
  ...
  * [SceneLab](https://scenelab.io)——在线样机图形编辑器，拥有不断扩展的免费设计模板集合。
  * [xLayers](https://xlayers.dev)——预览和将 Sketch 设计文件转换为 Angular、React、Vue、LitElement、Stencil、Xamarin 等（免费和开源，可在 [https://github.com/xlayers/xlayers](https://github.com/xlayers/xlayers) 上获得）
  ...
  * [Unicorn Platform](https://unicornplatform.com/)——无代码前端设计和开发平台。Unicorn Platform 是一个协作前端平台，可立即创建和发布无后端静态网站。三个免费项目，无限协作者和免费代码导出。
  ...
  * [svgrepo.com](https://www.svgrepo.com/)——使用各种向量库探索、搜索和找到最适合您项目的图标或向量。下载免费 SVG 向量用于商业用途。
  ...
  * [TeleportHQ](https://teleporthq.io/)——低代码前端设计和开发平台。TeleportHQ 是一个协作前端平台，可立即创建和发布无头静态网站。三个免费项目，无限协作者和免费代码导出。
  ...
  * [hypercolor.dev](https://hypercolor.dev/)——Tailwind CSS 颜色渐变的精选集合，还提供多种生成器来创建自己的渐变。
  ...
  * [Image BG Blurer](https://imagebgblurer.com/)——使用该图像源作为背景模糊，为 Notion、Trello、Jira 等工具生成图像的模糊背景框架。
  * [Webstudio](https://webstudio.is/)——Webflow 的开源替代方案。免费计划在其域名上提供无限数量的网站。五个网站使用自定义域名。每月一万次页面浏览量。2GB 资产存储。
  * [Nappy](https://nappy.co/)——免费的黑人棕色人物照片，可用于商业和个人用途。

**[⬆️ 返回顶部](#table-of-contents)**

## Design Inspiration

  * [awwwards.](https://www.awwwards.com/) - [顶尖网站]展示所有最佳设计的网站（由设计师投票）。
  * [Behance](https://www.behance.net/) - [设计展示]设计师展示作品的地方。可以按 UI/UX 项目的类别进行过滤。
  * [dribbble](https://dribbble.com/) - [设计展示]独特的设计灵感，通常不是来自真实应用程序。
  * [Landings](https://landings.dev/) - [网页截图]根据您的喜好，找到最适合您设计灵感的最佳登录页面。
  * [Lapa Ninja](https://www.lapa.ninja/) - [登录页面/UI 套件/网页截图]Lapa Ninja 是一个画廊，展示了最好的 6025 个登录页面示例，为设计师提供免费书籍和免费的 UI 套件来自网络。
  * [LovelyLanding.net](https://www.lovelylanding.net/) - [登录页面设计]经常更新的登录页面屏幕截图。包括桌面、平板电脑和移动屏幕截图。
  ...
  * [Mobile Patterns](https://www.mobile-patterns.com/) - [移动屏幕截图]一个设计灵感库，其中包含设计师、开发人员和产品制造商可以参考的 iOS 和 Android 上最优秀的 UI UX 模式（iOS 和 Android）。
...
## Data Visualization on Maps
  ...
  * [IP Geolocation](https://ipgeolocation.io/)——提供免费开发者计划，每月有 30K 请求。
...
## IDE and Code Editing
  ...
  * [BlueJ](https://bluej.org) - 一种免费的 Java 开发环境，专为初学者设计，全球有数百万人使用。由 Oracle 提供支持，具有简单的 GUI 可帮助初学者。
  ...
  * [Cody](https://sourcegraph.com/cody) - 免费的人工智能编码助手，可以编写（代码块、自动完成、单元测试）、理解（您整个代码库的知识）、修复和查找您的代码。可用于 VS Code、JetBrains 和在线使用。
  ...
  * [cocalc.com](https://cocalc.com/) - （以前是 cloud.sagemath.com 上的 SageMathCloud）- 云协作计算。浏览器访问带有内置协作功能的完整 Ubuntu，以及为数学、科学、数据科学预装的大量免费软件：Python、LaTeX、Jupyter Notebooks、SageMath、scikitlearn 等。
  ...
  * [Eclipse Che](https://www.eclipse.org/che/) - 适用于开发团队的基于 Web 和 Kubernetes 的多语言 IDE。开源且由社区驱动。红帽提供的在线实例可在[workspaces.openshift.com](https://workspaces.openshift.com/)上获得。
  ...
  * [jsbin.com](https://jsbin.com) - 另一个前端网页的游乐场和代码共享网站（HTML、CSS 和 JavaScript。它也支持 Markdown、Jade 和 Sass）。
  * [jsfiddle.net](https://jsfiddle.net/) - 前端网页的游乐场和代码共享网站，支持协作。
  * [JSONPlaceholder](https://jsonplaceholder.typicode.com/) 一些返回 JSON 格式虚假数据的 REST API 端点。如果您想在本地运行服务器，也可以提供源代码。
  ...
  * [Prepros](https://prepros.io/) - Prepros 可以开箱即用编译 Sass、Less、Stylus、Pug/Jade、Haml、Slim、CoffeeScript 和 TypeScript，重新加载您的浏览器，并使开发和测试您的网站变得容易，以便您专注于使其完美。您还可以通过点击几下添加自己的工具。
  ...
  * [VSCodium](https://vscodium.com/)——社区驱动的、无遥测/跟踪功能的、自由许可的微软编辑器 VSCode 的二进制分发版
  * [wakatime.com](https://wakatime.com/)——使用文本编辑器插件对您的编码活动进行自我量化指标分析，免费计划有限。
...
## Analytics, Events and Statistics
  ...
  * [Cauldron](https://cauldron.io) — 分析开源解决方案，允许用户将来自多个协作平台的信息聚合为不同类型的数据源（Git、Github 和 Gitlab）。免费套餐包括无限数量的报告。
  ...
  * [GoatCounter](https://www.goatcounter.com/) - GoatCounter 是一个开源的网络分析平台，可作为托管服务（免费供非商业使用）或自托管应用程序使用。它旨在提供易于使用和有意义的隐私友好型网络分析，作为 Google Analytics 或 Matomo 的替代方案。免费套餐适用于非商业用途，包括无限站点、6 个月的数据保留和每月 10 万次页面浏览量。
  * [Google Analytics](https://analytics.google.com/) - Google Analytics
  * [MetricsWave](https://metricswave.com) - 对开发人员友好的隐私替代方案 Google Analytics。免费计划允许每月 20k 次页面浏览量，无需信用卡。
  * [Expensify](https://www.expensify.com/) - 费用报告，免费个人报告审批工作流程
  * [getinsights.io](https://getinsights.io) - 关注隐私的分析，每月最多 3k 次事件免费
  * [heap.io](https://heap.io) - 在 iOS 或网络应用中自动捕获每个用户操作。免费提供高达 10K 个每月会话。
  * [Hotjar](https://hotjar.com) - 网站分析和报告。免费计划允许每天 2000 次页面浏览量。每天一百个快照（最大容量：300）。可以存储三个快照热图 365 天。无限团队成员。还包括应用内和独立调查、带有屏幕截图的反馈小部件。免费套餐允许每月创建 3 次调查和 3 次反馈小部件，并收集 20 次回复。
  ...
  * [Moesif](https://www.moesif.com) - API 分析，适用于 REST 和 GraphQL。（免费使用高达 500 万次 API 调用/月）
  ...
  * [StatCounter](https://statcounter.com/) - 网站访问者分析。免费计划适用于最近 500 名访问者的分析。
  * [Statsig](https://statsig.com) - 涵盖分析、功能标志和 A/B 测试的一体化平台。免费提供高达每月 100 万次度量事件。
  ...
  * [usabilityhub.com](https://usabilityhub.com/) - 在真实用户上测试设计和模型并跟踪访问者。免费供一个用户使用，无限次测试
  * [woopra.com](https://www.woopra.com/) - 免费的用户分析平台，适用于 500K 次操作，90 天数据保留，30 多个一键集成。
  * [counter.dev](https://counter.dev) - 简单易用的网络分析工具，注重隐私。免费或根据需要通过捐赠支付。
  * [PostHog](https://posthog.com) - 全面的产品分析套件，每月最多可跟踪 100 万次事件免费。还提供每月 250 次回复的无限应用内调查。
  * [Uptrace](https://uptrace.dev) - 分布式跟踪工具，可帮助开发人员找出故障并找到性能瓶颈。有免费计划，为开源项目提供免费的个人订阅，并且有开源版本。
  * [Microsoft Clarity](https://clarity.microsoft.com/) - 一种免费、易于使用的工具，可捕获真实用户如何使用您的网站。
  * [Beampipe.io](https://beampipe.io) - 简单、注重隐私的网络分析。免费提供最多 5 个域名和 10k 每月页面浏览量。
  * [Aptabase](https://aptabase.com) - 适用于移动和桌面应用的开源、隐私友好且简单的分析。适用于 Swift、Kotlin、React Native、Flutter、Electron 等的 SDK。免费提供每月最多 20,000 次事件。
  ...
  * [TrackWith Dicloud](https://dicloud.net/trackwith-privacy-focused-analytics/) - 免费的轻量级注重隐私的 Google Analytics 替代方案。无限制的页面浏览量、无限制的访问者、无限制的页面热点图和目标跟踪。免费提供最多 3 个域名和每个域名 600 次会话重放。
  * [AppFit](https://appfit.io) - AppFit 是一款全面的分析和产品管理工具，旨在促进跨平台的分析和产品更新的无缝管理。免费计划包括每月 10,000 次事件、产品日记和每周洞察。

**[⬆️ 返回顶部](# 目录 )**

## Visitor Session Recording

  * [Reactflow.com](https://www.reactflow.com/)——按站点计算：每天 1000 页面浏览量，三个热图，三个小部件，免费错误跟踪
  * [OpenReplay.com](https://www.openreplay.com)——开源会话重放，带有用于错误重现的开发工具，实时会话支持，以及产品分析套件。每月 1000 个会话，访问所有功能和 7 天保留期。
  ...
  * [hotjar.com](https://www.hotjar.com/)——按站点计算：每月 1050 页浏览量，无限热图，数据存储三个月
  ...
  * [UXtweak.com](https://www.uxtweak.com/)——记录并观看访问者如何使用您的网站或应用。小型项目免费无限时长
...
## Payment and Billing Integration
  ...
  * [Qonversion](http://qonversion.io/)——一体化跨平台订阅管理平台，提供分析、A/B 测试、Apple Search Ads、远程配置和增长工具，用于优化应用内购买和货币化。与 iOS、Android、React Native、Flutter、Unity、Cordova、Stripe 和网络兼容。免费使用，每月最高可追踪收入为 10 万美元。
  * [ParityVend](https://www.ambeteco.com/ParityVend/)——根据访问者的位置自动调整价格，以扩大您的全球业务并进入新市场（购买力平价）。免费计划包括每月 7500 次 API 请求。
  * [Glassfy](https://glassfy.io/)——应用内订阅基础架构，实时订阅事件和现成的货币化工具，适用于 iOS、Android、Stripe 和 Paddle。免费使用，每月最高收入为 10 万美元。
  * [Adapty.io](https://adapty.io/)——用于移动应用内订阅集成到 iOS、Android、React Native、Flutter、Unity 或网络应用的一站式解决方案，带有开源 SDK。免费使用，每月最高收入为 10 万美元。
  * [CoinMarketCap](https://coinmarketcap.com/api/)——提供加密货币市场数据，包括最新的加密和法定货币汇率。免费层每月提供 10K 调用信用。
  * [CurrencyFreaks](https://currencyfreaks.com/)——提供当前和历史货币汇率。免费的 DEVELOPER 计划，每月有 1000 次请求。
  * [CoinGecko](https://www.coingecko.com/en/api)——提供加密货币市场数据，包括最新的加密货币汇率和历史数据。演示 API 提供稳定的每分钟 30 次调用限制和每月 10,000 次调用上限。
  * [CurrencyApi](https://currencyapi.net/)——用于物理和加密货币的实时货币汇率，以 JSON 和 XML 格式提供。免费层每月提供 1250 次 API 请求。
  * [currencylayer](https://currencylayer.com/)——为您的企业提供可靠的汇率转换，每月免费 100 次 API 请求。
  * [exchangerate-api.com](https://www.exchangerate-api.com)——易于使用的货币转换 JSON API。免费层每天更新一次，每月限制 1500 次请求。
  ...
  * [RevenueCat](https://www.revenuecat.com/)——应用内购买和订阅的托管后端（iOS 和 Android）。免费使用，每月最高可追踪收入为 2500 美元。
...
## Dev Blogging Sites
  ...
  * [BearBlog](https://bearblog.dev/) - 简洁的、基于 Markdown 的博客和网站生成器。
...
## Commenting Platforms
  ...
  * [Remarkbox](https://www.remarkbox.com/) - 开源托管评论平台，“在几个域上有一个管理员，完全控制行为和外观”，你可以按需支付。
...
## Screenshot APIs
  ...
  * [ApiFlash](https://apiflash.com)——基于 Aws Lambda 和 Chrome 的屏幕截图 API。处理整页、捕获时间和viewport 尺寸。
  * [microlink.io](https://microlink.io/)——将任何网站转换为数据，例如元标记规范化、美化链接预览、抓取功能或屏幕截图即服务。每天免费 250 次请求。
  ...
  * [PhantomJsCloud](https://PhantomJsCloud.com)——浏览器自动化和页面渲染。免费层每天最多提供 500 页。自 2017 年以来一直是免费层。
  * [Webshrinker.com](https://webshrinker.com)——Web Shrinker 提供网站屏幕截图和域名智能 API 服务。每月免费 100 次请求。
...
## Flutter Related and Building IOS Apps without Mac
  ...
  * [CodeMagic](https://codemagic.io/) - CodeMagic 是一个完全托管和管理的移动应用 CI/CD。你可以使用基于 GUI 的 CI/CD 工具进行构建、测试和部署。免费套餐提供 500 分钟/月的免费时长和一个配备 2.3GHz 双核处理器和 8GB 内存的 Mac Mini 实例。
...
## Privacy Management
  ...
  * [Bearer](https://www.bearer.sh/) - 通过审计和持续工作流程帮助实现隐私设计，确保组织遵守 GDPR 等法规。免费版仅限于较小的团队，SaaS 版本也是如此。
  * [Osano](https://www.osano.com/) - 包含从 GDPR 代表到 cookie 横幅的同意管理和合规平台。免费版提供基本功能。
  * [Iubenda](https://www.iubenda.com/) - 隐私和 cookie 政策及同意管理。免费版提供有限的隐私和 cookie 政策以及 cookie 横幅。
  ...
  * [Concord](https://www.concord.tech/) - 完整的数据隐私平台，包括同意管理、隐私请求处理（DSARs）和数据映射。免费版包括核心同意管理功能，并且还为经过验证的开源项目提供更高级的计划。
...
## Miscellaneous
  ...
  * [BinShare.net](https://binshare.net) - 创建和分享代码或二进制文件。可以作为美丽的图像分享，例如用于 Twitter / Facebook 帖子，也可以作为链接分享，例如用于聊天或论坛。
  ...
  * [FOSSA](https://fossa.com/) - 端到端管理第三方代码、许可证合规性和漏洞的可扩展解决方案。
  * [fullcontact.com](https://www.fullcontact.com/developer/pricing/) - 通过在应用中添加社交个人资料，帮助用户更多地了解他们的联系人。每月 500 个免费个人 API 匹配。
  ...
  * [redirect.pizza](https://redirect.pizza/) - 轻松管理具有 HTTPS 支持的重定向。免费计划包括 10 个源和每月 100,000 次点击。
  ...
  * [snappify](https://snappify.com) - 使开发人员能够创建惊人的视觉效果。从美丽的代码片段到完整的技术演示文稿。免费计划包括一次最多 3 个快照，每月无限下载和 5 个 AI 驱动的代码解释。
  ...
  * [Versionfeeds](https://versionfeeds.com) - 您喜爱的软件版本的自定义 RSS 饲料。将您最喜欢的编程语言、库或工具的最新版本放在一个饲料中。（前 3 个饲料是免费的）
...
## Other Free Resources
  ...
  * [Wikimint 开发者](https://developer.wikimint.com/p/tools.html) - 为网络开发者提供的免费工具，包括 CSS 压缩、解压缩、图像优化、图像大小调整、大小写转换、CSS 验证、JavaScript 编译、HTML 编辑器等。
  ...
  * [Microsoft 365 开发者计划](https://developer.microsoft.com/microsoft-365/dev-program) - 获得免费沙盒、工具和其他资源，以构建适用于 Microsoft 365 平台的解决方案。该订阅是 90 天的 [Microsoft 365 E5 订阅](https://www.microsoft.com/microsoft-365/enterprise/e5)（不包括 Windows），可续订。如果您积极参与开发（通过遥测数据和算法衡量），则会续订。
  ...
  * [SimpleBackups.com](https://simplebackups.com/) - 针对服务器和数据库（MySQL、PostgreSQL、MongoDB）的备份自动化服务，直接存储到云存储提供商（AWS、DigitalOcean 和 Backblaze）。提供免费计划，可进行 1 次备份。
  ...
  * [Web.Dev](https://web.dev/measure/) - 这是一款免费工具，可帮助您查看网站性能并改善 SEO，以在搜索引擎中获得更高的排名。
  ...
  * [UseCSV by Layercode](https://layercode.com/usecsv) - 在您的网络应用程序中几分钟内添加 CSV 和 Excel 导入功能。为您的用户提供愉快且强大的数据导入体验。免费开始，无需任何信用卡详细信息，立即开始集成 UseCSV。您可以创建无限的导入器，上传的文件最大可达 100Mb。
  * [Buttons Generator](https://markodenic.com/tools/buttons-generator/) - 可在项目中使用的 100 多个按钮。
  ...
  * [QRCodeBest](https://qrcode.best/) - 使用 13 种模板创建自定义 QR 码，具有完全隐私和个人品牌。功能包括跟踪像素、项目分类和无限团队席位。
  * [PostPulse](https://post-pulse.io) - 通过每月由人工智能制作的文章提升在线形象，增强 SEO 和网站排名，这些文章针对 SEO 优化的域名进行优化。免费计划允许您每月在我们的网站上手动发布一篇文章。
  ...
  * [Killer Coda](https://killercoda.com/) - 在您的浏览器中进行交互式演练，以学习 Linux、Kubernetes、容器、编程、DevOps、网络
