...
以下是具有免费开发人员层的软件（SaaS、PaaS、IaaS等）和其他产品的列表。

本列表的范围仅限于基础设施开发人员（系统管理员、DevOps 从业人员等）可能发现有用的产品。我们喜欢所有免费的服务，但最好还是保持主题不变。有时这是一条灰色线，所以这是带有主观意见的列表; 如果我没有接受您的贡献，请不要感到冒犯。

该列表来自 1600+ 人的拉取请求、评论、想法和工作。您也可以通过发送[拉取请求](https://github.com/ripienaar/free-for-dev)来帮助添加更多服务或删除产品/服务以帮助其产品/服务被纳入此列表。

[![跟踪很棒的列表](https://www.trackawesomelist.com/badge.svg)](https://www.trackawesomelist.com/ripienaar/free-for-dev)

**注意**：此列表仅适用于即开即用服务，不适用于自托管软件。要符合条件，服务必须提供免费层，而不仅仅是免费试用。如果免费层是按时间分段提供的，则至少必须有一年。我们还从安全角度考虑免费层，因此 SSO 是可以接受的，但我不会接受将 TLS 限制为仅付费层的服务。
...
## 主要云服务提供商的永久免费限制
  ...
  * [谷歌云平台](https://cloud.google.com)
    ...
    * Compute Engine - 1 个非抢占式 e2-micro，30GB HDD，5GB 快照存储（限制在特定区域），每月 1 GB 网络出站流量从北美到所有区域目的地（不包括中国和澳大利亚）
    ...
    * Cloud Run - 每月 200 万个请求，360,000 GB-秒内存，180,000 vCPU-秒计算时间，每月 1 GB 网络出站到北美
...
* [亚马逊网络服务](https://aws.amazon.com)
    ...
    * [CodeCommit](https://aws.amazon.com/codecommit/) - 5 个活跃用户，50GB 存储和 10,000 次请求
...
* [微软 Azure](https://azure.microsoft.com)
    ...
    * [Azure DevOps](https://azure.microsoft.com/services/devops/) - 5 个活跃用户，无限量私人 Git 存储库
    * [Azure Pipelines](https://azure.microsoft.com/services/devops/pipelines/) — 10 个免费并行作业，为开源 Linux、macOS 和 Windows 提供无限分钟
    * [微软物联网中心](https://azure.microsoft.com/services/iot-hub/) - 每天 8,000 条消息
    ...
    * [带宽](https://azure.microsoft.com/pricing/details/bandwidth/) - 每月 15GB 入站（12 个月）和 5GB 出站
...
* [甲骨文云](https://www.oracle.com/cloud/)
    ...
    * 带宽 - 每月 10 TB 出站流量，速率限制为 x64 架构 VM 的 50 Mbps 和 ARM 架构 VM 的 500 Mbps * 核心数
...
* [Cloud 66](https://www.cloud66.com/) - 个人项目免费（包括一个部署服务器和一个静态站点），Cloud 66 为您提供在任何云上构建、部署和扩展应用程序所需的一切，而无需处理“服务器相关事宜”。
...
* [scalr.com](https://scalr.com/) - Scalr 是一个 Terraform 自动化和协作 (TACO) 产品，用于改善 Terraform 管理的基础架构和配置方面的协作和自动化。完全支持 Terraform CLI、OPA 集成和分层配置模型。没有 SSO 税。所有功能均包含在内。每月最多可免费运行 50 次。
* [deployment.io](https://deployment.io) - Deployment.io 帮助开发人员自动化 AWS 上的部署。在我们的免费层中，开发人员（单个用户）可以部署无限数量的静态站点、Web 服务和环境。我们每月免费提供 20 次作业执行，预览和自动部署也包含在免费层中。

**[⬆️ 返回顶部](#目录)**

## 源代码仓库

  * [Bitbucket](https://bitbucket.org/) — 为最多 5 个用户提供无限量的公共与私人 Git 仓库，并附带用于 CI/CD 的 Pipelines
* [chiselapp.com](https://chiselapp.com/) — 无限量的公共与私人 Fossil 仓库
* [codebasehq.com](https://www.codebasehq.com/) — 一个免费项目，拥有 100 MB 空间和两名用户
* [Codeberg](https://codeberg.org/) — 为自由开源项目提供无限量的公共与私人 Git 仓库（附带无限量的协作者）。由 [Forgejo](https://forgejo.org/) 提供支持。使用 [Codeberg Pages](https://codeberg.page/) 托管静态网站。使用 [Codeberg 的 CI](https://docs.codeberg.org/ci/) 托管 CI/CD。使用 [Codeberg Translate](https://translate.codeberg.org/) 托管翻译。包括软件包和容器托管、项目管理以及问题跟踪
...
* [gitlab.com](https://about.gitlab.com/) — 无限量的公共与私人 Git 仓库，附带最多 5 名协作者。包括 CI/CD、静态托管、容器注册表、项目管理以及问题跟踪
...
## API、数据和机器学习
  ...
  * [JSONGrid](https://jsongrid.com) - 一款可将复杂的 JSON 数据可视化、编辑和过滤为美观的表格格式的免费工具。可以通过链接保存和分享 JSON 数据。
...
* [Arize AI](https://arize.com/) - 机器学习可观察性，用于监控模型并跟踪数据质量和性能漂移等问题的根源。最多两个模型免费。
* [Atlas toolkit](https://atlastk.org/) - 轻量级库，用于开发可即时访问的单页 Web 应用程序。适用于 Java、Node.js、Perl、Python 和 Ruby。
...
* [Browse AI](https://www.browse.ai) — 从网页中提取和监控数据。每月 50 个免费积分。
* [BrowserCat](https://www.browsercat.com) - 用于自动化、抓取、AI 代理 Web 访问、图像/PDF 生成等的无头浏览器 API。每月 1k 次请求的免费计划。
* [Bruzu](https://bruzu.com/) — 自动化图像生产。使用 API、集成或无代码表生成大量图像变体。API 免费提供，但带有水印。
...
* [CometML](https://www.comet.com/site/) - MLOps 平台，用于实验跟踪、模型生产管理、模型注册表和完整的数据谱系，涵盖从训练到生产的整个工作流程。对个人和学术界免费。
* [Commerce Layer](https://commercelayer.io) - 可构建、放置和管理来自任何前端的订单的合成商业 API。开发人员计划每月免费提供 100 笔订单和最多 1,000 个 SKU。
...
* [Coupler](https://www.coupler.io/) - 同步应用程序之间的数据集成工具。它可以创建实时仪表板和报告、转换和操作值以及收集和备份见解。免费计划具有无限用户、100 次运行和 1000 行/月以及无限集成的功能。
...
* [CurrencyScoop](https://currencyscoop.com) - 用于金融应用程序的实时货币数据 API。免费计划每月包括 5,000 次调用。
* [Cube](https://cube.dev/) - Cube 帮助数据工程师和应用程序开发人员从现代数据存储中访问数据，将其组织到一致的定义中，并将其提供给每个应用程序。使用 Cube Cloud（具有免费层，每月 1GB 数据传输）是使用 Cube 的最快方式。
* [Data Dead Drop](https://datadeaddrop.com) - 简单的免费文件共享。数据在访问后自行销毁。通过浏览器或您最喜欢的命令行客户端上传和下载数据。
* [Data Fetcher](https://datafetcher.com) - 使用无代码连接 Airtable 到任何应用程序或 API。类似于 Postman 的界面可用于在 Airtable 中运行 API 请求。与数十个应用程序的预构建集成。免费计划每月包括 100 次运行。
...
* [Data Miner](https://dataminer.io/) - 用于从网页中提取数据的浏览器扩展（Google Chrome、MS Edge）。免费计划每月提供 500 页。
...
* [DB Designer](https://www.dbdesigner.net/) — 云数据库架构设计和建模工具，提供免费入门计划，包括 2 个数据库模型和每个模型 10 个表。
* [DeepAR](https://developer.deepar.ai) — 适用于任何平台的增强现实人脸滤镜，一个 SDK 即可实现。免费计划每月最多可跟踪 4 张人脸，支持 10 名月活跃用户 (MAU)。
* [Deepnote](https://deepnote.com) - 一种新的数据科学笔记本。与 Jupyter 兼容，具有实时协作和云端运行功能。免费层包括无限个个人项目、最多 750 小时的标准硬件和最多 3 位编辑器的团队。
...
* [DynamicDocs](https://advicement.io) - 这是一款基于 LaTeX 模板的 JSON 到 PDF API 生成器。免费计划每月允许 50 次 API 调用，并可以访问模板库。
...
* [Fern](https://buildwithfern.com) - 使用 API 定义在流行的语言中生成 SDK，并生成 API 参考文档网页。将 Markdown 页面添加到您的 API 参考中，并使用 Fern 托管它们，以获得完整的文档解决方案。完全支持 OpenAPI。
* [file.coffee](https://file.coffee/) - 一个平台，可以存储每个文件最高 15MB（每个文件 30MB）的文件。
* [FraudLabs Pro](https://www.fraudlabspro.com) — 这是一款针对信用卡支付欺诈的订单交易筛选 REST API。它将根据订单的输入参数检测所有可能的欺诈特征。免费的 Micro 计划每月有 500 笔交易。
* [Geekflare API](https://geekflare.com/api) - Geekflare API 允许您截图、审计网站、TLS 扫描、DNS 查找、测试 TTFB 等。免费计划提供 3,000 次 API 请求。
* [GeoCod](https://geocod.xyz) — 免费的地理编码 API：将邮政地址转换为地理坐标，或将地理坐标转换为邮政地址（反向地理编码）。
* [GeoDataSource](https://www.geodatasource.com) — 这是一款位置搜索服务，使用经纬度坐标查找城市名称。每月最多免费查询 500 次。
* [Geolocated.io](https://geolocated.io) — 这是一款 IP 地理位置 API，提供多大陆服务器，为爱好者提供永久免费计划，每月 60,000 次请求。
* [Glitterly](https://glitterly.app/) - 这是一个从基本模板程序生成动态图像的 API。免费版每月提供 50 张图像和五个模板。
* [GoodData](https://www.gooddata.com/) - 数据即服务 - 创建交互式和有见地的仪表板。免费版提供五个工作区和每个工作区 100 MB 空间。
* [Hex](https://hex.tech/) - 这是一个用于笔记本、数据应用程序和知识库的协作数据平台。免费的社区版本最多支持 3 位作者和五个项目。每位作者提供一个计算配置文件，具有 4GB RAM。
* [Hook0](https://www.hook0.com/) - Hook0 是一个开源的 Webhooks 即服务（WaaS），让在线产品提供 Webhooks 变得容易。免费计划每月最多分发 3,000 次事件，并保留七天历史记录。
* [Hoppscotch](https://hoppscotch.io) - 一个免费、快速且美观的 API 请求生成器。
* [Hybiscus](https://hybiscus.dev/) - 使用简单的声明性 API 构建 PDF 报告。免费版每月包括多达 100 份单页报告，并可以自定义配色方案和字体。
* [Invantive Cloud](https://cloud.invantive.com/) — 使用 Invantive SQL 或 OData4（通常为 Power BI 或 Power Query）访问 70 多个（云）平台，例如 Exact Online、Twinfield、ActiveCampaign 或 Visma。包括数据复制和交换。为开发人员和实施顾问提供免费计划。对于特定平台免费，但数据量有限。
* [ipaddress.sh](https://ipaddress.sh) — 一个简单的服务，用于以不同的[格式](https://about.ipaddress.sh/)获取公网 IP 地址。
* [ipbase.com](https://ipbase.com) - IP 地理位置 API - 永久免费计划，每月 150 次请求。
* [IP Geolocation](https://ipgeolocation.io/) — IP 地理位置 API - 针对开发人员的永久免费计划，每月限制 30,000 次请求（每天 1,000 次）。
...
* [IP2Location.io](https://wwwMultiplier.ip2location.io/) — 免费增值、快速可靠的 IP 地理位置 API，用于确定地理位置数据，例如城市、坐标、ISP 等。免费计划提供每月 30,000 个信用点。订阅付费计划以获得更多高级功能，或联系我们以获得个性化计划。
* [ipapi](https://ipapi.co/) - 由 Kloudend, Inc 提供的 IP 地址位置 API - 一个基于 AWS 的可靠地理定位 API，受 Fortune 500 公司的信赖。免费版每月提供 30,000 次查询（每天 1,000 次），无需注册。
* [ipapi.is](https://ipapi.is/) - 一个由开发人员为开发人员提供的可靠 IP 地址 API，具有最佳的托管检测功能。免费计划提供 1,000 次查询，无需注册。
* [IPinfo](https://ipinfo.io/) — 快速、准确且免费（每月最多 50,000 次）的 IP 地址数据 API。提供有关地理定位、公司、运营商、IP 范围、域名、滥用联系人等详细信息的 API。所有付费 API 都可以免费试用。
* [BigDataCloud](https://www.bigdatacloud.com/) - 提供现代 Web 所需的快速、准确且免费（无限或每月最多 10,000-50,000 次）API，例如 IP 地理位置、反向地理编码、网络洞察、电子邮件和电话验证、客户端信息等。
* [IPTrace](https://iptrace.io) — 一个令人尴尬的简单 API，为您的业务提供可靠且有用的 IP 地理位置数据。
* [JSON2Video](https://json2video.com) - 一个视频编辑 API，用于自动化视频营销和社交媒体视频，无论是程序方式还是无代码方式。
* [JSON IP](https://getjsonip.com) — 返回客户端请求的公网 IP 地址。免费版无需注册。使用 CORS，可以使用客户端 JS 直接从浏览器中请求数据。对于监控客户端和服务器 IP 更改的服务很有用。无限请求。
...
* [konghq.com](https://konghq.com/) — API 市场和强大的私有和公共 API 工具。在免费版中，某些功能（例如监控、警报和支持）是有限的。
* [Kreya](https://kreya.app) — 一个免费的 gRPC GUI 客户端，用于调用和测试 gRPC API。可以通过服务器反射导入 gRPC API。
* [Lightly](https://www.lightly.ai/) — 通过使用正确的数据来改进机器学习模型。免费数据集最多可包含 1,000 个样本。
* [LoginLlama](https://loginllama.app) - 一个登录安全 API，用于检测欺诈和可疑的登录，并通知您的客户。每月 1,000 次登录免费。

  - [Market Data API](https://www.marketdata.app) - 提供股票、期权、共同基金等实时和历史财务数据。免费版 API 每天允许 100 次免费 API 请求。
...
- [Mintlify](https://mintlify.com) — API 文档的现代标准。美观且易于维护的 UI 组件、内置搜索功能和交互式 playground。1 位编辑器免费。
- [monkeylearn.com](https://monkeylearn.com/) — 机器学习文本分析，每月免费 300 次查询。
- [MockAPI](https://www.mockapi.io/) — MockAPI 是一个简单的工具，允许您快速模拟 API、生成自定义数据并执行操作，使用 RESTful 接口。MockAPI 旨在成为一个原型/测试/学习工具。免费计划：4 个资源/每个项目 1 个项目。
...
- [microenv.com](https://microenv.com) — 为开发人员创建免费的 REST API，可以生成代码并在 Docker 容器中生成应用程序。
...
- [neptune.ai](https://neptune.ai/) - 记录、存储、显示、组织、比较和查询所有 MLOps 元数据。个人免费：1 位成员、100 GB 元数据存储空间、每月 200 小时的监控。
- [News API](https://newsapi.org) — 使用代码在网上搜索新闻，并获得 JSON 结果。每月开发人员免费获得 3,000 次查询。
- [GoCardless](https://gocardless.com/) — 免费的开源银行数据 API。PSD2。将 2300 多家欧盟和英国的银行与您的应用程序/软件连接。
- [Nyckel](https://www.nyckel.com) — 训练、部署和调用图像和文本 ML 模型。免费训练，最多 5,000 个训练数据集。每月免费调用 1,000 次模型。
...
- [Duply.co](https://duply.co) — 通过 API 和 URL 创建动态图像，一次设计模板，重复使用。免费版每月提供 70 张图像/通过 API 和 URL 创建，最多可通过表单创建 100 张图像。
- [OpenAPI3 Designer](https://openapidesigner.com/) — 免费创建 OpenAPI 3 定义。
...
- [PDF-API.io](https://pdf-api.io) - PDF 自动化 API、视觉模板编辑器或 HTML 到 PDF、动态数据集成和 PDF 渲染与 API。免费计划：1 个模板，每月 100 个 PDF。
- [Pixela](https://pixe.la/) - 免费的日志数据库服务。所有操作均通过 API 完成。也可以使用热图和折线图进行可视化。
...
- [Preset Cloud](https://preset.io/) - 一个托管的 Apache Superset 服务。对于最多 5 位用户的小组免费，具有无限数量的仪表板和图表、无代码图表生成器和协作 SQL 编辑器。
- [PromptLeo](https://promptleo.com/) - 创作者和开发人员的提示工程平台。它提供了一个提示工程库、表单和 API。免费计划：1 个提示形成、1 个提示 API 端点和每月 30 次生成。
...
- [Supportivekoala](https://supportivekoala.com/) — 通过模板根据您的输入自动生成图像。免费计划：每周最多创建 100 张图像。
- [QuickMocker](https://quickmocker.com/) — 在您自己的子域名下管理在线假 API 端点，将请求转发到用于 webhook 开发和测试的 localhost URL，对 URL 路径使用多个 HTTP 方法和 RegExp，对响应模板使用超过 100 个短代码（动态或假响应值），从 JSON 格式导入 OpenAPI（Swagger）规范，代理请求，通过 IP 地址和授权标头限制端点。免费帐户：1 个随机子域名、10 个端点、每个端点 5 个 RegExp URL 路径、50 个短代码、每天 100 次请求和请求日志中的 50 条历史记录。
- [Rapidapi](https://rapidapi.com/) - 全球最大的 API 中心。数以百万计的开发人员可以找到和连接数千个 API，使用有趣的挑战（附带解决方案！）和交互式示例进行 API 开发。
- [RequestBin.com](https://requestbin.com) — 创建一个免费的端点，向该端点发送 HTTP 请求。发送到该端点的任何 HTTP 请求都将记录下来，并附有相关的有效负载和标头，因此您可以观察 webhook 和其他服务提出的建议。
...
* [ScrapingAnt](https://scrapingant.com/) — 无头 Chrome 抓取 API 和免费代理服务。提供 JavaScript 渲染、高级轮换代理和 CAPTCHA 避免。提供免费计划。
* [ScraperBox](https://scraperbox.com/) — 一种不可被检测的网页抓取 API，使用真正的 Chrome 浏览器和代理轮换。使用简单的 API 调用抓取任何网页。免费计划每月包含 1000 次请求。
* [ScrapingDog](https://scrapingdog.com/) — Scrapingdog 处理数以百万计的代理、浏览器和 CAPTCHAs，以在单个 API 调用中为您提供任何网页的 HTML。它还包括 Chrome 和 Firefox 网页抓取器以及用于即时抓取需求的软件。提供免费计划。
...
* [Select Star](https://www.selectstar.com/) - 一个智能数据发现平台，可以自动分析和记录您的数据。免费的 Light 层包含 1 个数据源、最多 100 张表和 10 个用户。
...
* [SerpApi](https://serpapi.com/) - 实时搜索引擎抓取 API。返回 Google、YouTube、Bing、Baidu、Walmart 等机器的结构化 JSON 结果。免费计划每月包含 100 次成功的 API 调用。
* [SmartParse](https://smartparse.io) - SmartParse 是一个数据迁移和 CSV 到 API 平台，提供节省时间和成本的开发人员工具。免费层包括每月 300 个处理单元、浏览器上传、数据隔离、断路器和作业提醒。
* [Sofodata](https://www.sofodata.com/) - 从 CSV 文件创建安全的 RESTful API。上传 CSV 文件，并通过其 API 即时访问数据，从而加快应用程序开发。免费计划包括 2 个 API 和每月 2,500 次 API 调用。您无需信用卡。
...
* [TemplateTo](https://templateto.com) - 使用我们的拖放编辑器和简单的 API 从可重用模板自动生成 PDF/TXT 文档。免费计划包含 450 个 PDF 和三个模板。
* [TinyMCE](https://www.tiny.cloud) - 富文本编辑 API。核心功能可免费无限使用。
* [Treblle](https://www.treblle.com) - Treblle 帮助团队构建、发布和管理 API。提供高级 API 日志聚合、可观察性、文档和调试。免费层的所有功能均可使用，但每月请求限制为 25 万。
...
* [Weights & Biases](https://wandb.ai) — 开发人员优先的 MLOps 平台。通过实验跟踪、数据集版本控制和模型管理来构建更好的模型。免费层仅限个人项目，包括 100 GB 存储空间。
* [wit.ai](https://wit.ai/) — 开发人员的 NLP。
* [wolfram.com](https://wolfram.com/language/) — 内置基于知识的云算法。
* [wrapapi.com](https://wrapapi.com/) — 将任何网站转换为参数化 API。每月 3 万个 API 调用。
* [ZenRows](https://www.zenrows.com/) — 网页抓取 API 和代理服务器，可绕过任何反机器人解决方案，同时提供 JavaScript 渲染、代理轮换和地理定位。免费层的 API 调用次数为 1000 次。
* [Zenscrape](https://zenscrape.com/web-scraping-api) — 网页抓取 API，具有无头浏览器和住宅 IP，以及简单明了的定价。每月 1000 次免费 API 调用，并为学生和非营利组织提供额外学分。
* [ip-api](https://ip-api.com) — IP 地理位置 API，免费用于非商业用途，无需 API 密钥，对于免费计划，来自同一 IP 地址的请求限制为每分钟 45 个。
* [WebScraping.AI](https://webscraping.ai) - 一个简单的网页抓取 API，具有内置的解析、Chrome 渲染和代理。每月 2000 次免费 API 调用。
...
* [Query.me](https://query.me) - 协作数据笔记本，可以执行脚本，并允许通过 SQL、API 以及 Slack 和 Email 等许多自定义块来获取和发送数据。对于小型团队免费。
...
* [OpenWeb Ninja](https://www.openwebninja.com/) - 极其全面的实时 SERP 和公共数据 API：Google 搜索、购物、招聘、图片、镜头、新闻、Google 地图企业/地点、评论、照片、网站电子邮件和社交联系人抓取器、亚马逊、Yelp 等。所有 API 均包括免费层，每月提供 100 到 200 次免费请求。
* [Tavily AI](https://tavily.com/) - 用于在线搜索和快速洞察以及综合研究的 API，具有对研究结果进行组织的能力。免费层每月 1000 次请求，无需信用卡。
* [Composio](https://composio.dev/) - AI 代理和 LLMs 集成平台。集成 200 多款智能工具。
...
## 工件仓库
  ...
  * [Artifactory](https://jfrog.com/start-free/) - 支持多种软件包格式的工件仓库，例如 Maven、Docker、Cargo、Helm、PyPI、CocoaPods 和 GitLFS。包括软件包扫描工具 XRay 和 CI/CD 工具 Pipelines (前身为 Shippable)，每月提供 2,000 分钟的免费 CI/CD 服务。
  ...
  * [packagecloud.io](https://packagecloud.io/users/new?plan=free_usage_plan) — 易于使用的仓库托管服务，支持 Maven、RPM、DEB、PyPi、NPM 和 RubyGem 软件包 (包含免费版)。
  ...
  * [paperspace](https://www.paperspace.com/) — 构建和扩展 AI 模型，开发、训练和部署 AI 应用程序，免费计划：公共项目、5Gb 存储空间、基础实例。
  * [RepoForge](https://repoforge.io) - 适用于 Python、Debian、NPM 软件包和 Docker 注册表的私有云托管仓库。开源/公共项目免费计划。
  * [RepoFlow](https://repoflow.io) - RepoFlow 简化了 npm、PyPI、Docker、Go、Helm 等软件包的管理。免费试用，包括 10GB 存储空间、10GB 带宽、100 个软件包和无限用户，或个人免费自托管。
...
* [cally.com](https://cally.com/) — 寻找会议的理想时间和日期。简单易用，非常适合小型和大型团队。
* [Calendly](https://calendly.com) — Calendly 是一款用于安排会议的工具。免费计划为每个用户提供 1 个日历连接和无限次会议。还提供桌面和移动应用程序。
...
* [Dyte](https://dyte.io) - 功能最丰富的开发人员友好型音视频 SDK，具有增强生产力和参与度的协作插件。免费版每月包含 10,000 分钟的音视频使用量。
...
* [Gather](https://www.gather.town/) - 在线会面的更好方式。Gather 围绕完全可定制的空间展开，让您与社区交流变得与现实生活一样简单。最多 10 个并发用户免费。
* [gokanban.io](https://gokanban.io) - 用于快速使用的基于语法、无需注册的 Kanban 看板。免费且无限制。
* [flat.social](https://flat.social) - 用于团队会议和快乐时光社交活动的交互式可定制空间。无限次会议，最多 8 个并发用户免费。
...
* [Hackmd.io](https://hackmd.io/) - Markdown 格式文档/文件的实时协作和写作工具。类似于 Google Docs，但用于 Markdown 文件。免费的笔记数量不受限制，但私有笔记和模板的协作者（被邀请人）数量[是有限制的](https://hackmd.io/pricing)。
...
* [Keybase](https://keybase.io/) — Keybase 是 Slack 的 FOSS 替代品；它可以保护每个人（从家庭到社区到公司）的聊天和文件安全。
* [Google Meet](https://meet.google.com/) — 使用 Google Meet 满足您企业的在线视频会议需求。Meet 提供安全、易于加入的在线会议。
* [/meet for Slack](https://meetslack.com) - 在任何频道、小组或 DM 中使用 /meet 直接在 Slack 中启动 Google Meet 会议。免费且无限制。
...
* [Proton Pass](https://proton.me/pass) — 集电子邮件别名、2FA 身份验证器、共享和密码于一体的密码管理器。支持网页、浏览器扩展、移动应用程序和桌面应用程序。
* [Visual Debug](https://visualdebug.com) - 一种用于改善客户与开发人员之间沟通的可视反馈工具。
* [meet.jit.si](https://meet.jit.si/) — 单击即可免费视频通话和屏幕共享。
* [Microsoft Teams](https://products.office.com/microsoft-teams/free) — Microsoft Teams 是一个基于聊天的数字中心，将对话、内容和应用程序集中到一个地方，提供统一的体验。最多支持 50 万用户免费。
* [Miro](https://miro.com/) - 可扩展、安全、跨设备且企业就绪的分布式团队协作白板。提供免费增值计划。
* [nootiz](https://www.nootiz.com/) - 收集和管理任何网站视觉反馈的必备工具。
* [Notion](https://www.notion.so/) - Notion 是一款支持 Markdown 的笔记和协作应用程序，集任务、维基和数据库于一体。该公司将该应用程序描述为适用于笔记记录、项目管理和任务管理的综合工作区。除了跨平台应用程序外，它还可以通过大多数网络浏览器访问。
...
* [Quidlo Timesheets](https://www.quidlo.com/timesheets) - 一款简单的时间表和团队时间跟踪应用程序。免费计划支持最多 10 位用户的时间跟踪和生成报告功能。
* [PageShare.dev](https://www.pageshare.dev) - 在 GitHub Pull Requests 中添加可视化评审功能，而无需部署网站。每月最多 10 个页面和总共 100MB 存储空间免费。
* [Pendulums](https://pendulums.io/) - Pendulums 是一款免费的时间跟踪工具，它可以帮助您通过易于使用的界面和有价值的统计信息更好地管理时间。
* [Pumble](https://pumble.com) - 免费团队聊天应用。无限用户和消息历史记录，永久免费。
* [Raindrop.io](https://raindrop.io) - 适用于 macOS、Windows、Android、iOS 和 Web 的私人和安全书签应用。免费无限书签和协作。
...
- [Slab](https://slab.com/) — 一个现代化的团队知识管理服务。对最多 10 名用户免费。
- [slack.com](https://slack.com/) — 对无限用户免费，但部分功能有限制
- [Spectrum](https://spectrum.chat/) - 免费创建公共或私人社区。
- [StatusPile](https://www.statuspile.com/) - 一个状态页的集合页。可以跟踪上游提供商的状态页吗？
- [Stickies](https://stickies.app/) - 可用于头脑风暴、内容策划和记笔记的可视化协作应用。对最多 3 面墙、无限用户和 1 GB 存储免费。
- [Stacks](https://betterstacks.com/) - 集成的笔记、链接和文件存储内容工作区，以应对信息过载。永久免费个人计划。 
- [talky.io](https://talky.io/) — 免费的群组视频聊天。匿名、点对点。无需插件、注册或付费。
- [Teamhood](https://teamhood.com/) - 免费的项目、任务和问题跟踪软件。支持带有泳道的 Kanban 和完整的 Scrum 实现。集成了时间跟踪。对 5 名用户和 3 个项目组合包免费。
- [Teamplify](https://teamplify.com) - 使用团队分析和智能每日站立会议来改进团队开发流程。包括针对远程优先团队的完整的休假管理功能。对最多 5 个用户的小团队免费。
- [Tefter](https://tefter.io) - 带有强大 Slack 集成的书签应用。对开源团队免费。
- [TeleType](https://teletype.oorja.io/) — 共享终端、语音、代码、白板等。无需注册，即可为开发人员提供端到端加密协作。
- [TimeCamp](https://www.timecamp.com/) - 免费、无限用户使用的时间跟踪软件。可轻松与 Jira、Trello、Asana 等 PM 工具集成。
- [Teamcamp](https://www.teamcamp.app) - 软件开发公司的一体化项目管理应用程序。
- [twist.com](https://twist.com) — 一种适合异步协作的团队通信应用，可保持对话井井有条、有条不紊。提供免费和无限计划。为符合条件的团队提供折扣。
- [tldraw.com](https://tldraw.com) — 免费、开源的白板和绘图工具，具有智能箭头、吸附、便签和 SVG 导出功能。多人模式支持协作编辑。还有免费的官方 VS Code 扩展。
- [BookmarkOS.com](https://bookmarkos.com) - 免费的全合一书签管理器、标签管理器和任务管理器，具有可定制的在线桌面和文件夹协作功能。
- [typetalk.com](https://www.typetalk.com/) — 通过网络或移动设备上的即时通讯与团队分享和讨论想法。
- [Tugboat](https://tugboat.qa) - 预览每个 pull request，自动化和按需。对所有人免费，为非营利组织提供 Nano 层。
- [whereby.com](https://whereby.com/) — 单击视频对话，免费（前身为 appear.in）
- [windmill.dev](https://windmill.dev/) - Windmill 是一个开源的开发人员平台，可快速从最少的 Python 和 Typescript 脚本构建多步骤自动化和内部应用程序。作为免费用户，您最多可以创建 3 个非高级工作区，或成为其成员。
- [vadoo.tv](https://vadoo.tv/) — 简单的视频托管和营销。单击上传视频。记录、管理、分享等。免费版提供最多 10 个视频、1 GB 存储空间和每月 10 GB 带宽
- [userforge.com](https://userforge.com/) - 互联在线角色、用户故事和上下文映射。 帮助设计和开发保持同步，对最多 3 个角色和 2 个协作者免费。
- [wistia.com](https://wistia.com/) — 带有查看者分析、高清视频交付和营销工具的视频托管，可帮助您了解访问者，25 个视频和 Wistia 品牌播放器
- [wormhol.org](https://www.wormhol.org/) — 简单的文件共享服务。与您想要的任意数量的同行共享最多 5GB 的无限文件。
- [Wormhole](https://wormhole.app/) - 使用端到端加密共享最多 5GB 的文件，最多 24 小时。对于大于 5GB 的文件，它使用点对点传输直接发送您的文件。
- [zoom.us](https://zoom.us/) — 安全的视频和网络会议，可添加插件。免费计划限制在 40 分钟以内。
- [Zulip](https://zulip.com/) — 具有独特电子邮件式线程模型的实时聊天。免费计划包括 10,000 条消息搜索历史记录和 5 GB 的文件存储空间。还提供可自行托管的开源版本。
- [robocorp.com](https://robocorp.com) - 用于自动化运营的开源堆栈。试用云功能并实现简单的自动化，完全免费。机器人每月工作 240 分钟、10 次助理运行、100 MB 存储空间。
- [Fleep.io](https://fleep.io/) — Fleep 是 Slack 的替代品。它为小型团队提供免费计划，包括完整的消息历史记录、无限的 1:1 会话、1 个群组会话和 1 GB 的文件存储。
- [Chanty.com](https://chanty.com/) — Chanty 是 Slack 的另一种替代方案。它为小型团队（最多 10 人）提供永久免费计划，包括无限的公开和私人对话、可搜索的历史记录、无限的 1:1 音频通话、无限的语音消息、10 项集成和每个团队 20 GB 存储空间。
- [ruttl.com](https://ruttl.com/) — 一款全合一的收集反馈工具，可用于收集对网站、PDF 和图像的反馈。
- [Mattermost](https://mattermost.com/) — 技术团队的安全协作。免费计划包括无限通道、剧本、看板、用户、10GB 存储等。
...
- [Webex](https://www.webex.com/) — 视频会议，免费计划提供每次会议最多 100 人的 40 分钟会议。
- [RingCentral](https://www.ringcentral.com/) — 视频会议，免费计划提供每次会议最多 100 人的 50 分钟会议。
...
- [Revolt.chat](https://revolt.chat/) — 一个尊重隐私的 [Discord](https://discord.com/) 开源替代品。它还拥有免费的 Discord 专有功能。Revolt 是一款安全、快速且 100% 免费的综合应用程序。每个功能都是免费的。他们还支持（官方和非官方）插件，这与其他主流聊天应用程序不同。
- [Tencent RTC](https://trtc.io/) — 腾讯实时通信（TRTC）提供群组音视频通话解决方案。第一年每月 10,000 分钟免费。
- [Pastefy](https://pastefy.app/) - 美丽、简单的 Pastebin，可选客户端加密、多标签粘贴、API、带突出显示的编辑器等。
...
* [Crystallize](https://crystallize.com) — 支持电子商务的无头 PIM。内置 GraphQL API。免费版包括无限用户、1000 个目录项、每月 5 GB 带宽和每月 25k 次 API 调用。
* [DatoCMS](https://www.datocms.com/) - 为小型项目提供免费层。DatoCMS 是一个基于 GraphQL 的 CMS。在较低的层级，您每月有 100k 次调用。
...
* [Prismic](https://www.prismic.io/) — 无头 CMS。内容管理界面，具有完全托管的可扩展 API。社区计划为 1 个用户提供无限的 API 调用、文档、自定义类型、资产和语言环境。您下一个项目所需的一切。更大的免费计划可用于开放内容/开源项目。
...
* [GatsbyjsCMS](https://www.gatsbyjs.com/) - Gatsby 是一个快速、灵活的框架，可让您使用任何 CMS、API 或数据库来构建网站，让您再次享受乐趣。构建和部署无头网站，以吸引更多流量、提高转化率并增加收入！
* [Hygraph](https://hygraph.com/) - 为小型项目提供免费层。GraphQL 优先 API。从传统解决方案迁移到本机 GraphQL 无头 CMS - 并通过 API 优先的方式交付全渠道内容。
* [Squidex](https://squidex.io/) - 为小型项目提供免费层。API / GraphQL 优先。开源，基于事件源（自动记录每个更改）。
...
* [WPJack](https://wpjack.com) - 在 5 分钟内在任何云上设置 WordPress！免费版包括 1 个服务器、2 个站点、免费 SSL 证书和无限 cron 作业。没有时间限制或到期时间——您的网站，您做主。
...
* [DhiWise](https://www.dhiwise.com/) — DhiWise 的创新代码生成技术可以将 Figma 设计无缝地转变为动态 Flutter 和 React 应用程序，优化工作流程，并帮助您比以往更快地构建出色的移动和 Web 体验。
* [Codeium](https://www.codeium.com/) — Codeium 是一个免费的 AI 代码完成工具。它支持 20 多种编程语言（Python、JavaScript、Java、TypeScript、PHP、C/C++、Go 等），并可集成到所有主要的独立和 Web IDE 中。
* [Fern](https://buildwithfern.com) - 编写 API 定义，并使用它们以 TypeScript、Python、Java、Go 等流行语言生成 SDK/客户端库。完全支持 OpenAPI。免费版最多可为 20 个端点生成代码。
* [Metalama](https://www.postsharp.net/metalama) - 仅适用于 C#。Metalama 在编译时动态生成代码框架，使源代码保持简洁。它对开源项目免费，其商业友好型免费版包括三个方面。
* [Supermaven](https://www.supermaven.com/) — Supermaven 是一个快速的 AI 代码完成 VSCode、JetBrains 和 Neovim 插件。免费版提供无限的行内完成。
* [tabnine.com](https://www.tabnine.com/) — Tabnine 通过提供来自全球所有代码的见解来帮助开发人员更快地创建更好的软件。提供插件。
* [v0.dev](https://v0.dev/) — v0 使用 AI 模型根据简单的文本提示生成代码。它根据 shadcn/ui 和 Tailwind CSS 生成易于复制的 React 代码，人们可以在自己的项目中使用。每次生成至少需要 30 个积分。您将从 1200 个积分开始，每月获得 200 个免费积分。

**[⬆️ 返回顶部](#目录)**

## 代码质量

  * [beanstalkapp.com](https://beanstalkapp.com/) — 一个完整的代码工作流，包括编写、审核和部署，为单个用户提供免费账户，并提供一个拥有 100 MB 存储空间的仓库
  * [browserling.com](https://www.browserling.com/) — 实时交互式跨浏览器测试，免费会话仅限 3 分钟，支持 MS IE 9 下的 Vista 和 1024 x 768 分辨率
  * [codacy.com](https://www.codacy.com/) — 针对 PHP、Python、Ruby、Java、JavaScript、Scala、CSS 和 CoffeeScript 的自动化代码审查，免费提供无限量的公共和私有仓库
  * [Codeac.io](https://www.codeac.io/infrastructure-as-code.html?ref=free-for-dev) - 针对 DevOps 的自动化 Infrastructure as Code 审查工具，与 GitHub、Bitbucket 和 GitLab（包括自托管）集成。除了标准语言之外，它还分析 Ansible、Terraform、CloudFormation 和 Kubernetes 等。 （开源免费）
  * [CodeBeat](https://codebeat.co) — 适用于多种语言的自动化代码审查平台。对于公共仓库，免费提供 Slack 和电子邮件集成。
  * [codeclimate.com](https://codeclimate.com/) — 自动化代码审查，免费提供开源和无限量的组织私有仓库（最多 4 个协作者）。对于学生和机构也是免费的。
  ...
  * [coderabbit.ai](https://coderabbit.ai) — 与 GitHub/GitLab 集成的 AI 驱动的代码审查工具。免费版包括每小时 200 个文件、3 次审查和 50 次对话。对于开源项目，永久免费。
  * [codescene.io](https://codescene.io/) - CodeScene 根据开发人员如何处理代码来确定技术债务的优先级，并可视化组织因素，如团队耦合和系统掌握情况。开源免费。
  * [CodSpeed](https://codspeed.io) - 在 CI 管道中自动执行性能跟踪。在部署之前，通过精确和一致的指标来发现性能回归。对于开源项目，永久免费。
  * [coveralls.io](https://coveralls.io/) — 显示测试覆盖率报告，开源免费
  * [dareboost](https://dareboost.com) - 每月 5 次免费的 Web 性能、可用性和安全性分析报告
  * [deepcode.ai](https://www.deepcode.ai) — DeepCode 基于 AI 发现漏洞、安全漏洞、性能和 API 问题。DeepCode 的分析速度允许我们在 IDE 中保存按钮时实时分析代码。支持的语言包括 Java、C/C++、JavaScript、Python 和 TypeScript。与 GitHub、BitBucket 和 GitLab 集成。对于开源和私有仓库（最多 30 个开发人员）免费。
  ...
  * [eversql.com](https://www.eversql.com/) — EverSQL - 数据库优化的首选平台。自动获取数据库和 SQL 查询的重要见解。
  ...
  * [gtmetrix.com](https://gtmetrix.com/) — 提供报告和详细建议以优化网站
  ...
  * [sonarcloud.io](https://sonarcloud.io) — 针对 Java、JavaScript、C/C++、C#、VB.NET、PHP、Objective-C、Swift、Python、Groovy 等多种语言的自动化源代码分析，开源免费
  * [SourceLevel](https://sourcelevel.io/) — 自动化代码审查和团队分析。对于开源项目和最多 5 名协作者的组织免费。
...
## 持续集成和持续部署
  ...
  * [AccessLint](https://github.com/marketplace/accesslint) — 将自动化网页可访问性测试引入开发工作流程。该服务对开源和教育目的免费。
  * [appcircle.io](https://appcircle.io) — 一个企业级的移动 DevOps 平台，可自动化移动应用程序的构建、测试和发布，以实现更快、更高效的发布周期。免费版：每次构建最大构建时间为 30 分钟，每月 20 次构建，并发构建 1 次。
  * [appveyor.com](https://www.appveyor.com/) — 一个针对 Windows 的 CD 服务，对开源免费。
  * [Argonaut](https://argonaut.dev/) - 在几分钟内在云中部署应用程序和基础设施。支持在 Kubernetes 和 Lambda 环境中部署自定义和第三方应用程序。免费版允许在 5 个域名和 2 个用户下无限量部署应用程序。
  ...
  * [bytebase.com](https://www.bytebase.com/) — 一个针对数据库的 CI/CD 和 DevOps 服务。对 20 名以下用户和 10 个数据库实例免费。
  * [CircleCI](https://circleci.com/) — 一个全面的免费计划，包括所有功能，如 Docker、Windows、Mac OS、ARM 执行器、本地运行器、测试拆分、Docker 层缓存等。适用于 GitHub、GitLab 和 BitBucket 仓库的托管 CI/CD 服务。免费版：每月 6000 分钟执行时间，无限数量的协作者，私有项目 30 个并行工作，开源项目最高 80,000 分钟免费构建时间。
  ...
  * [deploybot.com](https://www.deploybot.com/) — 1 个仓库和 10 次部署，对开源免费
  ...
  * [LayerCI](https://layerci.com) — 一个针对全栈项目的 CI 服务。提供 1 个具有 5GB 内存和 3 个 CPU 的全栈预览环境。
  * [semaphoreci.com](https://semaphoreci.com/) — 对开源免费，每月 100 次私有构建
  * [Squash Labs](https://www.squash.io/) — 为每个分支创建一个 VM，并通过唯一的 URL 提供应用程序，无限量公共与私有仓库，VM 大小最高 2 GB。
  * [styleci.io](https://styleci.io/) — 仅支持公共 GitHub 仓库
  * [Mergify](https://mergify.io) — GitHub 的工作流自动化和合并队列，对公共 GitHub 仓库免费
  * [Make](https://www.make.com/en) — 一个工作流自动化工具，允许您通过 UI 连接和自动化应用程序。支持许多应用程序和最流行的 API。对公共 GitHub 仓库免费，免费版：100 Mb，1000 次操作，15 分钟的最小间隔。
  * [Spacelift](https://spacelift.io/) - 基础设施即代码管理平台。免费版功能：IaC 协作、Terraform 模块注册表、ChatOps 集成、使用 Open Policy Agent 的持续资源合规性、SAML 2.0 SSO 和公共工作者池访问权限：每月最高 200 分钟
  ...
  * [Nx Cloud](https://nx.dev/ci) - Nx Cloud 通过远程缓存、跨机器任务分布，甚至自动拆分端到端测试运行等功能，加快 CI 上的 monorepo 速度。它提供了一个免费计划，适用于最多 30 位贡献者，并附有 15 万积分。
  * [blacksmith](https://www.blacksmith.sh/) - 为 GitHub Actions 提供托管性能运行器，每月 3,000 分钟免费，无需信用卡。
  * [Terramate](https://terramate.io/) - Terramate 是一个用于基础设施即代码（IaC）工具（例如 Terraform、OpenTofu 和 Terragrunt）的编排和管理平台。对 2 个用户免费，包括所有功能。

**[⬆️ 返回顶部](#目录)**

## 测试

  * [Applitools.com](https://applitools.com/) — 针对 Web、Native Mobile 和桌面应用程序的智能视觉验证。它可以与几乎所有的自动化解决方案（如 Selenium 和 Karma）和远程运行器（如 Sauce Labs、Browser Stack）集成。开源项目免费。单用户每周有限检查点的免费版。
  * [Appetize](https://appetize.io) — 在基于云的 Android 手机/平板电脑模拟器和 iOS/iPad 模拟器上测试您的 Android 和 iOS 应用程序，直接在浏览器中运行。免费版包括两个并发会话，每月 30 分钟的使用时长。不限制应用大小。
  * [Apptim](https://apptim.com) — 一种移动测试工具，使没有性能工程技能的人也能评估应用程序的性能和用户体验 (UX)。桌面版在自己的设备上使用是完全免费的，iOS 和 Android 测试次数不限。
  * [Argos](https://argos-ci.com) - 开发人员的开源视觉测试。Unlimitedprojects，每月 5,000 张截图。开源项目免费。
  ...
  * [BugBug](https://bugbug.io/) - 一种轻量级的 Web 应用程序测试自动化工具。它易于学习，无需编码。您可以在自己的电脑上免费运行无限次测试。每月额外付费还可获得云监控和 CI/CD 集成。
  * [Checkly](https://checklyhq.com) - 针对现代 DevOps 的基于代码的合成监控。以传统提供商价格的一小部分监控您的 API 和应用程序。由监控即代码工作流程和 Playwright 提供支持。开发人员享有慷慨的免费层。
  * [checkbot.io](https://www.checkbot.io/) — 浏览器扩展程序，测试您的网站是否遵循 50 多个 SEO、速度和安全最佳实践。小型网站免费层。
  * [cypress.io](https://www.cypress.io/) - 针对任何运行在浏览器中的应用程序的快速、简单和可靠的测试。Cypress Test Runner 始终免费开源，没有限制。Cypress Dashboard 对开源项目免费，最多 5 个用户。
  * [Cypress Recorder by Preflight](https://cypress.preflight.com/) - 在浏览器中创建 AI 驱动的 Cypress 测试/POM 模型。除了 AI 部分之外，其余都是开源的。每月五个测试创建量免费，包括自我修复脚本、电子邮件和视觉测试。
  * [everystep-automation.com](https://www.everystep-automation.com/) — 记录和回放 Web 浏览器中的所有步骤并创建脚本，免费但功能较少
  * [Gremlin](https://www.gremlin.com/gremlin-free-software) — Gremlin 的混沌工程工具允许您安全地向系统注入故障，以在导致客户面临问题之前找到弱点。Gremlin Free 提供对最多 5 个主机或容器的关闭和 CPU 攻击的访问。
  * [gridlastic.com](https://www.gridlastic.com/) — Selenium Grid 测试，免费计划包括最多 4 个同时 Selenium 节点/10 个网格启动/每月 4,000 分钟测试时长
  * [katalon.com](https://katalon.com) - 提供了一个测试平台，可以帮助不同级别的测试成熟度团队，包括 Katalon Studio、TestOps (+ 免费视觉测试)、TestCloud 和 Katalon Recorder。
  * [Keploy](https://keploy.io/) - Keploy 是一个面向开发人员的功能测试工具包。录制 API 调用可为 API 生成端到端测试（KTests）和模拟或存根 (KMocks)。开源项目免费。
  * [knapsackpro.com](https://knapsackpro.com) - 通过在任何 CI 提供商上优化测试套件并行化来加快测试速度。在并行 CI 节点上拆分 Ruby 和 JavaScript 测试以节省时间。免费计划包括 10 分钟的测试文件，开源项目免费无限计划。
  ...
  * [loadmill.com](https://www.loadmill.com/) - 通过分析网络流量自动创建 API 和负载测试。每月免费模拟 50 个并发用户，持续 60 分钟。
  * [lost-pixel.com](https://lost-pixel.com) - 对您的 Storybook、Ladle、Histoire 故事和 Web 应用程序进行全面的视觉回归测试。团队成员数量不限，开源项目完全免费，每月 7,000 个快照。
  ...
  * [preflight.com](https://preflight.com) - 无代码自动化 Web 测试。在浏览器中录制可抵抗 UI 更改的测试，并在 Windows 机器上运行。可以与 CI/CD 集成。免费计划包括 50 次每月测试运行，并附带视频、HTML 会话等。
  * [qase.io](https://qase.io) - 针对 Dev 和 QA 团队的测试管理系统。管理测试用例、编写测试运行、执行测试、跟踪缺陷并衡量影响。免费层包括所有核心功能，附件空间 500MB，最多 3 个用户。
  * [Repeato](https://repeato.app/) 一种基于计算机视觉和人工智能的无代码移动应用程序测试自动化工具。适用于原生应用、Flutter、React-Native、Web、Ionic 等众多应用框架。免费计划限于 iOS 的 10 次测试和 Android 的 10 次测试，但包括付费计划的大部分功能，包括无限次测试运行。
  * [Requestly](https://requestly.com/) 一个开放源代码的 Chrome 扩展程序，用于拦截、重定向和模拟 HTTP 请求。包括 [Debugger](https://requestly.com/products/web-debugger/)、[Mock Server](https://requestly.com/products/mock-server/)、[API Client](https://requestly.com/products/api-client/) 和 [Session Recording](https://requestly.com/products/session-book/)。免费层可创建 10 条规则。开源免费。
  * [seotest.me](https://seotest.me/) — 免费的网页 SEO 网站测试工具。每天免费抓取 10 个网站。有用的 SEO 学习资源和建议，无论网站技术如何，都可以提高任何网站的网页 SEO 结果。
  * [snippets.uilicious.com](https://snippets.uilicious.com) - 它类似于 CodePen，但用于跨浏览器测试。UI-licious 让您像编写用户故事一样编写测试，并提供一个免费平台 - UI-licious Snippets - 允许您在没有注册要求的情况下在 Chrome 上运行长达 3 分钟的无限测试。发现了错误？您可以复制测试的唯一 URL，向开发人员显示如何重现错误。
  ...
  * [Testspace.com](https://testspace.com/) - 用于发布自动化测试结果的仪表板，以及用于使用 GitHub 将手动测试作为代码实现的框架。该服务对开源项目免费，每月 450 个结果。

  * [UseWebhook.com](https://usewebhook.com) - 在浏览器中捕获和检查 Webhook。转发到本地主机或从历史记录重播。免费使用。
  * [Vaadin](https://vaadin.com) — 使用集成工具、组件和设计系统以 Java 或 TypeScript 构建可扩展的 UI，加快迭代速度、改进设计并简化开发过程。无限项目，五年免费维护。
...
* [Altcha.org](https://altcha.org/anti-spam) - 一个基于自然语言处理和机器学习的网站和API防垃圾邮件过滤器。免费计划每天每个域名包括200次请求。
...
* [CyberChef](https://gchq.github.io/CyberChef/) — 一个简单、直观的网络应用程序，用于分析和解码/编码数据，无需处理复杂工具或编程语言。就像密码学和加密的瑞士军刀。所有功能均免费使用，且没有限制。开源，可自行托管。
...
* [Dotenv](https://dotenv.org/) — 同步你的 .env 文件，快速且安全。停止通过 Slack 和电子邮件等不安全渠道共享 .env 文件，再也不用担心丢失重要的 .env 文件。对于最多 3 名团队成员免费。
* [GitGuardian](https://www.gitguardian.com) — 通过自动机密检测和修复，防止机密信息泄露到源代码中。扫描 git 仓库中的 350 多种机密和敏感文件类型——对于个人和 25 名以下开发人员的团队免费。
...
* [keychest.net](https://keychest.net) - SSL 证书到期管理和购买，带集成 CT 数据库
* [letsencrypt.org](https://letsencrypt.org/) — 免费的 SSL 证书颁发机构，其证书受到所有主要浏览器的信任
* [meterian.io](https://www.meterian.io/) - 监控 Java、Javascript、.NET、Scala、Ruby 和 NodeJS 项目的依赖项中的安全漏洞。对于一个私有项目免费，对于开源项目则不限数量。
* [Mozilla Observatory](https://observatory.mozilla.org/) — 查找和修复站点中的安全漏洞。
* [opswat.com](https://www.opswat.com/) — 计算机、设备、应用程序、配置的安全监控，免费提供 25 个用户和 30 天的历史记录用户。
* [openapi.security](https://openapi.security/) - 一个免费工具，可以快速检查任何基于 OpenAPI / Swagger 的 API 的安全性。无需注册。
* [pixee.ai](https://pixee.ai) - 自动化产品安全工程师，作为免费的 GitHub 机器人，向 Java 代码库提交 PR 以自动解析漏洞。其他语言即将推出！
* [pyup.io](https://pyup.io) — 监控 Python 依赖项的安全漏洞，并自动更新。对于一个私有项目免费，对于开源项目则不限数量。
...
* [snyk.io](https://snyk.io) — 可以查找和修复开源依赖项中已知的安全漏洞。对于开源项目，测试和修复不受限制。对于私有项目，每月限制 200 次测试。
* [ssllabs.com](https://www.ssllabs.com/ssltest/) — 对任何 SSL Web 服务器配置进行深入分析
* [SOOS](https://soos.io) - 针对开源项目的免费、无限 SCA 扫描。在发布之前检测和修复安全威胁。使用简单有效的解决方案保护项目。
* [StackHawk](https://www.stackhawk.com/) — 在管道中自动化应用程序扫描，以在生产之前查找和修复安全漏洞。对于单个应用程序，扫描和环境数量不受限制。
...
* [Ubiq Security](https://ubiqsecurity.com/) — 使用三行代码和自动密钥管理加密和解密数据。对于最多 1,000,000 次加密的单个应用程序免费。
* [Virgil Security](https://virgilsecurity.com/) — 提供用于在数字解决方案中实现端到端加密、数据库保护、物联网安全等的工具和服务。对于最多 250 名用户的应用程序免费。
* [Vulert](https://vulert.com) - Vulert 会持续监控开源依赖项中的新漏洞，并推荐修复方法，而无需安装或访问代码库。对于开源项目免费。
* [Escape GraphQL Quickscan](https://escape.tech/) - 一键扫描 GraphQL 端点的安全性。免费，无需登录。
...
* [Authgear](https://www.authgear.com) - 在几分钟内为您的应用程序带来无密码、一次性密码、双因素身份验证、单点登录。包括所有前端。每月 5000 个 MAU 免费。
* [Authress](https://authress.io/) — 身份验证登录和访问控制，任何项目的无限身份提供商。Facebook、Google、Twitter 等。前 1000 次 API 调用免费。
* [Authy](https://authy.com) - 多设备上的双因素身份验证 (2FA)，并提供备份。Google 身份验证器的替代品。前 100 次成功身份验证免费。
...
* [Cloud-IAM](https://www.cloud-iam.com/) — Keycloak 即服务 (IDaaS) 身份和访问管理。免费版：100 个用户和 1 个领域。
* [Corbado](https://www.corbado.com/) — 为新应用或现有应用添加 Passkeys 优先身份验证。无限 MAU 免费。
* [Descope](https://www.descope.com/) — 高度可定制的身份验证流程，提供无代码和 API/SDK 方法，每月 7,500 个活跃用户/50 个租户免费（最多 5 个 SAML/SSO 租户）。
* [duo.com](https://duo.com/) — 网站或应用程序的双因素身份验证 (2FA)。免费版：10 个用户，所有身份验证方法，无限集成功能，硬件令牌。
* [Kinde](https://kinde.com/) - 简单、健壮的身份验证，可在几分钟内与您的产品集成。7,500 个免费 MAU。
* [logintc.com](https://www.logintc.com/) — 通过推送通知进行双因素身份验证 (2FA)，免费版：10 个用户，VPN、网站和 SSH。
* [MojoAuth](https://mojoauth.com/) - MojoAuth 让您轻松在几分钟内为任何应用程序实现无密码身份验证。
...
* [Ory](https://ory.sh/) - AuthN/AuthZ/OAuth2.0/Zero Trust 管理安全平台。免费的开发者账户，包括所有安全功能、无限团队成员、200 个每日活跃用户和每月 25k 次权限检查。
* [Phase Two](https://phasetwo.io) - Keycloak 开源身份和访问管理。免费领域：最多 1000 个用户，最多 10 个 SSO 连接，利用 Phase Two 的 Keycloak 增强型容器，包括 [Organization](https://phasetwo.io/product/organizations/) 扩展。
* [Stytch](https://www.stytch.com/) - 一个全合一的平台，提供身份验证和欺诈预防 API 和 SDK。免费计划包括 10,000 个每月活跃用户、无限组织、5 个 SSO 或 SCIM 连接和 1,000 个 M2M 令牌。
* [Stack Auth](https://stack-auth.com) — 不糟糕的开源身份验证。最适合开发人员的解决方案，5 分钟内即可开始使用。可免费自托管，或提供托管 SaaS 版本，包括 10k 个免费每月活跃用户。
* [SuperTokens](https://supertokens.com/) - 原生集成到您的应用程序中的开源用户身份验证 - 帮助您快速入门，同时控制用户和开发人员体验。每月 5000 个 MAU 免费。
* [Warrant](https://warrant.dev/) — 托管企业级授权和访问控制服务，用于您的应用程序。免费版包括 100 万次每月 API 请求和 1,000 条授权规则。
* [ZITADEL Cloud](https://zitadel.com) — 即用的用户和访问管理，支持多租户 (B2B) 用例。免费版：25,000 次经过身份验证的请求，包括所有安全功能（OTP、无密码、策略等，没有付费墙）。
* [PropelAuth](https://propelauth.com) — 通过几行代码向任何规模的公司销售，免费版：200 个用户和 10k 个事务性电子邮件（带有水印品牌：“由 PropelAuth 提供支持”）。
* [Logto](https://logto.io/) - 开发、保护和管理产品的用户身份 - 用于身份验证和授权。每月 5,000 个 MAU 免费，并提供开源自托管选项。
* [WorkOS](https://workos.com/) - 免费的用户管理和身份验证，支持多达 100 万个 MAU。支持电子邮件 + 密码、社交身份验证、Magic Auth、MFA 等。
...
## 移动应用分发和反馈
  ...
  * [InstallOnAir](https://www.installonair.com) - 通过空中下载分发 iOS & Android 应用。免费计划：无限上传、私有链接、访客 2 天过期、注册用户 60 天过期。
  * [GetUpdraft](https://www.getupdraft.com) - 测试版移动应用分发。免费计划包括一个应用项目、三个应用版本、500 MB 存储空间，以及每月 100 次应用安装。
...
## 管理系统
  ...
  * [Esper](https://esper.io) — 为 Android 设备提供 MDM 和 MAM，并附带 DevOps。一个用户许可证免费提供 100 台设备和 25 MB 的应用程序存储空间
...
* [cloudamqp.com](https://www.cloudamqp.com/) — RabbitMQ 即服务。小袋獼猴计划：每月最多 100 万条消息、最多 20 个并发连接、最多 100 个队列、最多 10,000 条排队的消息，以及位于不同可用区的多个节点。
* [courier.com](https://www.courier.com/) — 一个 API 支持推送、应用内、电子邮件、聊天、短信和其他消息渠道，并提供模板管理等功能。免费计划包括每月 10,000 条消息。
* [engagespot.co](https://engagespot.co/) — 针对开发人员的多渠道通知基础设施，提供预置的应用内收件箱和无需编码的模板编辑器。免费计划包括每月 10,000 条消息。
* [HiveMQ](https://www.hivemq.com/mqtt-cloud-broker/) - 将 MQTT 设备连接到云本地 IoT 消息代理。 免费连接最多 100 台设备（无需信用卡）且永久免费。
* [knock.app](https://knock.app) – 针对开发人员的通知基础设施。通过一次 API 调用，将消息发送到多个渠道，如应用内、电子邮件、短信、Slack 和推送。免费计划包括每月 10,000 条消息。
* [NotificationAPI.com](https://www.notificationapi.com/) — 5 分钟内向任何软件添加用户通知。免费计划包括每月 10,000 条通知 + 100 条短信和自动电话。
* [Novu.co](https://novu.co) — 针对开发人员的开源通知基础设施。简单的组件和 API，用于在一个地方管理所有通信渠道：电子邮件、短信、直接、应用内和推送。免费计划包括每月 30,000 条通知，保留 90 天。
...
* [eyeson API](https://developers.eyeson.team/) - 基于 WebRTC（SFU、MCU）的视频通信 API 服务，用于构建视频平台。支持实时数据注入、视频布局、录制，以及完全托管的 Web UI（快速启动）或自定义 UI 包。提供[针对开发人员的免费套餐](https://apiservice.eyeson.com/api-pricing)，每月 1,000 分钟会议时长。
* [webpushr](https://www.webpushr.com/) - 网页推送通知 - 对于最多 10,000 位订阅者免费，支持无限推送通知和浏览器内消息。
* [httpSMS](https://httpsms.com) - 使用您的安卓手机作为短信网关发送和接收短信。每月最多发送和接收 200 条消息免费。
* [EMQX Serverless](https://www.emqx.com/en/cloud/serverless-mqtt) - 可扩展且安全的无服务器 MQTT 代理，几秒钟内即可获得。每月 100 万个会话分钟数永久免费（无需信用卡）。
...
* [gitlocalize.com](https://gitlocalize.com) - 对于私有和公开存储库，免费且无限制
* [Lecto](https://lecto.ai/) - 机器翻译 API，提供免费版（30 次免费请求，每次 1000 个翻译字符）。与 Loco Translate Wordpress 插件集成
* [lingohub.com](https://lingohub.com/) — 免费支持 3 个用户，开源项目免费
...
* [Loco](https://localise.biz/) — 免费支持 2000 个翻译、无限名翻译人员、每个项目 10 种语言/1000 个可翻译资产
* [oneskyapp.com](https://www.oneskyapp.com/) — 限量免费版，支持 5 个用户，开源项目免费
...
* [Tolgee](https://tolgee.io) - 免费版 Saas 产品，提供有限的翻译量；免费自托管版本
...
* [weblate.org](https://weblate.org/) — 对于自由项目，免费版支持 10,000 个字符串源，无限制自托管本地版
* [Free PO editor](https://pofile.net/free-po-editor) — 免费面向所有人

## 监控

  * [UptimeObserver.com](https://uptimeobserver.com) - 提供 20 个每 5 分钟间隔的 uptime 监控和可定制的状态页面，甚至可以用于商业用途。享受无限的实时电子邮件和电报通知。无需信用卡即可开始使用。
* [Pingmeter.com](https://pingmeter.com/) - 5 个每 10 分钟间隔的 uptime 监控。监控 SSH、HTTP、HTTPS 和任何自定义 TCP 端口。
* [alerty.ai](https://www.alerty.ai) - 前端、后端、数据库等的免费 APM 和监控 + 免费代理运行。
* [appdynamics.com](https://www.appdynamics.com/) — 免费提供 24 小时指标，应用程序性能管理代理限制为 1 个 Java、1 个 .NET、1 个 PHP 和 1 个 Node.js
...
* [bleemeo.com](https://bleemeo.com) - 免费监控 3 台服务器、5 个 uptime 监控、无限用户、无限仪表板和无限警报规则。
* [checklyhq.com](https://checklyhq.com) - 用于开发人员的端到端/合成监控和深度 API 监控的开源工具。免费计划包括五个用户和 50,000+ 次检查运行。
* [cloudsploit.com](https://cloudsploit.com) — AWS 安全和配置监控。免费：无限按需扫描、无限用户和无限存储帐户。订阅：自动扫描、API 访问等。
* [cronitor.io](https://cronitor.io/) - Cron 作业、网站、API 等性能分析和 uptime 监控。免费级包括五个监控。
* [datadoghq.com](https://www.datadoghq.com/) — 最多 5 个节点免费
* [deadmanssnitch.com](https://deadmanssnitch.com/) — Cron 作业监控。一个免费的监控（snitch），如果推荐他人注册，可以获得更多监控。
* [downtimemonkey.com](https://downtimemonkey.com/) — 60 个 uptime 监控，每 5 分钟间隔一次。电子邮件、Slack 提醒。
* [economize.cloud](https://economize.cloud) — Economize 通过组织云资源来优化和报告 Google Cloud Platform 支出，帮助您了解云基础设施成本。每月支出 5,000 美元以内的 Google Cloud Platform 免费使用。
* [elastic.co](https://www.elastic.co/solutions/apm) — JS 开发人员的即时性能分析。免费，数据保留时间为 24 小时。
* [Grafana Cloud](https://grafana.com/products/cloud/) - Grafana Cloud 是一个可组合的可观察性平台，可将指标和日志与 Grafana 集成。免费：3 个用户、10 个仪表板、100 个警报、Prometheus 和 Graphite 中的指标存储（10,000 个序列、14 天保留期）、Loki 中的日志存储（50 GB 日志、14 天保留期）
* [healthchecks.io](https://healthchecks.io) — 监控您的 Cron 作业和后台任务。最多 20 次检查免费。
* [Hydrozen.io](https://hydrozen.io) — Uptime 监控和状态页面，免费计划：10 个 Uptime 监控、5 个心跳监控、1 个域名监控和 1 个免费状态页面。
* [incidenthub.cloud](https://incidenthub.cloud/) — 云和 SaaS 状态页面聚合器 - 20 个监控和 2 个通知渠道（Slack 和 Discord）永久免费。
* [inspector.dev](https://www.inspector.dev) - 一分钟内即可获得免费且无限期使用的完整实时监控仪表板。
* [instrumentalapp.com](https://instrumentalapp.com) - 免费提供漂亮且易于使用的应用程序和服务器监控，最多 500 个指标和 3 小时数据可见性。
...
* [newrelic.com](https://www.newrelic.com) — New Relic 可观察性平台旨在帮助工程师创建更完美的软件。从单体架构到无服务器架构，您可以对所有内容进行检测，然后分析、排查和优化整个软件堆栈。免费版每月提供 100GB 免费数据引入、1 个免费全访问用户和无限免费主要用户。
* [Middleware.io](https://middleware.io/) - Middleware 可观察性平台为您的应用程序和堆栈提供完全可见性，因此您可以大规模监控和诊断问题。他们为开发社区使用提供了一个永久免费的计划，包括 1M 日志事件日志监控、2 个主机的基础设施监控和 APM。
* [nixstats.com](https://nixstats.com) - 1 个服务器免费。电子邮件通知、公共状态页面、60 秒间隔等。
* [OnlineOrNot.com](https://onlineornot.com/) - OnlineOrNot 为网站和 API 提供 uptime 监控，为 Cron 作业和计划任务提供监控。还提供状态页面。前 5 次每 3 分钟间隔的检查免费。免费版通过 Slack、Discord 和电子邮件发送警报。
* [OntarioNet.ca CN Test](https://cntest.ontarionet.ca) — 检查某个网站是否被中国长城防火墙屏蔽。它通过比较在中国和美国服务器上检测到的 DNS 结果和 ASN 信息来识别 DNS 污染。
* [opsgenie.com](https://www.opsgenie.com/) — 强大的警报和随时待命服务管理。最多 5 个用户免费。
* [paessler.com](https://www.paessler.com/) — 强大的基础设施和网络监控解决方案，包括警报、强大的可视化功能和基本报告。最多 100 个传感器免费。
* [pagecrawl.io](https://pagecrawl.io/) -  监控网站更改，最多 6 个监控免费，每天检查。
* [syagent.com](https://syagent.com/) — 非商业性免费服务器监控服务，提供警报和指标。
* [pagerly.io](https://pagerly.io/) - 在 Slack 上管理待办（与 Pagerduty 和 OpsGenie 集成）。最多 1 个团队免费（一个团队是指一个待办）
* [pagertree.com](https://pagertree.com/) - 警报和待命管理简单界面。最多 5 个用户免费。
* [phare.io](https://phare.io/) - Uptime 监控，最多 100,000 个事件免费，无限项目和无限状态页面。
* [pingbreak.com](https://pingbreak.com/) — 现代 uptime 监控服务。检查无限个 URL，并通过 Discord、Slack 或电子邮件接收 downtime 通知。
* [pingpong.one](https://pingpong.one/) — 带有监控功能的高级状态页面平台。免费版包括一个可自定义的公共状态页面和一个 SSL 子域。向开源项目和非营利组织免费提供专业版。
* [robusta.dev](https://home.robusta.dev/) — 基于 Prometheus 的强大 Kubernetes 监控。自带 Prometheus 或安装全包。免费版包括最多 20 个 Kubernetes 节点。通过 Slack、Microsoft Teams、Discord 等发出警报。与 PagerDuty、OpsGenie、VictorOps、DataDog 等工具集成。
* [sematext.com](https://sematext.com/) — 免费提供 24 小时指标、无限服务器、10 个自定义指标、500,000 个自定义指标数据点、无限个仪表板、用户等。
...
* [speedchecker.xyz](https://probeapi.speedchecker.xyz/) — 性能监控 API，检查 Ping、DNS 等。
  ...
  * [thousandeyes.com](https://www.thousandeyes.com/) — 网络和用户体验监控。3 个位置和 20 个主要网络服务的实时数据免费
  * [uptimerobot.com](https://uptimerobot.com/) — 网站监控，50 个监控免费
  * [uptimetoolbox.com](https://uptimetoolbox.com/) — 五个网站免费监控，60 秒间隔，公共状态页面
  * [zenduty.com](https://www.zenduty.com/) — 端到端事件管理、告警、轮班管理和响应编排平台，适用于网络运营、站点可靠性工程和 DevOps 团队。5 个用户以内免费
  * [instatus.com](https://instatus.com) - 10 秒内获得漂亮的状态页面。永远免费，且订阅和团队数量无限制
  * [Squadcast.com](https://squadcast.com) - Squadcast 是一款端到端事件管理软件，旨在帮助您推广 SRE 最佳实践。提供免费计划，支持 10 个用户
  * [RoboMiri.com](https://robomiri.com/) - RoboMiri 是一款稳定的宕机时间监控工具，提供多种监控：cronjob、关键词、网站、端口、ping。25 个免费的宕机时间检查，每 3 分钟检查一次。通过电话、短信、电子邮件和 Webhook 告警
  * [Better Stack](https://betterstack.com/better-uptime) - 在单一产品中提供宕机时间监控、事件管理、轮班排班/告警和状态页面。免费计划包括 10 个监控，每 3 分钟检查一次，以及状态页面
  * [Pulsetic](https://pulsetic.com) - 10 个监控，6 个月的宕机时间/日志历史记录，无限状态页面和自定义域名！无限期免费，且无需信用卡
...
## 崩溃和异常处理
  ...
  * [elmah.io](https://elmah.io/) — 针对 Web 开发人员的错误日志记录和正常运行时间监控。对开源项目提供免费的小企业订阅。
  ...
  * [GlitchTip](https://glitchtip.com/) — 简单的开源错误跟踪。与开源 Sentry SDK 兼容。每月 1000 次免费事件，或可自托管以取消限制
  ...
  * [rollbar.com](https://rollbar.com/) — 异常和错误监控，每月 5,000 次错误、无限用户和 30 天保留期的免费计划
  * [sentry.io](https://sentry.io/) — Sentry 实时跟踪应用程序异常，并提供小型免费计划。每月 5k 次错误/1 用户免费，如果自托管则使用不受限制。
  * [Axiom](https://axiom.co/) — 存储容量达 0.5 TB 的日志，保留期 30 天。包括与 Vercel 等平台的集成以及与电子邮件/Discord 通知程序的高级数据查询。
  * [Semaphr](https://semaphr.com) — 适用于移动应用程序的免费一键式杀掉开关。
  * [Jam](https://jam.dev) - 只需单击一下即可获得对开发人员友好的错误报告。提供无限次“Jam”的免费计划。
  * [Whitespace](https://whitespace.dev) – 单击浏览器中的一键式错误报告。个人使用免费计划，无限次录制。
...
## 搜索
  ...
  * [CommandBar](https://www.commandbar.com/) - 统一的搜索栏即服务，是一个基于 Web 的 UI 小部件/插件，允许用户在产品内搜索内容、导航、功能等。对每月最多 1,000 名活跃用户和无限命令免费
  * [Magny](https://magny.io) - 一种 SaaS 服务，可帮助实现命令面板（例如应用内搜索），大大减少了用户在应用中查找任何内容的时间，从而提高用户体验和效率。
  * [Orama Cloud](https://orama.com/pricing) — 免费提供 3 个索引、10 万份文档/索引、无限次全文/矢量/混合搜索和 60 天分析
...
## 教育和职业发展
  ...
  * [Free Professional Resume Templates & Editor](https://www.overleaf.com/latex/templates/tagged/cv) - 一个提供大量简历模板的免费平台，经验丰富的专业人士可以克隆、完全编辑和下载，并支持 ATS 优化。
  * [DeepLearning.AI Short Courses](https://www.deeplearning.ai/short-courses/) - 来自业内领先专家的免费短课程，帮助您在 1 小时或更短的时间内掌握最新的生成 AI 工具和技术。
  * [LabEx](https://labex.io) - 通过交互式实验和真实世界项目，学习 Linux、DevOps、网络安全、编程、数据科学等方面的技能。
...
* [EmailJS](https://www.emailjs.com/) – 这不是完整的电子邮件服务器；这只是一个电子邮件客户端，您可以使用它直接从客户端发送电子邮件，而不会暴露您的凭证。免费版：每月 200 次请求，2 个电子邮件模板，请求上限为 50Kb，联系人历史记录有限。
* [EtherealMail](https://ethereal.email) - Ethereal 是一款假 SMTP 服务，主要面向 Nodemailer 和 EmailEngine 用户（但不限于此）。这是一款完全免费的反事务性电子邮件服务，邮件永远不会被投递。
* [Temp-Mail.org](https://temp-mail.org/en/) - 临时/可丢弃的邮件生成器，使用各种域名。每次刷新页面时，电子邮件地址都会更新。完全免费，且不包含任何付费服务。
...
* [FakeMailGenerator.com](https://www.fakemailgenerator.com/) - 德国临时/可丢弃的邮件生成器。支持 10 个域名，同时允许您创建无限数量的地址。（包含广告）除此之外，该服务完全免费。
* [forwardemail.net](https://forwardemail.net) — 免费为自定义域名转发电子邮件。使用您的域名创建和转发无限数量的电子邮件地址（**注意**：如果您使用 .casa、.cf、.click、.email、.fit、.ga、.gdn、.gq、.lat、.loan、.london、.men、.ml、.pl、.rest、.ru、.tk、.top 或 .work TLD，则必须付费，因为它们会产生大量垃圾邮件）
* [Imitate Email](https://imitate.email) - 用于测试跨构建/QA 和 CI/CD 的电子邮件功能的沙箱电子邮件服务器。免费帐户每天可接收 15 封电子邮件。
* [ImprovMX](https://improvmx.com) – 免费电子邮件转发。
* [EForw](https://www.eforw.com) – 为一个域名免费转发电子邮件。接收和发送来自您域名的电子邮件。
* [Inboxes App](https://inboxesapp.com) — 每天创建最多 3 个临时电子邮件，完成后从 Chrome 扩展程序中删除。适用于测试注册流程。
* [inboxkitten.com](https://inboxkitten.com/) - 免费临时/可丢弃的电子邮件收件箱，最多可自动删除 3 天的电子邮件。开源，可自托管。
...
* [Mailchimp](https://mailchimp.com/) — 订阅人数最多 500 人，每月 1,000 封电子邮件，免费。
* [Maildroppa](https://maildroppa.com) - 订阅人数最多 100 人，电子邮件和自动化功能不限量，免费。
* [MailerLite.com](https://www.mailerlite.com) — 每月订阅人数 1,000 人，发送 12,000 封电子邮件，免费。
* [MailerSend.com](https://www.mailersend.com) — 电子邮件 API，SMTP，每月 3,000 封电子邮件，免费发送事务性电子邮件。
...
* [Mailnesia](https://mailnesia.com) - 免费临时/可丢弃的电子邮件，会自动访问注册链接。
...
* [Postmark](https://postmarkapp.com/) - 每月 100 封电子邮件，每周无限量 DMARC 摘要，免费。
...
* [Sender](https://www.sender.net) 每月最多 15,000 封电子邮件，订阅人数最多 2,500 人。
* [SendGrid](https://sendgrid.com/) — 每天 100 封电子邮件，2,000 个联系人，免费。
* [Sendpulse](https://sendpulse.com) — 每月订阅人数 500 人，发送 15,000 封电子邮件，免费。
...
* [Substack](https://substack.com) — 无限量免费通讯服务。开始收费时才付费。
  ...
  * [Takeout](https://takeout.bysourfruit.com) - 一款不断更新的电子邮件服务，让发送电子邮件变得简单。每月 500 封事务性电子邮件免费。
  * [temp-mail.io](https://temp-mail.io) — 一次性临时电子邮件服务，可同时使用多个电子邮件并转发，免费。
  * [tinyletter.com](https://tinyletter.com/) — 每月 5,000 位订阅者免费。
  * [Touchlead](https://touchlead.app) - 一款多用途营销自动化工具，具有销售线索管理、表单生成器和自动化功能。对于有限数量的销售线索和自动化免费。
  * [trashmail.com](https://www.trashmail.com) - 免费的一次性电子邮件地址，支持转发和自动地址过期。
  * [Tuta](https://tuta.com/) - 免费的加密电子邮件服务提供商，内置端到端加密，无广告，无追踪。免费提供 1GB 存储空间和一个日历（Tuta 还提供[付费计划](https://tuta.com/pricing)。）。Tuta 也是[开源软件](https://github.com/tutao/tutanota)，因此您可以自行托管。
  * [Verifalia](https://verifalia.com/email-verification-api) — 实时电子邮件验证 API，具有邮箱确认和一次性电子邮件地址检测功能；每天 25 次免费电子邮件验证。
  * [verimail.io](https://verimail.io/) — 批量和 API 电子邮件验证服务。每月 100 次免费验证。
  * [Zoho](https://www.zoho.com) — 最初是电子邮件提供商，现在提供一系列服务，其中一些有免费计划。具有免费计划的服务列表：
     - [Email](https://zoho.com/mail) 对于 5 位用户免费。5GB/用户和 25 MB 附件限制，一个域名。
     - [Zoho Assist](https://www.zoho.com/assist) — Zoho Assist 的永久免费计划包括一个同时远程支持许可证和 5 个无人值守计算机许可证，可用于专业和个人用途，无限期使用。
     - [Sprints](https://zoho.com/sprints) 对于 5 位用户、5 个项目和 500MB 存储免费。
     - [Docs](https://zoho.com/docs) — 对于 5 位用户免费，附件限制为 1 GB，存储空间为 5GB。附带 Zoho Office Suite（Writer、Sheets 和 Show）。
     - [Projects](https://zoho.com/projects) — 对于 3 位用户、2 个项目和 10 MB 附件限制免费。Bugtracker 适用同样的计划。
     - [Connect](https://zoho.com/connect) — 团队协作工具，对于 25 位用户免费，具有 3 个组、3 个自定义应用、3 个看板、3 个手册和 10 个集成，以及频道、活动和论坛。
     - [Meeting](https://zoho.com/meeting) — 支持最多 3 位与会者和 10 位网络研讨会与会者参加会议。
     - [Vault](https://zoho.com/vault) — 密码管理服务，个人用户免费。
     - [Showtime](https://zoho.com/showtime) — 另一种远程会议软件，支持最多 5 位与会者参加培训。
     - [Notebook](https://zoho.com/notebook) — Evernote 的免费替代品。
     - [Wiki](https://zoho.com/wiki) — 对于 3 位用户免费，具有 50 MB 存储空间、无限页、zip 备份、RSS 和 Atom 提要、访问控制和可定制 CSS。
     - [Subscriptions](https://zoho.com/subscriptions) — 订阅和计费管理，对于 20 位客户/订阅和 1 位用户免费，所有付款均由 Zoho 托管。存储最后 40 个订阅指标。
     - [Checkout](https://zoho.com/checkout) — 产品计费管理，最多 3 个页面和 50 笔支付。
     - [Desk](https://zoho.com/desk) — 客户支持管理，3 位客服代表，私人知识库和电子邮件票证。与 Assist 集成，支持 1 位远程技术人员和 5 台无人值守计算机。
     - [Cliq](https://zoho.com/cliq) — 团队聊天软件，具有 100 GB 存储空间、无限用户和每个频道 100 位用户，支持 SSO。
...
## 功能切换管理平台
  ...
  * [ConfigCat](https://configcat.com) - ConfigCat 是一款以开发者为中心的功能标志服务，具有无限数量的团队大小、优质的支持和合理的价格。免费计划包括最多 10 个标志、两个环境、1 个产品和每月 500 万次请求。
  ...
  * [Hypertune](https://www.hypertune.com) - 具有类型安全的功能标志、A/B 测试、分析和应用程序配置，具有 Git 风格的版本控制和在内存、本地标志评估。对于最多 5 个团队成员，免费提供，且功能标志和 A/B 测试不受限制。
  ...
  * [Statsig](https://www.statsig.com) - 一个功能齐全的功能管理、A/B 测试和分析平台。它的免费版非常慷慨，提供无限制的座位、标志、实验和动态配置，支持每月最多 100 万次事件。
  * [Abby](https://www.tryabby.com) - 开源功能标志和 A/B 测试。代码配置和完全分型的 Typescript SDK。与 Next.js 和 React 等框架紧密集成。慷慨的免费层和便宜的扩展选项。

**[⬆️ 返回顶部](#目录)**

## 字体

  * [dafont](https://www.dafont.com/) - 该网站上呈现的字体是其作者财产，可以是免费软件、共享软件、演示版本或公有领域。
  * [Everything Fonts](https://everythingfonts.com/) - 提供多种工具; @font-face、Units Converter、Font Hinter 和 Font Submitter。
  * [Font Squirrel](https://www.fontsquirrel.com/) - 授权用于商业工作的免费字体。手动选择这些字体，并以易于使用的格式呈现。
  * [Google Fonts](https://fonts.google.com/) - 许多免费字体可以通过下载或链接到 Google 的 CDN 来快速轻松地安装在网站上。
  ...
  * [fonts.xz.style](https://fonts.xz.style/) - 一个免费的开源服务，用于使用 CSS 将字体系列交付给网站。
...
## 表格
  ...
  * [Form.taxi](https://form.taxi/) — HTML 表单提交的端点。带有通知、垃圾邮件阻止器和符合 GDPR 数据处理。提供用于基本使用的免费计划。
  ...
  * [formspark.io](https://formspark.io/) -  表单到电子邮件服务，免费计划允许无限数量的表单，每月 250 次提交，并支持客户支持团队。
  ...
  * [HeroTofu.com](https://herotofu.com/) - 带有机器人检测和加密存档的表单后端。通过 UI 将提交转发到电子邮件、Slack 或 Zapier。使用您自己的前端。无需服务器代码。免费计划提供无限数量的表单和每月 100 次提交。
  * [HeyForm.net](https://heyform.net/) - 拖放式在线表单生成器。免费版允许您创建无限数量的表单并收集无限数量的提交。带有预构建模板、防垃圾邮件和 100MB 文件存储。
  * [Tally.so](https://tally.so/) - 所有功能的 99% 都是免费的。免费版允许您拥有：无限数量的表单、无限数量的提交、电子邮件通知、表单逻辑、收集付款、文件上传、自定义感谢页等。
  * [Hyperforms.app](https://hyperforms.app/) — 在几秒钟内创建表单到电子邮件等功能，而无需后端代码。个人帐户免费提供每月最多 50 次表单提交。
  ...
  * [Screeb](https://screeb.app/) - 用于解码用户行为的应用内调查和产品分析。永久免费计划允许 500 个每月活跃用户、无限响应和事件、许多集成、导出和定期报告。
  ...
  * [stepFORM.io](https://stepform.io) - 测验和表单生成器。免费计划有五个表单，每个表单最多 3 个步骤，以及每月 50 次响应。
  ...
  * [forms.app](https://forms.app/) — 使用条件逻辑、自动评分计算器和 AI 等功能创建在线表单。使用免费计划收集多达 100 条回复，在网站上嵌入表单，或使用链接。
  ...
  * [feedback.fish](https://feedback.fish/) - 免费计划允许收集 25 条反馈提交。易于与 React 和 Vue 组件集成。
...
* [Portkey](https://portkey.ai/) - Gen AI 应用程序的控制面板，具有可观察性套件和 AI 网关。每月免费发送和记录多达 10,000 个请求。
...
* [Clair](https://askclair.ai/) - 临床 AI 参考。学生可以免费使用专业工具套件，其中包括开放搜索、临床摘要、Med Review、药物相互作用、ICD-10 代码和管理。此外，还提供专业套件的免费试用。
...
* [Ultra AI](https://ultraai.app) - 产品的 AI 指挥中心。功能包括多提供商 AI 网关、提示管理器、缓存、日志、分析、模型降级和用户速率限制。免费永久计划每月提供 10,000 多个请求并提供除缓存之外的所有功能。
...
* [ovh.ie](https://www.ovh.ie/ssl-gateway/) — 提供免费的 DDoS 防护和 SSL 证书
...
* [statically.io](https://statically.io/) — 一个为 Git 仓库 (GitHub、GitLab、Bitbucket)、WordPress 相关资源和图像提供服务的 CDN
* [toranproxy.com](https://toranproxy.com/) — Packagist 和 GitHub 的代理。永不失败的 CD。免费用于个人使用，一个开发人员，无支持
* [UNPKG](https://unpkg.com/) — 一个为 npm 上的一切提供服务的 CDN
* [weserv](https://images.weserv.nl/) — 一个图像缓存和调整大小服务。实时操作图像，并提供全局缓存
* [Namecheap Supersonic](https://www.namecheap.com/supersonic-cdn/#free-plan) — 提供免费的 DDoS 防护
...
## 平台即服务
  ...
  * [appwrite](https://appwrite.io) - 无限数量的项目，无项目暂停（支持 WebSocket）和身份验证服务。免费版包含每个项目 1 个数据库、3 个存储桶和 5 个函数。
  ...
  * [Deno Deploy](https://deno.com/deploy) - 一个在全球范围内运行 JavaScript、TypeScript 和 WebAssembly 的分布式系统。免费版包含每天 10 万次请求和每月 100 GiB 数据传输。
  * [domcloud.co](https://domcloud.co) – 一个提供 CI/CD、GitHub、SSH 和 MariaDB/Postgres 数据库的 Linux 托管服务。免费版包含 1 GB 存储空间和 1 GB/月的网络限制，并限制使用免费域名。
  * [encore.dev](https://encore.dev/) — 一种使用静态分析提供自动基础设施、无样板代码等的后端框架。包括为爱好者项目提供的免费云托管。
  * [flightcontrol.dev](https://flightcontrol.dev/) - 在您自己的 AWS 账户中部署 Web 服务、数据库等，并使用类似 Git push 的工作流。免费版适用于个人 GitHub 仓库的单个开发人员。AWS 费用通过 AWS 账单支付，但您可以使用 AWS 积分和免费套餐。
  * [gigalixir.com](https://gigalixir.com/) - Gigalixir 为 Elixir/Phoenix 应用提供一个永不休眠的免费实例和一个免费套餐 PostgreSQL 数据库，该数据库限制为 2 个连接、10,000 行，且不提供备份。
  * [Glitch](https://glitch.com/) — 免费的公共托管服务，具有代码共享和实时协作功能。免费套餐包含每月 1000 小时限制。
  * [Lade](https://www.lade.io/) - Lade 是一个为开发人员构建云平台。免费套餐包含三个服务，可以是应用程序或数据库。每个服务包含 128 MB RAM 和 1 GB 存储空间。
  * [pipedream.com](https://pipedream.com) - 一个为开发人员构建集成平台。根据任何触发器开发任何工作流。工作流是您可以[免费](https://docs.pipedream.com/pricing/)运行的代码。无需管理服务器或云资源。
  * [pythonanywhere.com](https://www.pythonanywhere.com/) — 云 Python 应用托管。初学者账户免费，1 个 Python Web 应用程序位于 your-username.pythonanywhere.com 域名，512 MB 私有文件存储，1 个 MySQL 数据库
  ...
  * [Koyeb](https://www.koyeb.com) - Koyeb 是一个友好的无服务器平台，可用于在全球部署应用程序。无缝运行 Docker 容器、Web 应用程序和 API，并提供基于 Git 的部署、本机自动扩展、全球边缘网络以及内置的服务网格和发现。免费实例允许您在德国法兰克福或美国华盛顿特区部署 Web 服务。免费托管的 Postgres 数据库可用于法兰克福（德国）、华盛顿特区（美国）和新加坡。512MB 内存、2GB 存储和 0.1 个 CPU。开始使用时无需信用卡。
  * [Napkin](https://www.napkin.io/) - FaaS，具有 500Mb 内存、15 秒的默认超时时间，以及每月 5,000 次免费 API 调用，每秒限额 5 次。
  * [Meteor Cloud](https://www.meteor.com/cloud) — Galaxy 托管。Meteor 的 Meteor 应用程序平台即服务包括免费的 MongoDB 共享托管和自动 SSL。
  * [Northflank](https://northflank.com) — 利用强大的 UI、API 和 CLI 构建和部署微服务、作业和托管数据库。免费套餐包含两个服务、两个 cron 作业和 1 个数据库。
  ...
  * [mogenius](https://mogenius.com) - 轻松构建、部署和运行 Kubernetes 上的服务。免费套餐支持将本地 Kubernetes 与 mogenius 连接，使个人开发人员能够在机器上创建类似于生产环境的测试环境。
...
## 后端即服务
  ...
  * [Activepieces](https://www.activepieces.com) - 构建自动化流程，将多个应用程序与应用程序的后端连接起来。例如，当应用程序中触发事件时，在 Slack 中发送消息或在 Google 工作表中添加行。每月最多 5,000 个任务免费。
  * [back4app.com](https://www.back4app.com) - Back4App 是一个基于 Parse Platform 的易于使用、灵活且可扩展的后端。
  * [backendless.com](https://backendless.com/) — 移动和 Web Baas，提供 1 GB 免费文件存储、每月 50,000 次推送通知和表格中的 1000 个数据对象。
  * [bismuth.cloud](https://www.bismuth.cloud/) — 我们的 AI 将使用函数运行时和托管存储来提升您的 Python API，在我们的在线编辑器中免费构建和托管，或使用您喜欢的工具本地构建和托管。
  * [BMC Developer Program](https://developers.bmc.com/site/global/bmc_helix_platform/program/overview/index.gsp) — BMC 开发人员计划提供文档和资源，以构建和部署企业的数字创新。访问一个全面的个人沙箱，其中包括平台、SDK 以及构建和定制应用程序时使用的组件库。
  ...
  * [darklang.com](https://darklang.com/) - 集成了编辑器和基础设施的托管语言。在测试版期间可以访问，测试版之后计划提供慷慨的免费层。
  * [Firebase](https://firebase.com) — Firebase 帮助您构建和运行成功的应用程序。Spark 计划免费提供身份验证、托管、Firebase ML、实时数据库、云存储、Testlab。A/B 测试、分析、应用程序分发、应用程序索引、云消息传递 (FCM)、Crashlytics、动态链接、应用内消息传递、性能监控、预测和远程配置始终免费。
  * [Flutter Flow](https://flutterflow.io) — 构建 Flutter 应用程序 UI，而无需编写任何代码。还具有 Firebase 集成。免费计划包括 UI 构建器和免费模板的完全访问权限。
  * [getstream.io](https://getstream.io/) — 在几小时而不是几周的时间内构建可扩展的应用内聊天、消息传递、视频和音频以及提要。
  * [hasura.io](https://hasura.io/) — Hasura 扩展您现有的数据库，无论它托管在何处，并提供安全的 GraphQL API，可用于 Web、移动和数据集成工作负载。每月 1GB 数据传输免费。
  * [iron.io](https://www.iron.io/) — 异步任务处理（例如 AWS Lambda），提供免费层和 1 个月的免费试用。
  ...
  * [paraio.com](https://paraio.com) — 具有灵活身份验证、全文搜索和缓存的后端服务 API。对于一个应用程序免费，1GB 的应用程序数据。
  * [progress.com](https://www.progress.com/kinvey) — 移动后端，入门计划具有无限次/秒的请求，1GB 数据存储。企业应用程序支持
  * [pubnub.com](https://www.pubnub.com/) — 每月最多 100 台活动设备，免费推送 100 万条消息。
  * [pushbots.com](https://pushbots.com/) — 推送通知服务。每月最多 150 万次推送免费。
  * [pushcrew.com](https://pushcrew.com/) — 推送通知服务。对于最多 2,000 个订阅者，无限量通知。
  * [pusher.com](https://pusher.com/beams) — 每月 2,000 名活跃用户免费、无限量推送通知。适用于 iOS 和 Android 设备的单一 API。
  ...
  * [Salesforce Developer Program](https://developer.salesforce.com/signup) — 使用拖放工具快速构建应用程序。使用单击自定义数据模型。使用 Apex 代码更进一步。使用强大的 API 与任何内容集成。使用企业级安全性进行保护。使用单击或任何领先的 Web 框架自定义 UI。免费的开发人员计划提供对完整 Lightning Platform 的访问权限。
  * [ServiceNow Developer Program](https://developer.servicenow.com/) — 快速构建、测试和部署应用程序，使您的组织的工作更加高效。免费实例和访问预览版。
  * [simperium.com](https://simperium.com/) — 即时、自动、跨平台地传输数据，无限量发送和存储结构化数据，每月最多 2,500 个用户。
  ...
  * [stackstorm.com](https://stackstorm.com/) — 应用程序、服务和工作流的基于事件的自动化，免费但不提供流程、访问控制和 LDAP。
  ...
  * [LeanCloud](https://leancloud.app/) — 移动后端。1GB 数据存储、256MB 实例、每天 3,000 个 API 请求和每天 10,000 次推送免费。（API 与 Parse Platform 非常相似）
...
* [DronaHQ](https://www.dronahq.com/) — DronaHQ 是一个低代码平台，帮助工程团队和产品经理 10 倍速地构建内部工具、自定义用户旅程、数字体验、自动化、自定义管理面板和运营应用程序。
...
* [outsystems.com](https://www.outsystems.com/) — 适用于本地或云部署的企业 Web 开发 PaaS，免费的“个人环境”产品允许无限代码和高达 1 GB 的数据库。
...
* [lil'bots](https://www.lilbots.io/) - 在线编写和运行脚本，利用免费的内置 API，例如 OpenAI、Anthropic、Firecrawl 等。非常适合构建 AI 代理/内部工具并与团队共享。 免费版包括对 API、AI 编码助手和每月 10,000 个执行积分的完全访问权限。
...
## 网络托管
  ...
  * [Drive To Web](https://drv.tw) — 直接从 Google Drive 和 OneDrive 在网上托管。仅限静态站点。永久免费。每个 Google/Microsoft 账户一个站点。
  ...
  * [Free Hosting](https://freehostingnoads.net/) — 免费托管服务，支持 PHP 5、Perl、CGI、MySQL、FTP、文件管理器、POP 电子邮件、免费子域名、免费域名托管、DNS 区域编辑器、网站统计信息、免费在线支持等其他功能，而其他免费托管服务则不提供这些功能。
  * [Freehostia](https://www.freehostia.com) — FreeHostia 提供免费的托管服务，包括业内领先的控制面板和 50 多个应用程序的一键式安装。即时设置，无强制广告。
  ...
  * [Lecturify](https://www.lecturify.net/index.en.html) - 提供 SFPT 访问以上传和下载文件，支持 PHP。
  ...
  * [tilda.cc](https://tilda.cc/) — 一个站点，50 页，50 MB 存储，仅 170 多个可用主预定义块，无字体、无 favicon 和无自定义域名
  ...
  * [fleek.co](https://fleek.co/) - 在开放网络及其协议上无缝免费构建现代网站和应用程序，支持无限个网站和 50 GB/月带宽。
  * [MDB GO](https://mdbgo.com/) - 免费托管一个项目，两个星期的容器 TTL，每个项目 500 MB RAM，SFTP - 1G 磁盘空间。
  * [Patr Cloud](https://patr.cloud/) — 一个易于使用的云平台，在其付费服务中提供托管三个静态站点的服务。
  * [Serv00.com](https://serv0.com/) — 免费提供 3 GB 的网络托管服务，并提供每日备份（7 天）。支持：Crontab 作业、SSH 访问、存储库 (GIT、SVN 和 Mercurial)、MySQL、PostgreSQL、MongoDB、PHP、Node.js、Python、Ruby、Java、Perl、TCL/TK、Lua、Erlang、Rust、Pascal、C、C++、D、R 等。
...
## 域名系统
  ...
  * [deSEC](https://desec.io) - 免费的 DNS 托管服务，支持 API，以安全为设计理念。基于开源软件，并得到 [SSE](https://www.securesystems.de/) 的支持。
  ...
  * [Dynv6.com](https://dynv6.com/) — 免费的 DDNS 服务，[支持 API](https://dynv6.com/docs/apis) 和多种 DNS 记录类型管理（如 CNAME、MX、SPF、SRV、TXT 等）。
  * [freedns.afraid.org](https://freedns.afraid.org/) — 免费的 DNS 托管服务。还提供基于众多公共用户 [贡献域名](https://freedns.afraid.org/domain/registry/) 的免费子域名。注册后，可在“子域名”菜单中获取免费子域名。
  * [luadns.com](https://www.luadns.com/) — 免费的 DNS 托管服务，支持最多 3 个域名和所有功能，但有合理限制
  * [namecheap.com](https://www.namecheap.com/domains/freedns/) — 免费的 DNS 服务，不限制域名数量
  * [nextdns.io](https://nextdns.io) - 基于 DNS 的防火墙，每月免费 30 万次查询
  ...
  * [zonewatcher.com](https://zonewatcher.com/) — 自动备份和 DNS 变更监控服务。免费支持 1 个域名
...
## 基础设施即服务
  ...
  * [4EVERLAND](https://www.4everland.org/) — 兼容 AWS S3 - API、界面操作、CLI 和其他上传方法，以安全、便捷、高效的方式上传和存储来自 IPFS 和 Arweave 网络的文件。注册用户可免费获得 6 GB IPFS 存储空间和 300 MB Arweave 存储空间。任何小于 150 KB 的 Arweave 文件上传都是免费的。
  * [backblaze.com](https://www.backblaze.com/b2/) — Backblaze B2 云存储。免费提供 10 GB 的对象存储，无限期使用（类似 Amazon S3）
  * [filebase.com](https://filebase.com/) - 区块链支持的与 S3 兼容的对象存储。无限期免费提供 5 GB 存储空间。
  * [Tebi](https://tebi.io/) - 与 S3 兼容的对象存储。免费提供 25 GB 存储空间和 250 GB 出站传输。
  * [Idrive e2](https://www.idrive.com/e2/) - 与 S3 兼容的对象存储。免费提供 10 GB 存储空间和每月 10 GB 下载带宽。
  * [C2 Object Storage](https://c2.synology.com/en-us/pricing/object-storage) - 与 S3 兼容的对象存储。免费提供 15 GB 存储空间和每月 15 GB 下载。
...
* [Upstash](https://upstash.com/) — 无服务器 Redis，每天最多 10,000 次请求，最大数据库大小 256MB，并发连接 20 个
...
* [SeaTable](https://seatable.io/) — Seafile 团队构建的灵活、类似电子表格的数据库。无限表格、2,000 行、1 个月版本控制，最多 25 位团队成员。
* [skyvia.com](https://skyvia.com/) — 云数据平台提供免费层，所有计划在测试版中都是完全免费的
* [StackBy](https://stackby.com/) — 一种结合了电子表格灵活性、数据库功能和内置集成功能的工具。免费计划包括无限用户、10 个堆栈和每个堆栈 2GB 附件。
* [TiDB Cloud](https://en.pingcap.com/tidb-cloud/) — TiDB 是一种开源的与 MySQL 兼容的分布式 HTAP RDBMS。TiDB Serverless 每月免费提供 5GB 行存储、5GB 列存储和 5000 万个请求单元 (RUs)。
* [Turso by ChiselStrike](https://chiselstrike.com/) - Turso 是一款带有 SQLite 开发体验的边缘数据库。Turso 提供免费入门版，9 GB 总存储空间，最多 500 个数据库，最多 3 个位置，每月 10 亿行读取，以及 SQLite 本地开发支持。
* [InfluxDB](https://www.influxdata.com/) — 时间序列数据库，免费版最多可写入 3MB/5 分钟、读取 30MB/5 分钟和 10,000 个基数系列
* [restdb.io](https://restdb.io/) - 一种快速简便的 NoSQL 云数据库服务。restdb.io 提供模式、关系、自动 REST API（带有 MongoDB 查询）和高效的多用户管理 UI 来处理数据。免费计划允许 3 个用户、2,500 条记录和每秒 1 个 API 请求。
...
* [Neo4j Aura](https://neo4j.com/cloud/aura/) — 托管本机图形 DBMS/分析平台，具有 Cypher 查询语言和 REST API。对图形大小（50k 节点、175k 关系）有限制。
* [Neon](https://neon.tech/) — 托管 PostgreSQL，0.5 GB 存储（总量），1 个项目，10 个分支，无限个数据库，始终可用的主分支（5 分钟后自动挂起），非主分支计算每月 20 小时活动时间（总量）。
* [Dgraph Cloud](https://cloud.dgraph.io/pricing?type=free) — 带有 GraphQL API 的托管本机图形 DBMS。每天限制 1 MB 数据传输。
* [Tinybird](https://tinybird.co) - 一种无服务器托管 ClickHouse，通过 HTTP 进行无连接数据引入，并允许您将 SQL 查询发布为托管 HTTP API。免费版没有时间限制，10GB 存储 + 每天 1000 次 API 请求。
* [TigerGraph Cloud](https://www.tigergraph.com/cloud/) — 托管本机图形 DBMS/分析平台，具有类似 SQL 的图形查询语言和 REST API。一个免费实例，具有两个 vCPU、8GB 内存和 50GB 存储，在 1 小时闲置后进入睡眠状态。
* [TerminusCMS](https://terminusdb.com/pricing) — TerminusDB 的托管免费服务，TerminusDB 是一种用 Prolog 和 Rust 编写的文档和图形数据库。开发免费，企业部署和支持付费。
* [YugabyteDB](https://cloud.yugabyte.com) - YugabyteDB 是一种与 PostgreSQL 兼容的分布式 SQL 数据库。云免费层包括两个 vCPU、4GB RAM 和 10GB 磁盘。
* [filess.io](https://filess.io) - filess.io 是一个平台，您可以在其中免费创建以下任一数据库：MySQL、MariaDB、MongoDB 和 PostgreSQL。
* [xata.io](https://xata.io) - Xata 是一种带有内置强大搜索和分析功能的无服务器数据库。一个 API、多个类型安全的客户端库，并针对开发工作流程进行了优化。免费版足够爱好者开发人员使用，每个 Xata 有三个单位，请参阅网站上的单位定义。
* [8base.com](https://www.8base.com/) - 8base 是一个为 JavaScript 开发人员构建的基于 MySQL 和 GraphQL 的全栈低代码开发平台，以及无服务器后端即服务。它允许您使用 UI 应用程序生成器快速构建 Web 应用程序，并快速扩展。免费版包括行：2,500 条、存储：500 条、无服务器计算：1Gb/h 和客户端应用程序用户：5 个。
* [Nile](https://www.thenile.dev/) — B2B 应用程序的 Postgres 平台。无限数据库，始终可用，无需关闭，1GB 存储（总量），5000 万个查询令牌，自动缩放，无限向量嵌入
...
## 隧道、WebRTC、Web套接字服务器和其他路由器
  ...
  * [Hamachi](https://www.vpn.net/) — LogMeIn Hamachi 是一款托管 VPN 服务，可让您安全地将 LAN 网络扩展到分布式团队。免费计划允许无限数量的网络，最多包含 5 个人。
  ...
  * [serveo](https://serveo.net/) — 将本地服务器暴露给互联网，无需安装或注册。免费子域名，无限制。
  ...
  * [Tailscale](https://tailscale.com/) — 零配置 VPN，使用开源 WireGuard 协议。支持 MacOS、iOS、Windows、Linux 和 Android 设备。免费计划供个人使用，最多 100 台设备和 3 位用户。
  * [webhookrelay.com](https://webhookrelay.com) — 管理、调试、扇出并代理所有 webhook 到公共或内部（例如 localhost）目标。还可以通过获取公共 HTTP 端点来通过隧道将位于私有网络中的服务器暴露给外部（`https://yoursubdomain.webrelay.io <----> http://localhost:8080`）。
  ...
  * [ZeroTier](https://www.zerotier.com) — 开源托管虚拟以太网服务。免费计划提供无限数量的端到端加密网络，最多 25 个客户端。支持桌面/移动/NA 客户端；提供 Web 界面用于配置自定义路由规则并批准私有网络中的新客户端节点。
  * [LocalXpose](https://localxpose.io) — 反向代理，可将本地服务器暴露给互联网。免费计划提供 15 分钟的隧道寿命。
  * [Traefik-Hub](https://traefik.io/traefik-hub/) - 通过隧道将本地运行的服务发布到公共的自定义 URL，并通过访问控制来保护它们。免费提供 1 个集群中最多 5 个服务。
...
- [Backlog](https://backlog.com) — 一个平台满足团队发布精彩项目的所有需求。免费计划包含 1 个项目、10 位用户和 100MB 存储空间
...
- [clickup.com](https://clickup.com/) — 项目管理。免费版提供付费版中的云存储。支持移动应用程序和 Git 集成
- [Clockify](https://clockify.me) - 时间跟踪和时间表应用程序，可跨项目跟踪工作时间。无限用户，永久免费
- [Cloudcraft](https://cloudcraft.co/) — 使用 Cloudcraft 可视化设计器在几分钟内设计专业架构图，针对 AWS 进行了优化，具有显示实时数据的智能组件。免费计划为单个用户提供无限私人图表
- [Codegiant](https://codegiant.io) — 项目管理与存储库托管和 CI/CD。免费计划为 5 位团队成员提供无限存储库、项目和文档。每月 500 分钟 CI/CD 时间和 30,000 分钟无服务器代码运行时间，1GB 存储库存储空间
...
- [Crosswork](https://crosswork.app/) - 多功能项目管理平台。免费版支持最多 3 个项目、无限用户和 1GB 存储空间
...
- [GForge](https://gforge.com) — 项目管理和问题跟踪工具，适用于复杂的项目，提供自托管和 SaaS 选项。SaaS 免费计划为前 5 位用户免费，并向开源项目免费提供
- [gleek.io](https://www.gleek.io) — 开发人员的免费描述-图表工具。使用关键字创建非正式的 UML 类、对象或实体关系图
...
- [WishKit](https://wishkit.io) — 为 iOS/macOS 应用收集应用内用户反馈，并根据用户投票对功能进行优先级排序。免费计划支持 1 个应用
- [Ilograph](https://www.ilograph.com/)  — 交互式图表，允许用户从多个角度和细节级别查看其基础设施。图表可以用代码表示。免费版包含无限量私人图表，最多 3 位查看者
- [Jira](https://www.atlassian.com/software/jira) — 高级软件开发项目管理工具，适用于许多企业环境。免费计划支持最多 10 位用户
- [kanbanflow.com](https://kanbanflow.com/) — 基于看板的项目管理。免费版提供更多选项的付费版
- [kanbantool.com](https://kanbantool.com/) — 基于 Kanban 看板的项目管理。免费计划包含 2 个看板、2 位用户，不提供附件或文件
- [Kitemaker.co](https://kitemaker.co) - 通过 Slack、Discord、Figma 和 GitHub 协作完成产品开发过程的所有阶段，并跟踪工作。无限用户，无限空间。免费计划支持最多 250 个工作项
- [Kiter.app](https://www.kiter.app/) - 让任何人都可以组织他们的求职活动并跟踪面试、机会和人脉。强大的 Web 应用程序和 Chrome 扩展程序。完全免费
- [Kumu.io](https://kumu.io/)  — 带有动画、装饰、过滤器、聚类、电子表格导入等的关联图。免费版允许无限量公共项目。图形大小无限制。为学生提供免费私人项目。如果您不希望将文件留在线上的公开访问状态，则提供沙盒模式（上传、编辑、下载、丢弃）
- [Linear](https://linear.app/) — 具有简化界面的主题跟踪器。免费版支持无限成员，文件上传大小上限为 10MB，250 个问题（不包括存档）
- [leiga.com](https://www.leiga.com/) — Leiga 是一种 SaaS 产品，它使用 AI 自动管理您的项目，帮助您的团队保持专注，释放巨大潜力，确保您的项目按计划进行。免费版支持最多 10 位用户、20 个自定义字段、2GB 存储空间、AI 视频录制时间限制为 5 分钟/视频，AI 自动化运行次数为 20 次/用户/月
...
- [Ora](https://ora.pm/) - Agile 任务管理和团队协作。免费版支持最多 3 位用户，文件限制为 10MB
- [pivotaltracker.com](https://www.pivotaltracker.com/) — 无限量公共项目免费，2 个私有项目免费，3 位活跃用户（读写）和无限被动用户（只读）
- [plan.io](https://plan.io/) — 项目管理与存储库托管和更多选项。免费版支持 2 位用户、10 位客户和 500MB 存储空间
...
- [point.poker](https://www.point.poker/) - 在线 Planning Poker（基于共识的估算工具）。免费版支持无限用户、团队、会话、回合和投票。无需注册
- [ScrumFast](https://www.scrumfast.com) - 直观的界面 Scrum 看板，免费版支持最多 5 位用户
  ...
  * [targetprocess.com](https://www.targetprocess.com/) — 可视化项目管理，从看板和敏捷到几乎任何运营流程。对于无限用户，最多 1,000 个数据实体免费 {[更多详细信息](https://www.targetprocess.com/pricing/)}
  * [taskade.com](https://www.taskade.com/) — 实时协作任务列表和团队大纲。免费计划包含一个工作区，无限任务和项目；1GB 文件存储；1 周项目历史记录；以及每一次视频会议最多 5 名参与者。
  ...
  * [terrastruct.com](https://terrastruct.com/) — 专门用于软件架构的在线绘图工具。每个图层最多 4 层免费。
  ...
  * [ubertesters.com](https://ubertesters.com/) — 测试平台、集成和众测，2 个项目，5 位成员
  ...
  * [YouTrack](https://www.jetbrains.com/youtrack/buy/#edition=incloud) — 适用于开源软件 (FOSS) 项目和私人项目的免费托管 YouTrack (InCloud)。包括时间跟踪和敏捷看板，3 位用户以内免费。
  * [zenhub.com](https://www.zenhub.com) — GitHub 内部的唯一项目管理解决方案。对于公共存储库、开源软件 (OSS) 和非营利组织免费
  * [zenkit.com](https://zenkit.com) — 项目管理和协作工具。5 位成员以内免费，附件 5GB。
  ...
  * [Sflow](https://sflow.io) — sflow.io 是一个项目管理工具，专为敏捷软件开发、营销、销售和客户支持项目而构建，特别适合外包和跨组织协作项目。免费计划最多支持 3 个项目和 5 位成员。
  * [Pulse.red](https://pulse.red) — 免费的极简计时器和项目时间表应用程序。
...
* [cloudinary.com](https://cloudinary.com/) — 提供图像上传、强大的图像处理、存储和交付服务，支持 Ruby、Python、Java、PHP、Objective-C 等多种库。免费版包括 25 个每月信用点。一个信用点等于 1,000 次图像转换、1 GB 存储或 1 GB CDN 使用量。
* [embed.ly](https://embed.ly/) — 提供嵌入网页的媒体、响应式图像缩放以及从网页提取元素的 API。每月免费处理 5,000 个 URL，每秒 15 个请求。
* [filestack.com](https://www.filestack.com/) — 文件选择、转换和交付服务，免费提供 250 个文件、500 个转换和 3 GB 带宽。
...
* [gumlet.com](https://www.gumlet.com/) — 图像和视频托管、处理和流媒体服务，通过 CDN 提供。提供慷慨的免费版，每月 250 GB 视频和 30 GB 图像。
* [image-charts.com](https://www.image-charts.com/) — 带有水印的无限量图像图表生成服务。
* [Imgbot](https://github.com/marketplace/imgbot) — Imgbot 是一个友好的机器人，可以优化您的图像并节省时间。优化后的图像意味着在不牺牲质量的情况下减小文件大小。它对开源项目免费。
* [ImgBB](https://imgbb.com/) — ImgBB 是一个无限量图像托管服务。拖放图像到屏幕上的任何位置。每个图像的最大大小为 32 MB。上传图像后，您将获得直接图像链接、BBCode 和 HTML 缩略图。登录后可以查看上传历史记录。
...
* [npoint.io](https://www.npoint.io/) — 一个具有协作模式编辑功能的 JSON 存储服务。
* [nitropack.io](https://nitropack.io/) - 自动优化网站的前端性能（缓存、图像和代码优化、CDN）。每月免费处理 5,000 次页面浏览。
* [otixo.com](https://www.otixo.com/) — 加密、共享、复制和移动您的所有云存储文件，在一个地方管理。基础版提供无限文件传输，最大文件大小为 250 MB，并允许加密 5 个文件。
* [packagecloud.io](https://packagecloud.io/) — 托管的软件包存储库，支持 YUM、APT、RubyGem 和 PyPI。提供有限的免费计划和开源计划，可以通过申请获得。
* [getpantry.cloud](https://getpantry.cloud/) — 一个简单的 JSON 数据存储 API，非常适合个人项目、黑客马拉松和移动应用！
* [Pinata IPFS](https://pinata.cloud) — Pinata 是将文件上传到 IPFS 的最简单方法。友好的用户界面和 IPFS API 使 Pinata 成为平台、创作者和收藏家最易于使用的 IPFS 固定服务。提供 1 GB 的免费存储空间，以及对 API 的访问。
* [placekitten.com](https://placekitten.com/) — 一个快速简单的服务，用于获取作为占位符的猫咪图片。
* [plot.ly](https://plot.ly/) — 绘制和分享数据。免费版包括无限量的公共文件和 10 个私人文件。
...
* [resmush.it](https://resmush.it) — reSmush.it 是一个免费的图像优化 API。reSmush.it 已经集成到 WordPress、Drupal、Magento 等最常见的 CMS 中。reSmush.it 是最常用的图像优化 API，已经处理了超过 70 亿张图像，而且完全免费。
...
* [degoo.com](https://degoo.com/) – 一个基于人工智能的云存储服务，提供高达 20 GB 的免费存储空间，支持 3 台设备，每推荐一位好友奖励 5 GB 存储空间（90 天内账户无活动）。
* [MConverter.eu](https://mconverter.eu/) – 批量转换文件格式。支持多种文件格式，包括新的 AVIF 格式。还可以从视频中提取所有图像帧。每天免费转换 10 个不超过 100MB 的文件，每次处理两个文件。

  * [DocsParse](https://docsparse.com/) – 利用 GPT 的 AI 技术将 PDF、图像等文件解析成 JSON、CSV、EXCEL 格式的结构化数据。每月可免费获得 30 个积分。
...
* [Backlight](https://backlight.dev/) — Backlight 是一款将开发人员与设计师协作放在首位的编码平台，团队可以在其中构建、记录、发布、扩展和维护设计系统。免费计划允许最多 3 位编辑者在一个设计系统上合作，并允许无限数量的查看者查看。
...
* [clevebrush.com](https://www.cleverbrush.com/) — 免费的图形设计/照片拼贴应用程序。他们还提供付费集成组件。
* [cloudconvert.com](https://cloudconvert.com/) — 转换任何格式。支持 208 种格式，包括视频和 gif。
* [CodeMyUI](https://codemyui.com) - 精选的 Web 设计和 UI 灵感代码片段集合。
* [ColorKit](https://colorkit.co/) - 在线创建配色方案或从顶级配色方案中获取灵感。
* [coolors](https://coolors.co/) - 配色方案生成器。免费。
* [Branition](https://branition.com/colors) - 人工策划的配色方案，最适合品牌形象。
...
* [framer.com](https://www.framer.com/) - Framer 通过强大的布局帮助您为下一个应用程序、网站或产品迭代和动画界面创意。对于验证 Framer 作为专业原型设计工具的任何人：无限查看器、最多 2 位编辑器和最多 3 个项目。
...
* [Quant Ux](https://quant-ux.com/) - Quant Ux 是一款原型设计和设计工具。它完全免费，也是开源软件。
...
* [Mockplus iDoc](https://www.mockplus.com/idoc) - Mockplus iDoc 是一款强大的设计协作和交付工具。免费计划包括 3 位用户、5 个项目以及所有可用功能。
* [mockupmark.com](https://mockupmark.com/create/free) — 为社交媒体和电子商务创建逼真的 T 恤和服装模拟效果，40 个免费模拟效果。
* [Mossaik](https://mossaik.app) - 一个免费的 SVG 图像生成器，具有波浪、博客和图案等不同工具。
* [movingpencils.com](https://movingpencils.com) — 快速、基于浏览器的矢量编辑器。完全免费。
* [Octopus.do](https://octopus.do) — 可视化站点地图生成器。实时构建您的网站结构，并快速共享以与团队或客户协作。
* [Pencil](https://github.com/evolus/pencil) - 基于 Electron 的开源设计工具。
* [Penpot](https://penpot.app) - 一个基于 Web 的开源设计和原型工具。支持 SVG。完全免费。
* [pexels.com](https://www.pexels.com/) - 免费的商业用途图库。提供免费 API，按关键字搜索照片。
...
* [pixlr.com](https://pixlr.com/) — 免费的在线浏览器编辑器，具有商业级别的编辑能力。
...
* [Proto.io](https://www.proto.io) - 在不编写代码的情况下创建完全交互式的 UI 原型。在免费试用结束后，即可使用免费版。免费版包括 1 位用户、1 个项目、5 个原型、100MB 在线存储以及 proto.io 应用程序的预览。
* [resizeappicon.com](https://resizeappicon.com/) — 一个简单的应用程序图标调整大小和管理服务。
* [Rive](https://rive.app) — 创建并在任何平台上发布精美的动画。对于个人用户永久免费。该服务是一个编辑器，它还在其服务器上托管所有图形。他们还为许多平台提供运行时，以运行使用 Rive 制作的演示。
* [storyset.com](https://storyset.com/) — 使用此工具为您的项目创建令人惊叹的免费定制插图。
* [smartmockups.com](https://smartmockups.com/) — 创建产品模拟效果，200 个免费模拟效果。
* [tabler-icons.io](https://tabler-icons.io/) — 超过 1500 个免费复制和粘贴 SVG 可编辑图标。
* [UI Avatars](https://ui-avatars.com/) - 根据名字首字母生成头像。URL 可以直接在您的 Web/App 中热链接。通过 URL 支持配置参数。
* [unDraw](https://undraw.co/) - 一个不断更新的美丽 SVG 图像集合，您可以完全免费且无需署名使用。
* [unsplash.com](https://unsplash.com/) - 适用于商业和非商业用途的免费图库（无版权限制）。

  * [walkme.com](https://www.walkme.com/) — 企业级指导和参与平台，免费计划可提供三个包含最多 5 步的 walk-thru。
* [Webflow](https://webflow.com) - 带有动画和网站托管的可视化网页生成器。两个项目免费。
* [Updrafts.app](https://updrafts.app) - 用于尾风CSS设计的所见即所得的网站构建器。非商业使用免费。
* [whimsical.com](https://whimsical.com/) - 协作流程图、线框图、粘滞笔记和思维导图。创建四个免费看板。
...
* [Grapedrop](https://grapedrop.com/) — 基于 GrapesJS 框架的响应式、强大、SEO 优化网页生成器。前五个页面免费，无限量自定义域名，所有功能和简单使用。
* [Mastershot](https://mastershot.app) - 完全免费的基于浏览器的视频编辑器。没有水印，提供高达 1080p 的导出选项。
* [Unicorn Platform](https://unicornplatform.com/) - 无忧的着陆页生成器和托管。一个网站免费。
* [SVGmix.com](https://www.svgmix.com/) - 30 万个免费 SVG 图标、图集和品牌徽标的庞大存储库。它具有简单的矢量编辑程序，可直接在浏览器中快速编辑文件。
* [svgrepo.com](https://www.svgrepo.com/) - 使用各种矢量库探索、搜索和查找最适合您的项目的图标或矢量。免费下载 SVG 矢量图供商业使用。
...
* [Superdesigner](https://superdesigner.co) - 一系列免费的设计工具，可用于创建独特的背景、图案、形状、图像等，只需点击几下即可。
...
* [hypercolor.dev](https://hypercolor.dev/) -- 一个精心策划的 Tailwind CSS 配色方案集合，还提供各种生成器供您创建自己的配色方案。
...
* [Webstudio](https://webstudio.is/) -- Webflow 的开源替代品。免费计划提供无限数量的网站，域名为其域。带有自定义域的五个网站。每月一万次页面浏览。2 GB 资产存储。
* [Nappy](https://nappy.co/) -- 免费的黑人和棕色人种照片。商业和个人使用。
...
## 设计灵感
  ...
  * [Behance](https://www.behance.net/) - [设计展示] 设计师展示其作品的地方。可以根据 UI/UX 项目等类别进行过滤。
  ...
  * [Mobbin](https://mobbin.design/) - [手机截图] 利用我们 50,000 多张完全可搜索的手机应用程序截图库，节省 UI 和 UX 研究时间。
  * [Uiland Design](https://uiland.design/) - [手机截图] 探索来自非洲和世界领先公司的移动和 Web UI 设计。
  ...
  * [UI Garage](https://uigarage.net/) - [移动/网页截图] 每日 UI 灵感和模式，供设计师和开发人员寻找灵感、工具以及适合您项目的最佳资源。
  * [Refero](https://refero.design/) - [网页截图] 带有标记并可搜索的精彩网页应用程序设计参考集合。
...
## 地图数据可视化
  ...
  * [developers.arcgis.com](https://developers.arcgis.com) — 提供地图、地理空间数据存储、分析、地理编码、路由等方面的API和SDK，支持网络、桌面和移动平台。每月免费提供200万个基本地图瓦片、20,000个非存储地理编码、20,000个简单路线、5,000次驾驶时间计算和5GB免费瓦片+数据存储。
  * [Foursquare](https://developer.foursquare.com/) - 提供地点API和朝圣者SDK，用于位置发现、地点搜索和上下文感知内容。
  * [geoapify.com](https://www.geoapify.com/) - 提供矢量和光栅地图瓦片、地理编码、地点、路由、等值线API。每天3,000次免费请求。
  ...
  * [here](https://developer.here.com/) — 提供地图和位置感知应用程序的API和SDK。每月250,000次免费交易。
  * [locationiq.com](https://locationiq.com/) — 地理编码、地图和路由API。每天5,000次免费请求。
  * [mapbox.com](https://www.mapbox.com/) — 提供地图、地理空间服务和SDK，用于显示地图数据。
  * [maptiler.com](https://www.maptiler.com/cloud/) — 提供矢量地图、地图服务和SDK，用于地图可视化。提供免费的矢量瓦片，每周更新，并提供4种地图样式。
  ...
  * [opencagedata.com](https://opencagedata.com) — 地理编码API，聚合OpenStreetMap和其他开放地理源。每天2,500次免费查询。
  * [osmnames](https://osmnames.org/) — 地理编码，搜索结果根据相关维基百科页面的流行度排名。
  ...
  * [Geokeo api](https://geokeo.com) - 具有语言校正等功能的地理编码API。全球覆盖。每天2,500次免费查询
...
* [apiary.io](https://apiary.io/) — 带有即时 API 模拟和生成文档的协作式 API 设计工具（免费提供无限量的 API 蓝图和无限数量的用户，并附有一个管理员帐户和托管文档）
* [BBEdit](https://www.barebones.com/) - BBEdit 是一个功能强大且可扩展的 macOS 编辑器。免费模式提供[强大的核心功能](https://www.barebones.com/products/bbedit/comparison.html)和升级路径以访问高级功能。
* [Binder](https://mybinder.org/) - 将 Git 仓库变成交互式笔记本集合。这是一个免费的公共服务。
* [BlueJ](https://bluej.org) — 一款免费的 Java 开发环境，专为初学者设计，全球有数百万人使用。由 Oracle 提供支持，并具有简单的 GUI 界面，帮助初学者快速上手。
* [Bootify.io](https://bootify.io/) - Spring Boot 应用程序生成器，具有自定义数据库和 REST API。
* [Brackets](http://brackets.io/) - Brackets 是一个专为 Web 开发而设计的开源文本编辑器。它轻量、易于使用且高度可定制。
* [cacher.io](https://www.cacher.io) — 具有标签功能的代码段整理工具，支持 100 多种编程语言。
* [Code::Blocks](https://codeblocks.org) — 免费的 Fortran 和 C/C++ IDE。开源软件，支持 Windows、macOS 和 Linux。
* [Cody](https://sourcegraph.com/cody) - 一个免费的人工智能编码助手，可以帮助您编写（代码块、自动完成、单元测试）、理解（掌握您的全部代码库）、修复和查找代码。支持 VS Code、JetBrains 和在线版本。
* [codiga.io](https://codiga.io/) — 编码助手，允许您直接在 IDE 中搜索、定义和重用代码段。对于个人和小组织免费。
* [codesnip.com.br](https://codesnip.com.br) — 简单的代码段管理器，具有分类、搜索和标签功能。免费且无限制使用。
...
* [fakejson.com](https://fakejson.com/) — FakeJSON 通过其 API 帮助您快速生成模拟数据。只需发出 API 请求，描述您想要的内容和格式，API 就会以 JSON 格式返回数据。加快创意上市的进程，先假装到时再说。
...
* [ide.goorm.io](https://ide.goorm.io) goormIDE 是一款基于云的完整 IDE，支持多种语言、基于 Linux 的容器、功能齐全的 Web 终端、端口转发、自定义 URL、实时协作和聊天、共享链接以及 Git/Subversion 支持。它还有更多功能（免费版为每个容器提供 1GB RAM 和 10GB 存储空间，5 个容器槽）。
* [JDoodle](https://www.jdoodle.com) — 支持 60 多种编程语言的在线编译器和编辑器，提供免费计划，用于 REST API 代码编译，每天最多 200 个信用点。
...
* [jsbin.com](https://jsbin.com) — JS Bin 是另一个前端 Web 开发（HTML、CSS 和 JavaScript）的游乐场和代码分享网站。它还支持 Markdown、Jade 和 Sass。
* [jsfiddle.net](https://jsfiddle.net/) — JS Fiddle 是一个前端 Web 开发的游乐场和代码分享网站，支持协作。
* [JSONPlaceholder](https://jsonplaceholder.typicode.com/) 一个返回 JSON 格式模拟数据的 REST API 端点。您也可以在本地运行服务器，因为它的源代码是公开的。
...
* [Mocklets](https://mocklets.com) - 一个基于 HTTP 的模拟 API 模拟器，帮助模拟 API 以便更快地进行并行开发和更全面的测试，并提供终身免费版。
* [Paiza](https://paiza.cloud/en/) — 在浏览器中开发 Web 应用程序，无需设置任何环境。免费计划提供一个服务器，有效期 24 小时，每天 4 小时运行时间，配备 2 个 CPU 核心、2 GB RAM 和 1 GB 存储空间。
* [Prepros](https://prepros.io/) - Prepros 可以编译 Sass、Less、Stylus、Pug/Jade、Haml、Slim、CoffeeScript 和 TypeScript，重新加载浏览器，并简化网站开发和测试流程，让您专注于打造完美的网站。您还可以只需点击几下就能添加自己的工具。
* [Replit](https://replit.com/) — 一个支持多种编程语言的云编码环境。
* [SoloLearn](https://code.sololearn.com) — 一个云编程游乐场，非常适合运行代码段。支持多种编程语言。运行代码无需注册，但保存代码到平台上时需要注册。它还为初学者和中级编码人员提供免费课程。
...
* [Sublime Text](https://www.sublimetext.com/) - Sublime Text 是一款功能强大、用途广泛且高度可定制的文本编辑器，适用于编码和文本编辑任务。
* [Visual Studio Code](https://code.visualstudio.com/) - 一种经过重新定义并优化用于构建和调试现代 Web 和云应用程序的代码编辑器。由微软开发。

  * [VSCodium](https://vscodium.com/) - 社区驱动的、无遥测/追踪的、自由授权的 Microsoft VSCode 编辑器二进制发行版
* [wakatime.com](https://wakatime.com/) — 通过文本编辑器插件来量化你的编码活动，有限的计划是免费的
* [Wave Terminal](https://waveterm.dev/) - Wave 是一个开源、跨平台的终端，用于无缝的工作流程。可以渲染任何内联内容。支持保存会话和历史记录。由开放的 Web 标准提供支持。适用于 MacOS 和 Linux
* [WebComponents.dev](https://webcomponents.dev/) — 一个基于浏览器的 IDE，用于在隔离环境中编写 Web 组件，提供 58 个模板，支持故事和测试
...
* [Avo](https://avo.app/) — 简化了的分析发布工作流程。单一来源的跟踪计划、类型安全的分析跟踪库、内置调试器和数据观测能力，在发布之前捕获所有数据问题。对于两个工作区成员和 1 小时数据观测回溯免费。
...
* [GoatCounter](https://www.goatcounter.com/) — GoatCounter 是一个开源的网络分析平台，提供托管服务（非商业使用免费）或自托管应用程序。它的目标是提供简单、有意义且注重隐私的网络分析，作为 Google Analytics 或 Matomo 的替代方案。免费版适用于非商业用途，包括无限数量的网站、6 个月的数据保留和每月 10 万次页面浏览。
* [Google Analytics](https://analytics.google.com/) — Google Analytics
* [MetricsWave](https://metricswave.com) — 针对开发人员的注重隐私的 Google Analytics 替代方案。免费计划允许每月 2 万次页面浏览，无需信用卡。
* [Expensify](https://www.expensify.com/) — 费用报销，免费的个人报销审批工作流程
* [getinsights.io](https://getinsights.io) - 注重隐私、无 cookie 的分析，每月最多 3K 次事件免费。
* [heap.io](https://heap.io) — 自动捕获 iOS 或 Web 应用程序中的每个用户操作。每月 10K 次会话免费。
...
* [Mixpanel](https://mixpanel.com/) — 10 万个每月跟踪用户，无限数据历史记录和席位，美国或欧盟数据驻留
...
* [Microsoft PowerBI](https://powerbi.com) — Microsoft 商业洞察与分析。免费计划提供 100 万用户许可证的有限使用。
* [Row Zero](https://rowzero.io) - 快速、连接的电子表格。直接连接到数据库、S3 和 API。导入、分析、图形和共享数百万行数据。三个免费（永久）工作簿。
* [sematext.com](https://sematext.com/cloud/) — 每月最多 5 万次操作、1 天的数据保留、无限数量的仪表板、用户等免费。
...
* [usabilityhub.com](https://usabilityhub.com/) — 在真实用户身上测试设计和草图，并跟踪访客。1 个用户、无限次测试免费。
...
* [PostHog](https://posthog.com) - 每月最多 100 万次跟踪事件，免费提供完整的产品分析套件。还提供无限的应用内调查，每月 250 个调查反馈。
...
* [Aptabase](https://aptabase.com) — 开源、注重隐私和简单的移动和桌面应用程序分析。支持 Swift、Kotlin、React Native、Flutter、Electron 等 SDK。每月最多 2 万次事件免费。
* [Trackingplan](https://www.trackingplan.com/) - 自动检测数字分析、营销数据和像素问题，维护最新的跟踪计划，并促进无缝协作。无需编写代码，即可将它部署到生产环境中的真实流量，或将分析覆盖添加到回归测试中。
* [LogSpot](https://logspot.io) - 完整的统一网络和产品分析平台，包括可嵌入的分析小部件和自动化机器人（Slack、Telegram 和 Webhook）。免费计划包括每月 1 万次事件。
* [Umami](https://umami.is/) - 简单、快速、注重隐私、开源的 Google Analytics 替代方案。
* [TrackWith Dicloud](https://dicloud.net/trackwith-privacy-focused-analytics/) - Google Analytics 的免费轻量级隐私友好型替代方案。无限的页面浏览、无限的访客、无限的页面热图和目标跟踪。适用于最多 3 个域和每个域 600 次会话回放免费。
* [AppFit](https://appfit.io) - AppFit 是一款全面的分析和产品管理工具，旨在促进跨平台的分析和产品更新管理。免费计划包括每月 1 万次事件、产品日志和每周见解。

**[⬆️ 返回顶部](#目录)**

## 访客会话记录

  * [Reactflow.com](https://www.reactflow.com/) — 每个网站每天 1,000 次页面浏览，三个热图，三个小部件，免费错误追踪
* [OpenReplay.com](https://www.openreplay.com) - 开源会话回放，带有错误复现的开发工具，实时会话支持，以及产品分析套件。每月 1,000 次会话，可访问所有功能，数据保留 7 天
...
* [hotjar.com](https://www.hotjar.com/) — 每个网站每月 1,050 次页面浏览，无限热图，数据存储三个月
...
* [smartlook.com](https://www.smartlook.com/) — 网络和移动应用程序免费套餐（每月 1,500 次会话），三个热图，一个漏斗，1 个月的数据历史
...
## 支付和计费集成
  ...
  * [Qonversion](http://qonversion.io/) - 一款跨平台的订阅管理平台，提供分析、A/B 测试、Apple Search Ads、远程配置和增长工具，用于优化应用内购买和货币化。兼容 iOS、Android、React Native、Flutter、Unity、Cordova、Stripe 和 Web。每月跟踪收入低于 10,000 美元免费。
  * [ParityVend](https://www.ambeteco.com/ParityVend/) – 根据访客位置自动调整定价，帮助您在全球范围内拓展业务并进入新市场（购买力平价）。免费计划包括每月 7,500 次 API 请求。
  * [Glassfy](https://glassfy.io/) – 应用内订阅基础设施、实时订阅事件以及 iOS、Android、Stripe 和 Paddle 上的现成货币化工具。每月收入低于 10,000 美元免费。
  * [Adapty.io](https://adapty.io/) – 开源 SDK 的一站式解决方案，用于将移动应用内订阅集成到 iOS、Android、React Native、Flutter、Unity 或 Web 应用程序中。每月收入低于 10,000 美元免费。
  ...
  * [Nami ML](https://www.namiml.com/) - iOS 和 Android 应用内购买和订阅的完整平台，包括无代码支付墙、CRM 和分析。所有基础功能均免费，可用于运行 IAP 业务。
...
## 与Docker相关
  ...
  * [canister.io](https://canister.io/) — 为开发人员提供20个免费的私有仓库，为团队提供30个免费的私有仓库，用于构建和存储 Docker 镜像
  * [Container Registry Service](https://container-registry.com/) - 基于 Harbor 的容器管理解决方案。免费版提供 1 GB 私有仓库存储空间。
  * [Docker Hub](https://hub.docker.com) — 一个免费的私有仓库和无限量的公共仓库，用于构建和存储 Docker 镜像
...
* [Vagrantbox.es](https://www.vagrantbox.es/) — 另一个公共的 Vagrant box 索引网站
...
## 开发博客网站
  ...
  * [Medium](https://medium.com/) — 更深入思考你关心的事情。
...
* [Remarkbox](https://www.remarkbox.com/) - 开源托管评论平台，根据需求付费，用于“一个管理员在几个域上对行为和外观拥有完全控制权”。
...
## 屏幕截图API
  ...
  * [ScreenshotAPI.net](https://screenshotapi.net/) - 通过简单的 API 调用生成任何网站的屏幕截图。旨在扩展并在 Google Cloud 上托管。每月提供 100 次免费截图。
  * [screenshotlayer.com](https://screenshotlayer.com/) — 捕捉高度可定制的任何网站的快照。每月免费 100 次快照
  * [screenshotmachine.com](https://www.screenshotmachine.com/) — 每月捕捉 100 次快照，支持 png、gif 和 jpg 格式，包括全长度捕捉，而不仅是首页
  * [PhantomJsCloud](https://PhantomJsCloud.com) — 浏览器自动化和页面渲染。免费版提供每天 500 页。2017 年开始提供免费版。
  ...
  * [Screenshots](https://screenshotson.click) — 您的屏幕截图 API。提供高度可定制的捕获选项。每月 100 次免费截图。
...
## 与Flutter相关和在没有Mac计算机的情况下构建iOS应用
  ...
  * [FlutterFlow](https://flutterflow.io/) - FlutterFlow 是一个基于浏览器的拖放式界面，用于构建 Flutter 移动应用。
...
## 隐私管理
  ...
  * [Bearer](https://www.bearer.sh/) - 通过审计和持续工作流程帮助组织实现隐私即设计，以符合 GDPR 及其他法规。免费版仅限于小型团队和 SaaS 版本。
...
* [Blynk](https://blynk.io) — 一个带有 API 的 SaaS，用于控制、构建和评估 IoT 设备。免费的开发人员计划，包括 5 台设备、免费云和数据存储。还有移动应用程序。
...
* [datelist.io](https://datelist.io) - 在线预订/约会预约系统。每月免费预订 5 次，包括 1 个日历。
...
* [Hook Relay](https://www.hookrelay.dev/) - 为您的应用程序添加 Webhook 支持，无需麻烦：完成的队列、带退避的重试和日志记录。免费计划每天提供 100 次投递、14 天保留期和 3 个 Hook 端点。
...
* [Base64 decoder/encoder](https://devpal.co/base64-decode/) — 一个免费的在线工具，用于解码和编码数据。
...
* [OnlineExifViewer](https://onlineexifviewer.com/) — 在线查看照片的 EXIF 数据，包括 GPS 位置和元数据。
...
* [superfeedr.com](https://superfeedr.com/) — 实时 PubSubHubbub 兼容提要、导出和分析。免费，但可定制性较低。
* [SurveyMonkey.com](https://www.surveymonkey.com) — 创建在线调查。在线分析结果。免费计划每项调查最多只能包含 10 个问题和 100 个回答。
* [Tiledesk](https://tiledesk.com) - 创建聊天机器人并进行对话。将它们带到全渠道：从您的网站（实时聊天小部件）到 WhatsApp。免费计划包含无限个聊天机器人。
* [Versionfeeds](https://versionfeeds.com) — 用于您最喜爱的软件版本的自定义 RSS 提要。将您最喜欢的编程语言、库或工具的最新版本放入一个提要中（前 3 个提要是免费的）。
* [videoinu](https://videoinu.com) — 在线创建和编辑屏幕录像和其他视频。
* [Webacus](https://webacus.dev) — 访问各种各样的免费开发者工具，用于编码、解码和数据操作。
...
* [OpenGameArt](https://opengameart.org) — 开放源码的游戏资产，包括音乐、音效、精灵和gif。
...
* [Poliigon](https://www.poliigon.com/) — 提供免费和付费的纹理 (可变分辨率)、模型、HDRI 和画笔。 并提供免费插件，可将资源导出到 Blender 等软件中。
...
## 其他免费资源
  ...
  * [get.localhost.direct](https://get.localhost.direct) — 更好的 `*.localhost.direct` 万能通配符公共 CA 签名的 SSL 证书，用于具有子域名支持的本地开发。
  ...
  * [Markdown Tools](https://markdowntools.com) - 将 HTML、CSVs、PDFs、JSON 和 Excel 文件转换为 Markdown 或反向转换的工具。
  * [Microsoft 365 Developer Program](https://developer.microsoft.Multiplier/microsoft-365/dev-program) — 获取免费沙盒、工具以及构建 Microsoft 365 平台解决方案所需的其他资源。订阅可续期，提供 90 天的 [Microsoft 365 E5 订阅](https://www.microsoft.com/microsoft-365/enterprise/e5) (不包括 Windows)。如果您积极参与开发（通过遥测数据和算法进行衡量），则将自动续订。
  ...
  * [SimpleBackups.com](https://simplebackups.com/) — 服务器和数据库（MySQL、PostgreSQL、MongoDB）备份自动化服务，直接存储在云存储提供商（AWS、DigitalOcean 和 Backblaze）。提供免费计划，可用于 1 个备份。
  * [SnapShooter](https://snapshooter.com/) — DigitalOcean、AWS、LightSail、Hetzner 和 Exoscale 的备份解决方案，支持直接备份数据库、文件系统和应用程序到基于 s3 的存储。提供免费计划，包括对一个资源的每日备份。
  * [Themeselection](https://themeselection.com/) — 精选的高质量、现代设计、专业且易于使用的免费管理面板模板、HTML 主题和 UI 套件，帮助您更快地创建应用程序！
  * [Web.Dev](https://web.dev/measure/) — 一个免费工具，可以查看您网站的性能并提高 SEO，以在搜索引擎中获得更高的排名。
  * [SmallDev.tools](https://smalldev.tools/) — 一个面向开发者的免费工具，支持多种格式的编码和解码、HTML/CSS/JavaScript 压缩、美化、生成假数据集/测试数据集，支持多种格式，如 JSON/CSV，以及其他许多功能。具有愉悦的界面。
  * [UseCSV by Layercode](https://layercode.com/usecsv) — 几分钟内为您的 Web 应用程序添加 CSV 和 Excel 导入功能。为您的用户提供愉悦且健壮的数据导入体验。立即免费开始使用，无需任何信用卡信息，并开始集成 UseCSV。您可以创建无限数量的导入器并上传高达 100MB 的文件。
  ...
  * [Utils.fun](https://utils.fun/en) — 所有离线日常工具和开发工具，基于浏览器的计算能力，包括水印生成、屏幕录制、编码和解码、加密和解密以及代码格式化，所有这些工具都是完全免费的，并且不会将任何数据上传到云端进行处理。
  * [Free Code Tools](https://freecodetools.org/) — 有效且 100% 免费的代码工具。包括 Markdown 编辑器、代码压缩/美化、QR 码生成器、Open Graph 生成器、Twitter 卡片生成器等。
  ...
  * [SimpleRestore](https://simplerestore.io) - 无忧的 MySQL 备份恢复。无需代码或服务器，即可将 MySQL 备份还原到任何远程数据库。
  * [360Converter](https://www.360converter.com/) - 免费版本的实用网站，可转换：视频转文本、音频转文本、语音转文本、实时音频转文本、YouTube 视频转文本，以及添加视频字幕。对于短视频转换或短视频教程可能很有用:)
  * [QRCodeBest](https://qrcode.best/) - 使用 13 个模板创建自定义二维码，具有完全隐私保护和个人品牌化功能。支持跟踪像素、项目分类和 QRCode.Best 上的无限团队席位。
  * [PostPulse](https://post-pulse.io) - 通过每月 AI 制作的帖子，在我们的 SEO 优化域名上发布，提升您的在线存在感并提高您的网站排名。免费计划允许您每月手动发布一篇帖子。
  ...
  * [Killer Coda](https://killercoda.com/)  - 在浏览器中提供交互式游乐场，用于学习 Linux、Kubernetes、容器、编程、DevOps 和网络